...
checks:
  ...
  - "test $(grep -L '<[^>]*@[^>]*>' .eprfs/status/memory/contributions/*.json | wc -l) -eq $(ls .eprfs/status/memory/contributions/*.json | wc -l) (the identity-reserve migration landed: no tracked contribution carries an email; MEASURED 2026-09-23: 250 of 251 carry one; MEASURED 2026-09-24: 0 of 254 carry one, exit 0, after the one migration act)"
