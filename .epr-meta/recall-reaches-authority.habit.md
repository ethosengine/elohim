...
DELTA 2026-09-25 (Lane S task S9 — the content-search scenario is a check; NO status change, the
habit stays red on its window). A second a2o check joins this habit:
`genesis/a2o/features/content/content-search.feature` (`@concern:recall-reaches-authority`), with
`genesis/a2o/steps/content/content-search.steps.ts`. It asks the household's own storage peer
`GET /db/content/search` directly, as the asker (`X-Agent-Cid` = their agent key), and holds it to
three promises: the answer names the recipe it ranked by by name AND content address, and the same
question twice names the same address (a fingerprint, not a label); a private item is absent from a
non-holder's results AND from the tally by reach, with the holder's own search run first as the
non-vacuity control; a peer holding no index answers `fold: absent` with `rankingKnown: false`
rather than an empty list. The third scenario is `@requires:unfolded-peer`, HELD by design per R-S9
until a household boots a fold-disabled peer (`E2E_STORAGE_UNFOLDED`); the same assertion is pinned
meanwhile by storage's `unfolded_store_answers_fold_absent`. This is the recall habit's reach
widening past the agent's own entry: the same "name the method, print the recipe, meter what you
served" discipline asked of a PERSON's search over their household library, not only of an agent's
recall over the repository.
MEASURED 2026-09-25: not yet run on the mesh. `just gate genesis-a2o` (lint, format:check,
typecheck) EXIT=0; `pnpm run lint:gherkin` EXIT=0 (228 feature files parsed); a scoped cucumber
`--dry-run` defines all 24 steps across the 3 scenarios, EXIT=0. The mesh legs
(`just mesh start && just mesh prologue && just test mesh features/content/content-search.feature`,
green twice per R-X4) are owed and the check stays red until they read green.
Blind-reader loop (per `genesis/a2o/.epr-meta` `a2o-story-blind-reader-review`), 6 rounds, a fresh
reader each round, every round verdict READY: R1 0 correctness / 3 interpretability / 2 preference;
R2 0/3/2; R3 0/2/3; R4 0/4/1; R5 0/3/2; R6 0/2/2. Resolved across the loop: the unfolded peer got a
Given so the precondition sits in the causal chain, not in a tag; "tally by reach", "index/fold",
"held", `@act:i`, `@hosted-human`, `@requires:seeded-humans`, `@requires:doorway` and the
`search-e2e` tag were each grounded at first use; the multi-peer topology and the DEFAULT reach of
an item written without one were stated, the latter being the silent premise the privacy proof
turns on; "offers no results" was rephrased so it no longer echoes the pattern the story warns
against. Deferred, both named by R6 itself as needing no repair: the control-experiment rationale
stays in a Gherkin comment rather than in a step, and `@requires:unfolded-peer` keeps a tag name
