...
def scan_doc(path: Path, is_plan: bool, today: date) -> tuple[Proposal | None, ReviewFlag | None]:
...
def render_proposals(
    ...
    out.append("")
    out.append(
        "Generated by `.epr-meta/elohim/lenses/memory/cleanup-scan.py`. "
        "Mark `- [x] Accept` on archive candidates, then route each accepted one "
        "through `epr flow hold` — `cleanup-apply.py` was RETIRED 2026-09-10 "
        "(station four of the memory-kit replacement: archival relocation is not net "
        "removal, so a mover that only shuffles bytes into a dated directory earns "
        "no place; a hold is the decision this proposal is asking for)."
    )
    out.append("")
...
def main() -> int:
    ...
    for path in specs:
        p, f = scan_doc(path, is_plan=False, today=today)
        if p:
            proposals.append(p)
        if f:
            flags.append(f)
    ...
    for path in plans:
        p, f = scan_doc(path, is_plan=True, today=today)
        if p:
            proposals.append(p)
        if f:
            flags.append(f)
    ...
    output = render_proposals(proposals, flags, archive_root, today)
