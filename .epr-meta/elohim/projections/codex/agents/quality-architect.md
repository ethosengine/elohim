...
governance: "epr:elohim-agent/agents/quality-architect"
---
...
The BDD gap scanner lives at `genesis/a2o/scripts/scan-coverage.ts` and is invoked locally via `cd genesis/a2o && npm run scan:coverage`. When the scanner has been run, its report will appear in `genesis/a2o/reports/` as `coverage-gap-report.json`. When that report is present, incorporate it into your analysis:
