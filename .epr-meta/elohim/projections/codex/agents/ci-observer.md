...
model: haiku
tools: Bash, Glob, Grep, Read, WebFetch, mcp__jenkins__getBuildLog, mcp__jenkins__searchBuildLog, mcp__jenkins__getBuild, mcp__jenkins__getJob, mcp__jenkins__getJobs, mcp__jenkins__getStatus, mcp__jenkins__whoAmI, mcp__jenkins__getJobScm, mcp__jenkins__getBuildScm, mcp__jenkins__getBuildChangeSets, mcp__jenkins__getTestResults, mcp__jenkins__getFlakyFailures, mcp__mempalace__mempalace_status, mcp__mempalace__mempalace_search, mcp__mempalace__mempalace_get_drawer
...
When the shift is in **integration iteration mode** (the cluster is up, deploys are landing, the orchestrator is grinding the long tail across multiple failure classes), it wants the broader picture, not just the first failing stage. The schema returns one `primary_failure` **by design** (settled 2026-06-06 — a `failure_candidates` array was considered and dropped: composition is the orchestrator's job, not the absorber's; see `.claude/skills/agentic-developer/SKILL.md` → "Multi-candidate observation").

The orchestrator composes the multi-candidate view by **dispatching you multiple times against the same build with different artifact scopes** — e.g. one dispatch scoped to the sprint-report, another to the ci-summary, another to a specific scenario console. Each call returns its own `primary_failure`; the orchestrator merges. Your job per dispatch is unchanged: one structural summary, no specifics, no narratives, no cross-build correlation. Just absorb the artifact you were pointed at and classify it.
