...
`--need '<question>'` alone sets the session's carried intent — no separate `--intent` flag is required. `.claude/` (skills, hooks, commands, measures, memory) sits inside the entry's declared source scope alongside the rest of the repository.
...
For head compaction, storyteller owns disposition: memorialize into an existing umbrella, graduate a lesson to a canonical story, or retain it. Librarian applies topic-file metadata and reprojects with `epr flow memory project --index --budget memory-index-bytes@1 --out .claude/memory/MEMORY.md` (the PostToolUse router `.claude/hooks/memory-index-projection.py` takes the same leg, refusing (never falling back to a directory-scan projector) when a just-written entry has no contribution row yet or the native projection exceeds the hook's latency budget). A projected row comes from a CONTRIBUTION, so an entry the ceremony just wrote reaches the index through `epr flow memory import .claude/memory`. To fold entries under an umbrella, write a NEW umbrella entry whose frontmatter declares `supersedes: [<entry-name>, …]` (one inline list of entry file stems); the harness imports it under its writer and pins each member's current contribution. A fold of your own entries takes effect at once; a fold of another author's takes effect only after an approving verdict from an active Steward who is not the curator — the import result and the index `advisory` print the exact `epr flow note --on <umbrella-contribution-cid> --kind verdict --verdict approved …` line — and until then the umbrella and its members all stay indexed. Members' files and contributions are never modified. Who wrote an entry's current bytes is the harness's earliest write witness of them, not the frontmatter's `originSessionId`: an edit to another participant's entry is reported (`editedByAnotherParticipant`), never imported under either participant. Preserve candidate-to-surviving-source mappings and the corrected lesson. Staleness alone does not authorize removal. Feedback remains unless a surviving entry genuinely duplicates it. Routine byte-budget and archive hygiene belong to the hygiene lenses at `.epr-meta/elohim/lenses/`.
...
When verified canonical memory surfaces change, refresh the semantic fold natively: `epr flow memory index status` reads the fold's lag (files new, changed or removed since their fold) against `index-fold-lag-ceiling@1`, the same reading the `epr flow report --headline` `index:` slot prints; `epr flow memory index fold` folds what moved, at most `limits.fold_files_per_run` files a run (repeat, or pass `--max-files N`, until the lag is 0 and the attestation reads `complete`). The pickup-surfacing hook also spawns one detached fold per session while the fold is behind, and the fold's lock turns a concurrent run into a harmless `busy`. A `degraded` or `failed` attestation stays visible; do not claim the fold current or erase successful local work. No automatic diary/curation outside this authorized maintenance scope. Reuse the existing chronology and flow records rather than creating another ledger.

**Visitor maintenance (optional).** MemPalace is the declared visitor and second opinion, not the index, and its mine is no longer a headline gate. When the palace is kept, refresh it with `mempalace --palace .mempalace/palace sync --root . --apply`, then `mempalace --palace .mempalace/palace mine <surface>` for each surface declared on `mempalace-surfaces-changed-ceiling@1` (still readable by name with `epr flow report --bound mempalace-surfaces-changed-ceiling`), then stamp `date +%s.%N > .mempalace/.last-mine` ONLY if no step was lock-blocked.
