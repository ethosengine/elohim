...
checks:
  ...
  - "python3 -c \"import yaml,sys;p=yaml.safe_load(open('.claude/epr-meta/policies.yaml'))['policies'];r=[x for x in p if x.get('id') in ('sovereignty-ontology-guard','ownership-ontology-guard') and x.get('status')=='active'];sys.exit(0 if len(r)==2 and all(x.get('frame') and x.get('frame-ref') for x in r) else 1)\" (both ACTIVE guard rows declare `frame:` and `frame-ref:` — @5 since R-C13, the filter was `version==4` until then; MEASURED 2026-09-24: exit 1 — no @4 row exists, and `grep -cE '^\\s+frame:'` and `grep -cE '^\\s+frame-ref:'` over policies.yaml both count 0; MEASURED 2026-09-24 after C6 (c0b6d9fc7): exit 0; MEASURED 2026-09-24 after the review fixes (e5f8e4a61): exit 0 over the @5 rows, and exit 0 under the old `version==4` filter too)"
...
DELTA 2026-09-24 (REVIEW FINDINGS CLOSED — stays green: checks (a), (b), (c) each exit 0 again by their own commands). Executed by agent:implementer@claude-opus-5-5. M1 575d8ad38 (both loaders refuse a schema-breaking atom: an empty phrase no longer hangs Python or underflows Rust, R-C12); W2 575d8ad38 (evidence carries `ontologyRef`, equal in both hosts); M2 e5f8e4a61 (guard rows @5 without the raw `contains-any` pre-filter, @4 superseded, re-pinned, canon-lift fresh, both vector runners seed the registry and bind @5, R-C13); M3 W1 W4 0ad3fe292 (probe deadline 30 s and one recall open per session, R-C10; rows carry `classification_scope: "document"` and a nonce the classify child rewrites by, R-C14); W3 this commit (parity test widened: non-ASCII byte offsets, drift, confidence, rubricAnswer, ownership silence, cases told apart). Pool `epr` rebuilt at 575d8ad38. Evidence: (a) `frame classifier parity: native == python ✅` exit 0; (b) exit 0; (c) `golden_fixture_classification_cid ... ok`, EXIT=0. Check (b) now selects the active rows rather than `version==4`. Open by name: the canon manifest still binds @3, so live writes there keep the raw pre-filter until the operator rebinds it — to @5 now (R-C9, R-C13).
