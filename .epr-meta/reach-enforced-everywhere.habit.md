...
evidence:
  - "2026-08-27c: the conductor admin socket's DEV_MODE hole is CLOSED ON THE FLEET — edge #1386 (SUCCESS, image 1.0.0-dev-9a207277, all 7 statefulsets + both doorways rolled ~23:26Z) carries 13ed1721e: one predicate (native_local_first_operator = loopback ∧ pre-coordination ∧ no declared JWT_SECRET) decides the credential-free grant, every admin frame is filtered, and the keyless-API `|| !is_configured()` arm no longer hands Public to a remote caller. Probed 2026-08-28T00:12Z: anonymous WebSocket upgrade to /hc/admin → HTTP 401 {\"error\":\"Authentication required. Use POST /hc/connect.\"} on BOTH doorway-alpha.elohim.host and elohim.host (was an unfiltered admin passthrough — install_app/uninstall_app/revoke_agent_key reachable from the open internet). Same build: the quiesce gate ADMITTED a measure (A-QUIESCED, first since #1383) and Dataplane Validation ran 87 scenarios (3 passed, 1 failed = inventory-convergence caughtUp 30 min post-restart, 83 HELD @act:i). Stays RED on the invariant: /head-record · /epr-head · /apps/{cid} · direct /shard·/ipfs plane conflation is untouched, and the flip needs the household lane's byte-route scenario green on a fleet build, not a probe."
