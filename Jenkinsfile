...
def buildSophiaPlugin() {
    ...
    sh 'git submodule update --init --recursive app/sophia'
    ...
    sh '''
        mkdir -p app/elohim-app/src/assets/sophia-plugin

        # Copy UMD bundle from sophia-element
        cp app/sophia/packages/sophia-element/dist/sophia-element.umd.js app/elohim-app/src/assets/sophia-plugin/

        # Copy base CSS from main sophia package
        cp app/sophia/packages/sophia/dist/index.css app/elohim-app/src/assets/sophia-plugin/

        # Copy UMD CSS if it exists (generated by rollup)
        cp app/sophia/packages/sophia-element/dist/sophia-element.umd.css app/elohim-app/src/assets/sophia-plugin/ 2>/dev/null || true

        # Create stub CSS files for theme overrides (actual theming is via Sophia.configure() API)
        echo "/* Sophia theme overrides - Configure via Sophia.configure() API */" > app/elohim-app/src/assets/sophia-plugin/sophia-theme-overrides.css
        echo "/* Sophia styles - bundled in UMD */" > app/elohim-app/src/assets/sophia-plugin/sophia.css

        echo "✅ Sophia plugin assets copied to elohim-app"
        ls -la app/elohim-app/src/assets/sophia-plugin/
    '''
}
