        ...
        stage('SonarQube Analysis') {
            steps {
                container('builder'){
                    dir('elohim-app') {
                        script {
                            // Debug: Show what files were generated by tests
                            sh '''
                                echo "🔍 DEBUG: Checking for coverage files after test stage..."
                                echo "Current directory: $(pwd)"
                                echo "Contents of current directory:"
                                ls -la
                                echo ""
                                echo "Looking for coverage directory:"
                                find . -name "coverage" -type d 2>/dev/null || echo "No coverage directory found"
                                echo ""
                                echo "Looking for any lcov files:"
                                find . -name "*.lcov" -o -name "lcov.info" 2>/dev/null || echo "No lcov files found"
                                echo ""
                                echo "Looking for any coverage-related files:"
                                find . -path "*/coverage/*" 2>/dev/null || echo "No files in coverage directories found"
                                echo ""
                                echo "Checking if specific expected path exists:"
                                ls -la coverage/elohim-app/lcov.info 2>/dev/null || echo "coverage/elohim-app/lcov.info not found"
                                echo ""
                                echo "Alternative coverage paths to check:"
                                ls -la coverage/lcov.info 2>/dev/null || echo "coverage/lcov.info not found"
                                ls -la coverage/*/lcov.info 2>/dev/null || echo "No lcov.info in coverage subdirectories"
                                echo "🔍 DEBUG: Coverage file search complete"
                                echo ""
                            '''
