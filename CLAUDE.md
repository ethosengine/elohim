...
MESH_KEEP_DOORWAY_DB=1 just mesh start  # a cold start (peer 0's admin port silent) now drops the doorways' Mongo account archive along with the conductor sandboxes it names cells on — a kept row 409s `/auth/register` and logs a stale human in against a cell that no longer exists; set this to study that skew deliberately
