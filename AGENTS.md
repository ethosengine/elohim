...
just mesh start                # conductor = the PINNED FORK, auto-detected at /projects/.claude-config/tools/hc-fork-<submodule-pin12>/bin (MESH_TOOLS_DIR); a conductor whose line differs from the DNA's hdk line (0.7) is REFUSED, not warned (MESH_ALLOW_TOOLCHAIN_SKEW=1 overrides) — the workspace image still ships stock 0.6.0 in /opt/holochain/bin until it is rebuilt (backlog: workspace-image-ships-stock-holochain-0-6)
