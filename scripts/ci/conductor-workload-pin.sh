...
#
# Prints `conductor-<hc12>` on stdout, exactly the tag suffix
...
#
# Exits non-zero with an empty stdout if the pointer is unreadable — the
...
if [ -z "${CONDUCTOR_SHA}" ]; then
    echo "conductor-workload-pin: cannot read fork submodule pointer (holochain-conductor='${CONDUCTOR_SHA:-<unreadable>}')" >&2
    exit 1
fi

echo "conductor-$(echo "${CONDUCTOR_SHA}" | cut -c1-12)"
