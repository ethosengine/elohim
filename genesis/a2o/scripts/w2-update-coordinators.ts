...
const APP_ID = 'elohim';
const WASM = '/projects/elohim/elohim/holochain/target/wasm32-unknown-unknown/release/content_store.wasm';
async function main() {
  ...
  await admin.updateCoordinators({
    ...
    source: {
      type: 'bundle',
      value: {
        manifest: { zomes: [{ name: 'content_store', path: 'content_store.wasm', dependencies: [{ name: 'content_store_integrity' }] }] },
