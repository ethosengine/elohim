...
async function main() {
  ...
  await fetch(`${STORAGE}/db/content/bulk`, {
    ...
    body: JSON.stringify([
      {
        ...
        description: 'Digital Infrastructure for Human Flourishing',
        contentType: 'article',
    ...
    .then(r => r.text())
    .then(t => console.log('bulk', t.slice(0, 80)));
  ...
  console.log(
    ...
    patch.status,
    'anchor',
    String(target).slice(0, 22),
    'reach',
    patched.reach ?? patched.content?.reach
  );
