...
async function main() {
  ...
  const appWs = await AppWebsocket.connect({
    url: new URL('ws://127.0.0.1:4485'),
    token: (await admin.issueAppAuthenticationToken({ installed_app_id: app.installed_app_id }))
      .token,
    wsClientOptions: { origin: APP_ID },
  });
  const ev: any = await appWs.callZome({
    cell_id: lamad,
    zome_name: 'content_store',
    fn_name: 'get_canonical_election_evidence',
    payload: 'manifesto',
  });
  if (!ev) {
    console.log('NO election evidence (extern answered null)');
    process.exit(2);
  }
  console.log(
    'EVIDENCE:',
    JSON.stringify({
      winner: String(ev.election.winner_target),
      earned: ev.election.canonical_earned,
      declared_at: ev.election.canonical_declared_at,
      linkBytes: ev.link_record?.length,
    })
  );
  process.exit(0);
}
