@e2e @qahal @browser-only @feedback-gate @regression @act:i
Feature: Feedback dialogue panel — accountable peer surface
  ...
  Scenario: Learner reaches the feedback dialogue from a content page
    When Matthew navigates to a markdown content page
  ...
  Scenario: Trigger reveals the four governance feedback choices
    Given Matthew is viewing a markdown content page
    When Matthew opens the feedback trigger menu
  ...
  Scenario: Selecting a category opens a readable, selectable card above page chrome
    Given Matthew is viewing a markdown content page
    And Matthew has opened the feedback trigger menu
    When Matthew selects "Feedback" from the menu
  ...
  Scenario Outline: Each category renders the spec's grievance vocabulary
    Given Matthew is viewing a markdown content page
    And Matthew has opened the feedback trigger menu
    When Matthew selects "<menu-label>" from the menu
  ...
  Scenario: Learner can close the dialogue without submitting
    Given Matthew has the feedback dialogue panel open with "Feedback" selected
    When Matthew clicks the dialogue close button
    Then the feedback dialogue panel should not be visible

  @dismiss @backdrop
  Scenario: Clicking outside the card dismisses the dialogue
    Given Matthew has the feedback dialogue panel open with "Feedback" selected
    When Matthew clicks the dialogue backdrop
    Then the feedback dialogue panel should not be visible

  @dismiss @keyboard
  Scenario: Pressing Escape dismisses the dialogue
    Given Matthew has the feedback dialogue panel open with "Feedback" selected
    When Matthew presses the Escape key
    Then the feedback dialogue panel should not be visible
