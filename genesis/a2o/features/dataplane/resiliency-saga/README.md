...
Eleven ordered `a2o` chapters narrating one continuous story of the Elohim Protocol's
...
| 6 | Blobs sync to one head | `@concern:saga-06-heads-converge` | served head matches declared head locally on alpha-A (2026-08-16 stage split: the whole-node `caughtUp=true` / `elohim_projection_reconcile_converged>=1` / `elohim_projection_heal_outcomes_total{outcome="healed"}>=1` gauges moved to the stage-1 leg — `scripts/ci/fleet-quiesce-gate.sh` gates pre-suite, `scripts/ci/post-deploy-saga-probe.sh` runs the converged/divergent honesty fence); served head matches on both federation doorways | Local-converged leg GREEN in steady state — races the same deploy-window restart as chapter 4 (`elohim_projection_reconcile_converged` reads 0 within minutes of a restart, then recovers). Cross-node one-head leg (needs `@requires:alpha-cluster-6peer`) still RED, pending B reaching `caughtUp=true` plus one `authorHeadOnce` declare cycle (a `[build:app]` push) — mechanism proven, documented in the same `self-heal-adam-projection-catchup-exhaustion-full-arc.md` backlog entry. The three ghost-witness sweep stations (chain-contention livelock, stale-anchor create collision, wall-clock-budget abandonment) are no longer `@wip` (de-@wip'd 2026-07-29): their counters — `elohim_content_witness_reauthor_failed_total` (labeled `class="chain_head_moved"` / `class="already_exists"`) and `elohim_content_witness_sweep_abandoned_total` — are wired in `elohim-storage/src/metrics.rs` and incremented at the wire sites in `p2p/projection_reconcile.rs::witness_ghost_anchors`; all three PASSED at the #1267 dataplane-validation report |
...
| 9 | Projector caches carry the head | `@concern:saga-09-projectors-carry` | `GET /api/v1/resilience/elohim-host-landing/household` `commitmentBackedReplication.commonsCommitments >= 1` | GREEN — stable across 3+ consecutive builds since 2026-07-29. The field rides the served `ResilienceSnapshotView` (the prior wiring gap is closed); `commonsCommitments` flips 0→1 the same moment chapter 5's mirror cure lands, which it now has. Re-aimed off `totalPledgedBytes` at the #1267 basis measurement, when that field stayed unreachable by design from a content-tier commitment (`replication_commitment.rs:182-194` pledges 0 bytes for `replicates-content`/`replicates-commons` content commitments; they're counted, not summed) and no capacity-tier pledge producer existed in the codebase. A producer has since landed (2026-07-31, see Documented residue below) and the chapter's assertion has been re-widened to pair `totalPledgedBytes >= 1` with `commonsCommitments`; that widened assertion has not yet been measured against a recorded dataplane-validation run, so this row's GREEN verdict still reflects the pre-widen finish line |
...
| 11 | The pull queue can finish | `@concern:saga-11-pull-queue-retires` | within 60s, the household's own pin (`elohim-host-landing`) reaches `caughtUp=true` via `/api/v1/pins/{id}/pull` (2026-08-16 stage split off whole-node `/p2p/status pull.caughtUp`, now gated pre-suite by `scripts/ci/fleet-quiesce-gate.sh`) + `elohim_acquisition_pins_retired >= 0` (the gauge that keeps a drained queue honest) + both `elohim_acquisition_pin_retirements_total{reason}` directions | Born with the cure (2026-07-31). alpha-A read 73 total / 3 fetched / 70 failed with `caughtUp` false for 12h+: no runtime path ever retired an unsatisfiable pin (the only status-flip caller was the HTTP DELETE route), so `fetched < total` was pinned permanently and the provide loop could never graduate this node to providing. The cure retires such pins so the queue drains, sizes the retry budget from the live peer count so retirement means actually-unsatisfiable, and re-admits on new inventory or a 6h cooldown — retirement is a HOLD, never a delete |
...
A capacity-tier pledge producer landed 2026-07-31 (commit 6103694f9): an
authenticated `POST /api/v1/commitments/capacity` route reuses the existing
Mishpat Commitment entry/coordinator/signal to author a `replicates-dwelling` /
commons-`capacity` commitment, mirrored per commitment into `rea_commitments`,
where the replication fold (`elohim-facings/src/folds/replication_commitment.rs`)
reads it into `commons_bytes`. The same landing also gave `distribute_shards`
self-placement a conductor-anchored deterministic custody-blob commitment
producer. Chapter 9's assertion has been re-widened accordingly to pair
`commonsCommitments >= 1` with `totalPledgedBytes >= 1` — but that widened
assertion has not yet been measured against a recorded dataplane-validation run;
the board's last recorded pass (report #1267 / 2026-07-30) predates this commit,
so chapter 9's status above still reflects the pre-widen finish line rather than
a green/red verdict on the new one.
