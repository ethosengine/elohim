...
| 11 | The pull queue can finish | `@concern:saga-11-pull-queue-retires` | `/p2p/status` `pull.caughtUp=true` + `elohim_acquisition_pins_retired >= 0` (the gauge that keeps a drained queue honest) + both `elohim_acquisition_pin_retirements_total{reason}` directions | Born with the cure (2026-07-31). alpha-A read 73 total / 3 fetched / 70 failed with `caughtUp` false for 12h+: no runtime path ever retired an unsatisfiable pin (the only status-flip caller was the HTTP DELETE route), so `fetched < total` was pinned permanently and the provide loop could never graduate this node to providing. The cure retires such pins so the queue drains, sizes the retry budget from the live peer count so retirement means actually-unsatisfiable, and re-admits on new inventory or a 6h cooldown — retirement is a HOLD, never a delete |
