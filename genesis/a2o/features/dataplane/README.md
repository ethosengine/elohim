...
| `trust-priced-sync` | A peer derives a CLASS for each edge (`unverified` / `public` / `community` / `trusted`) from what it can verify of the other peer's membership, relationship and replication-commitment records on the DHT; the class labels the sync-request counter and keys that edge's catch-up budget (fetch window, patience, backoff cap, admission weight) — trust widens, never gates. **RED-FIRST** — scenario 1 fails today because the trust handshake is a stub that prices every edge `public`; scenarios 2–5 are `@wip` until their stations land. Spec: `genesis/docs/superpowers/specs/2026-09-01-trust-priced-sync-edge-design.md`. |
| `accountable-correction` | Who may amend a record, how a witnessed correction (a `FeedbackSignal`) is discovered and applied once, and what a peer shows when it cannot know. Slice 1 of the reimplementation sequence: the root author alone may accept a correction (`create_vouch` accept-correction) and publish its successor (`amend_content` naming an explicit predecessor); a correction alone never debits standing; discovery is durable and unordered so a late arrival converges without a double-apply; two same-root Updates naming one predecessor leave a visible `contested` fork. **Born `@wip`, every station** — none of slice 1's coordinator surface exists yet. Contract: `genesis/docs/superpowers/specs/2026-09-06-accountable-correction-contract.md`. Env: `README-accountable-correction.md`. |
| `push-delivers-within-budget` | `app-delivery-refuses-fast.feature` (Act I, household-owned) — a deploy that meets a doorway not ready for a write is told so in seconds and waits no longer than it was told: the fleet write-readiness probe (`scripts/ci/fleet-write-readiness.sh`) names a shedding doorway by its origin and face while the other stays ready; a real conductor-restart window is reported by name and closes on its own; a shorter shed is waited out by one deploy, a longer one stops at its deadline naming the face and leaves the previous head. Faces are one vocabulary shared by the probe and the stations: `scripts/ci/lib/readiness-faces.json`. Habit: `genesis/orchestrator/.epr-meta/push-delivers-within-budget.habit.md`. |
