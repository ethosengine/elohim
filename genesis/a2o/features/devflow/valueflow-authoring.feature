...
# @act:host — the a2o act for scenarios that run on the developer's host
# against local tooling, with no mesh or fleet.
...
Feature: A task moves through one readable valueflow from claim to verdict
  ...
  Background:
    Given an isolated scratch repository with its own valueflow, never the live checkout
  ...
  #   verdict      — a reviewer's typed note; its value (approved or
  #                  changes-requested) is stored in a named field separate
  #                  from the note's prose, so it can be filtered and rendered
  ...
  #   equilibrium habit — the repository promise that commitments drain at
  #                  least as fast as they fill; a fulfilment is the drain it
  #                  measures, which is why a fulfilment must never be counted
  #                  twice.
  ...
  Scenario: Only a discharging report closes its claimed commitment
    Given two gap items are claimed as open commitments
