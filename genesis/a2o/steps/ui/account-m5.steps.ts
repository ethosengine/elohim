...
When(String.raw`I navigate to \/account\/security`, async function (this: E2EWorld) {
...
When(String.raw`I navigate to doorway\/account`, async function (this: E2EWorld) {
  const device = requirePlaywright(this);
  if (!device) return 'pending';
...
Then(String.raw`I am redirected to \/identity\/recover`, async function (this: E2EWorld) {
  const device = requirePlaywright(this);
  if (!device) return 'pending';
...
Then(
  String.raw`I land on \/account\/security authenticated as the same identity`,
  async function (this: E2EWorld) {
    const device = requirePlaywright(this);
    if (!device) return 'pending';
...
Then(
  String.raw`\/api\/v1\/account\/portal-hosts returns the host in the list`,
  async function (this: E2EWorld) {
    const doorwayUrl = requireDoorwayUrl(this);
    const token = getFirstAuthToken(this);
    const headers: Record<string, string> = {};
