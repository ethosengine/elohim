...
interface PathFixture {
  ...
  chapters: Array<{
    ...
    description: string;
    order: number;
    steps: Array<{ teachingPartner: string }>;
  }>;
}

function loadNegotiationFixture(): NegotiationFixture {
  const filePath = resolve(__dirname, '../../data/lamad/negotiations/negotiation-adam-eve-001.json');
...
Given(
  'Adam has attested {string} relationship to Eve',
  function (this: E2EWorld, relType: string) {
...
Given(
  'Adam has proposed a love map negotiation',
...
Given(
  'the love map path {string} exists',
...
When(
  'Adam tries to initiate a love map negotiation with Eve',
  function (this: E2EWorld) {
...
When(
  'Adam initiates a love map negotiation with Eve',
  function (this: E2EWorld) {
    const negotiation = loadNegotiationFixture();
    this.contentIds.set('negotiationId', negotiation.id);
...
When(
  'Adam selects the {string} bridging strategy',
  function (this: E2EWorld, strategy: string) {
    this.contentIds.set('bridgingStrategy', strategy);
  }
);

When(
  'Adam sends proposal message {string}',
  function (this: E2EWorld, _message: string) {
    this.contentIds.set('negotiationStatus', 'proposed');
  }
);

When(
  'Eve views the negotiation proposal',
  function (this: E2EWorld) {
    const negotiation = loadNegotiationFixture();
    this.contentIds.set('negotiationData', JSON.stringify(negotiation));
  }
...
When(
  'Eve accepts with strategy {string}',
  function (this: E2EWorld, strategy: string) {
    const negotiation = loadNegotiationFixture();
    assert.strictEqual(
...
When(
  'Eve revokes her {string} attestation to Adam',
  function (this: E2EWorld, _relType: string) {
...
Then(
  'the negotiation should not be created',
  function (this: E2EWorld) {
...
Then(
  'a message about requiring mutual attestation should appear',
  function (this: E2EWorld) {
...
Then(
  'the path content should not be visible',
  function (this: E2EWorld) {
...
Then(
  'an emergent path {string} should be generated',
  function (this: E2EWorld, pathTitle: string) {
    const negotiation = loadNegotiationFixture();
...
Then(
  'both Adam and Eve should be listed as participants',
  function (this: E2EWorld) {
    const pathData = loadPathFixture();
    assert.ok(
...
Then(
  'the path should become inaccessible to both Adam and Eve',
  function (this: E2EWorld) {
...
Then(
  'the negotiation status should be updated to reflect the change',
  function (this: E2EWorld) {
    const status = this.contentIds.get('negotiationStatus');
    assert.ok(
