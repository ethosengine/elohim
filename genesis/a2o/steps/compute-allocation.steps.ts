/* eslint-disable sonarjs/no-os-command-from-path -- testnet steps intentionally spawn shell processes */
...
import { isProvisionEnvelope, isSettleEnvelope } from '../src/framework/envelope-validator.js';
...
Given(
  'Matthew has a simulation requiring {int} peer nodes',
...
Given("{int} conductors are running for Matthew's simulation", async function (count: number) {
  if (!isTestnetActive()) {
    await startTestnet({
...
Then('the compute summary appears in the test report', function () {
  ...
  assert.ok(Object.keys(summary.perPersona).length > 0, 'Compute summary has no persona data');
});
...
When('that persona exceeds its budget', { timeout: 120_000 }, async function () {
...
Then('the remaining {int} conductors continue', function (count: number) {
  const status = execSync('bash elohim-node/simulation/spawn-persona-testnet.sh status 2>&1', {
    encoding: 'utf-8',
    timeout: 10_000,
...
Then(
  'a {string} commitment exists for Matthew with {int} cpu-seconds',
  async function (action: string, budget: number) {
...
Then(
  'a {string} commitment exists for each of the {int} personas',
  async function (action: string, count: number) {
...
Then('Matthew has a {string} EconomicEvent for the total', async function (action: string) {
  const res = await fetch(
    `http://localhost:8090/api/v1/economic-events?action=${action}&provider=matthew`
  );
  assert.ok(res.ok, `Failed to query events: ${res.status}`);
  const events = (await res.json()) as Record<string, unknown>[];
...
Then('each event fulfills its corresponding commitment', async function () {
  const res = await fetch('http://localhost:8090/api/v1/economic-events?action=deliver-service');
  const events = (await res.json()) as { fulfills?: string[] }[];
...
Then('each event has resourceQuantity in cpu-seconds', async function () {
  const res = await fetch('http://localhost:8090/api/v1/economic-events?action=deliver-service');
  const events = (await res.json()) as { resourceQuantityUnit?: string }[];
  for (const event of events) {
...
Then('each event has effortQuantity in megabytes', async function () {
  const res = await fetch('http://localhost:8090/api/v1/economic-events?action=deliver-service');
  const events = (await res.json()) as { effortQuantityUnit?: string }[];
  for (const event of events) {
...
Then('all persona commitments are marked {string}', async function (expectedState: string) {
  ...
  for (const c of commitments) {
    assert.equal(
      c.state,
      expectedState,
      `Commitment ${c.id} state is '${c.state}', expected '${expectedState}'`
    );
  }
});
