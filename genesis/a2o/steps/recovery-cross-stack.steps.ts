...
Given(
  ...
  function (this: E2EWorld, _table: DataTable) {
    // Pending plan-1: wire MultiStackFixture admin URLs into E2EWorld state.
...
Given(
  ...
  function (this: E2EWorld, _requiredWitnessCount: number) {
    // Pending plan-1: POST to Abby's node to create human record.
...
Given(
  ...
  function (this: E2EWorld) {
    // Pending plan-1: POST HumanRelationship on each share-holder's node.
...
Given('share-holder {word} is offline', function (this: E2EWorld, _name: string) {
  // Pending plan-1: call MultiStackFixture::take(name) to shut down the named NodeSlot.
  // Admin port: fixture.admin_port(name) — kills iroh + libp2p handles for that slot.
  return 'pending';
});

Given('Ben, Dan, Faye have each submitted submit_intimate_witness', function (this: E2EWorld) {
  // Pending plan-1: submit witness for Ben, Dan, Faye against the open recovery request.
...
When('Abby invokes create_recovery_request from a fresh agent key', function (this: E2EWorld) {
  // Pending plan-1: POST to Abby's node creating a recovery request.
...
When(
  ...
  function (this: E2EWorld, _a: string, _b: string, _c: string) {
    // Pending plan-1: POST to Abby's node to commit key rotation with IntimateQuorum.
...
When(
  ...
  function (this: E2EWorld, _name: string) {
    // Pending plan-1: POST the named share-holder's node to trigger self-revocation.
...
Then(
  ...
  function (this: E2EWorld, _expectedCount: number) {
    // Pending plan-1: GET /api/v1/account/pending-recovery (account.rs:386) from Abby's node.
...
Then('the rotation succeeds', function (this: E2EWorld) {
  // Pending plan-1: assert HTTP 200 from commit_key_rotation POST.
  return 'pending';
});
...
Then(
  ...
  function (this: E2EWorld, _transport: string) {
    // Pending plan-1: read per-node tracing ring-buffer from MultiStackFixture admin port.
...
Then(
  ...
  function (this: E2EWorld, _transport: string) {
    // Pending plan-1: read per-node tracing ring-buffer; assert count > 0 for given transport.
    return 'pending';
  }
);
...
Then(
  ...
  function (this: E2EWorld) {
    // Pending plan-1: assert Cara's GET /api/v1/account/pending-recovery returns empty list
...
Then(
  ...
  function (this: E2EWorld) {
    // Pending plan-1: GET /api/v1/account/pending-recovery; assert Ben's witness entry
...
Then(
  ...
  function (this: E2EWorld) {
    // Pending plan-1: POST commit_key_rotation; assert HTTP 4xx.
...
Then('the rejection reason references the revoked HumanityWitness rule', function (this: E2EWorld) {
  // Pending plan-1: assert response body contains reference to revocation rule
