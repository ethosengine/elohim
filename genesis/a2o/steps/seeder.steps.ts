 ...
 * a2o harness today (tracked under @wip — see TODO notes below).
 */
...
Given(
  'account packages exist for {string}',
  function (this: E2EWorld, _names: string) {
...
Given('a successful seed run completed', function (this: E2EWorld) {
  ...
  // TODO(a2o-framework): bring up an ephemeral doorway + storage fixture so
  // the seeder can be run twice and outcomes inspected.
  return 'pending';
});

Given(
  "Adam's account package declares spouse relationship with Eve",
...
Given(
  "Eve's account package declares spouse relationship with Adam",
  function (this: E2EWorld) {
...
Given(
  "Adam's account package has been imported successfully",
  function (this: E2EWorld) {
    // TODO(a2o-framework): needs a live storage service fixture the harness
...
Given('a clean database', function (this: E2EWorld) {
  // TODO(a2o-framework): needs a storage-service fixture with a reset endpoint
...
When(
  'the seeder runs against {string}',
  function (this: E2EWorld, doorwayUrl: string) {
    const s = state(this);
    assert.ok(s.registryPath, 'registry not initialised — add a "Given the deployment registry …" step');
    s.lastResult = runSeeder([
      `--registry=${s.registryPath}`,
      `--doorway-url=${doorwayUrl}`,
    ]);
  },
);
...
When(
  'the seeder runs with {string}',
  function (this: E2EWorld, flag: string) {
    const s = state(this);
...
When(
  'the seeder runs a second time with unchanged packages',
  function (this: E2EWorld) {
...
When(
  'both packages are imported in sequence',
  function (this: E2EWorld) {
    // TODO(a2o-framework): direct /account/import invocation against a
...
When(
  "Adam's account package is imported a second time",
  function (this: E2EWorld) {
    return 'pending';
  },
);
...
When(
  "Adam's account package is imported",
  function (this: E2EWorld) {
    return 'pending';
  },
);
...
When(
  "Eve's account package is imported",
  function (this: E2EWorld) {
    return 'pending';
  },
);
...
Then('the seeder marks {string} as staged', function (this: E2EWorld, namesCsv: string) {
  ...
  for (const n of names) {
    ...
    assert.ok(
      pattern.test(stdout),
      `expected seeder to mark "${n}" as staged, got:\n${stdout}`,
    );
  }
});
...
Then(
  'the seeder emits warning {string}',
  function (this: E2EWorld, expectedWarning: string) {
...
Then(
  'all deployed humans report outcome {string}',
  function (this: E2EWorld, _outcome: string) {
    // TODO(a2o-framework): requires the seeder to have actually run against a
...
Then(
  'no package reports outcome {string}',
  function (this: E2EWorld, _outcome: string) {
    return 'pending';
  },
);
...
Then(
  'exactly one human_relationships row exists for the pair',
  function (this: E2EWorld) {
    // TODO(a2o-framework): needs SQLite inspection against the scenario
