...
// =============================================================================
// Constants Sync: lamad-generated domain types across app and seeder
//
// Validates that the files generated by `pnpm run lamad:codegen` are
// identical between app/lamad/src/app/generated/ and
// genesis/seeder/src/generated/. content-node-types.ts has different imports
// (app uses ../../generated/, seeder uses ./) so we compare non-import lines.
//
// Path note: lamad was split out of elohim-app into its own SPA bundle
// (pillar-EPR decomposition B19, merge 0b36155e9). Codegen target moved
// from app/elohim-app/src/app/lamad/generated/ to app/lamad/src/app/generated/.
// =============================================================================

describe('Constants Sync: lamad-generated domain types (app ↔ seeder)', () => {
  const APP_LAMAD_GENERATED = path.join(
    REPO_ROOT,
    'app',
    'lamad',
    'src',
  ...
  const SEEDER_GEN = path.join(REPO_ROOT, 'genesis', 'seeder', 'src', 'generated');

  // Files that should be byte-identical between app and seeder
  ...
  for (const filename of IDENTICAL_FILES) {
    it(`${filename} should be identical between app and seeder`, () => {
      const appPath = path.join(APP_LAMAD_GENERATED, filename);
      const seederPath = path.join(SEEDER_GEN, filename);

      if (!fs.existsSync(appPath)) {
        throw new Error(
          `App-side generated file not found at ${appPath}. Run: pnpm run lamad:codegen`,
        );
      }
      if (!fs.existsSync(seederPath)) {
        throw new Error(
          `Seeder-side generated file not found at ${seederPath}. Run: pnpm run lamad:codegen`,
        );
      }
  ...
  it('content-node-types.ts should have identical type definitions (imports differ)', () => {
    const appPath = path.join(APP_LAMAD_GENERATED, 'content-node-types.ts');
    const seederPath = path.join(SEEDER_GEN, 'content-node-types.ts');

    if (!fs.existsSync(appPath) || !fs.existsSync(seederPath)) {
      throw new Error(
        'content-node-types.ts not found. Run: pnpm run lamad:codegen',
      );
    }
    ...
    const appContent = stripImports(fs.readFileSync(appPath, 'utf8'));
    const seederContent = stripImports(fs.readFileSync(seederPath, 'utf8'));
    expect(appContent).toEqual(seederContent);
  });
});
