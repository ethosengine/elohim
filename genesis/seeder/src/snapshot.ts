...
// Configuration
const LOCAL_DEV_DIR =
  process.env.LOCAL_DEV_DIR || "/projects/elohim/elohim/holochain/local-dev";
...
const HAPP_PATH =
  "/projects/elohim/elohim/holochain/dna/elohim/workdir/elohim.happ";
const APP_ID = "elohim";
...
async function createSnapshot(name?: string): Promise<void> {
  ...
  const conductorProc = spawn(
    "hc",
    ["sandbox", "run", "--root", CONDUCTOR_DATA_DIR, "-p=4445"],
    {
  ...
  await new Promise<void>((resolve) => {
    ...
    conductorProc.stdout?.on("data", (data) => {
      ...
      const match =
        output.match(/admin_port[=:\s]+(\d+)/i) ||
        output.match(/Conductor admin interface running at.*:(\d+)/i);
      if (match) {
        adminPort = parseInt(match[1], 10);
      }
      if (
        output.includes("Conductor ready") ||
  ...
  const seedResult = await runCommand("npx", ["tsx", "src/seed.ts"], {
    cwd: "/projects/elohim/genesis/seeder",
  });
...
async function restoreSnapshot(name?: string): Promise<void> {
  ...
  if (fs.existsSync(metadataPath)) {
    ...
    console.log(
      `   Contains: ${metadata?.conceptCount} concepts, ${metadata?.pathCount} paths`,
    );
  }
  ...
  if (restoredSize < 100 * 1024) {
    ...
    console.log(
      "      just dev start                    # Start fresh conductor",
    );
    console.log("      just seed apply local content      # Seed content");
    console.log("      npm run snapshot:save  # Save the seeded state");
  } else {
    console.log("\n   ✅ Snapshot contains data");
    if (metadata) {
      console.log(
        `   Content: ${metadata.conceptCount} concepts, ${metadata.pathCount} paths`,
      );
      if (metadata.seedDuration) {
        console.log(`   Original seed time: ${metadata.seedDuration}`);
      }
      if (
        metadata.seedReport?.skippedFiles?.total &&
        metadata.seedReport.skippedFiles.total > 0
      ) {
        console.log(
          `   Skipped files: ${metadata.seedReport.skippedFiles.total}`,
        );
      }
    }
    console.log("\n   To start conductor: pnpm run hc:start:snapshot");
  }
}
  ...
  if (snapshots.length === 0) {
    ...
    for (const snap of snapshots) {
      ...
      if (snap.metadata) {
        ...
        console.log(
          `      Content: ${snap.metadata.conceptCount} concepts, ${snap.metadata.pathCount} paths`,
        );
        if (
          snap.metadata.seedDuration &&
          snap.metadata.seedDuration !== "N/A"
        ) {
        ...
        if (snap.metadata.seedReport?.results) {
          ...
          if (r.conceptErrors > 0 || r.pathErrors > 0) {
            console.log(
              `      Errors: ${r.conceptErrors} concept, ${r.pathErrors} path`,
            );
          }
        }
        if (
          snap.metadata.seedReport?.skippedFiles?.total &&
          snap.metadata.seedReport.skippedFiles.total > 0
        ) {
          console.log(
            `      Skipped: ${snap.metadata.seedReport.skippedFiles.total} files`,
          );
        }
      }
    }
...
async function saveSnapshot(name?: string): Promise<void> {
  ...
  if (!fs.existsSync(CONDUCTOR_DATA_DIR)) {
    ...
    console.error(
      "   Run hc:start first, or use snapshot:create for fresh seed",
    );
    process.exit(1);
  }
  ...
  if (dataSize < 100 * 1024) {
    ...
    console.log("   Make sure you ran: just seed apply local content");
    console.log("\n   Continuing anyway...");
  }
  ...
  if (fs.existsSync(SEED_REPORT_PATH)) {
    try {
      ...
      console.log(
        `      Concepts: ${conceptCount} created, ${seedReport?.results?.conceptErrors || 0} errors`,
      );
      console.log(
        `      Paths: ${pathCount} created, ${seedReport?.results?.pathErrors || 0} errors`,
      );
      console.log(`      Duration: ${seedDuration}`);

      if (
        seedReport?.skippedFiles?.total &&
        seedReport.skippedFiles.total > 0
      ) {
    ...
    console.log(
      "      Run just seed apply local content first to generate the report",
    );
  }
  ...
  console.log(
    `   Compression: ${((1 - snapshotSize / dataSize) * 100).toFixed(1)}%`,
  );
  console.log("\n   To restore: npm run snapshot:restore");
}
...
Typical workflow:
  1. just dev start                    # Start fresh conductor
  2. just seed apply local content     # Seed with content
  3. npm run snapshot:save   # Save the seeded state  <-- YOU ARE HERE

  Later:
  4. npm run snapshot:restore  # Restore saved state
  5. pnpm run hc:start:snapshot        # Start from restored state
