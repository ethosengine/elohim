...
This is fine for a CI gate (`saga-sync.sh` calls it once per pipeline run and tolerates the
...
whatever dev-tool/CI path currently `cargo run`s it debug (this repo's own `saga-sync.sh` included —
