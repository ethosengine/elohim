...
## Root cause + resolution of recurrence 3 (operator incident report, 2026-08-28)

Not a rotated key. **Garage's LMDB metadata emptied at 2026-08-23 ~01:50Z**: `garage status` HEALTHY, layout intact (300G/dc1, node id preserved — `cluster_layout` and `node_key` are separate files from `db.lmdb`), every table in `garage stats` reading 0. Against an empty metadata view every block is unreferenced, so the resync worker reaped all of it (`garage_block::resync: Deleting unneeded block … offload finished (0 / 0)`) — the data loss was the cascade. Contributing (strong, unproven): `limits.memory: 1Gi`, working set 688→825 MB over the preceding hours, termination `reason: Unknown` (not a clean OOMKilled); LMDB is mmap-backed so DB pages count against the memcg — the same 1Gi-thrash shape as Loki's. It looked days late because Tempo/Pyroscope read S3 only at init and ran on until their own restart.

Resolution 2026-08-28: all three buckets re-provisioned (`sccache-elohim`, `tempo`, `pyroscope`), three scoped RWO keys minted, five secrets updated (Che auto-mount labels verified). The bootstrap Job covers only sccache — run alone it would have left Tempo/Pyroscope broken. Unrelated kubelet orphan local-volume wedge found and lazy-unmounted (runbook path). **Left open, operator-owned:** the 1Gi limit (raise to ~4Gi, bounce correlated with the wipe — timing is the operator's call); `garage-rpc-secret` SecretSyncedError for 49 days (OpenBao almost certainly sealed — no rotation possible). Loki uses `object_store: filesystem`, not Garage — only Tempo, Pyroscope and sccache consume Garage.

Repo side, landed this cycle: the flake shellHook probes `sccache rustc -vV` and cold-compiles instead of failing (53e662caa; proven on holochain #1405). Front 2 of this entry ("retry-without-sccache") is therefore DONE for the DNA lane; front 1 (a clean cache-miss on NoSuchKey) is the same class as "LMDB wipe → 404 sentinel leak" and stays open.
