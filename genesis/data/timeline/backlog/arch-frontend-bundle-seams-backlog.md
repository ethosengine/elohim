...
| 11 | **lamad's production build ships the dev environment, and the leak is the bundle seam again.** `app/lamad/angular.json` declares no `fileReplacements` in any configuration, so the deployed bundle carries `src/environments/environment.ts` verbatim (`production: false`, `logLevel: 'debug'`, `gitHash: 'local-dev'`, `client.storageUrl: http://localhost:8090`). Compounding: lamad's injected services are elohim-app's (`app.config.ts:165` provides `LAMAD_STORAGE_CLIENT` with `StorageClientService`), and those import **elohim-app's** environment by relative path (`storage-client.service.ts:27` → `../../../environments/environment`) — elohim-app's `fileReplacements` live in elohim-app's `angular.json`, which lamad's build never consults, so the deployed lamad bundle also carries elohim-app's dev env (`useLocalProxy: true`, `connectionMode: 'auto'`, `adminUrl: wss://doorway-alpha…`). Found 2026-09-11 by the doorway-federation sprint Task 18b (the serving defect it surfaced — SSR baking `localhost:8090` blob URLs — is cured independently at runtime, e09eec608, and its general cure is the standalone atom `ssr-connection-mode-conflates-node-with-native-2026-09-11`). | Architect decision | (a) lamad gets its own `fileReplacements` + per-env files AND the cross-workspace services stop importing elohim-app's environment by relative path, or (b) the shared environment is extracted into a package both bundles consume — the `TODO(B18c-followup)` in lamad's environment file already anticipates (b), which is the seam-correct shape; (a) alone leaves the relative import leaking. Either way a served bundle must not say `production: false`. Probe: `curl -s https://alpha.elohim.host/lamad/ \| grep -c 'local-dev'`. |
