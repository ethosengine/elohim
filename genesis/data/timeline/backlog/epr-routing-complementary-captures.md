...
## Commitment-system reconciliation captures (2026-06-08, Slice-2a brainstorm)

- **History-record-worthy: "why both `create_commitment` (mishpat) AND `create_rea_commitment` (content_store) exist"** — the two-commitment-writer split is canonical-by-pattern but NOT yet canonized-as-decision (historian: palace carries it pattern-only). When Slice 2a/2b lands the Mishpat-commitment projection + ProjectionCommitmentFetcher, write the `chronicle/` record: Mishpat=policy-envelope/compute-bounds substrate primitive; content_store=REA/VF economic fact; one event references both (`bounded_by`→Mishpat, `fulfills`→content_store). Domain D9. Compose-target: `rea-compute-substrate-native-roadmap`.
- **deprecation-sentinel doc-grep FP class** (process-meta, ci/hooks subdomain) — the sentinel matches the literal token `deprecated` inside `.md`/source prose captured by read-only grep/cat (2 FPs so far: `802862c393b2` DEAD_WORDS const, `a890d56fab81` roadmap prose). Gate capture on originating-command class (build/test/lint vs read-only grep/cat over docs) to stop the family. Flagged by deprecation-triage 2026-06-08.
