...
- **`LamadNotFoundComponent` → designed gate experience** — upgrade the lamad `**` page to the §6 outward-face pattern (preview + hints); gated on the gate-face follow-up plan (gap `#6-2` of the 2026-06-06 Slice-3 spec, `epr-route-claims-link-conformance-design`), which carries the §6 gate-face UI explicitly deferred out of the Slice-3 implementation.
