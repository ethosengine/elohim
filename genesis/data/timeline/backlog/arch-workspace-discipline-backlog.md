...
| 11 | **canon-lift is blind to half the policy registry** | Found by audit 2026-08-05. `.claude/epr-meta/generated/canon-lift/precedent-standing.json` still carries `sovereignty-ontology-guard@1` as `status: active` / `superseded_by: null` after the `@2` bump, and **`ownership-ontology-guard` was never canon-lifted at all — neither version is present.** So the canon-lift view of governance is missing a whole policy and mis-reports the supersession lineage of another. Generated artifact (epr-cli `canon_lift`), so the fix is in the generator or its inputs, not the JSON. Not a gate defect — the live resolver binds `@2` correctly — but canon-lift is precisely the surface that should answer "what governs this repo," and today it answers wrong. | Owned by whoever is mid-change on `epr-cli/src/canon_lift/` | rust-architect |
