...
| 8 | **Witnessed-harm limit — T&S sight as a bounded, highest-Mishpat stewardship role** | [witnessed-harm survey](epr:witnessed-harm-limit-research-2026-08-09) + blind-custody design seed OQ6 (operator note 2026-08-09): privacy is qualified, not absolute — CSAM (strict-liability contraband) and adjacent threshold classes (violent extremism, NCII, imminent threat-to-life, trafficking; each on different possession/identifiability/latency/counter-duty axes) need a designed answer BEFORE the private layer ships. The surveyed grain is compatible with blind custody: duty-on-knowledge not duty-to-scan (§2258A shape), hash-ID at the authoring edge where plaintext exists (PhotoDNA/StopNCII shape), franking-style accountable witnessed reporting — never unsealing custody. Frontier gap this protocol would be first to formalize: agent exposure/memory (classifier-first so agents see verdicts; compelled sight quarantined with no memory formation; disposal as governed ceremony). Only contraband-per-se + known-item admits automated non-sight adjudication; every other class needs bounded witnessed judgment. | council decision (not engineering): T&S role commitment shape, list governance, reporting interfaces (NCMEC/INHOPE), evidence-preservation counter-duty (Berkeley Protocol). Position deliberately TBD — the survey is the evidence base | council + p2p-design-gate when any mechanism is proposed |

| 9 | **Swarm-curve + blind-custody design (2026-08-23) — decisions closed for #3, #5, #6; new bounded rows** | [spec](epr:swarm-curve-and-blind-custody-design), red-teamed by three lenses (36 findings folded, §12). **#5 decided:** dual-key — storage-minted X25519 reader key, notarized by reusing `attestation:key-stewardship` (metadata `reader_pk`; DNA-hash-neutral — integrity validates subtype membership/expiry/supersedes only), authority = the conductor-signed record, not DHT residency (Floor 2 is accept-all). **#3 decided:** KeyEnvelope = a bound, signed, unlabeled key ring that travels with the ciphertext (B2 bytes), epoch-superseded, with a MANDATORY floor envelope (sealed-against-self, mishpat ⊗ imagodei) — no Shamir shares of the DEK pre-placed (that would bypass the per-release `recovery-approval` gate). **#6 path:** `StorableBytes::{Public, Ciphertext}` typed at `BlobStore::store` AND the iroh `add_bytes` (`PUT /blob` dual-writes the plaintext composite into the iroh `FsStore` today). Stated limits: revocation recalls nothing replicated; replica map is public at manifest granularity; reader *count* visible to custodians. | #1 (fail-closed) is a precondition of C6-c; the doorway blob pantry re-serving any 200 body ungated is a live hole on the serve path → standalone atomic row `security-doorway-blob-pantry-ungated` | rust-architect (C6-a/b/c), Sonnet (C6-d actuator wiring); issuing a quarantine/refuse verdict stays council-gated (#8) |
