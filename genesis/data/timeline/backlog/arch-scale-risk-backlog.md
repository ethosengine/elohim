...
| 7 | **A hosted human costs a full hApp install on the pool conductor — ~786 MB of conductor heap each, measured.** Real hosted provisioning (live since 60fb28a39 retired the dev_mode singleton path) installs the whole bundle per registrant: 5 DNAs → 5 cells; on the household mesh 29 hosted personas took matthew's conductor from 1.1 GB to 22.8 GB `RssAnon` (145 cells over a 693 MB corpus ≈ 157 MB/cell) and the workspace RAM guard shed the a2o lane. Until 2026-09-10 every hosted persona was free because none was ever provisioned. Compounded by the doorway registry double-counting `capacity_used` (fleet ceiling 50 ≈ 25 real cells). | `doorway/doorway-service/src/conductor/provisioner.rs` (installs the full bundle per agent); `elohim/holochain/dna/elohim/workdir/happ.yaml` (5 roles); `main.rs:467-474` (`DOORWAY_MAX_AGENTS_PER_CONDUCTOR`) | Any doorway pool: RAM ≈ hosted humans × roles × per-cell cost (~157 MB at genesis corpus size; grows with corpus at full arc). Measure: `grep RssAnon /proc/<conductor>/status` after `just mesh prologue`; alpha conductor-diagnostics after N hosted registrations. | **Now** — the household mesh cannot carry its own standing cast; alpha's pool at 25 real cells is ~20 GB | high / high | (a) hosted humans get a LIGHT app — imagodei + the role the app they arrived through needs, other roles provisioned on first write (holons-are-spaces: a cell exists when there is something to write); (b) arc policy for hosted cells (spoke arc, not full arc); (c) the cast allow-list on the mesh (sprint 2026-09-10 Task 17b) as the interim → doorway pool provisioning design; conductor-capacity-represented habit gains a per-cell heap measure | open — found 2026-09-11 by the doorway-federation sprint's first real hosted run |
