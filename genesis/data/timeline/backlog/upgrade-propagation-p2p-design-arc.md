...
| coordinator zome — node_registry `export_records`, epic Task 1 (mesh, 2026-09-04) | 2-4 h | **DELIVERED, fix round 1.** The candidate (unpacked installed bundle + swapped-in `node_registry_coordinator.wasm`) was correct all along — `happ_manager::bundle_dna_hashes` on it resolves node_registry to the LIVE installed hash `uhC0kNpWca3kckKu8YbNFbYT2Dva296lTxdRjcFS4_YLk6ZS12x1K` for every role; the earlier "matches none of 173 `.dna` files" finding was a false alarm from comparing `hc dna hash` on a BARE `.dna` (no happ-role-modifier folding) against the conductor's installed hash (which folds the happ.yaml role's `modifiers.properties`) — repacking with `integrity.properties: {progenitor_pubkey: null}` (the exact shape happ.yaml declares) reproduces the installed hash exactly, confirmed both by direct repack and by the committed diagnostic `happ_manager::tests::probe_bundle_dna_hashes`. The channel was wedged at an unattestable earned head (fix round 1, see below); recovered via `release-ceremony.ts revert` (mishpat's real "supersede-in-place" verb, `--attestation-threshold 0`, `--lineage-parent <immediate chain predecessor>` — a `lineage_parent_mismatch` refusal on the first two revert attempts named the exact required parent CID each time, since each `revert` call itself adds a chain entry). Final revert declared at 03:14:38Z; **revert→3/3 applied in 49 s** (matthew t+32s, jessica t+34s, james/canary t+49s); `GET /version` per peer confirms `node_registry` coordinator wasm hash changed `uhCok825ERx6…→uhCok1d81AQi…` on all three, `node_registry` DNA hash and every other role (lamad/infrastructure/mishpat/imagodei) byte-identical before/after. Channel configs (james=canary, matthew/jessica=apply) left registered per the a2o fixture's expectation — not reset to empty. |
| coordinator zome — mishpat lineage arms, epic Task 2 (mesh, 2026-09-04/05) | 2-4 h | **DELIVERED 3/3** on a fresh channel `runtime:coordinators:elohim:epic-t2-20260904035403` (the driver refuses `publish` over an earned head — rung-5 seam filed). james (canary): publish → applied → attested → promoted, the full rung-5 order. matthew/jessica then wedged `threshold_unmet`: the manifest carried the packager DEFAULT attestationThreshold 2 and a 3-peer household has one attester archetype → RULE: household manifests use `--attestation-threshold 1`. Recovery = threshold-0 revert of the same candidate (head `uhCkktI48…`, 05:10:58Z); matthew+jessica converged on the next sweeps; `/version` ×3: mishpat coordinator `uhCokfEmz…→uhCokomHU…`, every other role byte-identical, node_registry still on Task 1's `uhCok1d81…`. Two deliveries, two wedges, two lessons — both filed. |
| coordinator zome — content_store staging-candidate election (shift c3, mesh 2026-09-04) | 2-4 h | **DELIVERED 3/3** on channel `runtime:coordinators:elohim:c3-20260904t062743z`, discipline 30/1/[james] explicit. publish 06:30:14Z → 3/3 staged +68 s → james (canary) applied +139 s → attestation authored +164 s (qualifying 1/1 at +230 s) → promote +248 s → matthew/jessica applied +297/+300 s: publish→3/3 earned-applied 300 s; conductor PIDs unchanged; every other role byte-identical. Blob-lag trap fired on BOTH followers (`artifact_unavailable`, two operator-side `PUT /blob` pushes needed) — the finding that drove the peer-pull cure (storage `35f0746ad` + `c8930c2f2`). |
| coordinator zome — **rung 5 LEAVES THE HOUSE: workspace→ALPHA FLEET, first canary apply (2026-09-06)** | 2-4 h (image bake + pod roll; and the same-wave edge bake had shipped the PREVIOUS mishpat happ, so the fleet ran pre-#1b-b `uhCokfb6q…` while `dev` carried post-#1b-b) | **DELIVERED on a deployed peer, no pipeline in the path.** Release it8 `uhCkkaqtOU_V…` (coordinator-bundle, channel `runtime:coordinators:elohim:workspace`, cut FOR the fleet: mishpat binding = the wasm every alpha peer self-reported) published from the workspace peer 14:24:34Z → matthew+jessica verify `ok` +29 s → **james (canary) hot-swap `sync_coordinators` +30 s (14:25:04Z), alreadyCurrent false** → james's soak attestation readable through the workspace conductor +303 s (14:29:37Z: linked 1, qualifying 1/1). Publish→attested ≈ 5 min vs 2-4 h. Read entirely through `doorway-alpha /db/p2p/adoption?peer=all`; no pod access, no roll. Two typed refusals became two driver cures on the way: `artifact_unavailable` = wire address form (CIDv1 on the wire, `sha256-` key on disk → honest NotFound from every holder; `shard_service::on_disk_key` + `artifact_pull::wire_address`) and `coordinator_lineage_mismatch` = `appliesTo` read from the BUILDER's passport (cure: `--applies-to-from-adoption`, reading the target's `installedReality`). Promotion deliberately NOT declared: no apply-mode peer exists to move, and the steward's workspace cannot adopt a release cut for the fleet (seam filed: `runtime-steward-adoption-of-fleet-cut-release.md`). Release 1 of the retire-when's three. |
| coordinator zome — **workspace→ALPHA FLEET, release 2 of three: lamad-only ROLLBACK by election (2026-09-06)** | 2-4 h | **DELIVERED, single-role.** Release `uhCkkV8l1w…` (coordinator-bundle carrying ONLY `lamad`, coordinator bytes = the fleet baseline `uhCokBbQH…`, cut FOR james via `--applies-to-from-adoption ?peer=james --applies-to-role lamad`, lineage parent it8) published 19:06:58Z → james (canary) pulled the artifact from a peer in 640 ms and hot-swapped lamad `uhCokL5-8…→uhCokBbQH…` at 19:07:52Z (+54 s; `drifted=1, applied=1`) → soak attestation authored 19:08:24Z (+86 s), qualifying 1/1 through the workspace conductor. Every observer running the target read the by-bytes exit (`ok`, already current) — the canary rejoined the fleet line through the channel, no pod hand. Why a rollback was release 2: release 1's artifact was a 5-role .happ rebuilt from the workspace tree, and james's 14:25Z apply logged `drifted=5, applied=1` — lamad hot-swapped, the other four roles REFUSED on DNA lineage (workspace toolchain moves integrity bytes) — a mixed peer no later 5-role release could verify (`coordinator_lineage_mismatch` ×5 sweeps after edge #1438 rolled the pods; the adopted lamad SURVIVED the roll, the controller's in-memory `appliedRelease` did not). Cures landed tonight: single-role candidates (`coordinator-candidate.ts --role --rollback-to-baseline`, packager refuses an unscoped multi-role appliesTo), verify refuses an artifact whose DNA line contradicts installed reality BEFORE any role is swapped, a partial hot-swap is a refusal, every divergent role named in one refusal, boot-sweep per-role logs. |
| coordinator zome — **workspace→ALPHA FLEET, release 3 of three: lamad-only FORWARD marker (2026-09-06)** | 2-4 h | **DELIVERED, single-role, cut for the whole fleet.** Release `uhCkkd02F1…` (lamad-only, marker `workspace-release-3-…`, `--applies-to-from-adoption ?peer=matthew --applies-to-role lamad`, parent release 2) published 19:16:45Z → james applied `sync_coordinators` at 19:17:53Z (+68 s) → attestation qualifying 1/1 readable through the workspace conductor by 19:21Z; matthew/adam/jessica/susan verified `ok` within two sweeps. **Three coordinator releases reached the alpha fleet through their release channel today (14:24Z, 19:07Z, 19:17Z), each applied by the canary and attested, no pipeline in any delivery path** (edge #1438 rolled the pods BETWEEN releases 1 and 2 for the serve-side fix — a roll, not a delivery). Not yet: promotion + observer convergence (six peers follow at `observe`, so an earned head moves nobody — the =observe→apply data flip is the next deliberate change); gertrude and eve still resolved release 1's head 15 min after release 3 (election read lag on two peers — seam filed in the habit atom). |
