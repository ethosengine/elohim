...
| coordinator zome — node_registry `export_records`, epic Task 1 (mesh, 2026-09-04) | 2-4 h | **BLOCKED, not delivered.** publish→staged ≤5 s (3/3, `sha256-c200b41…` PUT+replicated to all peers); promote→earned 1 s — but promote was called BEFORE any canary apply, wedging the channel's attestation threshold (2 required, only reachable via a prior successful apply — `(Canary, Earned, threshold_met=false) ⇒ Refused` per `watch.rs`'s `decide_post_verify_action`; publishing a fresh staging candidate over an already-earned head is itself refused, so THIS channel-id cannot recover without an `adopt`/`resolve` verb not exposed by `release-ceremony.ts`). Independently structural: the live mesh's installed `node_registry` DNA hash (`uhC0kNpWca3kckKu8YbNFbYT2Dva296lTxdRjcFS4_YLk6ZS12x1K`, confirmed identical on matthew/jessica/james via `/version`) matches NONE of the 173 `.dna` files found anywhere in-tree (including the exact bundle path this task was pointed at) — so no coordinator-only candidate buildable from disk can pass `happ_manager::lineage_mismatch_error`; the apply vehicle would refuse with `dnaHashMismatch: bundle carries a different DNA lineage (integrity change) — hot-swap refused; this needs the reinstall/migration path (installed=uhC0kNpWca3…, bundle=uhC0kyvKwO2…)` even with correct promote ordering. Runtime-config channel watches reverted to empty on all three peers afterward; conductors and DNA untouched. Coordinator-level work (extern + tests + hash-neutral pack) is proven green in sweettest; only the rung-5 mesh leg is outstanding — retry needs (a) canary-before-promote ordering and (b) locating or re-deriving the live-installed node_registry integrity bytes (or a fresh consented reinstall) before a hash-matching candidate can be minted. |
