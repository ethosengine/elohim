...
| 12 | **Declare `kind: level \| rate \| ratio` (+ `per` period for rates) on a Measure family** | [Meadows](epr:meadows-systems-dynamics-cross-pollination-2026-08-11) §3.1 — *"Stocks describe the state of the system at any particular time… Flows are the inputs or outputs (measured per time unit)"* (Indicators p.28). We model both and **declare neither**, so nothing catches a limit written against the wrong one. The live proof is `spatial.rs:169` (`max_sustainable_yield` — a rate, with a `unit: String` carrying no time denominator) compared at `spatial_capacity.rs:150` to an all-time cumulative sum. **Prerequisite for rows 13–15**, exactly as row 1 is for the rest of the cluster | **Landed 2026-08-11** — `MeasureKind::{Level, Rate{per}, Ratio}` (`elohim/epr/src/measure.rs:25-29`), tested `elohim/epr/tests/measure_ontology.rs` + `measure_dimensional.rs` | rust-architect / small |
| 13 | **Harvest/regeneration and emission/absorption as first-class family members** — index > 1.0 = unsustainable, as a **leading** signal | [Meadows](epr:meadows-systems-dynamics-cross-pollination-2026-08-11) §3.2–3.3 — *"the essential measure of sustainable use of a renewable resource… If the index is above 1.0, the harvest is not sustainable"*, and *"Deforestation is indicated not when the forest is gone, but when the rate of harvest first exceeds the rate of regrowth"* (Indicators p.29). **We have no inflow term anywhere** — `grep -rn "regenerat" --include=*.rs` over `elohim/ doorway/ steward/ crates/` returns only codegen comments (verified 2026-08-11). The sink half is the same shape and is the one nobody models: an emission is only sustainable against an absorption process. Fires long before a stock is visibly depleted, which is what makes it worth having | **Landed (partial) 2026-08-11.** `Stock::harvest_regeneration()` / `Stock::emission_absorption()` (`elohim/epr-rea/src/stock.rs:199-212`, the exact reciprocal shapes this row names) are real, dimensionally-refusing constructors — tested by `elohim/epr-rea/tests/stock_dynamics.rs::the_sink_index_and_the_source_index_are_reciprocals_and_naming_them_apart_is_the_point` (asserts `sink.value == 2.5`, `source.value == 0.4`, `sink.kind == Ratio`, and `sink × source == 1.0`) and `::a_stock_with_no_outflow_reports_unbounded_turnover_as_absence_not_infinity`. **What did NOT land: the `> 1.0 = unsustainable` threshold semantics.** No `is_unsustainable()` predicate or threshold constant exists anywhere in `stock.rs` — the `>1.0` reading is doc-comment narrative only (`stock.rs:24-27,193,203`); the one place it is actually *applied* is `habits.yaml`'s 2026-08-11 delta (commit `100c433f7`, doc corpus 90d 320/98 → 3.27, "confirmed overshoot"), which is a manual narrative application in a Python script (`doc_dynamics.py`), not a pinned Rust construct or test. So: the ratio-as-family-member half is landed and tested; the leading-signal threshold is not encoded anywhere — still design/convention-only | rust-architect |
| 14 | **Turnover time and coverage time as derived quantities** — stock ÷ change-rate, and stock ÷ drain-rate | [Meadows](epr:meadows-systems-dynamics-cross-pollination-2026-08-11) §3.9 (Indicators p.29). Trivial arithmetic once row 12 exists, and the bridge from a level to a *response-rate* estimate — *"the size and lifetimes of stocks can give us useful indicators of response rates."* Her caveat carries into the design: coverage at steady draw and coverage at exponentially rising draw are different numbers and must render as such | **Landed (turnover time only) 2026-08-11.** `Stock::turnover_time()` (`elohim/epr-rea/src/stock.rs:189-191`, `level ÷ outflow`) — tested `stock_dynamics.rs::turnover_time_is_a_time_and_says_how_long_the_corpus_takes_to_replace_itself` (300 docs ÷ 4/week = 75) and `::a_stock_with_no_outflow_reports_unbounded_turnover_as_absence_not_infinity` (honest-absence, not `+∞`). `MeasureKind::divide`'s `Level ÷ Rate → Level` rule (`elohim/epr/src/measure.rs:74-75`) names this row's *arithmetic* generically as "turnover and coverage time" — one shape covers both — but **no distinct `coverage_time()` constructor exists**, and the row's own caveat (steady vs. exponentially-rising draw give different numbers) is not modelled: nothing in `stock.rs` distinguishes a draw regime. Also carries spec Q15 (open, `measure-dynamics-confidence-ontology-design.md` §3): the result's period survives only in the caller's `basis` string, never in the type — a turnover of `43` is 43 weeks or 43 years depending on a divisor the type no longer holds | rust-architect / small |
| 15 | **An aggregation-facing measure — the fold that carries a global query down and an anonymized answer up** | [Meadows](epr:meadows-systems-dynamics-cross-pollination-2026-08-11) §7, the survey's largest take. A global fan-out (*"what is the total estimated result of your carbon measures?"*) is answered locally under whichever Mishpat lens is contextually correct, then folded upward via `select → fold → aggregate` on the **projection layer** — no global clock needed, because aggregation is not synchronization. The measure half is here; the governance/orchestra half is [commons-holonic](epr:commons-holonic-stewardship-backlog) row 9. **Two conditions bind:** the fold must stay walkable downward (an aggregate whose value theory is buried is un-arguable — our own [trap detectors](epr:comparative-political-economy-trap-detectors-2026-08-07) rule, and Meadows' *hierarchical* + *appropriate in scale* indicator criteria), and per-fold anonymity is an open design constraint, not a footnote | **Still design-only, confirmed 2026-08-12.** Row 12 landed and row 13 landed (partial, see its row); neither the fold-carries-a-global-query-down shape nor the anonymized-answer-up shape exist anywhere in `stock.rs` or elsewhere. The 2026-08-11 systems-discipline slice explicitly scoped this out: its handoff (`genesis/docs/superpowers/plans/2026-08-11-systems-discipline-slice2-handoff.md` §0) names "slice 2 (network)" — per-fold anonymity, the aggregation/lens layer — as a *different* slice, "blocked by [anonymity], NOT this sprint," and the sealed spec's §4 lists it as explicitly not-inherited, blocking "the network rung outright." Composes onto [plural-mishpat-lenses](epr:plural-mishpat-lenses-over-epr-design)'s two-layer law | rust-architect design pass |
...
**Status: spec landed (2026-08-11); a second, same-day slice then built the stock spine
(2026-08-11, commits `9e7065d48`→`100c433f7`, plan:
`genesis/docs/superpowers/plans/2026-08-11-systems-discipline-slice2-handoff.md`).** Rows 12–18
are spec'd by [measure-dynamics-confidence-ontology-design](epr:measure-dynamics-confidence-ontology-design),
...
analog, enforced on both the manifest and registry paths, Task 4).

**Rows 12, 13, and 14 have since moved off design-only — do not re-propose them.** Row 12 was
already landed (`MeasureKind`, above). Row 13 (harvest/regeneration ↔ emission/absorption) and row
14 (turnover time) are now real, tested constructs in `elohim/epr-rea/src/stock.rs` —
`Stock::harvest_regeneration()`, `Stock::emission_absorption()`, and `Stock::turnover_time()`, all
pinned by `elohim/epr-rea/tests/stock_dynamics.rs`. **Both landed only in part** — see their own
rows for the exact split: row 13's `>1.0 = unsustainable` *threshold* reading has no enforcing
construct (doc-comment narrative only, one manual application in `habits.yaml`); row 14 built
turnover time but not a distinct "coverage time" construct, and does not model the
steady-vs-rising-draw distinction the row itself asks for. Both also inherit spec Q15 (a derived
duration's period survives only in `basis`, not the type).

Row 15 (the aggregation-facing fold) and row 18 (the uncertainty work-queue) remain genuinely
design-only, confirmed against the tree 2026-08-12 — they ride L1 (row 12) and L5 (row 17)
respectively, both landed, but neither has been built. The 2026-08-11 stock-and-flow slice was
explicitly scoped away from both: its handoff document names row 15's network/anonymity half a
*different* "slice 2" ("blocked by [per-fold anonymity], NOT this sprint"), and spec Q3 names row
18 out of scope for the ontology spec itself. Do not re-propose rows 12–14; do re-propose rows 15
and 18 only against the still-unbuilt anonymity/decomposition mechanisms named above, not against
this spec or the stock-and-flow slice. The invariants (interval-inside-the-hash, propagating
folds, the widen-free/narrow-costly asymmetry) are the kind that must be pinned by contract test
at birth or they erode silently; this spec is that pin.
