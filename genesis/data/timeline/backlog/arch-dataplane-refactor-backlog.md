...
| 19 | **Object runtime facet + install ladder (open with → install to device → install to hub)** [DESIGN] | Operator thought 2026-09-08: the resilience chip reads only the custody plane; selecting an object also needs the RUNTIME facet (which epr-apps can act on it, where each runs, each with its own reach/staleness) and a one-click install ladder that composes the four installer acts of the reimplementation plan §10.3 without equating them — browser (obtain+enable), this device (replicate the executable as an elected artifact under an ark envelope), hub (always-on + a separate 'offer runtime to peers' commitment through the existing compute-grant flow, `api/compute_grants.rs` + `services/rea_observed_compute.rs`). Install-to-hub = the hub adopting an app release by election (`services/release_adoption/`), so it dogfoods rung 5 rather than adding a channel; Shefa's per-device view = custody rows + hosting commitments + governance holder. Full shape, chain/between/missing-node and guards in the standalone entry [object-open-with-runtime-facet-and-install-ladder](epr:object-open-with-runtime-facet-and-install-ladder). | M-L | Med — every record through p2p-design-gate; D6 cell-qualifier contract is a precondition; hosted sessions keep hosted trust limits | Hub stewardship becomes visible contribution; the resilience card gets its missing columns; first interactive instance of the peer-runtime edge after sprint 2026-09-08 T10 | backlog-only until T10's receipt lands; then spec, then browser rung on the household mesh |
| 20 | **`metrics.rs` decomposition — one concern per submodule, four-place-per-metric trap retired** [MECH] | `elohim-storage/src/metrics.rs` hit the `source-file-loc-ceiling@1` hard ceiling (finding `fe9edbbb11cd`): 6,797 lines on `dev`, 7,018 / 7,106 on the doorbell-latency / pointer-audit-1.4d sprint trees, +764 lines in five days. 149 metric statics across five `lazy_static!` blocks (split only for the macro-recursion limit), a ~540-line `register_all`, 122 typed setters + 15 `ReasonLabel` enums, and one 1,515-line `mod tests` — each metric lives in four places ~2,500 lines apart. Keep `metrics.rs` as root + a sibling `metrics/` of 10 concern modules (node_resources, conductor, http, sync_plane, acquisition, projection_reconcile, content_head, content_adopt, custody, identity) re-exported by glob so every `crate::metrics::X` path is unchanged; then per-module `register(&Registry)` (the payoff). Exposition is order-independent (`Registry::gather` sorts by name), so an exposition-skeleton diff proves no family or pre-touched zero was dropped. Full plan in [metrics-rs-loc-ceiling-decomposition](epr:metrics-rs-loc-ceiling-decomposition). | M (9 small waves) | Low — plain native Rust, no ts-rs type, no zome; only real trap is a silently dropped `register`/pre-touch (skeleton diff + item-count guard) | Adding an instrument touches one ~300–1,100-line module; ceiling ratchets down | rust-architect shift (**BLOCKED until in-flight commits `19c78d4af` / `fef7f7702` / `ef752ee26` land on `dev`; then Wave 0 + Wave 1 at once and announce the layout — no quiet window will open across ~20 worktrees**; re-measured 2026-09-23) |
