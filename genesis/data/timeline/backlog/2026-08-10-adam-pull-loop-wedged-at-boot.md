...
1. Why does the pull loop report total=0 — never scheduled (init-order gate
   ...
   for the loop's first log line vs its absence.
...
3. Whether `caughtUp` should distinguish "never started" from "started and
   behind" (C4 — the two absences are currently collapsed).
