...
| 13 | iroh backend for the acquisition plane: `acquisition.rs` provider rotation over `IrohPeerBook`, fetches through `IrohBlobStore` (the swarm-curve blob milestone) — closes P3 on pure iroh (measured 2026-08-24: 4/4 `homo-iroh` runs fail P3/P4 with P0 green; bytes flow, the pull queue has no iroh path) | 7 | acceptance = `homo-iroh` warm/cold rows green on the same matrix |
| 14 | doorway admission on a transport-agnostic health signal: `catching_up.rs` `ShedCause::Upstream` re-admits the recovering storage from a signal that is true on any live plane, not one coupled to the libp2p pull state — closes P4 on pure iroh | 13 | verify the exact admission predicate first (not yet confirmed); acceptance = P4 green within the P1–P3 window |
