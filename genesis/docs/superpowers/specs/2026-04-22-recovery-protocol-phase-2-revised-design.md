...
| **M3** | Coordinator functions: `create_recovery_request`, `commit_key_rotation`, `submit_intimate_witness`. Post-commit signals for `RecoveryRequest` + `KeyRotation`. Doorway libp2p invitation flow on topic `recovery.invitation.{request_hash}`. | a2o: intimate-quorum end-to-end passes on shem cross-node topology |
