...
**Taxonomy** — schema-rooted like `substrate-signal.schema.json`: a closed, DNA-notarizable enum of signal varieties, first cut: `completion-count`, `engagement-duration`, `timing-pattern`, `sequence-trace`, `content-response` (free-text/answer payloads), `presence-coincidence` (who-was-together). Canonical homes: `elohim/sdk/schemas/v1/enums/signal-variety.schema.json`, `elohim/sdk/schemas/v1/enums/aggregation-lane.schema.json`, and the adoption ceremony's wire contract `elohim/sdk/schemas/v1/commitments/adopts-content.schema.json` (schema-first: DNA arm and validator check conform to these). Wiring campaign: `genesis/data/timeline/backlog/2026-07-31-care-aggregation-lanes-adoption-wiring.md`. Each variety names what a flow atom *reveals*, independent of its EprKind. One atom may carry several varieties; the taxonomy exists so policy can address them separately.
...
**The elohim as policy reader (steering decision 2026-07-31).** The machine-comparability guard above is not only anti-EULA hygiene — it is what makes adoption policies *elohim-readable*. The ceremony's presenter is the household's local agent: it diffs an incoming policy against the household's values profile and the commons norm for that content class, flags anomalies ("content-response at k=5 where the category norm is suppress"), proposes modified adoptions, and remembers precedent. The ToS-nobody-reads failure was an asymmetry (one drafting team vs. a million tired adopters); the household box inverts it — every intimate space has a tireless reader with standing to negotiate what enters. At network scale the inversion compounds: distributed elohim review makes bad-faith terms a *reputation signal* on the policy atom itself, so good faith becomes a measured property of policies — terms that respect where they go at every reach tier — rather than a hope. As on-network intelligence matures, this is the good-faith-ToS equilibrium the ceremony is designed to reach.

**The maturity end.** The same records drive the positive closure arcs. An indemnification claim is adjudicable directly from the ledger — adopted in good faith (epistemic digest), practiced faithfully (flow atoms), harmed per the ratified pattern (cohort evidence), repaired when the path arrived (map-edit ceremony) — so the shefa indemnity pool pays against witnessed process, not litigation. And reconciliation is itself witnessable: the author's revision re-earning reach, the repaired maps, the acknowledgment atoms — mishpat gets a ledger to *complete* stories against (restored capability), not merely to charge against.
