...
| R3 | warm/cold recovery `time_to_recover` durable — `just mesh recovery <shape> <peer>` → `genesis/a2o/reports/recovery/recovery-timeline.jsonl` (`/tmp` path is a symlink) | T2 | 🟡 DURABLE since M0 (2026-08-28): warm jessica←matthew 62 s libp2p · 58 s dual · 902 s FAIL homo-iroh (P0 only); 2026-08-29: 16 records (homo-iroh warm ×11 incl. 3 PASS + cold ×1 PASS, homo-libp2p ×1, dual ×1), rows carry `scenario=`; `recovery-detail:` names failing ids; mesh runs dual by default (`STORAGE_BIN` dev-slot fallback) | N=3 per shape × transport |
...
| R7 | fleet A `caughtUp=true` — `@concern:inventory-convergence` | T4 validate-only | 🟡 2026-08-29 edge #1389 Dataplane Validation: A-caughtUp=True AND B-caughtUp=True for the whole 45-min gate (one B blip 06:18Z) — first run where the fleet read caught-up on both storages; A-quiesced never (actionable 12→20→16→4→3, tol 2) so the gate DID-NOT-MEASURE (post-deploy churn; measurement-by-deploy) | green ×2 consecutive fresh triggers |
...
| P3 | `homo-iroh` warm/cold P0–P4 green; dual pull leg carries bytes on iroh | T2 matrix | 🟢 2026-08-29 **pure-iroh PARITY**: warm N=3 61/60/72 s, cold 113 s, all PASS P0–P4 (`cut=reconcile-peers-1`, durable rows 01:16–01:21Z, `scenario=homo-iroh`); homo-libp2p warm 58 s the same hour. The gap was the projection-reconcile discovery + anchor heal being `P2PHandle`-gated (homo-iroh sat P1 red 469 s on a row homo-libp2p healed in 58 s) — cured by `p2p::reconcile_peers::ReconcilePeers` (libp2p + `IrohReconcilePeers`). Earlier 258 s pass (2026-08-28) stood on P1 luck. Residual, plane-neutral: converged doc `blobHash` form ≠ author row (backlog `content-doc-blobhash-representation-drift`; reverse heal now idempotent + names from/to at INFO) | FLEET 2026-08-29: edge #1389 deployed elohim-storage-iroh:1.0.0-dev-71f310ce to 7/7 alpha peers + 2/2 doorways Ready (dual); iroh families live on all 7 pods. Next: a fleet recovery-shaped read (a pod restart with `route_total`/`iroh_blob_fetches` observed) |
| P4 | `PathObservation`/`select_path` live — `elohim_transport_route_total{reason}` on both planes | T1 contract tests + T2 | 🟢 2026-08-29 LANDED (spec 2026-08-24 §3.1 rows 1–4 + the write-time push): `p2p::transport_paths` ring + `select_path` (registry row, C3/C4/Degraded contract tests), `/p2p/status.transportPaths`, both metric families; Small races sample both planes (jessica→matthew iroh **15 ms** vs libp2p **816 ms degraded**), Bulk selects (pull leg via `plan_acquisition_targets_selected`, custody push via `push_shard_routed` — probe content's shards went `transport=iroh` to both holders). Before/after on the household LAN: warm recovery on 62/59/61 s · off 61/60/58 s · on-v2 61/57/60 s — inside noise, as the spec predicted, so the fleet default stays on-by-code with `ELOHIM_TRANSPORT_SELECTION=off` as rollback until a fleet-side difference is read. Gap filed: a warm-restarted peer drains its pulls before its iroh book warms (backlog `pull-leg-drains-before-iroh-book-warms`) | FLEET 2026-08-29 (edge #1389, 71f310ce live 7/7): `elohim_transport_route_total` / `_path_rtt_ms` / `acquisition_dispatch_total` NOT yet present — label-vecs materialise on first decision and a converged fleet routed nothing in the window (genesis #1518's upload, the first routed push, failed on the doorway rollout collision); backlog `transport-route-metrics-pretouch-zero`. Local corpus (3.4k rows) delta: bulk EWMA iroh 153–191 ms vs libp2p 1120–1220 ms, small 16 ms vs 425 ms; selection picks iroh on evidence (best_rtt 43:11, explore floor live) but whole-recovery/quiesce wall-clock is cadence-bound (on/off inside noise). genesis #1519's upload PATCHes blobHash onto existing rows (update path) so `distribute_shards` never ran — still nothing routed on the fleet. Next: a create-with-blob on alpha (or a read-miss heal) is the first fleet route decision |
...
| F9 | fleet lane measures federation at all — `@act:ii` siblings for read-only concerns | T4 | 🟡 2026-08-29: first Act II siblings landed — auth-discovery-neighbourhood (4, API leg) + doorway-portal-login-neighbourhood (2, playwright leg, @browser-only) ride the existing genesis E2E lane with zero pipeline edits; genesis #1518 failed upstream of E2E (doorway rollout collision); **#1519: auth-discovery twins 4/4 PASS on the deployed doorway (API leg reports them every run); portal twin 1/2 — wrong-password PASS, sign-in FAILED on the domain-scoped identifier `<name>@alpha.elohim.host` the deployed doorway returns (backlog `portal-login-step-domain-scoped-identifier`; session was real)** | ≥ 7 federation/notary scenarios banked |
...
| D3 | a browser regression on elohim.host is seen by SOME lane — replace the Cypress ghost | App Jenkinsfile | 🔴 confirmed 2026-08-29 (#1678): Cypress binary missing, catchError→UNSTABLE, prints ✅ — a ghost; the a2o playwright leg in genesis (e2e-verify-browser.sh) is the real browser lane, now carrying the portal twin | stage runs `@act:ii @browser`-safe a2o or is deleted and routed to genesis |
