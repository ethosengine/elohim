...
- Federation is a *separate spec*: a doorway projection adapter translates this primitive into `org.dds.result.*` records for AT Protocol consumers. The signing question (doorway-as-relying-party vs. opportunistic did:plc per peer) is settled there, not here. AT Proto interop lives at doorway by architectural memory.
