...
- **LAW-3 — HEAD stays DECLARED/notarized, never recency.** The CRDT converges the *version DAG*; the notary *elects HEAD* as a declared dependency (cid-pin = lockfile semantics — MEMORY `project_versioned_entity_head_is_declared_dependency`). Convergence ≠ binding-selection.
  > **⚠ First clause SUPERSEDED 2026-08-07.** The CRDT does **not** converge the version DAG — it converges VALUES (a grow-only version set). The version lineage AND the head election both live at **L2**. The second clause stands and is exactly what `select_canonical_winner` does. See [`history/2026-08-07-version-dag-lives-at-l2-not-in-the-crdt-doc.md`](../../content/elohim-protocol/history/2026-08-07-version-dag-lives-at-l2-not-in-the-crdt-doc.md).
...
- **REQ-F4 — HEAD selection is declared/notarized.** The serving field applied MUST correspond to the *declared/notarized* version HEAD, never "latest Automerge heads." **[MUST-CLOSE: no declared-HEAD / version-DAG / cid-pin logic exists; the content doc is a FLAT single-value LWW map (`projector.rs:118-144`, `tx.put(ROOT,"blobHash",…)`); `upsert_with_anchor` is pure recency (`content_diesel.rs:682-720`) — VERDICT L1 R1, VERDICT L2 #3, VERDICT coherence R3. A real notarized HEAD pointer AND a multi-version doc structure MUST be added before any HEAD-is-declared claim holds.]**
  > **⚠ PARTIALLY SUPERSEDED 2026-08-07** — the *notarized HEAD pointer* half LANDED (`declared_head_action_hash`, migration `2026-07-04-120000_content_add_declared_head`). The *multi-version doc structure* half is **superseded**: the version lineage and the canonical-head election live at **L2 (the DHT)** — anchor `canonical_head`, elected by `content_store::select_canonical_winner` — and the CRDT doc's `headActionHash` is a HINT that REQ-N5 forbids consuming as authority. Do **not** build lineage edges or head election inside the Automerge doc. See [`history/2026-08-07-version-dag-lives-at-l2-not-in-the-crdt-doc.md`](../../content/elohim-protocol/history/2026-08-07-version-dag-lives-at-l2-not-in-the-crdt-doc.md).
...
- **OD-1 resolution:** couple HEAD-selection to the notarized declared DAG HEAD — add the multi-version doc structure and the `resolve_head(id)→declared_head_action_hash` selector (net-new substrate, VERDICT L1 R1). Replace the LWW scalar `blob_hash` with a HEAD-pointer-carried value so convergence stops being recency.
  > **⚠ SUPERSEDED 2026-08-07** — do not add a multi-version *doc* structure; the selector was not net-new. Both live at L2 (`resolve_content_head` / `select_canonical_winner`). See the OD-1 annotation in §7.1 and [`history/2026-08-07-version-dag-lives-at-l2-not-in-the-crdt-doc.md`](../../content/elohim-protocol/history/2026-08-07-version-dag-lives-at-l2-not-in-the-crdt-doc.md).
...
- **OD-1 (the hardest): `blob_hash` is class-A notarized AND the field the CRDT wants to own.** Options: (a) reclassify — the CRDT owns only *non-notarized* operational fields, and `blob_hash` heals via a *different* non-notarized producer that populates the real hash at deploy time (decoupling field-population from the PATCH) — VERDICT L3 #1's suggested close; (b) carry `blob_hash` under a notarized declared-HEAD pointer that the CRDT converges as a DAG of versions, not an LWW scalar.
  **✅ RESOLVED 2026-07-01 (architect): option (b) — FULL 1c NOW.** The version-DAG + notarized declared-HEAD substrate moves from Phase C **into the core build** (see re-sequenced §6). `blob_hash` is carried under a notarized declared-HEAD pointer the CRDT converges as a DAG of versions, never an LWW scalar. The interim empty-guard/amber-only stopgap is SUPERSEDED by the real HEAD selector (it may still land first as a scaffold, but the target is the DAG, not the stopgap).
  > **⚠ SUPERSEDED 2026-08-07 — option (b)'s "the CRDT converges a DAG of versions" is NOT what was built, and should not be built.** What landed instead: the notarized declared-HEAD pointer (`declared_head_action_hash`) plus a **multi-candidate election at L2** — `select_canonical_winner` over the `canonical_head` anchor link set, arbitrated on `(tier, notarized-DHT-link-clock, create_link_hash)`. The SQL `canonical_declared_at` / `canonical_earned` columns are a **projection of that L2 election, not a second ledger** (see migration `2026-08-02-120000_content_add_canonical_election`). The CRDT plane received only the *value-coexistence* half — a grow-only `versions` map so divergent peer versions are added rather than LWW-clobbered — and never lineage edges or an election. Also note: this resolution called `resolve_head(id)→declared_head_action_hash` "net-new substrate"; `resolve_content_head` already existed in the `content_store` zome.
  >
  > **The process lesson, recorded because it is repeatable:** this spec cited the *policy seed* (`project_versioned_entity_head_is_declared_dependency`) but never the *running instances* of the same primitive — grep finds zero occurrences here of `version_parent`, `Mishpat::Commitment`, or the `2026-06-27-lens-version-dag-epr-policy-dependency-design` spec sealed four days earlier. That makes this an **uncomposed fork** of a recurring primitive, not a reasoned divergence. Full record: [`history/2026-08-07-version-dag-lives-at-l2-not-in-the-crdt-doc.md`](../../content/elohim-protocol/history/2026-08-07-version-dag-lives-at-l2-not-in-the-crdt-doc.md).
