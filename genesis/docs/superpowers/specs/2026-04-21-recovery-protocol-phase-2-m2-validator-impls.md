...
3. **Coordinator pre-commit gate (M3)** — the real enforcement point. Before calling `create_entry(KeyRotation)`, the M3 coordinator (`commit_key_rotation`) queries `get_links` on the `ActiveFreezes` anchor, feeds the results to `check_freeze_floor_rules`, and bails if a blocker exists. The gate goes live with the coordinator in M3 regardless of whether freezes are being authored in volume yet — it is a structural commitment, not gated on the defender. Defender-authored freezes arrive in M5 and immediately benefit from the pre-existing gate. This matches the Stage-1 social-bootstrap model (spec §3.2) — coordinator owns correctness; validator enforces what is cheaply verifiable; elohim defender escalation (M5+) is the compensating control for adversarial coordinators.

The dispatch wiring stays in `validate_key_rotation` as a documentation anchor for the enforcement split. The HDI wrapper returns `Ok(None)` permanently; the coordinator is the authoritative enforcement point. Pure-logic rules (`check_freeze_floor_rules`) are the portable piece — shared between the coordinator's pre-commit gate and any future integrity-level enforcement if HDI primitives evolve to permit it.
