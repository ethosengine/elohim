...
5. **The vocabulary routes; it never names the resolver's species** (clarified 2026-07-23). `Refer` carries layer + reason — whether it lands with elohim counsel (machine-speed, reversible moves: pause/freeze/surface/verify), a human, or a quorum is **governed routing**, itself Knowledge-level policy and lens-plural per §2b. Bounded non-negotiably: genuine value conflicts reach humans (`constitution.md` — "Genuine conflicts flagged for human resolution"); the sortition floor stands; the algedonic channel bypasses every elohim. Between those floors, a community's routing table is its own witnessed choice — "human must decide" is the guaranteed floor of the routing space, not the definition of the term.
