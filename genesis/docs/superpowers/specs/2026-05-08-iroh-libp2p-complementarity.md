...
| **Identity-handshake (`/elohim/identity-handshake/2.0.0`)** | **libp2p-canonical, iroh-receive** | Sensitive flow. No n0 in the auth path is a security property. iroh ALPN remains for hub-to-hub auth where convenient, but libp2p is canonical. |
| **Trust (`/elohim/trust/2.0.0`)** | **libp2p-canonical, iroh-receive** | Same security posture as identity-handshake. |
| **Reach-authorization** | **libp2p-canonical, iroh-receive** | Reach-gate decisions feed elohim-mediated matchmaking; security-sensitive. |
