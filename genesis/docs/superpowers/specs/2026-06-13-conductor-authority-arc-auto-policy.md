...
## 2. Feasibility verdict — the lever is STATIC, and the *fractional* lever is UNVERIFIED
...
**Consequence — and the actuation tiers this forces.** A fractional `a` (our policy output, §3) cannot be expressed at the `target_arc_factor` field. The real continuous lever is the `LocalAgent` **target-arc hint** — a `DhtArc` range, e.g. `Arc(0, u32::MAX / k)` (`kitsune2_api-0.4.1/src/agent.rs:55-76`, `set_tgt_storage_arc_hint`; `DhtArc::FULL = Arc(0, u32::MAX)` at `arc.rs:14-26`) — reachable only via the kitsune2 `network.advanced` block (`NetworkConfig.advanced: Option<serde_json::Value>`, `conductor.rs:312-317`) or upstream/custom sharding work. We therefore commit to **three actuation tiers, ranked by confidence**, and gate the fractional path behind a P0 spike:

| Tier | Mechanism | Granularity | Status | Fixes james? |
|---|---|---|---|---|
| **T1** | `network.target_arc_factor` | `{0, 1}` only | **VERIFIED** | No — only full or leecher; leecher breaks the floor |
| **T2** | `LocalAgent` target-arc **hint** via `network.advanced` | fractional `a ∈ (0,1)` | **UNVERIFIED on deployed 0.3.2** — the P0 spike | Yes, if it works |
| **T3** | upstream/custom kitsune2 sharding policy | fractional, N-aware | fallback if T2 fails | Yes, with carrying cost |

**Version skew to confirm against (the must-confirm item, §8).** Deployed: `holochain 0.6.0` / `holochain_p2p 0.6.0` / `kitsune2 0.3.2` (`elohim/holochain/tests/sweettest/Cargo.lock:2120,2390,3431`; edgenode base image `…edgenode:v0.0.8-alpha31-hc0.6.0-go-pion-custom`, `elohim/holochain/edgenode/Dockerfile:10`). The `target_arc_factor` `{0,1}` clamp and `LocalAgent` trait shape were byte-read at `0.4.1`/`0.7.0-dev` and confirmed stable against upstream `main`; the **0.3.2 `network.advanced` hint surface is NOT vendored here and must be byte-confirmed by the spike.** (Steward mobile is a different line again — `kitsune2 0.4.1` — and already sets `target_arc_factor = 0` under `cfg!(mobile)` at `steward/device/src-tauri/src/lib.rs:140`; that is the T1 leecher case, legitimate for a phone, not the model for a laptop.)

**Two cheap confirms to run before the spike, to de-risk it:**
1. **Discriminator #3 (scaling note line 64):** set `target_arc_factor: 0` (T1 leecher) on one loaded node; if the working-set climb collapses, the conductor DHT arc is confirmed as the RAM driver and the field provably moves the resident set.
2. **Bound the T2 spike** by reading the vendored `0.4.1` `network.advanced` surface first — but the deployed **0.3.2** remains the gate.

**Design rule from this verdict:** everything in §3 (the `derive()` function), §4 (coverage invariant), §6 (observability), and the operator override ships **regardless of the spike outcome** — they are config/derivation/read-model work. Only *fractional actuation* (and therefore `SAFE_MIN_ARC > 0`, see §4) depends on T2/T3. If the spike fails, T1 is a coarse but real lever: a rich always-on node can be the full-arc anchor while lean devices are explicit, accountable leechers — degraded, but a coherent stopgap that ends the RAM-bump treadmill on the lean devices.
...
- **`mem_ceiling`** — the cgroup memory limit of the container the conductor shares with storage. **This is the headline gap: it is unobservable today.** `system_metrics::total_memory_bytes()` (`elohim/elohim-storage/src/services/system_metrics.rs:99-115`) calls `libc::sysinfo() → info.totalram` = **HOST RAM**; `/sys/fs/cgroup/memory.max` is read *nowhere* (grep over `elohim-storage/src` + `genesis/orchestrator` = 0 hits). A 512Mi pod sees host GBs. **The cgroup MEM reader (work item M1 / vision §9 #13) is this function's hard precondition: you cannot size an arc to a ceiling you can't read.** It must cover cgroup v2 (`/sys/fs/cgroup/memory.max`) and v1. Note: conductor + storage share **one** cgroup in the consolidated edgenode (scaling note line 40), so `memory.max` is the joint ceiling — the arc budget must leave headroom for the storage parent.
...
- **MUST-CONFIRM (the §2 spike): does a fractional arc lever exist on deployed kitsune2 0.3.2?** `target_arc_factor` is a VERIFIED `{0,1}` switch; the fractional `LocalAgent` hint via `network.advanced` is UNVERIFIED on 0.3.2 (vendored sources are 0.4.1/0.7.0-dev only). If neither T2 nor a tractable T3 works, fractional `SAFE_MIN_ARC` and per-archetype fractional targets are not actuatable — the policy degrades to T1 (full-anchor vs accountable-leecher). This is the single gate; everything else ships regardless.
