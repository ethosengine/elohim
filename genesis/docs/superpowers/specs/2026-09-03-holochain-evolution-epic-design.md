...
- **Coordinator Zome** (as landed, Task 9): `node_registry_coordinator::carry_from(CarryInput{v1_cell, cursor, limit}) -> CarryReceipt{carried, self_carried, next_cursor, v1_digest, v1_total, witness_hash}` — one page (≤16), ONE witness per page, `witness_hash` a base64 string (`""` = empty page), `v1_total` read from v1's export page (never derived from `carried`), `self_carried` = records re-created natively (a re-created entry whose hash differs from the carried action's is refused, never held); `get_witnesses_for(entry_hash) -> Vec<Link>`. Measured on 0.7.0: a same-agent cross-cell call needs no cap grant in `init`. **Landed since (Tasks 18, 20, 14a, 13b):** v1 `export_held_records(agent, cursor, limit) -> ExportPage{…, observed_head}` and `known_agents()` (the caller included); v2 `carry_from` takes `source: Own | Held(agent)` and is idempotent by entry hash (`already_carried`); v2 `seal_close(v1_cell) -> SealReceipt{close_hash, open_hash, witness_hash, already_sealed, resumed}` (base64 strings; resumes a half-seal) and `get_closes_for`; v1 `readopt_from(v2_cell, cursor, limit) -> ReadoptReceipt{readopted, already_present, foreign, next_cursor, v2_digest, v2_total}` re-authors the agent's own window-time v2 facts on v1 with the same entry hash (in flight).
...
|---|---|---|
| 1 admissibility (verify's positive lineage branch) | **GREEN on the household mesh 2026-09-04** | `cucumber-stations-mvp-r3.{log,json}` (run 20260904123730, 9/9, 2m18s) |
| 2 notarized path (`PathNotNotarized`) | **GREEN on the household mesh 2026-09-04** | `cucumber-stations-mvp-r7.{log,json}` (run 20260904130252, 13/13, 2m01s; reds r5/r6) |
| 3 install beside, same key | **GREEN on the household mesh 2026-09-04** | `cucumber-stations-mvp-r8` (11/11, 50s) and `-r12` (Stations 1-4 together, 44/44) |
| 4 self-carry, notarization re-verified | **GREEN on the household mesh 2026-09-04** | `cucumber-stations-mvp-r10` (11/11, 31s) and `-r12`; kernel green in sweettest (Probe A) |
| 5 held-carry | **GREEN on the household mesh 2026-09-05** (the 2026-09-04 zome BLOCK is retired — Task 12's bridge ticker and Task 24/26's agent-scoped export closed it) | `cucumber-stations-mvp-r20.{log,json}` (run 20260905025626, 11/11, 1m12s; red r18) |
| 6 bridge direction | red | — |
| 7 revert before sunset | red | — |
| 8 sunset | red; REWRITTEN 2026-09-04 (the fence is ours, not the conductor's); NARROWED 2026-09-05: the fence covers a close + post-close fact in one witness and any later carry by a peer that sealed or carried the close — a courier that never saw the close is NOT fenced on HDI 0.8 (filed gap G6) | Probe B; Task 14a `5de545477` (fix round in flight) |
| 9 forged witness refused | **GREEN on the household mesh 2026-09-04** | `cucumber-stations-mvp-r13` (11/11, 44s), three peers dual-celled, both refusals on all three |
| 10 quorum / root refused | red — **BLOCKED**: no roster check anywhere; `constitution_root` is `None` for every installed role | `cucumber-stations-mvp-r14` (9 passed / 1 failed) |
...
- **Integrator (push ordering) — UPDATE 2026-09-05 (later):** rakia now carries TWO unpushed submodule commits — `4947469` (happ-lineage class) and `13cb31d` (`roleBinding.constitutionRoot`, Task 17b); the parent gitlink is at `13cb31d` (`6cfdf3663`). Push rakia first, then dev; a dev push before rakia leaves the gitlink dangling for every fresh clone.

### 11.4 Ledger (newest first)

- 2026-09-05 — **Station 5 GREEN on the household mesh** (`cucumber-stations-mvp-r20`, 11/11, 1m12s; red `r18`). jessica never adopted and never moved; 16 of her 180 v1 records are readable in james's v2 cell at her own entry hashes, each witnessed by a link james authored, and her v1 digest and count are unchanged — a HELD carry, not a write into her chain. The 2026-09-04 "BLOCKED (zome)" reading is retired: Task 12's `LineageBridge` ticker plus Task 24/26's agent-scoped export are the substrate that was missing. Two harness facts: `RoleLineageView.sweep` is `skip_serializing_if = Vec::is_empty`, so it is ABSENT from `/version` until the first tick (the step's non-optional index into it was the first red); and the sweep budget cannot be a constant — it moves 16 records per neighbour per tick, so Stations 5/6 now derive `(⌈N/16⌉ + 2) × 30 s` from the neighbour's own record count. **Risk row R1 has its first live numbers**, appended per run to `.claude/data/lineage-carry-metrics.jsonl`: a 186-record self-carry in 40 s, the three-page v1 export walk in 0.2 s, `ExportPage.scanned` peaking at 634 — the whole chain's action count, which is what an UNPINNED page costs by construction and the baseline the pinned-resume path has to beat.
- 2026-09-05 — **Task 27 landed and approved; the mesh is prepared for the second live run.** A migration's successors are now linked from its own anchor (`sunset|<cid>` / `revoked|<cid>` on the state-link type — an address segment is unambiguous against a time), `get_lineage_successors` reads them, and storage's sunset read picks the ACTIVE successor (a revoked one cannot mask a live one); mishpat hash unchanged. The mishpat and node-registry coordinators are hot-swapped onto all three peers (drift was exactly those two roles), storage is rebuilt feature-full into the slot. **Every code task the rehearsal needs is landed and reviewed: 1–20, 24, 26, 27.** Tasks 21–23 and 25 are the sunset-hardening crossing (hash-moving, deferred by design). Next: the live run for Stations 5, 6, 7, 8 and 10.
- 2026-09-05 — **Tasks 13, 14, 26 reviewed approved; Task 24's fix landed with the metric** (`eee48965d`): a resumed export page now reads only its window — measured on a 1204-record chain, the unpinned first page scanned 1204 headers and the resumed pages 512, 432 and 48 — and the pin is verified at zero queries because the conductor's own `agent_info` reports the chain head hash, which on an append-only source chain proves the chain below (the two bounded queries were redundant and were not written). `scanned` on the page is the always-on metric the risk row R1 reads. The test-only carry extern now sits behind a `lineage-test` feature: the deployable witness bundle exports none of it, the test bundle two. **In flight:** Task 27 (a migration's successors linked from its anchor so the sunset is discoverable on the canary). Then: hot-swap the mishpat and node-registry coordinators onto the mesh, rebuild storage into the slot, and measure Stations 5–10.
- 2026-09-05 — **Tasks 13c, 14b, 26 landed** (`707dbd4ec`, `2db2d9830`, `4c99b070f`; under review): the revert now re-authors the agent's window-time v2 facts on v1 through `readopt_from` — and it must run BEFORE the side app is disabled, since a disabled app answers no cross-cell call (asserted in code); the sunset arm seals close→open on the canary through `seal_close`, closes the window, records `sunsets[]`, disables nothing (the admin API disables whole apps and the base app carries every role), and installed reality now reads the AUTHORING cell on a crossed role so the installed-root check is live there. Task 26: entry types travel by NAME across lineage ends; the carried index is read only on the C10 fallback for a predecessor that sends no names. **Gap G11 filed → Task 27:** sunset discovery is author-scoped (mishpat has no query-by-target), so the sunset arm is dormant on non-authoring peers until a migration→successor link exists; needed before Station 8 is measured on the canary. **In flight:** Task 24's fix round (a resumed page reads only its window), the 14b and 26 reviews.
- 2026-09-05 — **Task 24 landed** (`3b3e4eeb6`, under review; G8): the export walk is linear — a headers-only ordinal index plus a sequence-range-bounded entry read, the digest computed once per walk and carried on an additive `resume` pin that names the chain head; a later page whose head or record count moved is refused `chain moved — restart at 0` (which the storage fold and sweep already do for a digest change); witness lookups memoised per page. A sealed predecessor (`ChainStatus::Closed`) gets its own pin rendering. Sweettest 13/13, default hash unchanged. Follow-up for storage: thread `CarryReceipt.resume` back into `CarryInput.resume` so the pin actually skips the walk on the mesh. **In flight:** Task 26 (entry types by NAME across lineage ends, G10) and the storage seat for 14b + 13c (sunset arm; `readopt_from` wired).
- 2026-09-05 — **Task 13a landed** (`9aadf8213`, under review): the revert arm — when a role's open window reads its path as REVOKED (Task 19's DHT-first evidence; `Unreachable`/`Absent` never revert) or the prior head is re-elected, the controller reverts routing (v1 authors again, the v2 side app stays as the reading pointer — the landed `LineageRoles::revert` semantics; my earlier "authoring == reading == base" wording is withdrawn), disables the side app (the admin seam has one method, so uninstall is structurally impossible), clears the bridge's sweep state, and records the revert on `/admin/adoption`; the re-authoring of window-time v2 facts on v1 (`readopt_from`, Task 13b) is wired next (13c, with 14b). The revert horizon is not enforced at MVP — the sunset bounds it; said in the docs, not assumed. Gate: the full crate gate links ~170 integration binaries and was shed four times at 80% RAM; the light recipe (lib filters + all-targets clippy at two jobs) is the box's honest gate. **In flight:** Task 24 (linear export walk).
- 2026-09-05 — **Task 13b landed** (`285a5303e`, under review): v1 `readopt_from(v2_cell, cursor, limit)` re-authors the agent's OWN window-time v2 facts on v1 with the same entry hash at a new action (guarded by `hash_entry`; other authors ignored — v1 has no witness type, so revert is a re-authoring, never a carry); a v2 entry type v1 does not declare (the witness) is counted `foreign`, never an error; retry idempotent by entry hash. Sweettest 12/12; default hash unchanged. Also this stretch: the operator's fork landed the **risk discipline** (`ae95cfb29`) — `risk`-tagged backlog rows with triggers, inject rules beside the coordinator and the bridge, guard rows R1–R4 on this habit — so G8/G9's scale arithmetic now has a reader at edit time. **In flight:** Task 13a (storage revert trigger + path; its full gate was shed at 80% and runs the light recipe). Then 14b, then the live run.
- 2026-09-05 — **Growth shapes filed (G8, G9) from a read of the landed code**, none biting at rehearsal scale, all at lamad scale or after a few migrations: the export re-walks and re-digests the whole chain per 64-record page (≈N²/64 record loads in WASM); after-close validation walks the carrier's whole v2 activity per witness (O(W²), re-run by every validating authority); chains grow ≈1.06× per migration and the held sweep writes peers × records to the DHT during a window (a 3.5k-record neighbour takes ≈110 min per courier at 16 records / 30 s); dual cells per role double conductor RAM, arc and gossip until the sunset (a memory event on the household tier); the controller sweep is peers × channels DHT reads per minute forever on a converged fleet. Minted: Task 24 (G8, coordinator-side: digest once per walk, cursor carries the chain head, pages verify it; batched witness lookups) and Task 25 (G9, integrity-side: the close travels in the witness so validation is O(1) after the seal; rides Tasks 21–23). The §3 head-plane budget counted witnesses only; it now points here.
- 2026-09-05 — **Task 14a closed** (`5de545477` + fix `27532918a`, approved): `seal_close` seals close→open→witness in order and now RESUMES a half-seal instead of closing v1 twice (Probe B2's close+1 warrant), its existing-seal check keys on the open's `close_hash`, and the after-close fence stands as narrowed above (v2 hash moves to `uhC0ktG7w…`; default unchanged). **Task 11 part 2c fixed** (`defe73ba9`): Station 10 mints a REAL roster commitment and asserts the membership and root refusals verbatim; the story's Station 8 Then is narrowed to the peers the fence reaches and names the courier gap as a limitation, two blind-reader passes READY. **In flight:** Task 13a (revert trigger + vehicle path, storage) and 13b (`readopt_from` on v1, the agent's own window-time v2 facts re-authored with the same entry hash). Then 14b, then the live run for Stations 5–10.
- 2026-09-05 — **Task 19 reviewed: the boundary it closed exposed a wider one (gap G7).** Lifecycle now rides `CommitmentByState` links, but mishpat integrity validates only their tag SHAPE and `create_commitment_state_link` is a public extern — any agent can author `revoked` on any lineage anchor (terminal, unioned, unreopenable: a permanent denial on every peer) or `active`. Storage reads those links as evidence it cannot yet verify; the source carries the trust boundary named, not closed. The fix is integrity-side (the link author must be the anchor's author for `active`, or the author of a quorum-checked revocation naming it for `revoked`), hash-moving on mishpat, so it rides the sunset-hardening crossing together with G1 (roster chain) and G4. **Trust-boundary ledger as of now:** G1 roster not bound to the elohim's key/root · G4 closed on the read path (Task 19) · G6 cross-courier after-close fence not expressible on HDI 0.8 · G7 state links forgeable. All four are one crossing's worth of integrity change; none blocks the rehearsal's measurement.
- 2026-09-05 — **Task 14a reviewed: changes requested; Station 8's claim narrowed to what HDI 0.8 can enforce.** The after-close rule as landed fences (i) a close and a post-close fact in the same witness batch and (ii) any later carry by a peer that itself sealed or carried the close (it walks the carrier's own v2 chain); a courier that never sealed and never carried the close is not fenced, and that half is not buildable in validation on 0.8 (no link reads) — **filed gap G6**. Two fixes requested before 14b: a retry after a failed open must not close v1 twice (resume the half-seal), and the existing-seal check must key on the close hash. Hash discipline holds structurally (every integrity hunk inside the gated block).
- 2026-09-05 — **Task 19 landed** (`751fba917` mishpat coordinator, `938cf8215` storage; under review): path lifecycle is now DHT truth — `create_commitment` authors the `CommitmentByState` link in the same call the validator accepts the quorum (`active` on a lineage commitment's own anchor; `revoked` on the path a quorum-checked revocation names), and storage reads `state` from those links through the peer's own conductor, the projection row demoted to a cache (`revoked_at` a monotone union of both; unreadable stays `Unreachable`). The entry-action inference is gone — that removal is the trust boundary G4 named. Gate: `just gate elohim-storage` green, mishpat sweettest 19/19, mishpat hash unchanged. **Operating rule minted:** the mishpat coordinator hot-swap reaches every peer at or before the storage roll that reads links, and lineage commitments notarized before the swap are re-notarized, not re-read. Also landed: Task 14a (`5de545477`, the after-close fence and `seal_close`, DNA side, under review) and Task 11 part 2c. **In flight:** Task 13a (revert trigger + vehicle path, storage).
- 2026-09-05 — **Task 11 part 2c landed** (`e2ecbd01e` + `47119c1f5`): the steps for Stations 5, 6, 7, 8 and 10 are authored against the landed contracts (Station 10 was already real from 2b) — 10 scenarios / 128 steps bind with zero undefined in a dry run; Station 8 rides Task 14a's `seal_close` / `get_closes_for` / after-close rule as they sit in the tree. No live run yet; that is the next measurement seat once Tasks 19, 14a, 13 and 14b land. Gate debt from a neighbour lane (`app_deliverability.rs` needless_borrow, `fb51227d9`) landed as a scoped prerequisite so the crate gate is green for every lane.
- 2026-09-05 — **Task 20 landed** (`a991bb0ea`, under review): the carry is now idempotent by entry hash — one chain `query` per page, an already-present entry counts as carried but is neither re-created nor re-proved, a held entry already witnessed from this lineage gets no second proof, and a page with nothing new authors no witness (`witness_hash: ""`, `already_carried` on the receipt). Retry sweettests for both own and held carries: same `carried`, `self_carried 0`, exactly one witness link. C6b in §7 is true again once this lands; the bridge's halt-on-missing-`already_carried` (Task 12) lifts on a v2 that carries it. Default hash unchanged. **In flight:** Task 19 (DHT-visible path lifecycle; storage + mishpat coordinator) and Task 14a (the after-close rule and `seal_close`, DNA side).
- 2026-09-05 — **Task 12 closed** (`b3b444344` + review fix `985f2a30c`; verdict approved and fulfil recorded through `epr flow`): the sweep now enumerates neighbours from the READING cell (a mid-window reconnect would otherwise have asked v2), and the fix exposed a latent bug — a REVERTED role's ids are inverted, so the old `authoring != reading` shorthand read it as an open window; the predicate now names the open shape. A v2 whose page omits `already_carried` halts the re-walk (Task 20's contract, in flight) rather than authoring a duplicate witness every tick. Ops in the same stretch: three seats sat parked ~3 h on wait loops whose `pgrep -f` pattern matched other waiter shells (hook hint fixed, `1dcdfa5c5`); the mesh pid files had gone stale after the morning respawn, so both restart scripts were no-ops until the real pids were signalled; a neighbour lane's build had overwritten the slot binary without the iroh feature. Peers now run the feature-full Tasks 4–12 binary at baseline.
- 2026-09-05 — **Task 12 landed** (`b3b444344`): the lineage bridge — a ticker that, for every role with an open window, held-carries one page per tick per neighbour (`CarrySource::Held`, neighbours from `known_agents` minus self, per-agent cursors, restart at 0 when a neighbour's digest changes mid-walk, errors never abort a tick, the idle tick makes no conductor call); the passport's lineage view now carries the sweep (`observed_head`, `total`, `digest`, `carried`, `last_sweep` per agent) and `backwardCarry: unavailable` while v1 has no witness type. Storage claims no completeness: the a2o Station 6 step compares james's view of jessica against jessica's own export. `/admin/lineage/reset` also clears the sweep. Gate green (all targets), which also closes the deferred clippy leg on the Task 17b coverage round. From this task on, claims, fulfils and verdicts are recorded through `epr flow` (the valueflow-authoring surface another session shipped today) — the SDD ledger becomes a projection. Under review: `known_agents` is read through the role's client, which resolves to the AUTHORING app mid-window.
- 2026-09-05 — **Task 17b + Task 16 hardening landed** (`9d4753a93`, coverage `0fe678ef9`; rakia `13cb31d`, packager `02062dbd2`): the root now reaches an installed cell — `install_lineage` writes `constitution_root` beside `lineage` in the side app's modifiers (`roleBinding.constitutionRoot` in the manifest, `--constitution-root` in the packager) — and `verify_path` checks the path's root against the INSTALLED root first (a fact about this peer that no release can declare its way out of), else the manifest's declared root, else skips and logs `root: undeclared` (skipping is not passing). The roster arm hardened: a roster is bound to its own `constitution_root`, signers are deduped, an explicit quorum of 0 floors to 1 at parse and at use, an unaddressable path cid is absence (`path_not_notarized`) never our outage, and the refusal text separates "is not an address" (terminal) from "not found" (may gossip). Two fixtures were fake hashes and are now real — a hardening that refuses non-addresses catches its own tests. Design note for Station 8: the root minted into the SIDE app never reaches installed reality until the sunset makes v2 the role's reading cell.
- 2026-09-05 — **Tasks 16, 17, 18 landed** (`825a090df` `10cb3dc00` `4425bb6fb`; `4fe69b918`): `verify_path` now reads the roster commitment through the peer's own conductor and refuses `quorum_unmet` for off-roster signers (only members count; an unaddressable roster cid is absence, never our outage); the passport reads each role's `constitution_root` from the installed cell's modifiers so `root_mismatch` is reachable; v1 gained `export_held_records` + `known_agents` and v2's carry a `source: Held(agent)` — a neighbour's record held-carried with the courier's witness, proven with two agents in the sweettest (default hash unchanged). **Stated plainly for Station 10:** the roster check as landed is a coherence check against an author-supplied, author-MINTABLE electorate — mishpat integrity verifies no signatures and no arm binds a roster to the elohim's key or root — so it raises forgery cost from one commitment to two; it is not yet a trust boundary. The boundary is the integrity-side lineage arm (hash-moving; sunset hardening) or storage re-verifying the roster's chain to the root. Hardening in flight (17b): root written at install and compared installed-or-declared; roster bound to its own root; signers deduped; quorum floor; unaddressable path cid is absence. **Held-view rule for the sweep (Task 12):** on the held path `total`/`digest`/`next_cursor` describe the courier's integrated view, never the neighbour's chain — a held receipt is never self-evidencing; completeness is `observed_head` against local `total` or the neighbour's own export.
- 2026-09-05 — **MVP measured; gaps minted from measurement (controller).** Tasks 1–11 landed; Stations 1, 2, 3, 4, 9 green in one run (r15) and 1–2 re-green after the fix round (r16). The review of the live run found and the fix round closed a path forgery: a peer decided the lineage class from the commitment BODY's self-declared `action`, and one mishpat validator never pins that field to the entry's — now both storage homes key on the entry's own action, with forgery tests. Four gaps the mesh exposed, none of them in the plan before it was measured: **(G1) roster check** — nothing anywhere verifies that a path's signers are members of the earned roster under the declared root (`validate_lineage_signatures` never reads `roster_cid`; `verify_path` counts signatures only) → storage `verify_path` reads the roster commitment through the peer's own conductor and refuses `quorum_unmet` for non-members (evidence, not authority); the integrity-side lineage arm is sunset hardening (hash-moving). **(G2) constitution root in the passport** — `constitution_root` is `None` for every installed role, so `root_mismatch` can never fire → the passport reads each role's root from the installed cell's modifiers. **(G3) held-view export** — `export_records` is a local `query()`, so a v1 cell cannot hand a sweep its neighbours' records → v1 gains `export_held_records(agent, cursor, limit)` over `get_agent_activity` + DHT `get` (Task 12's sweep enumerates agents from the registry's own registrations); Station 5 is measured through that sweep. **(G4) revocation visible to every peer** — a non-authoring peer holds no projection row, so `revoked_at` reads `None` and `verify_path` checks it first → path lifecycle from DHT truth (`get_commitment_state_links` through the peer's own conductor), Task 13. **Trust boundary, stated:** today a receiving peer establishes a path from a body that only the AUTHOR's conductor validated (mishpat integrity has no lineage arm) — G1 and G4 are that one hole seen from two sides. **(G5) carry idempotency** (query the chain for the entry hash before `create_entry`) — a retried page commits a second witness today. Measured facts kept: the installed v2 hash folds the role modifiers (`uhC0kN2gubu9…`, not the packed `uhC0kEKiI…`); a same-agent cross-cell call needs no cap grant on 0.7.0 in both the sweettest and the fleet shape.
- 2026-09-05 — **Task 11 part 2b, fix round 1** — one CRITICAL, found in review, fixed with tests. `path_evidence`'s lineage-class decision read `payload_json["action"]`, which is SELF-DECLARED: `mishpat::validate_commitment_payload` dispatches on the ENTRY's action and only some arms pin the body's second copy to it (`validate_ratifies_limit_gradient` never does), so a commitment created as `ratifies-limit-gradient` whose body declares `"action": "migrates-lineage"` reaches the coordinator carrying NO signatures — and would have read as an ACTIVE migration path on every peer without a projection row, i.e. every peer but the author. Both sides now key on the ENTRY (`GetCommitmentOutput.action` on the read side, `commitment.action` on the projection side, which `signals.rs` already passed), each pinned by a forgery test. **Two claims qualified**: `validate_lineage_signatures` runs AUTHOR-SIDE ONLY — mishpat integrity's `commitment_action_requirements` has no lineage arm, so receiving peers never re-verify a lineage quorum on gossip; "active" states the LIFECYCLE, never the quorum's soundness on this peer. **OPEN TRUST BOUNDARY, named not closed:** a `revokes-commitment` projects off the AUTHOR's post-commit signal too, so on every other peer `path_evidence` reads "no row" and therefore "not revoked" — and `verify_path` checks `revoked_at` FIRST. An elohim's revocation of a migration path is invisible to the peers it is meant to stop. That is the SAME hole as Station 10's missing roster check, pointed at the revert instead of the quorum, and it is why Station 7 is not yet measurable. The seam that closes it: read the revocation where every peer can see it — `mishpat::get_commitment_state_links` through the peer's OWN conductor, the same C5 rail the commitment body already comes down — rather than off a projection only the author holds (Task 13 / Station 7). Also tightened: Station 1 now asserts the PATH arm BY NAME (`path_not_notarized`) rather than merely "not `dna_lineage_mismatch`", which would have passed on a stray `manifest_schema_invalid` or `conductor_unavailable`; and `readV1Export` refuses a baseline that hit its page cap, since a PREFIX of the chain would make every "untouched" and `carried == v1Count` comparison compare the wrong set. Gate: storage lib tests + `cargo clippy --features "p2p p2p-iroh" --all-targets -- -D warnings` EXIT=0 + fmt; Stations 1 and 2 re-run live on the rebuilt mesh.

- 2026-09-05 — **Task 11 part 2b closed: Stations 1, 2, 3, 4 and 9 pass TOGETHER in one run** (`cucumber-stations-mvp-r15.{log,json}`, 5 scenarios / 55 steps, 6m44s, three peers, conductors untouched). Five of the seven MVP stations are measured green on the household mesh; Stations 5 and 10 are red and BLOCKED with their gaps named in the entry below. The story's own harness notes for whoever runs it next: the feature is `@wip`, so a live run needs `A2O_RUN_WIP=1` or every scenario is HELD and the run is a green no-op; `--name '^Station 1'` also matches Station 10, so the em dash is what makes the anchor exact; and each scenario is its own world — the fixture drops its cross-step memo and mints fresh channels per scenario alongside the peer reset it already did, because a second scenario inheriting the first's memo goes looking for a side app the `Before` has just uninstalled.

- 2026-09-05 — **Station 9 GREEN; Stations 5 and 10 RED and BLOCKED with the gaps named** (`cucumber-stations-mvp-r13` 11/11 44s; `-r14` 9 passed / 1 failed). **Station 9:** all three peers dual-celled (carried 261/261, 135/135, 141/141), then the forgery committed through EACH peer's own v2 cell under that peer's own key — harder than the story's fourth peer, because the refusal must come from v2's validation rather than from the network declining a stranger. Both arms refused on all three in v2's own words ("carried signature does not verify against the action's signer" = signature invalid; "lineage_dna_hash … is not declared in this DNA's lineage property" = lineage unrecognized), and all 141 honestly-carried records still answer a witness afterwards. **Station 5 is BLOCKED on the zome:** `export_records` is `query(ChainQueryFilter…)` — the calling agent's own chain, local by construction — so james's v1 cell can only export james's records and `carry_from`'s held-carry branch is unreachable from the only caller that exists; and no bridge sweep exists in elohim-storage. It needs a v1 extern that exports another agent's records off the DHT plus a sweep caller. **Station 10 is BLOCKED on two absences.** Measured live: a `migrates-lineage` commitment notarized through jessica's mishpat cell — a household peer, not the bootstrap steward, so off the declared roster — was accepted by every peer. There is NO roster check anywhere: `verify_path` compares only the COUNT of `signatures` against `required_signatures`, and `validate_lineage_signatures` verifies each signature against its own claimed key and never reads `roster_cid`; `quorum_unmet` is further unreachable from any commitment mishpat will create, since that validator refuses an under-quorum payload at commit. Measured from the code: `verify_path` compares roots only when `inst.constitution_root` is `Some`, and `from_happ_passport` sets it `None` for every role (verify.rs:136-138), so `root_mismatch` cannot fire on a live peer. Station 10 needs a machine-readable council roster — the story declares one in prose and nothing on the substrate holds it — and a per-role constitution root on the passport.

- 2026-09-05 — **Stations 3 and 4 GREEN on the household mesh, and Stations 1-4 green TOGETHER** (`cucumber-stations-mvp-r8` S3 11/11 50s, `-r10` S4 11/11 31s, `-r12` Stations 1-4 in one run, 4 scenarios / 44 steps, 6m11s): james's runtime installed v2 BESIDE v1 under his one key and carried his whole v1 chain, and every record is in v2 at the same entry hash under a witness v2's own validation accepted. On the wire: one agent key on both apps (`elohim` and `elohim@EKiIscIk5BDd`), the passport's role-level `lineage` view with `closed: false`, the conductor pid unchanged, james's v1 chain still at its pre-crossing digest and count, and a carry receipt with `carried == v1Count` and `digest == v1Digest` and one witness per page. **The installed v2 hash is NOT the packed one** — james authors on `uhC0kN2gubu9…` while `hc dna hash node-registry-v2.dna` says `uhC0kEKiIscI…`; the install folds the hApp role modifiers (inherited network seed plus the lineage property `install_lineage` writes). Task 1's trap, now measured on the authoring side, and the reason the manifest's `migrateFrom` must come from a live passport. **Three more measured facts.** (a) The 0.7 `SignedActionHashed` wire shape splits the action into `content.header` + `content.data` and the entry hash lives on `data` — read at the pre-0.7 flat level it yields nothing and reads as an empty chain, which was Station 4's first red. (b) `LineageCarryReceipt` reached a tracing line and nothing else; it is now kept on `appliedRelease.carry` (`state.rs`/`watch.rs`), because a completeness proof no operator and no check can read is not a proof. (c) Above `staging` the floor enforces the attestation threshold for EVERY mode, canary included, so an earned-first ordering is circular — the canary cannot adopt what it must first attest — and the canary necessarily acts at `staging`. That is rung 5's ceremony unchanged, and it means the story's "the release is earned" at Station 3 is satisfied by the notarized path plus a release the canary's mode acts on, with promotion belonging to the peers who follow james.

- 2026-09-05 — **Station 2 GREEN on the household mesh** (`cucumber-stations-mvp-r7.{log,json}`, run 20260904130252, 13/13, 2m01s; driven from the reds in `r5`/`r6`): the path is refused before it is notarized (`path_not_notarized` on all three) and admitted after (`state: ok` on all three), with no peer asked anything. The elohim's signature is the zome's — `create_lineage_commitment` took `signatures: []` and returned a commitment signed by matthew's own agent key; the harness supplied none. **Two storage defects found and fixed on the way, both of which made the positive branch structurally unreachable.** (a) `mishpat_projection` had no arm for `migrates-lineage`, so it fell to the unknown-action fallback and projected `state: "proposed"` — while `verify_path` establishes a path only on `"active"`, and nothing in the tree ever wrote that value (`set_state`'s only caller was a unit test). A lineage commitment's notarization IS its activation (quorum verified in-wasm before the entry could exist; its only other state is revoked, which `verify_path` checks first), so the two lineage actions now project active and every class with a real acceptance step keeps `proposed`. (b) `CommitmentCommitted` is a post-commit signal on the AUTHOR's conductor, so only the notarizing peer holds a row at all: jessica and james read the same commitment off their own DHT view, found no row, and `path_evidence::lifecycle` read that absence as `proposed` — our own index gap asserted as a statement about the elohim's governance, the same fail-open-into-a-fact inversion the Task 7 review caught, pointed the other way. An absent row is now an ANSWER of no-row, and a lineage action's notarized entry carries its own `active`. **Still open (Station 7's):** a REVOCATION reaches only the peer that projected it — the same signal asymmetry, on the field `verify_path` checks first. **Two harness facts.** `path_not_notarized` needs a DECODABLE cid (`HoloHash`'s base64 decoder verifies the DHT-location bytes, so a hand-written pointer errors and correctly answers `conductor_unavailable`); and the release→path pointer runs ONE way, so the commitment is minted first and the release names it, on a FRESH channel — `assertAdmissibleOverEarnedHead` refuses a publish over an earned head the acting peer has not adopted, which for the release Station 2's first half exists to have refused it never will.

- 2026-09-05 — **Station 1 GREEN on the household mesh** (`cucumber-stations-mvp-r3.{log,json}`, run stamp 20260904123730, 1 scenario / 9 steps, 2m18s): the first station measured live. A `happ-lineage` release whose `migrateFrom` names the INSTALLED v1 node_registry hash (`uhC0kNpWca3k…`, read from each peer's own `/version` passport — NOT the packed `node-registry-v1.dna` hash `uhC0kyvKw…`, which differs because the install folds the hApp role modifiers) is admitted by all three peers; the same v2 with no parent named is refused by all three with `dna_lineage_mismatch` → "lineage mismatch". **Two measured facts.** (1) The story's literal channel name `runtime:lineage:node_registry:commons` is not a legal channel id: the release-manifest schema's `channelId` pattern (`^runtime:[a-z0-9][a-z0-9-]*:…$`, mirrored by `verify::is_channel_id`) has no underscore, and packaging refuses before anything reaches the mesh — the a2o run scopes to `runtime:lineage:node-registry:a2o-<stamp>`, and the story line should be re-spelled with a hyphen when it is next touched. (2) The positive branch's live refusal is `conductor_unavailable`, not `path_not_notarized`, because Station 1's placeholder path-commitment cid is not a decodable EntryHash — `fetch_path_evidence`'s `get_commitment` errors and answers `Unreachable`, which is C4 holding exactly as designed. Since `verify_envelope` (verify.rs:1046) runs before `verify_path` (:1059), REACHING the path arm is itself the admissibility proof, and that is what the step asserts. Harness notes for the stations that follow: the feature is `@wip`, so a live run needs `A2O_RUN_WIP=1`; `--name '^Station 1'` also matches Station 10 (use `'^Station 1 —'`); and 60s is too short a convergence budget — a release published on matthew needs the other peers' own 60s sweep after gossip, so the polls now carry rung 5's `240_000`.

- 2026-09-05 — **Task 10 landed** (`ad5144d24` + `622596db4`): the fixture — `lineage-candidate.ts` mints the `happ-lineage` manifest from `node-registry-v2.happ`; `lineage-commitments.ts` shapes the elohim's commitments; `POST /admin/lineage/reset` (reset every role's window first, then disable and, with `{"uninstall":true}`, uninstall every `elohim@…` side app) converges a peer to the v1 baseline; `hc-mesh.sh lineage-reset` fans it across the household; the story's `Before`/`AfterAll` call it so every run starts and ends at baseline (rung 5's lesson). All nine code tasks are landed; what remains is the measured half — the live stations.
