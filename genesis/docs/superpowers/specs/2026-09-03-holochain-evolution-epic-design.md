...
| 5 held-carry | red — **BLOCKED (zome)**: `export_records` is a local `query()`, so `carry_from`'s held-carry branch is unreachable; no bridge sweep exists | task-11-part2b report |
