...
- The local mesh for this arc is **two peers, two doorways** by default: `MESH_PEERS=matthew,jessica`. Never add a process to prove a *transport* scenario — add a scenario to the library. The one exception is the **fanout family** (spec §3.3): 1 recoverer ← N survivors, which needs N+1 peers; it runs with `MESH_DOORWAYS=0` (no doorway A/B, no mongod) to stay under the compute ceiling, capped at 3 peers in this plan (the prologue casts a 3-member household).
...
- Produces: `peer_transport <name>` → `libp2p|dual|iroh` (per-peer declaration, falls back to `MESH_TRANSPORT_BACKEND`); `restart_env_overlay <captured-environ> <peer-name>`; `mesh_transport_backend_from_status` prints a homogeneous mode OR a `name=mode,name=mode` set (never `unknown` for a readable mixed mesh); `MESH_DOORWAYS=0` makes `start` skip mongod + doorway A/B (storage peers launch without `ELOHIM_DOORWAY_URL`), `status` print `doorways: disabled (MESH_DOORWAYS=0)`, and `stop` unchanged.
...
- [ ] **Step 5b: `MESH_DOORWAYS=0` (fanout family runs doorway-less)** — add beside the knob block: `MESH_DOORWAYS="${MESH_DOORWAYS:-1}"`. In `start`: wrap the mongod launch (the block at "# 0. mongod") and the doorway A and doorway B launches in `if [ "$MESH_DOORWAYS" = "1" ]; then … fi`; in the storage launch replace `ELOHIM_DOORWAY_URL="http://localhost:$DOORWAY_PORT" \` with `${MESH_DOORWAYS:+ELOHIM_DOORWAY_URL="http://localhost:$DOORWAY_PORT"} \` guarded so it is emitted only when `MESH_DOORWAYS=1` (simplest: compute `doorway_env=()` / `doorway_env=(ELOHIM_DOORWAY_URL=http://localhost:$DOORWAY_PORT)` above the loop and prefix the launch with `env "${doorway_env[@]}"`); in `restart_env_overlay` emit the `ELOHIM_DOORWAY_URL` line only when `MESH_DOORWAYS=1`; in `status_all` print `doorways: disabled (MESH_DOORWAYS=0)` instead of the two doorway probes. Add to the test file:
...
**Interfaces:**
- Consumes (sourced from `hc-mesh.sh`): `PEERS` (2 or more; survivors = all but the recovering peer), `MESH_DOORWAYS`, `http_port <idx>`, `storage_pid_for_port <port>`, `restart_storage <peer>` (the function behind the `storage-restart` verb, hc-mesh.sh:678), `peer_transport <name>`, `MESH_DIR`, `DOORWAY_PORT`, `DOORWAY_B_PORT`.
...
```
# scenario	peers	doorways	survivor	recovering	expect
# The configurations are the library; the slots cycle through it. `peers`
# names the mesh shape a scenario needs; the matrix reshapes the mesh only
# when consecutive scenarios differ in peers/doorways (a full stop/start).
homo-libp2p	matthew,jessica	1	libp2p	libp2p	recover
homo-iroh	matthew,jessica	1	iroh	iroh	recover
homo-dual	matthew,jessica	1	dual	dual	recover
mixed-dual-libp2p	matthew,jessica	1	dual	libp2p	recover
mixed-dual-iroh	matthew,jessica	1	dual	iroh	recover
split-libp2p-iroh	matthew,jessica	1	libp2p	iroh	no-shared-transport
# fanout family — the torrent effect (spec §3.3): 1 recoverer <- N survivors, doorway-less
fanout-1	matthew,jessica	0	dual	dual	recover
fanout-2	matthew,jessica,james	0	dual	dual	recover
```
...
( set +e; RECOVERY_MATRIX_SOURCE_ONLY=1 source "$here/../hc-mesh-recovery-matrix.sh"
  ...
  t "fanout-2 declares 3 peers, no doorways" '[ "$(matrix_scenarios | awk -F"\t" "\$1==\"fanout-2\"{print \$2\"/\"\$3}")" = "matthew,jessica,james/0" ]'
...
rc=0; rows=(); shape_now=""
reshape_mesh() { # <peers-csv> <doorways 0|1> — full stop/start/prologue; the ONLY place the process count changes
  echo "=== reshaping mesh: peers=$1 doorways=$2 ==="
  MESH_PEERS="$1" MESH_DOORWAYS="$2" "$SCRIPT_DIR/hc-mesh.sh" stop; true   # 144 = own process group, fine
  MESH_PEERS="$1" MESH_DOORWAYS="$2" "$SCRIPT_DIR/hc-mesh.sh" start && MESH_PEERS="$1" MESH_DOORWAYS="$2" "$SCRIPT_DIR/hc-mesh.sh" prologue
}
while IFS=$'\t' read -r name peers doorways t_surv t_rec expect; do
  if [ "$shape_now" != "$peers/$doorways" ]; then reshape_mesh "$peers" "$doorways" || { echo "reshape failed for $name" >&2; rc=1; continue; }; shape_now="$peers/$doorways"; fi
  export MESH_PEERS="$peers" MESH_DOORWAYS="$doorways"
  set +e; source "$SCRIPT_DIR/hc-mesh.sh" >/dev/null 2>&1; set -u
  for shape in "${SHAPES[@]}"; do
    for run in $(seq 1 "$RUNS"); do
      # two peers: roles alternate; N>2 (fanout): the LAST peer always recovers, the rest survive
      if [ "${#PEERS[@]}" -eq 2 ]; then ri="$(matrix_recovering_index "$run")"; else ri=$(( ${#PEERS[@]} - 1 )); fi
      rec="${PEERS[$ri]}"; surv=""; pt=""
      for k in "${!PEERS[@]}"; do if [ "$k" -eq "$ri" ]; then pt+="${pt:+,}${PEERS[$k]}=$t_rec"; else pt+="${pt:+,}${PEERS[$k]}=$t_surv"; surv+="${surv:+,}${PEERS[$k]}"; fi; done
      export MESH_PEER_TRANSPORTS="$pt"
      echo; echo "=== $name · $shape · run $run · survivor=$surv($t_surv) recovering=$rec($t_rec) ==="
      # put every SURVIVOR into its declared plane first (they must be serving when the loser returns)
      for s1 in ${surv//,/ }; do MESH_RESTART_APPLY_PROFILE=1 "$SCRIPT_DIR/hc-mesh.sh" storage-restart "$s1" >/dev/null 2>&1 || echo "  survivor $s1 restart non-zero; continuing"; done
      sleep 10
      ...
      # doorway-less runs cannot take a quiesce measure (the gate needs both doorways)
      [ "$doorways" = "0" ] && continue
...
- [ ] **Step 4: Unit test, then one cheap live cycle** (one scenario, one shape, one run — the honest red; the matrix will reshape the mesh to `matthew,jessica` + doorways first):
...
- [ ] **Step 1: Run the full baseline** (8 scenarios × 2 shapes × 3 runs = 48 recoveries plus two mesh reshapes; budget ~3–5 h at the dev-tier pacing profile — run in the background, foreground-check with `tail`):
...
fan=[r for r in rs if (r["labels"].get("scenario") or "").startswith("fanout")]
if fan:
    print("## Fanout (the torrent effect): time_to_recover vs survivors — the swarm-curve claim\n")
    print("| scenario | survivors | shape | median s | min | max |\n|---|---|---|---|---|---|")
    for (sc,sh),v in sorted(g.items()):
        if not sc.startswith("fanout"): continue
        t=[r["time_to_recover_s"] for r in v if r["recovered"]]
        print(f"| {sc} | {v[0].get('survivors','?')} | {sh} | {int(st.median(t)) if t else '—'} | {min(t) if t else '—'} | {max(t) if t else '—'} |")
    print()
