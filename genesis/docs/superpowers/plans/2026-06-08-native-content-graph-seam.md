...
- [ ] **Step 1: For each new doc, build the JSON** by mirroring `manifesto.json` and inlining the source markdown body into `content` (read the source `.md`, JSON-escape it). Use these field values:
  ...
  - Do **not** add `did`/SEO blocks if minimizing; a minimal node (id/title/contentType/contentFormat/content/reach/tags/relatedNodeIds/metadata) seeds and serves fine. Match manifesto's richer shape only if SEO/federation is wanted.
