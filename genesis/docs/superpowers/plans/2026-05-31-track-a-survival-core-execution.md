...
- **Resiliency (durability) — this IS the graduation gradient:** `local-personal (1 replica)` → `dwelling-backed (2, a reciprocal pact)` → `collective-backed` → **High-Available (HA)**. **HA is a useful *convention*, not a literal count** — it names the *floor* at which an EPR is reliably stable + available in the p2p data layer (**~5 hub-quality replicas across distinct failure domains** is the working heuristic for that floor). A *hub-quality replica* = a full copy on a stewarded, always-on, WAN-reachable hub (not a transient peer cache). **Grounding (2026-05-31):** the HA floor must reconcile with existing machinery, not replace it — (a) `distribution_view.rs::replica_target_for(reach)` already scales replica *targets* by reach (`Private 2 → Public 16`; enum lacks `commons` — a reach-vocab **drift to fix**); (b) `household_resilience.rs` classifies `protected = ≥3 households + ≥2 online peers`. **OPEN (Phase 4, not Phase 1):** express the HA floor as a convention layered over `replica_target_for` (likely "met the reach-scaled target across ≥N distinct hub-quality failure domains"), not a hardcoded number → story-harvest.
