...
landed: 2026-09-11
verified_by: |
  All seven gaps approved by independent reviewer seats 2026-09-10/11 (verdicts on each gap ledger); native parity fold 53/53; kit directories deleted; habit dev-system-equilibrium stays RED per the plan verdict rule (byte leg unmet for one reader).
class: devflow
...
cites:
  ...
  - "private-thought-governed-fruit | private-thought-governed-fruit | sha256:c7adde5bd6ad4520 | status: stale — target content moved on; re-verify | path: genesis/docs/architecture/private-thought-governed-fruit.md"
---
...
## The privacy line (operator ruling, 2026-09-10, constitutional)

Thoughts are private; what is said and done is the fruit the governed world may judge
(`genesis/docs/architecture/private-thought-governed-fruit.md`). For this plan: station four
imports only entries a participant authored for others (`.claude/memory/*.md` notes written
as shared knowledge); recall receipts, continuation files, transcripts, scratch and any
reasoning trace stay private records and are never imported, projected, witnessed or
targeted by feedback. Station five's native recall executor keeps receipts private to the
session that made them and exposes only what was read and concluded. Feedback signals and
verdicts target contributions, notes, edits and verdicts by CID, never a private store. No
review or ceremony surface may require a transcript of reasoning as the price of standing.
...
- [x] Every threshold the kit hard-codes is a declared bound, the native `epr flow report` folds each bound into a witnessed outcome, the SessionStart headline is a projection of that fold, and the drift-signal hooks append structured observations instead of writing JSON state.
