...
use it, bounded from inside; (strategic, T13 spike) extend the conductor fork we ALREADY
carry (the kitsune2 `store_slice_hash` patch ships on that lineage, operator-gated on
image build) with app-interface call deadlines/cooperative cancellation for read-only
zome calls, written upstreamable (holochain / holochain-client-rust PR sketch). Nothing
is off-limits: if upstream declines, the fork carries it — the substrate owns its
scheduling floor.
...
| T13 | Conductor-fork spike: app-interface call deadline/cooperative cancellation for read-only zome calls, on the existing fork lineage (kitsune2 patch rail); deliverable = design note + upstream PR sketch + fork-vs-upstream decision memo for the operator | **Opus** (spike, design-bearing; no fleet deploy without operator gate) | — (independent; informs post-sprint) | conductor fork repo, hc_client.rs | fork branch + design note |
