...
> **LANDING OUTCOME 2026-08-05: Wave 2 is ON `dev` and DEPLOYED — everything except the flip itself.** Landed `e3c155638..43c05a5e4` (14 commits) via the overnight integrator shift `2026-08-05T03-57-land-shakeout-wave2-relay-dual`; deploy evidence verified green at 7h and re-verified at close. **Sovereign relay fleet is LIVE:** `iroh-relay 0.95.1` built `--locked` in CI and pushed to Harbor (closes U1/U2), both per-doorway relays serving (`relay.alpha.elohim.host` + `relay.elohim.host`, `/ping`=200, `/generate_204`=204, TLS verified), per-human `resolveRelayUrl` rendering confirmed live in the edge console with zero mapping errors. **Dual dataplane is LIVE:** `"Dual: DualGossipPublisher wired into P2PNode"` on all 7 alpha peers, `irohNodeId` present on BOTH doorways with distinct identities (A `0e517f68…`, B `e627d641…` — the per-doorway design's prediction, confirmed), mesh at 6 peers. **n0 contamination: clean** across 708k log lines — the never-n0 defaults held. E2 prep landed too, so the flip is now a pure image-tag change. **What remains for Wave 2 is Task E2 alone**, gated on the operator ratification checklist (design doc §7).
>
> Two things the landing surfaced that the campaign should carry forward: (1) **flip-checklist item A4** — `edgenode/alpha.yaml` pairs signal_url=doorway-A with relay_url=doorway-B's host, a pre-existing mismatch under the per-doorway design; correct it when touched at flip time. (2) **A live blocker adjacent to, but not caused by, this wave** — adam's conductor panic/write-guard class is steady-to-escalating (23→34 panics across two 2h windows, adam >90%), and doorway-B pins its ZomeCaller to adam alone, so `GET /api/v1/federation/doorways` has been dead for hours. Backlog `conductor-source-chain-unwrap-panic-db-timeout` (critical). Whether the dual fan-out *raised* adam's rate is unresolved — worth settling before Stage 1 puts the whole namespace on a new transport.

> **OUTCOME 2026-08-05 (E1): DONE — design + manifest authored, two campaign assumptions corrected.** Deliverables: `genesis/docs/content/elohim-protocol/architecture/2026-08-05-wave2-relay-sovereignty-design.md` (decisions D1-D10, operator ratification checklist §7, STILL-UNKNOWNs U1-U7) + relay Deployment/Service/Ingress sections in `genesis/orchestrator/manifests/doorway/{alpha,alpha-b}.yaml` (inert until deployed). **OPERATOR RULING 2026-08-05: the relay is a doorway concern** — the signal function's iroh-generation successor, so PER-DOORWAY relays mirroring the signal topology (`relay.alpha.elohim.host` ↔ doorway A, `relay.elohim.host` ↔ doorway B, per-human homing via `primaryDoorway` like `signalUrl`); this restores the dual-WAN redundancy the single-relay draft had conceded (design D2/D3/D7 revised; peers dial through each other's home relay, so heterogeneous relays are protocol-native). Server verdict: stock `iroh-relay 0.95.1` built `--locked` into Harbor (four evidence lines; `--locked` load-bearing on the dalek pre-release pin); the iroh-0.92 storage dataplane can share the instance (wire-identical 0.92↔0.95.1) — two config keys, one hostname. Corrections, both source-verified by the orchestrator: (a) **the flip is an image-feature change, not config** — kitsune2 0.4.1 cfg-gates tx5-wins-when-compiled, so E2 = second Harbor tag `hc-elohim-0.6.3-iroh` (`HC_FEATURES` build-arg swap), rollback = tag repoint; (b) **a partial flip transport-partitions the DHT** (tx5 `wss://` vs iroh `https://` peer URLs can't interdial) — E2's household-first staging is replaced by Stage-0 off-DHT mechanism proof + one atomic alpha-namespace window (genesis-pair rule generalized to the whole namespace). Also found: coturn retirement is blocked on re-homing the `relay-addr-beacon` (it owns the zone's dynamic-WAN A records), and the storage dataplane carried its own n0 defaults (see E3 outcome).

- [x] **Task E1 (design doc, Opus-assisted):** self-hosted `iroh-relay` deployment design — manifest under `genesis/orchestrator/manifests/infra/`, relay URL + `base64_auth_material` wiring into the conductor config templates, explicit decision record rejecting the n0 public-relay default (sovereignty stance), retirement plan for the SBD signal server (`doorway/doorway-service/src/signal/` — dead code once no conductor speaks SBD) and the coturn pair (verify nothing else consumes STUN/TURN first: grep manifests + doorway for coturn/stun references). Include rollback: tx5 feature stays compiled for one full wave; flip is per-env config, alpha first.
- [ ] **Task E2 (execution):** flip `transport-iroh` default on the rebased conductor in alpha config; deploy; watch the substrate trust-contract probes (per-seam smoke, `conductor-diagnostics`, canonical-head propagation line). @requires:alpha-cluster-6peer for the full 6-peer soak; the M/J/J household mesh proves the mechanism first (`feedback_household_nodes_is_the_stable_floor`).
  > **CORRECTED 2026-08-05 by E1 evidence (design doc §4.1/§5.2):** the flip is a build-feature change (image tag `hc-elohim-0.6.3-iroh`), not a config edit, and household-first staging on the live DHT is NOT executable — a partial flip transport-partitions the namespace. Execute per the design doc: Stage 0 off-DHT two-conductor proof against the live relay → Stage 1 atomic alpha-namespace rollout (all live peers one window, genesis pair included) → Stage 2 six-peer soak; the `direct=` ratio from Stage 2 decides Phase-B QAD. Gated on the operator ratification checklist (design doc §7) and a live `relay.elohim.host`.
> **OUTCOME 2026-08-05 (E3): DONE in-repo, verification pending deploy.** Manifests `4cb207de3` — `ELOHIM_TRANSPORT_BACKEND: "dual"` in the humans template, adam-firstman (hand-rendered, template-external — a coverage site the task text missed), and edgenode/alpha.yaml (dead wrong-name `TRANSPORT_BACKEND: "dual-stack"` block retired); staging/prod untouched (alpha-first). Prerequisite fix `f9087cfad` — E1 review found the dataplane carried its OWN n0 defaults (`use_n0_relays: true` AND n0 pkarr discovery with address publication): flipped to sovereign never-n0 defaults, added `relay_url`→`RelayMode::Custom` (hard-fail on bad URL) + `ELOHIM_IROH_RELAY_URL`/`ELOHIM_IROH_USE_N0_*` opt-in envs; dual-mode reach verified independent of n0 discovery (iroh addressing rides the Phase-12 `peer_transport_manifest` plane; LAN/direct now, WAN when the relay is live). Gates: fmt/clippy/default-release 0, p2p-iroh 156 targets green, sole red = parked known-red `seed_e2e_dual_address`. Post-deploy proof points: the dual sentinel line + `irohNodeId` in `/p2p/status` — closes `project_iroh_dataplane_actual_state`'s "STILL OPEN" check.

- [x] **Task E3 (dataplane dual enablement — the separate layer):** add `ELOHIM_TRANSPORT_BACKEND: "dual"` to `genesis/orchestrator/manifests/humans/_edgenode-consolidated.template.yaml` env block; DELETE the dead `TRANSPORT_BACKEND: "dual-stack"` block from `genesis/orchestrator/manifests/edgenode/alpha.yaml:286-290`; verify via the true sentinel log line + `/p2p/status` `irohNodeId`. Rides the existing `=0.92` build already shipped in the image (decision 2026-08-04: does NOT wait for the iroh 1.0 bump, which deferred to Wave 3). This closes the "STILL OPEN: live 2-node dual-delivery check" from `project_iroh_dataplane_actual_state`.
...
## Wave-1 residuals (tracked)

- Known-red `seed_e2e_dual_address::seeded_blob_is_fetchable_by_both_addresses` — exclusion contract lives in backlog `genesis/data/timeline/backlog/2026-08-04-dual-address-blob-router-unmanifested-blake3-400.md`.
- Deferred minor: `elohim/elohim-storage/src/p2p_iroh/sync.rs:190-207` `echo_backend_dispatches_each_variant` asserts only `GetHeads` despite its name (pre-existing; fix opportunistically when that file is next touched).
