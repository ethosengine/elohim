...
- [x] Failing tests with a mock `CommitmentAuthor`/`HcClient` seam (copy the salvage tests' harness at `salvage_commitment_author.rs:~700`): `expands_one_custody_blob_per_advertised_witness`, `is_idempotent_across_ticks`, `skips_when_self_agent_unresolved`, `refuses_beyond_atoms_per_hour_with_a_logged_reason`, `ignores_blobs_the_ward_did_not_advertise`.
- [x] Implement; wire the dispatch on the custody sweep cadence; `cargo test --lib spool_custody_author`; clippy clean.
- [x] Commit `feat(storage): custody-spool expands to per-hash custody-blob on the custodian's own conductor (SpoolCustodyAuthor)`.
...
- [x] Shape test (`genesis/seeder` test runner; grep the precedent for `buildCeremonyCustodyInput` tests): the id is deterministic and equals the Rust `deterministic_spool_custody_id` for the same inputs (pin one vector in both languages).
- [x] Prologue leg; the orchestrator runs `just mesh prologue` and asserts `GET :8090/api/v1/commitments?action=custody-spool` returns rows with `provider == matthew`'s agent for `receiver == jessica`'s agent.
- [x] Commit `feat(seeder): spool custody — each household custodian authors a custody-spool toward every other member on its own conductor`.

### Task 5: mesh dials

**Executor:** Codex. **Reviewer:** Opus. **Files:** `app/elohim-app/scripts/hc-mesh.sh` only.

- [x] In ark mode, each storage peer's launch environment gains `ELOHIM_ARK_SPOOL_PATH=$LOCAL_DEV_DIR/<peer>/ark`, `CUSTODY_SWEEP_SECONDS=${CUSTODY_SWEEP_SECONDS:-15}`, `INVENTORY_BROADCAST_SECONDS=${INVENTORY_BROADCAST_SECONDS:-15}`; `just mesh status` prints the spool path per storage peer; header doc updated. `bash -n` and shellcheck unchanged. Commit `feat(mesh): ark mode points each storage peer at its ark spool and dials custody/inventory to 15 s`.
...
- [x] `Given Matthew and James have each counter-signed a commitment to custody Jessica's witnesses` — `GET {matthew}/api/v1/commitments?action=custody-spool` and `{james}` → a row whose `provider` is that peer's own agent and `receiver` is Jessica's agent; fail with "run `just mesh prologue`" when absent.
- [x] `Then within 60 seconds Matthew and James each hold a copy of the witness with the same content hash` — poll ≤60 s: on each custodian, `GET /api/v1/commitments?action=custody-blob&limit=500` has a row whose `resourceClassifiedAs` digest equals the station-1 witness CID's digest, AND the bytes exist on that peer's disk (`peerStorageDir`), AND `GET {custodian}/blob/<witness cid>` returns the bytes whose sha256 equals the digest.
- [x] `And Matthew and James each record on their own peer that they received that witness from Jessica` — `GET {custodian}/api/v1/economic-events` → `action == 'serve-blob'`, `provider` resolves to Jessica's agent (via the fixture's agent keys, never a transport id string-compare), `resourceInventoriedAs` digest matches.
- [x] `pnpm exec tsc --noEmit -p .` green; the orchestrator runs `just test mesh '@concern:death-witness and @station-2'` on the live mesh.
...
- Station 2 decision (2026-09-03): the custodian recognises a ward's witness by its LOCAL content row, which arrives via the shard replication plane (`run_replication_cycle` → `ShardRequest::ListContent{reach_filter: None}`; `shard_service.rs` serves all local rows, no reach/anchor gate; `store_acquired_record` writes them with dht_anchor_hash None). No new mechanism for station 2; measure the cycle cadence on the mesh. Fallback if the row does not land: D1 — persist a `content_kind` field on the additive `BlobHint` (inventory_gossip.rs:87; gather_hints; peer_blob_inventory column) and read it in `local_death_witnesses`. Rejected: ward-authored per-hash rows (B bound by A's pen; and the ward is dead when custody is needed); anchoring at household reach (needs the dead conductor; nothing between private and community passes the projector).
- M9 sharpened: the shard replication plane replicates PRIVATE rows to every household peer with no reach gate — that IS the custody-scoped read gate gap (station 3b), now with a named site (shard_service.rs ListContent, p2p/mod.rs run_replication_cycle, p2p_iroh/pull_core.rs).
- M10: `peer_blob_inventory` persists no kind/type/reach — hints (`BlobHint`) are consumed for fetch scoring and dropped (gossip_dispatch.rs:451-456).
- TRAP HIT (2026-09-03, edge #1421 red): station-2 Task 1 added `elohim-ark-core` as a path dep of elohim-storage; the storage Docker build context copies a fixed dir set and elohim/ark/* was not in it → `failed to get elohim-ark-core as a dependency`. The local storage gate cannot catch it. Rule for every prompt that may add a path dep: update elohim/elohim-storage/Dockerfile + scripts/ci/build-storage-image.sh COPY set in the SAME commit. Integrator fixed the COPY set (their window). Fold into the S1 plan constraints and into the memory project_pipeline_dispatch_ordering (the trap already lived there; the executor prompt did not carry it).
- FINDING (integrator, 2026-09-03): `just mesh stop` did not take the `ark run` supervisors down — the arks and their conductors outlived it. Cause candidates: stop_all SIGTERMs the recorded conductor child (refresh_mesh_pidfiles records conductor-<peer> too) before/without the ark, and the ark restarts a killed child by design; or the ark's own SIGTERM was delivered during a restart backoff (the S1 gap: shutdown during backoff spawns once). Fix (mesh script, Task 7-adjacent): in ark mode stop_all SIGTERMs ark-* pids FIRST, waits for each ark to exit (its child stops with SIGINT + grace), then proceeds to the rest; never signal a conductor whose ark is alive. Fix (ark, S1): SIGTERM must be honored in every lifecycle state including Spawning/backoff (a `step` transition that cancels a pending spawn). Add a station-2-adjacent a2o assertion: after `just mesh stop`, no ark-* pid and no holochain child survive.
- M11 (storage HTTP view): `GET /api/v1/commitments` returns `resourceClassifiedAs: null` when the projected row stores a bare string (projection_reconcile writes first_or_none → bare; the view decodes only a JSON list). Custody-spool and projected custody-blob rows are affected. Fix in the view: render a bare string as a one-element list. Probe: the row that the DB shows with `resource_classified_as='spool:witness:…'` appears with `resourceClassifiedAs: ["spool:witness:…"]`.
- FOUND ON MESH: shard replication delivers a ward's content row with `blob_hash` in the CID rendering (bafkrei…) while inventory rows are `sha256-<hex>`; any join by string equality across the two tables is wrong — join by digest (parse_content_address). Fixed in SpoolCustodyAuthor (3R); worth a rail: `.epr-meta` inject on `blob_hash ==` string compares.
- MEASURED (2026-09-03 03:30Z, mesh): station-2 chain end-to-end on the fresh witness sha256-c7ee0b24…: kill 03:30:4x → Jessica row 03:30:57 (ingest ≤5 s) → Matthew "Queued content gaps for replication" 03:30:58 (the 60 s tick happened to align) → blob pulled 03:31:03 → custody-blob authored by BOTH custodians (origin spool-custody), bytes on disk at blobs/c7ee/sha256-…, serve-blob receipt provider=12D3KooWHA4k (jessica) — all present within ~130 s, outside the 60 s budget on the first run. Bottleneck: replication_interval hard-coded 60 s (p2p/mod.rs:3601) → made a dial (REPLICATION_INTERVAL_SECONDS, ark mode 10 s). Author budget at 120/h with 15 s ticks = 1 authoring/tick — enough for one death (two witness records = same blob? no: write-ahead and verdict-filled are TWO blobs/digests; the second is authored the next tick).
