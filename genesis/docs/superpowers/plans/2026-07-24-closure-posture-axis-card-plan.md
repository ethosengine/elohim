...
- [ ] **Task 1 — `closure` vocabulary.** `v1/enums/closure.schema.json`: `["open","closed"]`, house
...
- [ ] **Task 2 — axis-card meta-schema.** `v1/registries/axis-card.schema.json` formalizing the
...
- [ ] **Task 3 — registries validation leg** in `scripts/test-schema.mjs`: load every
...
- [ ] **Task 4 — backfill `epistemic.axis.json`.** Add `closure: {facts: open, verdicts: closed}`
...
- [ ] **Task 5 — card `reach`.** `registries/axes/reach.axis.json`, `kind: two-layer` (declared
...
- [ ] **Task 6 — a2o capture.** `genesis/a2o/features/content/closure-posture.feature`: absence on an
...
- [ ] **Task 7 — gates + commits.** `pnpm run schema:test`, `schema:validate`, `schema:codegen:ts`
