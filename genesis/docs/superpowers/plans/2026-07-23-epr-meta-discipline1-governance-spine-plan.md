...
4. **Agency charter (the ladder).** New policy `governance-escalation-ladder@1`: agents self-grant `measure`/`inject` (+ `dispatch` with named agent+prompt); `ask`/`deny` authorship or promotion requires `established_by: operator-*` ratification on the bound policy. Enforced by validator `epr:validator-escalation-ladder` (Python AND Rust) on `.epr-meta`-file writes: an agent-authored manifest introducing/raising to ask|deny without a ratified policy pin → **refer** (`escalation-requires-ratification`). The charter policy row itself ships `established_by: operator-ratification-pending-2026-07-23` per the existing pending convention — the operator stamps it.
