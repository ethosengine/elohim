...
- **#1 (CONFIRMED) — fractional lever does not exist.** `target_arc_factor` is `u32`, hard-clamped `{0,1}` (`apply_arc_factor`: `factor>1 -> ERROR LOG + forced to 1`, "multi-factor sharding isn't yet implemented"; `genesis/docs/content/elohim-protocol/history/2026-06-14-arc-factor-feasibility-spike-findings.md:39-44`). `arc_policy::derive` returns `f64 arc_factor` as a **signal only**; `arc_actuator::authorize` REFUSES non-`{0,1}` as `NotActuatable` (`arc_actuator.rs:115-124`). **DECISION FORCED** (see Decision Memo §A). This plan does NOT add a fractional actuator — it is spike-verified infeasible on 0.3.2/0.4.1 without forking `holochain_p2p`.
