...
tier: architecture
status: Accepted
landing_state: "Implemented (Phase-2a sprint, commit 34fcf1070, 2026-05-15) — canonical reference; §7 migration executed Stage A→G. Residual codegen-$ref tail closed 2026-06-20 (commit b78908924). One known Phase-2a incompleteness remains: backlog content-attestations-table-dropped-but-still-consumed.md."
created: 2026-05-11
...
2. **`ContentMastery` overlap with `attestation:mastery`** — RESOLVED (2026-05-11 user review): `ContentMastery` stays as the agent's private progress record (Category B2 — agent-scoped with attestation). It is **not** consolidated into core. The PROCESS of minting `attestation:mastery` from a learner's mastery state IS core because the lamad manifest declares the subtype and the minting policy is subject to EPR. Said differently: the private progress data is app/UX layer (lamad's domain); the attestation that publicly recognizes mastery is a core operation governed by the manifest. ContentMastery's storage projection stays in elohim-storage; the attestation flows through the unified pipeline defined in this spec.
