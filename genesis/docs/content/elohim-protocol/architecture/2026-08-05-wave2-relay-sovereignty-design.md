...
cites:
  ...
  - "substrate-trust-contract-runbook | the invariant/probe contract the transport flip must preserve — its seam-smoke, conductor-diagnostics and canonical-head probes carry over unchanged, seam-smoke[signal-bus] is retired by the flip and replaced by this design relay-reachability and n0-contamination probes | sha256:e47d962ca7259c79 | path: genesis/docs/content/elohim-protocol/architecture/2026-07-12-substrate-trust-contract-runbook.md"
...
| 8 | **Ratify the two image tags** (D9) — `hc-elohim-0.6.3` and `hc-elohim-0.6.3-iroh` both live in Harbor for the wave. **Artifacts EXIST as of 2026-08-06** (edgenode job, verified): `elohim-edgenode-iroh:{latest,2026-08-06,067ec55,hc-elohim-0.6.3-iroh}` + `elohim-storage-iroh:{same}` embedding the iroh conductor by digest; tx5 fleet tags confirmed untouched (0 pushes). The ratification (keep both lines live for the wave) remains open | registry footprint + the rollback contract |
