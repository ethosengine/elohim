...
cites:
  ...
  - "holons-are-spaces-how-we-use-holochain | The companion diagnosis and Holochain reassessment that this human story turns into a sequenced reimplementation decision. | sha256:931b9b05cae78c40 | status: stale — target content moved on; re-verify | path: genesis/docs/content/elohim-protocol/architecture/2026-09-06-holons-are-spaces-how-we-use-holochain.md"
...
#### Chosen operating budgets (2026-09-08)

The target device/host profile is the `device-raspberry-pi-4` / `device-recycled-laptop` row of `genesis/data/devices/archetype-resource-budgets.json`: 768Mi request / 3Gi limit on memory for both, 250m CPU request on both, with a 1000m CPU limit on the Pi and a 2000m limit on the recycled laptop. Phone-class (a 2019 Android handset, 3 GB RAM, carrier NAT) is a declared floor never measured on this pass; it stays an open acceptance prerequisite rather than a passing row. Every budget below distinguishes a CHOSEN requirement — a threshold this plan commits to — from a DEMONSTRATED capability — a threshold actually observed on real hardware under a real workload. A chosen number that is not demonstrated is a red, not a pass. Clone, publication, and rotation evidence comes from the 3-peer household mesh (HC 0.7.0 fork, iroh dual transport, runs on 2026-09-06/07); memory and recovery evidence comes from the 7-peer alpha fleet.

| Budget | Chosen threshold | Class | Demonstrated? | Evidence (path, date, context) |
|---|---|---|---|---|
| Local-save latency | p95 ≤ 250 ms, p99 ≤ 500 ms per single local content commit, modality reported | CHOSEN | NO — needs n≥100 on the target device via `latency-scoreboard.py --local` | `create_content` 48 ms single call (`genesis/data/timeline/backlog/p1-dht-authored-content-not-projected.md`, 2026-08-28, x86); clone create 128–191 ms (`genesis/a2o/fixtures-clone-2026-09-06-report.md`, 2026-09-07); bulk seed 11.3 ms/row; bimodality forbids a median-only budget. |
| Healthy-network publication | readable on every household peer ≤ 300 s; applied ≤ 600 s at N ≤ 128 members | CHOSEN (the 300 s floor is substrate) | PARTIAL — floor observed in 5 rounds; the N-scaled bound needs the sweep cold-retire measure | 300 s DHT propagation floor (`genesis/a2o/features/dataplane/README-accountable-correction.md`, 2026-09-07); rotation 106.6→332.8 s at N=44 (`genesis/data/timeline/backlog/feedback-discovery-sweep-is-o-n-in-history.md`). |
| Working memory | ≤ 2 GiB peak RSS, conductor + storage combined, measured during catch-up at 3,500 records × 5 cells, with OS + client reserve inside the 3Gi limit | CHOSEN, provisional | RED — james ~2.1 GiB on x86; ARM unmeasured | conductor RSS 872/866/896 MiB at 5/10/20 cells, +1.62 MiB per clone (`genesis/a2o/fixtures-clone-2026-09-06-report.md`); fleet james ~2.1 GiB @3,654 docs, jessica ~1.4 GiB, idle 1.2 GiB (holons doc §9.3); arc is not the memory lever (`genesis/data/timeline/backlog/arc-shrink-ineffective-memory-soak.md`). |
| Disk growth | ≤ 2.5 MiB per additional cell; separately, ≤ 1 GiB total at 3,500 records and ≤ 8 GiB/year as a DESIGN TARGET | CHOSEN (per-cell); DESIGN TARGET (total/annual) | YES on the per-cell figure (one gossip run, linear); NO on the total/annual figure — needs a defined write/revision/retention workload (the year-equivalent shape §8 names) | +2.11 MiB/clone under gossip, ~8× idle (`genesis/a2o/fixtures-clone-2026-09-06-report.md`). 704→738 MB over 5→20 cells is a snapshot, not a workload. |
| Idle bandwidth | ≤ 50 MiB/day/peer on a metered link | DESIGN TARGET (not chosen) | NO — REJECTED AS UNMEASURABLE until byte counters exist on all three anti-entropy loops, then a ≥1 h idle soak | no byte counter (`genesis/data/timeline/backlog/2026-07-27-anti-entropy-egress-baseline.md`); doc-sync ~1,430 rounds/24 h/pod is a count, not bytes. |
| Background CPU / energy | idle participation ≤ 25 % of one Pi-4 core for conductor + storage combined; catch-up bounded to the publication budget; battery-class deferred with phone-class | DESIGN TARGET | NO — x86 idle shows ~10 % per conductor (`just mesh status` footprint, 2026-09-07); no ARM or energy measurement | added on second-model review: the budget a phone participant feels first (battery, thermal throttling) even when RSS and bandwidth pass. |
| Recovery | warm ≤ 120 s (dual/libp2p); cold ≤ 1,200 s to serving-ready with a PASS at corpus ≥ 3,400 | CHOSEN | warm YES on dual/libp2p (57–62 s, n≈12); warm homo-iroh UNSUPPORTED (1/8 pass, 258 s); cold RED (906/1,830/1,842 s all `recovered:false`) | `genesis/a2o/reports/recovery/recovery-timeline.jsonl` 2026-08-28/29; ≈20 min churn (`genesis/docs/content/elohim-protocol/architecture/2026-07-12-substrate-trust-contract-runbook.md` I6). |
| Witness unavailability | PRECONDITION: zero permanent-block states, with observable failure criteria for the rejected-op cell block and the null-storage-arc no-authority state. THRESHOLD (not chosen until the partition experiment runs): 1 of 3 household peers absent → local reads/writes continue; reconvergence ≤ 1,200 s on return | PRECONDITION | NO — quiesce time_to_verdict (362 s) is not a reconvergence measure | HC 0.7 forever-block on first rejected op (`genesis/docs/superpowers/specs/2026-09-03-holochain-evolution-epic-design.md`, measured 2026-09-05); storageArc null on all alpha agent-infos (`genesis/data/timeline/backlog/sovereign-peer-network-read-no-authorities.md`). |

**Second-model read (Codex, 2026-09-08):** the table above adopts its framing — chosen vs. demonstrated, two rows rejected as unmeasurable, a CPU/energy row added, and homo-iroh warm recovery marked unsupported.

> Local-save latency — TIGHTEN: the evidence column's 48 ms single call, 128–191 ms clones, and bulk per-row average do not establish p95/p99 for individual saves; require repeated target-device measurements and enough observations to characterize p99.
>
> Healthy-network publication — TIGHTEN: five three-peer rounds support the observed 300 s delay, but neither that observation nor rotation reaching 332.8 s at N=44 establishes an every-peer upper bound or application within 600 s at N=128.
>
> Working memory — TIGHTEN: the evidence column's conductor-only clone measurements and fleet RSS snapshots do not establish combined conductor-plus-storage peak RSS at the specified workload; retain 2 GiB provisionally, but define peak measurement and reserve device memory for the OS and client.
>
> Disk growth — TIGHTEN: the measured 2.11 MiB/clone supports the incremental allowance, but the 704–738 MB snapshot does not establish total disk at 3,500 records, and no evidence supports annual growth without a defined write, revision, and retention workload.
>
> Idle bandwidth — REJECT-AS-UNMEASURABLE: the evidence column explicitly lacks byte counters, and 1,430 rounds/day cannot establish MiB/day; preserve 50 MiB/day as an intended ceiling until instrumentation covers all traffic, with longer runs to capture periodic work.
>
> Recovery — TIGHTEN: the evidence column supports warm recovery on dual/libp2p at 57–62 s, but the homo-iroh 258 s result with only 1/8 passing defeats a transport-independent 120 s claim, while all full-corpus cold runs failed regardless of elapsed time.
>
> Witness unavailability — REJECT-AS-UNMEASURABLE: rejected-op blockage, null storage arcs, and a 362 s quiesce verdict do not measure reads/writes during one-peer loss or reconvergence afterward; define observable permanent-block failure criteria and run the specified partition experiment.
>
> Numbers outrunning evidence: local-save percentiles rely on isolated calls and mismatched bulk averages; publication extrapolates scope and scale; memory lacks the specified combined-resource workload measurement; disk's total and annual limits lack matching workload evidence; bandwidth lacks byte measurements; witness recovery substitutes quiesce for reconvergence. Recovery's warm result is transport-limited, and cold recovery has no successful full-corpus anchor. These may be chosen requirements, but they are not demonstrated operating capabilities.
>
> The missing budget is background CPU/energy consumption, measured during idle participation and catch-up: a household phone participant would feel battery drain and thermal throttling even while RSS and bandwidth pass.
>
> Recording memory and cold recovery as chosen-and-failing is honest and preferable to holding them: §8 requires explicit acceptance budgets, not convenient passing thresholds. Keep them active RED, label memory's evidence as x86-specific, and hold any supported-footprint or "keep Holochain" acceptance claim until representative hardware and workloads pass. Warm homo-iroh recovery also needs an explicit failing or unsupported status.

The keep/deepen/replace decision in the table below stays open until every CHOSEN row is demonstrated on the target hardware; operator ratification of the chosen numbers is recorded in the run-plane note.
