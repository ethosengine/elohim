...
| **D8** | **Web2 Projection & Doorway** | `…doorway-access-tier-patterns`, `…doorway-ssr-runtime`, `…doorway-two-axis-scaling`, `…doorway-auth-posture-declared-stage` | **doorway** · projection + SSR compute | access-tiers = *reach-gating + cache-scoping*; ssr-runtime = *compute capability*; two-axis-scaling = *the projection read path vs the conductor/identity-hosting pool, and the graduation flywheel between them* (graduation is **accounting only** as-implemented — no source-chain export moves a hosted user's history to their device); auth-posture = *write authority derived from declared network stage, distinct from per-doorway admin identity*. The Track-4 projection boundary; peer-native stays clean behind it. **Doorway is OPTIONAL, not architectural** — the D5 mesh *is* the hosting layer (peers who care about content shard+replicate it); doorway only projects mesh-hosted content to HTTP browsers and absorbs the read-mass a peer can't. A peer functions with zero doorway (layers 1+2 only). |
...
[`doorway-two-axis-scaling`](./2026-06-11-doorway-two-axis-scaling.md) before touching capacity or
graduation, and [`doorway-auth-posture-declared-stage`](./2026-08-25-doorway-auth-posture-declared-stage.md)
before changing a write gate); for a refusal symptom, use the operational companion
[`doorway-auth-refusal-runbook`](./2026-08-25-doorway-auth-refusal-runbook.md); (3)
...
pillar guide), and points at its tracked entry in `.eprfs/status/gap-items/` (the decomposed
...
| **records-lifecycle ↔ memory-lifecycle reconciliation** — both say "lifecycle," distinct subjects (substantive records vs data retention); frontmatter admits "sibling vocabulary; reconciliation incomplete" | STRADDLE | OPEN (known todo) | `gap-items/plans__2026-05-24-records-lifecycle-part-d-substrate-gaps-plan.json` |
...
| **Cold-archive terminus** — tiered-quilt names the shelved terminus; records-lifecycle defers "organization-dissolution"; neither closes "what happens at the terminus" | STRADDLE | OPEN | `gap-items/architecture__2026-05-11-tiered-quilt-stewardship-design.json` (the terminus is not yet among its items — needs agent decomposition) |
