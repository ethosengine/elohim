...
> These eighteen failure modes recurred across **≥3 distinct agentic shifts each** (the starred rows are
...
| 2a | **Frozen global baseline over-build** (2026-09-22, #1888) — a red orchestrator run does not advance `__global__` while per-pipeline baselines keep advancing, so the single `git diff <__global__>..HEAD` changeset re-dispatches pipelines whose own baseline already covers the range (the ~53min DNA pipeline for an empty `elohim/holochain` diff). Closed by `applyAlreadyBuiltFilter` on any trigger — with the narrow re-walk done by the SAME `build-graph.groovy::walkBuildGraph`, never a second matcher, and an optimistic (`longRunning`) producer kept whenever any surviving pipeline reaches it through `dependsOn` — with NO controller-evidence exception, because `dev-latest` is a floating tag a later failed run can overwrite, so a historical SUCCESS at that sha proves nothing about the bytes the consumer will fetch. The 2026-09-21 TIMER exact-sha case (#1886) is its degenerate instance. | 2 | `genesis/orchestrator/README.md` §Baseline state |
...
| 18 | **A per-item "partial failure is just a test failure" posture, correct for an INDEPENDENT phase, is inherited by a SHARED-ARTIFACT phase — where the first failure is a leading indicator for every item behind it, so continuing walks the whole fleet** — and the safety pause is conditioned on success, so the loop accelerates exactly when it should stop. `deployHumansInParallel` (`elohim/holochain/Jenkinsfile`) runs two phases. The parallel STORAGE phase's continue-on-failure/UNSTABLE-not-FAILURE rationale is sound and documented in the function's own doc comment (a peer's resource floor is a per-peer test failure — the 2026-05-22 james OOM). The sequential rung-3 CONDUCTOR phase inherited it, but a conductor only rolls when a SHARED artifact moved (hApp digest / conductor pin), so a peer whose conductor will not come Ready predicts the next one. On 2026-09-02 the phase walked `ordered` — non-genesis first, matthew last — at one `--timeout=600s` per peer, attempting every peer behind the first failure; the alpha fleet's conductors were meanwhile reinstalling a drifted DNA and tearing their source chains. (Scope note, because the museum must not overstate: the *loop-never-halts* half is proven — #1414 attempted jessica→james→gertrude→susan, three confirmed failed, in one build; the *loop-caused-the-tear* half is *not*, since #1414's conductor image was demonstrably `holding` and its pod template is stable by design, making it a witness. Which builds rolled versus waited is recorded as open on the backlog entry.) **Diagnostic tells, in order of cheapness:** (a) the failing items appear in the loop's own declared order at intervals equal to the loop's own timeout — that cadence is the pipeline, not the substrate; (b) `getBuild tree=artifacts[relativePath]` shows evidence bundles for MULTIPLE items from ONE build (#1413 archived four conductor bundles) — that list is a picture of a loop that never halted; (c) the soak/backoff sits AFTER the throw point, so a failing item advances faster than a healthy one. **Cost even in the pure-witness reading:** `600s × remaining peers` of dead wall clock (#1414 burned 40.7 min in one stage without reaching its last three peers), which justifies the halt before any blast-radius argument. **Rule: continue-on-failure is a property of the phase's independence, never of the file it is written in — a shared-artifact roll halts on first failure and leaves the rest on their last-known-good.** The escape hatch is an env flag an operator sets deliberately, never the default. | 1* | `backlog/ci-edge-conductor-roll-no-halt-walks-the-fleet.md` (#1413/#1414, fps `ca8705d89578`/`e63e3dcd0771`/`d89606ca5cde`); `backlog/alpha-conductor-crash-loop-after-wave4-roll-and-moved-dna-hashes.md` (the substrate half); adjacency: #16 (a pipeline that inflicts what it reports) |
| 19 | **An error that establishes NOTHING about the work is surfaced as a VERDICT — and the cure has to be re-applied at every site that answers for itself** — a shed, a dropped socket, a `BrokenPipe`: in each case the far side never saw the work, so the condition says nothing about whether it can succeed on a retry or on a sibling. Surfacing it as a refusal converts a seconds-long transient into a permanent-looking red, and (worse) into a *user-facing* one: `elohim-genesis/dev` #1576 answered a hosted human `503 PROVISIONING_FAILED` because `conductor-3`'s admin websocket died mid-`install_app`, while every other conductor in the pool sat unasked. The class is stubborn because **every fix is site-local**: elohim-storage classified the shed correctly at `services::response::error_response`, then again at `HttpServer::escaped_error_response` (4,538 sheds had already reached the wire as bare 500s), then again at `api::source_chain::zome_error`, then again at the three `/db/content/{id}/…head` routes that `return Ok(...)` inline and therefore escape to no classifying seam at all — and then the doorway needed it twice more, once in `forward_to_storage` and once in `AgentProvisioner::provision_agent`. A route or function that answers its own errors **opts out of every classification the shared seams provide**, and "we fixed that" is false comfort every time. **Diagnostic tells, in order:** (a) the error TEXT names a transport or admission condition (`shed`, `nothing was dispatched`, `BrokenPipe`, `ConnectionAborted`, `error sending request`) while the wire SHAPE says the work was judged (502, `PROVISIONING_FAILED`, `forwarded_to_storage:false`); (b) the log names exactly ONE member of a pool that has others — the siblings are absent because none was asked; (c) the same input succeeds on a later identical attempt. **Rules:** a transport/admission condition is re-offered (bounded, with the failing member EXCLUDED so one sick peer cannot absorb the whole budget), never verdicted; a re-offer re-runs its own idempotency check first, because an aborted call may have landed before it aborted; and an error that crosses a process boundary carries its CAUSE chain, not only its wrapper — `reqwest::Error`'s `Display` stops at `error sending request for url (…)`, and that missing `source()` walk is the difference between a finding CI can route and one that costs a triage dispatch. | 3* | `backlog/ci-deploy-reads-storage-backpressure-as-failure.md` (#1683, fps `f8003a16a985`/`5ee767181c07` — the shed at two seams); `backlog/ci-genesis-hosted-provision-conductor-admin-socket-abort.md` (#1576, fps `885aee5f78f7`/`46c20a2dfea8` — the dropped admin socket); `backlog/ci-apex-doorway-cannot-reach-adam-storage-blob-forward.md` (#1704/#1705, fps `5ee767181c07`/`82831e35dcd7` — the truncated cause chain); `elohim/elohim-storage/src/conductor_admission.rs` module doc; `genesis/docs/content/elohim-protocol/architecture/2026-07-12-substrate-trust-contract-runbook.md`
...
2026-09-02 walk attempted every peer behind a known-bad one, one 600s timeout at a
time, while the fleet lost seven source chains irrecoverably; every subsequent edge build
re-entered the same loop. The row is worded around
...
#19 is 3 occurrences across 11 days (#1683 2026-09-02, #1576 and #1704/#1705 2026-09-11), but its
frequency count understates it: those are only the instances that reached CI *after* the class had a
name. The cure had already been applied and re-applied at five in-tree sites before the first backlog
entry was written, one incident at a time — the same reactive shape #17 earned its row on, in a
different layer. It is promoted here on two grounds beyond recurrence. First, it is the one class in
this table whose blast radius reaches a **human**: #1576's `503 PROVISIONING_FAILED` was a stranger
being told the network could not host them while the network could. Second, it is **self-concealing by
construction** — the site that laundered the error is the only site that knew what the error was, so
the evidence of misclassification is destroyed at the moment of misclassification and every downstream
reader, including the sentinel, sees a clean verdict. That is why the third rule (carry the cause
chain) is stated as a rule and not as an optimization: an error that crosses a process boundary
without its `source()` is a finding that cannot be routed, and an unroutable finding costs a triage
dispatch every single time it recurs. The mechanical audit question when adding ANY error path:
*does this site answer for itself, and if so, which classification did it just opt out of?*
