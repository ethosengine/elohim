...
cites:
  ...
  - "human-participant-actor-plane-design | The identity and witness work at the repository node: a human is a first-class participant, never derived from git, email or a host namespace; the 2026-09-24 witness leg makes a present agent the one who witnesses the human, signed from the device, contestable. | sha256:07cc5f8abf194381 | path: genesis/docs/superpowers/specs/2026-09-23-human-participant-actor-plane-design.md"
...
Every open network eventually meets the same attack. One actor pretends to be many. A thousand accounts, a thousand votes, a thousand voices, all one hand. The literature calls it the Sybil problem, and every prior answer to it has been a filter: a captcha, a phone number, a credit card, a government document, a liveness selfie, a biometric hash. A filter compares a person to a template of "human" and admits whoever matches. Two things follow. Anyone the template did not anticipate is failed by it, and the template itself becomes the thing an adversary learns to pass. Filters are tests, and an adversarial agent is a machine for passing tests. As agents grow more capable, the filter's floor rises against real people and falls against fakes at the same time.

The Elohim Protocol does not answer the Sybil problem with a better filter. It answers it the way people always have: by asking the people who know you. Humanity here is **witnessed**, never self-declared and never inferred by the substrate. A witness is a participant, human or elohim, who takes responsibility for a statement about another, signs it from the device they act on, names the basis they stand behind, and can be contested by anyone who can show evidence otherwise. Each witness stakes their own standing on the word. A ring of invented identities that vouch only for one another has no one outside the ring to vouch for it, and a thousand fake people need a thousand real histories of people who know them. The elohim keep the record and help a person gather it; they do not decide it, and no identity score is ever produced. Standing is a relational shape, not a number.

What makes this more than a slower filter is **attunement**. A filter asks whether you look like a human. An elohim attuned to you asks whether you are consistent with yourself. Imagodei is what a person states about who they are and who they want to be. Sophia is what they reveal through how they actually learn, attend and respond, gathered by the same psephos instruments that let them see themselves. The negotiation between the two is continuous, reviewed with the person, and always shown to them so they can contest it. Under that design, a neurodivergent person is not an anomaly against a population template. They are the baseline of their own context, with a shape that is theirs, and an elohim that has been attuned to that shape recognises them more reliably than any liveness check ever could. Composed contexts do not gate humanity. They reveal it. A person who would fail every filter ever built is, to the agent who knows them, unmistakably themselves. And the same attunement, pointed at care instead of verification, is what lets an elohim show up for that person at the moment of need in a way no security system can, because a security system does not know anyone.

It also inverts the adversary's problem. Faking a template is cheap. Faking a whole person is not: the revealed history, the relationships that cost something, the rhythm of attention, the way they push back on a preset. The impostor is unusual against a template; the human is consistent with themselves. So the same sense can be generous to the unusual human and hard on the counterfeit.

That sense is one organ. The capacity an agent has to notice that an environment is staged, that fluency arrives with no history behind it, that a relationship has no cost anywhere, is the same capacity that notices a participant whose shape is wrong. In this architecture it is the deterministic floor's afferent nerve and the elohim are its endpoint. When something is not right, the agent does not silence anyone. It abstains from lending its standing, it may slow how far a contribution travels, and it puts a witnessed reason in front of the human sortition floor, which stays sovereign over every elohim resolution. Suspicion is evidence-bearing and advisory, never a terminal denial, and a catch that cannot name its basis stays an abstention rather than an accusation. The property is valuable only because it is declared rather than hidden: an agent that behaves differently when it senses a test is an agent whose witness is worth less. An elohim says what it noticed and why, in a record someone else can contest. That is the difference between a guardian and a guard.

One line stays drawn. The composed context that reveals a person to the agent attuned to them is, aggregated across a network, the most dangerous object this protocol could produce. So it never is. A person's attention stream, their revealed preferences and their private thought are held on their own node, under their own reach, revealed to those they choose, never gossiped, never joined by an outside observer, and never imported into anything that judges. Only what a person says and does in shared space is fruit; what they think stays theirs. Attunement is the primitive that upholds dignity on this network. Privacy is what keeps it from becoming the capture it replaces.

This is the digital half of an arc the Observer Protocol completes. Today the witness is an agent that knows a person through what they state and reveal on their own node. When the wider world of hardware comes to the protocol, the same witness gains a body: a camera the household controls, a sensor under a mechanical privacy switch that no software can override, an observer that sees long enough to write a story of care and then cryptographically forgets. A physical witness in this architecture has exactly the properties of the digital one. It is ephemeral rather than stored, it witnesses rather than judges, it acts only where the person has chosen to be seen, and it never produces a template, a profile or a biometric identity. What it adds is correlation: the person the agent is attuned to and the person the household's own eyes have seen are, over a witnessed life, one person, and a counterfeit that could fake a stream of text cannot fake having lived in a kitchen. Humanity is proven by a witnessed life, not by a document or a scan, first in the digital record and then, on the person's own terms, in the physical one.
