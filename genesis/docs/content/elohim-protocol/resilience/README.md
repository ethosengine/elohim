...
These are three different *postures* of pro-social participation. The substrate has been quietly carrying them as separate dimensions of the Commitment ledger; the UX makes them legible to the human stewarding the hardware. Note on vocabulary: the five values threaded below (`household`, `neighborhood`, `community`, `organization` in social custody; `commons` in commons custody) are **custody** tiers — who holds/replicates for whom, anchored on the `CustodianCommitment` / `Mishpat::Commitment` lineage — a distinct axis from the reach vocabulary (declared sensitivity floor), per `reach-ontology-vocabulary-split-spec` §1.
...
Every hub in the substrate runs an **elohim-operator** — a context-bound specialist agent that fills the role a household's devops/IT person would fill if the household had one. The operator's definition lives in the iroh-libp2p complementarity spec (`genesis/docs/content/elohim-protocol/architecture/2026-05-08-iroh-libp2p-complementarity.md`) and is anchored by memory `project_elohim_subagent_specialists`:
...
**The matrix is this chapter's load-bearing honesty mechanism, and it only stays honest if it migrates as the substrate lands.** A `GAP` row left reading `GAP` after the work ships does not make the chapter *cautious* — it makes the chapter *wrong*, and silently overclaims by understating what's built (or, inversely, a `LIVE` row whose code-path rotted overclaims the other way). The migration triggers are concrete: a `signal_kind` whitelist edit in `content_store_integrity/lib.rs`, a new `resource_classified_as` value, a role record landing in `genesis/data/lamad/content/`, a feature file moving from `@wip` to passing on alpha, a `ReconcileController` handler wired, a new view module under `services/`, the substrate-native node-health observable closing, the recognition transfer-on-claim executor landing, or the compute-class/attribution-class typed partition replacing the documented discipline. Each such commit should name the matrix row(s) it migrates so this chapter's author has a clean trigger. Reviewing this matrix against the substrate as it actually exists is a standing memory-ceremony beat — and a row migrating GAP→LIVE is not just a closed backlog item, it *removes a hedge from the civic claim*, which is exactly the readiness signal the cartographer scores "what's next" against.
...
| Reach enum drift — five strands, now dispositioned per `reach-ontology-vocabulary-split-spec` §1: (1) schema-8 (`private…commons`) is **CANONICAL**, contract-tested slice-1 (`elohim-storage/tests/reach_vocabulary_contract.rs`); (2) Rust services kebab-8 (`Personal…District/Public`) **RETIRED** slice-1 — re-exports `elohim_epr::Reach`; (3) TS `VALID_REACH_LEVELS`-6 **RETIRED** slice-1 — zero consumers, deleted; (4) TS geographic-8 (`private/invited/local…bioregional/regional/commons`) **RENAMED → `LocalityLevel`** slice-2 (`elohim/sdk/storage-client-ts/src/protocol-core.model.ts`, single edit point + 3 re-exports), old names kept as `@deprecated` aliases pending consumer burn-down; (5) Part-V custody 5 (`household / neighborhood / community / organization / commons`) is a **distinct custody vocabulary**, named not renamed — see Part V note | **PARTIAL** | remaining: `LocalityLevel` alias burn-down (37 consumers, +23 deprecation-lint findings tracked); `elohim-library`'s `ContentReach` (`content-node.model.ts:46`) + `trust.service.ts:14` `ReachLevel` duplicate the retired-6 vocabulary independently (~41 refs, newly-discovered, slice-3 scope — not yet touched); `steward/node/src/storage/reach.rs` dormant 6-value enum (`#[allow(dead_code)]`) unreconciled; verdict surface (spec §7 item 3) not yet speced; disposition detail: `genesis/data/timeline/backlog/reach-vocabulary-frontend-strand.md` |
...
*Related testbeds: [`a2o/features/rms/human-resilience.feature`](../../../../a2o/features/rms/human-resilience.feature) · [`a2o/features/deployment/compute-commitment-bounds.feature`](../../../../a2o/features/deployment/compute-commitment-bounds.feature) · [`a2o/features/elohim/compute-allocation.feature`](../../../../a2o/features/elohim/compute-allocation.feature)*
