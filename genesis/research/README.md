...
## The Limits Problem — dynamics under a carrying capacity

We arrived at limits through philosophy — Robeyns' limitarianism, Georgist common inheritance, the Donut's floor and ceiling — and built governors accordingly: friction-gradient limitarianism, the per-substrate limitarian governor, Mishpat bounds. What we never wrote down is the *physics* underneath: the stock-and-flow model of the thing being bounded. **Donella Meadows** is the complement to the Beer reading in Foundations above — where Beer gives the shape of the regulator (variety, recursion, algedonic channels), Meadows gives the units the regulated system must be denominated in. The two are read together or neither is complete.

The survey covers the whole corpus — *Limits to Growth* and its 1992/2004 updates, *Thinking in Systems*, the twelve-rung leverage-point ladder, "Dancing with Systems", and the under-read 1998 Balaton Group report *Indicators and Information Systems for Sustainable Development*, which is where the stock/flow vocabulary becomes indicator design. The concepts we had no word for: **regeneration rate** (harvest ÷ regeneration > 1.0 is unsustainable, and it fires as a *leading* signal long before a stock is visibly depleted), **sinks as well as sources**, **erodible carrying capacity** (overshoot degrades the limit itself), **resilience as redundancy of balancing loops** — strippable without immediate cost until the crisis that needs it — and the sharpest borrow, the **respite-time / response-time ratio**: any system generating problems faster than it can respond to them is, in her words, "quite simply, out of control", and the only two cures are quickening the response or slowing the growth. Effort is not on the list. Also carried: her "forbidden numeraire" — that *power* should be published as a stock, a flow, and a distribution, which our governors act on but never show.

Adjudicated against build state, the ladder reading is a barbell: strong at information flows, rules, goals, and paradigm; near-empty at buffers, stock-and-flow structure, and delays — precisely the middle rungs that require a dynamics model. The concrete find is a live unit error in `spatial_capacity.rs`, which compares an all-time cumulative sum to a sustainable *yield* (a rate), so utilization rises monotonically and every Place eventually reads as overshot.

**The largest take runs the other way, and is a contribution back to her.** World3 was one model with one parameterization computed centrally, and *Groping in the Dark* is her own self-critique of what that opacity cost the argument. This substrate already carries the machinery to build the version she could not: **plural per-EPR Mishpat lenses executing in parallel**, the **two-layer law** that puts rollups on the aggregate-projection layer where no global clock is required (aggregation is not synchronization — the constraint bounds the wrong thing), and `select → fold → aggregate` to carry a global query down and an anonymized answer back up. The **dataset becomes primary and World3 becomes one lens over it**, live alongside the Donut and planetary boundaries, swappable when a parameterization ages out — which is the plural-lens anti-regime-drift argument applied to global models, where it bites hardest. That aggregate is the dataset the **Global Orchestra** epic deliberates on. Two conditions carry: the lens must show its mechanism downward or a council can only accept-or-reject it (our own trap-detector rule, reached independently), and per-fold anonymity is an unsolved design constraint, not a footnote.

**Underneath the limits is a standing question the survey could not avoid**, and which `constitution.md` App. C has carried unanswered as deliberation question 5 ("the role of non-human entities"). Every limit here assumes the living world is the kind of thing whose capacity we owe something to, and imago dei does not on its face supply that — but our own canon does, in the verse `values-forward` already quotes for economics: *"the land is mine; you are but sojourners with me"* (Lev 25). Read plainly it grants the land standing **prior to human title** — not ours to damage because never ours — and the corpus has only ever drawn the economic half. **Kami** (coined by Botao "Amber" Hu; extended by civic.ai) is recorded as *bridge-legibility*: a second lineage reaching the same floor from a different ontology, which matters for a protocol meant to meet people who got here the other way. And the anthropocentric focus resolves as a **control-point claim rather than a rank** — we, our values, and the AI acting on them are the requisite-variety attenuator at biospheric scale, which puts human values on Meadows' two highest rungs (goals, paradigm), while the regulator sitting *inside* the plant means damaging nature is self-damage. An apparent conflict between human and ecological thriving is therefore the signature of a broken loop, not a contest of interests — which makes the **honest limit signal, not the declaration, the ethically decisive artifact**, and makes a broken carrying-capacity check a values failure rather than an arithmetic one.

Two confirmations from a second lineage: friction-gradient limitarianism is Meadows' own Way Out of the success-to-the-successful trap, and `habits.yaml`'s evidence rule is her Way Out of seeking-the-wrong-goal ("don't confuse effort with result"). Two declines, stated plainly: her privatization horn for the commons (our witness layer is a third horn she lacked — make the consequence *felt* without making the resource *ownable*), and leverage point #1's relativism, since two floors here are claim-not-frame — imago dei and physics — non-negotiable for different reasons. One correction offered to canon: the orchestra epic's "overshoot impossible" overstates — a protocol cannot remove the perception-delay term, only collapse it, so the keepable promise is that overshoot becomes *witnessed and priced*.

[Meadows and the Elohim substrate →](epr:meadows-systems-dynamics-cross-pollination-2026-08-11)
...
### The Encoding Half — canonical bytes vs. schema-relative compactness

Content addressing names bytes; something has to *produce* those bytes identically on peers that share no code. dag-cbor buys that byte-identity (RFC 8949 §4.2.1) and pays for it by repeating every map key as a string in every value. The [August 2026 survey →](epr:serialization-canonicality-cross-pollination-2026-08-11) adjudicates whether a schema-relative format (schemaboi, Borsh, SSZ) is the more native route, and finds the framing mis-located twice: `serde` is not a wire format, and serde *derive* is not on the canonical-hash path anywhere in `elohim/epr` — every `canonical_bytes()` is a hand-built `BTreeMap<String, Ipld>`, so swapping the framework changes zero content-addressed bytes. **Canonicality and schema-relative compactness are separable properties**, and the survey maps who mastered each: Borsh and SSZ for canonical-binary-for-hashing (SSZ especially — many independently-written Ethereum clients agreeing byte-for-byte is the production answer to *peer diversity*), Avro for schema evolution via a content-addressed schema fingerprint. Cap'n Proto/FlatBuffers/rkyv are structurally disqualified — fast, but not byte-stable. The survey's second axis is **schema evolution**, where [Cambria](https://github.com/inkandswitch/cambria-project) (Ink & Switch) supplies the half Avro and SSZ don't — *transformation* rather than mere identity: a graph of schema versions with invertible edges, composed along the shortest path. Adopt the model, not the dependency (TypeScript-only, dormant since 2024, self-declared pre-production), and mind two traps it surfaces — translation must be a read-time projection rather than a re-notarization (a translated doc has a different CID, signed by nobody), and **"lens" is already taken** by Mishpat governance standing (`find_lenses_governing_epr`), so schema work must say *migration*. Verdict: keep dag-cbor; the real question is *should schemas be content-addressed EPRs?*, which is a substrate redesign sharing one migration story with the DNA-upgrade path. The drill-in also found a live defect — eprfs tags non-CBOR bytes with the dag-cbor codec, so the same file gets two different CIDs depending on which code path addresses it.
...
## The Harness Problem — shared context between agents

How do many agents, across many sessions, hold one coherent picture of the work? Every serious team publishing on long-horizon agent runs (OpenAI Codex, Anthropic Research and Engineering, Arize) has converged on the same move: stop treating the conversation transcript as the agent's state, and make the context window a **projection of durable state** that outranks it. The sharpest form is Anthropic's session log living *outside* the harness, which makes compaction and handoff the same operation — two slices of one append-only log that survives both. The field's open problem is what we already hold: a workflow representation that is not bound to a vendor's ticket board. Ours is REA valueflows over EPRs — recipes as ProcessSpecifications, `commitment:claim:` state, admission-controlled by `habits.yaml`'s WIP fence. Our gap is the reciprocal one: nothing re-projects that state into the window mid-run. The survey's Codex pass also measured a live local defect: the generated root `AGENTS.md` (42,327 B) exceeds Codex's 32 KiB combined instruction budget and truncates at line 308 — the Critical Gotchas, CI/CD, and Code Style sections never reach a Codex run.

This is also where the Beer reading needs amending. `.epr-meta`'s cascading compose-gates are **stigmergic System 2** — place-bound rules that fire at the moment of action, coordinating autonomous agents with no coordinator and no message-passing — which partly supersedes the 2026-06-04 critique's finding that System 2 is underbuilt.

**Key references:**

- OpenAI, ["Harness engineering: leveraging Codex in an agent-first world"](https://openai.com/index/harness-engineering/), Feb 2026 — Three engineers (later seven), ~1,500 PRs, ~1M lines, five months, zero human-written code. `AGENTS.md` shrunk to a ~100-line **table of contents** pointing into `docs/{design-docs,product-specs,exec-plans,references}/`, because "a bloated instruction file crowded out the actual task"; architecture enforced by linters and structural tests rather than prose; scheduled doc-gardening agents open cleanup PRs against stale docs and update per-domain/per-layer quality grades. (Read directly by the survey's Codex pass — all claims primary-verified, including the literal "graveyard of stale rules" phrasing.)
- Anthropic, ["Scaling managed agents"](https://www.anthropic.com/engineering/managed-agents), Apr 2026 — The highest-signal source in this area. The durable session log sits outside the harness (`getSession`/`emitEvent`/`wake`/`getEvents`), so nothing in the harness needs to survive a crash, and compaction is a projection-time transform that never touches the log.
- Arize, ["How to build planning into your agent"](https://arize.com/blog/how-to-build-planning-into-your-agent/) — The **PlanMessage**: the plan is lifted out of conversation history entirely and rebuilt from state on every loop iteration, pinned immediately after the system prompt. Plus a `finish` tool that refuses to terminate while tasks remain pending.
- Anthropic, ["Long-running Claude for scientific computing"](https://www.anthropic.com/research/long-running-Claude) — The progress file (`CHANGELOG.md`) as portable long-term memory, recording **failed approaches and why they failed** so successive sessions don't re-walk dead ends. Real artifacts: [`smsharma/clax`](https://github.com/smsharma/clax).
- Anthropic, ["Effective harnesses"](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents) (Nov 2025) and ["Harness design for long-running application development"](https://www.anthropic.com/engineering/harness-design-long-running-apps) (Mar 2026) — Read as a pair: four months apart, the sprint construct *and* the context resets were deleted on a model-tier upgrade, planner and evaluator kept. Harness scaffolding depreciates; stress-test it for load-bearingness rather than accumulating it.
- Claude, ["When to use multi-agent systems (and when not to)"](https://claude.com/blog/building-multi-agent-systems-when-and-how-to-use-them) — Multi-agent is a **read-heavy** technique. Splitting write work by role on a shared surface is named an anti-pattern: "each handoff loses context," and teams observed agents spending more tokens coordinating than working.
- [`openai/symphony`](https://github.com/openai/symphony) — Ticket as the sole control plane: `WORKFLOW.md` rendered per issue, a claim-state machine, per-issue workspaces, and `Human Review` as a first-class terminal state. The container-bound version of what our commitment graph already represents.

[Survey notes →](epr:context-engineering-primary-sources-cross-pollination-2026-08-13)
