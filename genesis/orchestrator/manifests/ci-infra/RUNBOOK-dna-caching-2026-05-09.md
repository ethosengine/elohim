...
kubectl exec -n <workspace-ns> <workspace-pod> -c tools -- \
    cat /projects/elohim/genesis/orchestrator/manifests/ci-infra/nix-cache-pvc.yaml | \
    kubectl apply -f -
```
...
```bash
kubectl apply -f genesis/orchestrator/manifests/ci-infra/nix-cache-pvc.yaml
```
...
`storageClassName: openebs-jiva-csi-default` is the same class used by the existing nix-cache and cargo-cache PVCs. Per `genesis/orchestrator/manifests/ci-infra/nix-cache-pvc.yaml` header:
...
- Manifest: `genesis/orchestrator/manifests/ci-infra/nix-cache-pvc.yaml`
