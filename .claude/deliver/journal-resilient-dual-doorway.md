...
| G | **P1 projection reconciliation stream** — peers = discovery only (connected-peer fan-out, NOT same-agent bindings — the incident is cross-agent); own conductor = truth via `get_rea_commitment`; shared `project_commitment_from_wire` mapping (signal arm rewired, behavior unchanged); two-pass anchor-divergence through the single GapTracker; status on `P2PStatusInfo.projectionReconcile` (mirrors `pull`); inventory cap 2000 newest-first under the 256KiB wire cap; boot+30s then `PROJECTION_RECONCILE_SECS` (300 default, 0 off); heal logs WARN naming id + discovering peer; v1 table = rea_commitments (agreements/events seam documented, returns honest-empty); +1 scenario in substrate-reconciliation.feature (own-conductor-truth invariants) | elohim-views shared.rs + elohim-storage (new p2p/projection_reconcile.rs ~370 + 8 files) + p2p-status-view schema + generated TS (21 files, +1061/−31) | lib 1483/1483, schema_contract 212/212, view-federate 6/6, export_bindings green, codegen verify clean, clippy+fmt clean | `d88bba0a1` |
