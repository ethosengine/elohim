...
- ethosengine I/O wedge (root-caused 2026-08-17): ~11 `hard,timeo=600` NFS4 mounts to in-cluster ClusterIPs (openebs-rwx servers for SonarQube + its Postgres, scheduled on ethosengine itself). When the NFS server pod dies or reschedules (or kubelite stops at shutdown) the VIP vanishes, hard mounts retry forever, `sync()` sits in D-state holding `s_sync_lock`, and even LOCAL ext4 flushes stall — power button is the only exit. Ruled out: NVMe, APST, kernel 6.8.0-137, AMD-Vi IO_PAGE_FAULT. RESOLVED at source the same day (SonarQube PVC moved to openebs-hostpath). Residual watch: any future openebs-rwx consumer scheduled on a node reintroduces the class; the systemd shutdown unit that unmounts kubelet nfs4 in ExecStop stays the backstop. (Folded 2026-09-24 from the former `project_ethosengine_wedge_nfs_hardmount` entry.)
- `gh` is baked into the udi-plus image (pinned + checksummed, `che-devworkspaces/containers/udi-plus/Dockerfile`, 2026-09-05) and survives container recycles; `GH_TOKEN` (EthosengineBot classic PAT, repo+admin:org+workflow) auths it with no login step; curl against the REST API needs no install. (Retired 2026-09-24: the former `reference_gh_cli_install` entry's manual restore procedure.)
...
- Whole-host power cuts (Sept 2026, PSU fault; PSU replaced by 2026-09-22 — see [[reference_node_hardware_ethosengine_shem]] for the load-triggered crashes that continued after): a WHOLE-HOST cut is not a container recycle — mesh, a running pre-push gate, every background agent and watch die at once, /tmp is wiped, /projects survives and `berth` leases stay held by dead agents. Run long legs re-entrant from durable state (logs under /projects/.claude-config/jobs/), commit early and path-limited, `berth release` stale leases first thing after a restart; a sudden RAM-guard "committed 1.7G" at session start is the fingerprint.

**Restart → `dubious ownership` (2026-09-24).** After a workspace restart (kernel 7.0.0-31 → -34, Claude Code process exited mid-sprint), every git command in `/projects/elohim` failed with `detected dubious ownership` — the checkout is owned by uid 1234 while the container runs as uid 0. Remedy: `git config --global --add safe.directory /projects/elohim` (and `'*'` for the many `.worktrees/`), then re-read state before resuming agents: background subagents stopped by the restart keep their transcripts and resume with `SendMessage` to their id; their uncommitted work is still in the shared tree, so read `git status` first and hand each agent what the tree now shows.
