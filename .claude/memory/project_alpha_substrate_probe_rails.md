...
- **Jenkins without the MCP log search (live consoles time it out):** `curl -sS https://jenkins.ethosengine.com/job/<job>/job/dev/<n>/wfapi/describe` (stage list + status + durations; anonymous read works), `…/<n>/consoleText | tail -40` (full console, ~35k lines fine), `…/execution/node/<stageId>/wfapi/describe` (first 100 step nodes only). `logText/progressiveText` returns no `X-Text-Size` here — use consoleText. Prometheus uid `prometheus`, Loki uid `loki` via the observability MCP: `kube_deployment_status_replicas_unavailable`, `kube_pod_container_status_waiting_reason{reason="CrashLoopBackOff"}`, `kube_pod_container_info` (image tags per pod), `kube_service_info`, `kube_ingress_path{host=~…}` answer most "what is the cluster doing" questions with no kubectl.
- **Two Jenkins reading traps (harvested 2026-09-22 from archived sprint-results):** `curl` needs `-g` for `tree=…{0,N}` range queries (unescaped braces are curl globs); and a WebFetch *summary* of `wfapi/describe` once attributed one build's stage table to another — cross-check any summarized stage table against that build's own `durationMillis` before trusting it.
