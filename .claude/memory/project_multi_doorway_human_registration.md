---
id: project-multi-doorway-human-registration
name: Humans register with multiple doorways for resiliency
description: A human is not bound to a single doorway — they register with multiple doorways simultaneously, so any one can service their access; design flows must be doorway-agnostic
type: project
originSessionId: 253292ea-69ea-4e76-86e3-6d87ebdac46c
cites:
  - doorway-access-tier-patterns | the pattern catalog where these doorway-agnostic flows surface as access tiers A / B / Recovery | sha256:347f4a6ee935b18f
---
A human registers with multiple doorways (doorway-A, doorway-B, doorway-C...) as a resiliency pattern. No single doorway is the human's "home." Any registered doorway can facilitate that human's access to the network — login, content fetch, recovery ceremony, governance participation.

**Why:** single-doorway binding recreates corporate-custody failure modes (that doorway fails → human is locked out). The protocol commits to never-absolute-lockout (graduated recovery authority), and single-doorway binding would violate that. Multi-registration puts the resilience at the human-level, not the doorway-level.

**Architectural implications:**

- **Flows must be doorway-agnostic.** Recovery ceremonies, login, content operations — none should assume "the human's doorway." The shared substrate (DHT as notary, libp2p mesh as data-ops) is what actually carries state; doorways are interchangeable entry points.
- **Invitations and signals fan out over libp2p mesh**, not via doorway-to-doorway routing. Gossipsub topics reach all elohim-storage pods regardless of which doorway-steward they belong to.
- **Data replicates across doorway-stewards' pods.** Sharded/replicated backups form the view of a human's data across multiple elohim-storage pods, so losing access to one doorway doesn't lose access to the human's history.
- **Hosted-conductor ceremonies can initiate at any registered doorway.** If doorway-A is offline, the claimant walks into doorway-B and starts the ceremony there. The ceremony references the human's DHT state which is shared.

**How to apply:**

- Any design featuring "the human's doorway" is suspect — name the plural. "A doorway the human has registered with" or "any of the human's doorways."
- Cross-doorway scenarios belong in a2o testing. A single-doorway a2o test is insufficient to prove multi-doorway claims.
- Doorway failure is a resiliency test case: if doorway-A goes down mid-ceremony, the claimant can retry at doorway-B and the ceremony state (on DHT) is still valid.
- Do not design doorway-to-doorway routing for claimant flows. Route via libp2p mesh; doorway is a local entry, not a forwarder.

**Current state (as of 2026-05-27):** Single-primary per-human. The alpha cluster runs two doorways (doorway-A on-prem, doorway-B on shem). The Jenkinsfile selects ONE primary doorway per human based on `nodeTypes` (containing `"remote"` → doorway-B, else → doorway-A) and writes a single `bootstrap_url` + `signal_url` pair into that human's conductor-config.yaml. Each human still discovers peers through exactly one doorway's signal server. Multi-registration is **not yet wired** at three layers:

1. **Conductor config (Holochain 0.7.0-dev.5):** `bootstrap_url` and `signal_url` are singular fields in the conductor's YAML schema. The conductor cannot fail over to a second signal server within its own networking stack — multi-bootstrap requires either an upstream HC schema change OR a substrate-layer dual-registration shape where elohim-storage maintains its own peer-discovery channel in parallel to the conductor's.
2. **Doorway federation (doorway-service):** outbound peer-discovery + heartbeat + cached peer list are wired (`services::federation::{spawn_peer_discovery_task, register_doorway_in_dht, spawn_heartbeat_task, refresh_peer_cache}`). Inbound bidirectional reconciliation is stubbed — `services::dashboard_topology` hardcodes `federation_peers.online = true` (always), `direction = OutboundOnly` (always), `shared_cid_count = 0` (always). Federation can SEE peers; it does not yet reconcile shared content state.
3. **Session + recovery binding:** `local_sessions.doorway_url` is `TEXT NOT NULL` — sessions are pinned to one doorway URL. A human who registers via doorway-A and attempts recovery through doorway-B will fail session lookup. Both `auth_routes.rs` and `disaster_recovery.rs` thread a single `doorway_url` through; per-session doorway-list / doorway-agnostic resolution is the missing piece.

The single-primary shape was a deliberate stepping stone, not the destination — it removes the "all peers depend on doorway-A's signal server" failure mode (so adam-on-shem can bootstrap independently of doorway-A) without yet solving the harder substrate question of how a conductor that's already configured with `bootstrap_url=A` learns about doorway-B if A becomes unreachable.

See sprint result genesis/docs/superpowers/sprints/2026-05-27-federation-wiring-audit.md for the full sequenced backlog from this state to true multi-registration.
