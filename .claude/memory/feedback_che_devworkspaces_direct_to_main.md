...
description: "Push policy by repo: che-devworkspaces (CI/image infra) main = push DIRECTLY (no PR, no dev→main); elohim MONOREPO main = NO direct push (reviewed dev→main release path)."
metadata: 
  node_type: memory
  type: feedback
