...
**Load-triggered crashes, 2026-09-22/23:** ethosengine went down four times in one night (~19:51, 21:47, 00:25, 02:26) while a session stacked up to three concurrent `cargo` builds (~5 GB each), the household mesh and full a2o suites; rustc/rust-lld died with SIGBUS (the signature of a read error on an mmap'd file) in between. A new PSU was fitted; 2x 4 TB durable NVMe were on order (~48 h). The operator observed that light work and waiting on Jenkins never crash it. **How to apply:** until the drives are replaced, run ONE heavy job at a time (single cargo build, `CARGO_BUILD_JOBS=1`, no build while a mesh suite runs, no parallel agent gates); treat household results gathered under load as noise. After a crash, a fleet conductor can crash-loop on an empty Lair `pid_file` — elohim-storage heals it before spawning the conductor (fix pushed 2026-09-23).

**Incoming 2026-09 (operator):** shem gets 3x refurbished 4 TB SAS SSDs (its onboard LSI takes SAS) — with its 135 GB DDR3 it becomes a dev-capable node, not just a test-fixture host; ethosengine gets 2x durable 4 TB NVMe. Once landed, heavy builds and the household mesh can move to / split onto shem, and the one-heavy-job rule becomes an ethosengine-only constraint. Re-probe both nodes after install before relying on these numbers.
