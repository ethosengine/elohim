...
**2026-09-12 (verified live):** a day of agents building elohim-storage under three feature sets (default gate, `p2p`, `p2p p2p-iroh`) in the `family/dev` storage slot grew `debug/incremental` to **56 GB** (deps 29 GB beside it) and pinned the volume at the 92 % hard ceiling; `cargo-pool enforce --yes` freed 0 B every time ("keep-warm over cap … no evictable slots") because the incremental dirs were < 3 days old and the dev family is protected — meanwhile it LRU-evicted every `family/sprint` slot agents were building in (three cold restarts in one day). Surgical rung that works: `rm -rf <slot>/dev/debug/incremental` (rebuildable cache; deps intact; 92 % → 85 % in one step). Rule for sprint days: give storage agents ONE slot and one feature set per gate, and clear the slot's incremental cache before the disk reaches the ceiling, not after the evictions start.
