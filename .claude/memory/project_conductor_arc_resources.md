...
description: "Sys-validation CPU spin; CPU storm starves storage; glibc leak → jemalloc; RAM ∝ corpus at full arc; pool admission shed."
metadata:
  node_type: memory
  type: project
---
...
- [[project_fleet_conductor_admission_ceiling]] — folded 2026-09-22 (index: false); On alpha the pool conductors (matthew/adam) shed interactive content_store writes and time out heal reads; the quiesce gate and the app head PATCH both fail on that, not on storage code
