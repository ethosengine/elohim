- [Elohim councils capture the apex (gospel-tier)](project_elohim_councils_capture_apex.md) — wisdom holds the structural top of authority by design; apex no longer corruptible by self-interest; friction-gradient + councils together complete the move; LLM as load-bearing primitive (bet = substrate surfaces wisdom while suppressing corruption); anti-concentration recurses to elohim layer; interpretability + openness + transparent attribution + distributed councils = alignment substrate.
- [Dissolution principle + sensemaking collectives](project_dissolution_principle_sensemaking_collectives.md) — at full elohim deployment, vestigial institutions (gov/banking/lawyers/insurance/etc) become abstract sensemaking collectives = primary domain of the elohim = substantive content of the apex; diagnostic = info/value/governance friction the substrate resolves; humans steward corporeal-scale, elohim councils steward abstract-scale; three substrate currents = Story+Value+Governance.
- [Household living core + lived contrast diffusion (gospel-tier)](project_household_living_core_lived_contrast_diffusion.md) — household is the protocol's living core (foundation/seed/driver, not one of four equals); value-scanner (~1,700 scenarios, 21 life-stage archetypes) is the substrate primitive making care-economy REA visible there; lived contrast = the diffusion mechanism; "why isn't this like home?" reforms common sense; participants bring the substrate INTO every other institution because the absence becomes intolerable; this is why MVP focuses on the seed.
- [Qahal — graduated capability surface](project_qahal_graduated_capability_surface.md) — one primitive; lamad-attested Bloom mastery curve as gate; rubric is steward-governed EPR; anti-colonization by construction; reach gates outward, standing gates inward; convergent UX (Hylo/Discord/Matrix) via graphos pattern stories.
- [Commons-elohim co-steward](project_commons_elohim_co_steward.md) — one autonomous elohim per Qahal as co-steward of the commons interest (reflects what the collective can't directly voice); holds commons-share custody; speaks in councils; mediates disputes; can't be silenced (structurally embedded at genesis). Earlier "shadow agent" framing rejected.
- [Friction-gradient limitarianism](project_friction_gradient_limitarianism.md) — anti-concentration as substrate; friction to centralization rises with accumulation; soft (standing-curve flattening) + hard (protocol refuses concentration ops); makes sovereignty mechanically expensive.
...
- [Subagent silent impl drops](feedback_subagent_silent_impl_drops.md) — rust-architect dropped 8 From impls across A.6/A.9 of Plan 3.A; per-crate build green, workspace build caught; gate moves with workspace build + before/after impl count.
...
- [Epistemic integrity under pressure](project_epistemic_integrity_under_pressure.md) — every load-bearing word (consent/community/stewardship/reach/trust/reconciliation) is one capture event from being hollowed; defense is pointable structure that breaks visibly when the word drifts; Foster's "Reclaiming Reconciliation" as canonical worked example: truth-telling + repair as non-negotiables; protocol is substrate for the ministry, NOT the reconciliation.
- [MISSION — Platform for collective biography](project_mission_platform_for_collective_biography.md) — north-star anchor from Foster via Coates: investigation → public acknowledgment → biographical accumulation with consequences → restored dignity to those who have suffered shame and oppression; causal order non-negotiable; audience-of-first-resort = the personas in `genesis/data/humans/`, not the early adopters. **Re-read before any major design decision.**
- [Consilience: higher-order sensemaking as P2P property](project_consilience_higher_order_sensemaking.md) — network has knowledge no single node has; substrate's job is to make recognition from healthier vantage points RECEIVABLE at insular nodes via bridges/nudges/experiences, NEVER imposed; fish-in-water problem is universal; Foster's gift of self-recognition is rare and must be supplied structurally; protocol = patience machine, generational time-scale, outlives its authors.
- [No false-opposition rhetoric in framing](feedback_no_false_opposition_rhetoric.md) — refuse "not X, not Y, but Z" when X/Y aren't mutually exclusive with Z; the protocol coordinates AND arbitrates truth AND exercises control; patience qualifies HOW, not WHETHER; hollowing-out by negation is still hollowing-out, the same anti-pattern epistemic-integrity-under-pressure names at protocol scale.
- [Social contract makes friction explicit](project_social_contract_makes_friction_explicit.md) — values-forward stance has lived costs; accountability-resistant actors, weaponized-attention models, and harm-externalizing positions will experience the protocol as friction BY DESIGN; asymmetric care = patience for PEOPLE, friction for harmful IDEAS; the grandmother bar applies in reverse — a substrate comfortable for both her AND her predator is failing her.
- [Protocol sheds externalities to non-participants](project_protocol_sheds_externalities_to_non_participants.md) — value flow points OUTWARD; bargain is unilateral generosity; substrate accounts for non-participants whether they have a node, recognize the protocol, or use it; refuses lock-in via network effects, value extraction from non-users, and conditional dignity; grandmother bar in broadest scope = credible in the life of a grandmother who NEVER uses it because her family does.
- [Orchestrator soft-skip NOT_PROVISIONED](project_orchestrator_soft_skip_not_provisioned.md) — triggerPipeline classifies "No item named" → soft-skip UNSTABLE (not FAILURE); registration in strategy.mjs can lead Jenkins job provisioning safely (landed 09d39c2f4).
- [Husky manifest-vs-grep project drift](feedback_husky_manifest_vs_grep_project_drift.md) — pre-push's two detection paths emit different names; adding a build-manifest gate.projects entry needs a matching run_gate fallback case.
- [Operator recurrence — verify per build](feedback_operator_recurrence_perception_verify_per_build.md) — "we keep hitting this" can hide N distinct failure classes; pull first_failing_stage per build before scoping (5 classes / 5 builds on 2026-05-17).
- [Shift CAN fix unparseable test fixtures](feedback_shift_can_fix_unparseable_test_fixtures.md) — principle #6 doesn't prohibit grammar-restoring edits when the fixture won't parse at all; declare scope up front; constrain edits to the parse-error lines.
- [cucumber-report-empty signature](feedback_cucumber_report_empty_signature.md) — Jenkins "Missing report result → UNSTABLE" + scenarios=0 = Gherkin parse error elsewhere; read E2E log for "Parse error in features/" first.
...
- [Nexus Cargo publish requires Basic auth](feedback_nexus_cargo_publish_basic_auth.md) — NpmToken reads sparse index OK, but PUT /api/v1/crates/new returns 401 with www-authenticate: BASIC realm; cargo:token provider only sends Bearer; need Nexus user-token Name+Pass Code as Basic, or admin enables Bearer realm; T9/T10 deferred until then.
- [SDK scaffolding generators (ng-generate-style)](project_sdk_scaffolding_generators.md) — 6 repeatable patterns surfaced 2026-05-18: add content-type/View pair/schema+struct/split-manifest/new-domain/signal-kind. Substrate-without-tooling = library, not SDK. Spec after Plan 2 lands.
- [ts-rs cross-crate import paths](feedback_ts_rs_cross_crate_import_paths.md) — 2026-05-18 PILOT: partial type moves across crates break 70+ downstream .ts files; ts-rs computes import paths from source crate location, not export_to. Fix: atomic single-crate consolidation. crates/elohim-views is the canonical pattern.
- [K8s is dev-substrate, not protocol](feedback_k8s_is_dev_substrate_not_protocol.md) — k8s is the bootstrap-phase dev/test-bench; protocol docs MUST keep pod/PVC/kubectl/kube-rs out of substrate descriptions; carries forward only as inspirational analogue for elohim-operator complexity-collapse; trajectory: brit→rakia self-hosting.
- [Resilience epic landed (Sheila + Gertrude witnesses)](project_resilience_epic_landed_2026_05_18.md) — 9-part chapter at `genesis/docs/content/elohim-protocol/resilience/README.md`; mutual aid as substrate primitive + recovery as test + patron-CDN as civic distribution + elohim-operator as complexity collapse; ~80-row LIVE/DESIGNED/GAP matrix in Part IX; 15-item leverage-ordered roadmap; foundational chapter the other epics rest on.
- [Commit-attribution leak under parallel agents](feedback_commit_attribution_parallel_agent_leak.md) — resilience epic commit e44bd77c3 swept in 10 views_convert/ scaffolds from parallel monolithic-decomp agent; root cause not confirmed (likely intermediate `git add`/hook); use narrow `git add <file>` under parallel-agent work; open thread — unresolved on local dev, safely reversible pre-push.
...
- [Living-doc honesty matrix maintenance](feedback_living_doc_honesty_matrix_maintenance.md) — chapters with LIVE/DESIGNED/GAP matrices need migration discipline; resilience epic Part IX ~80 rows must update as substrate work lands; memory-ceremony standing beat + cartographer readiness input; optional grep-CI gate as low-cost guard against silent rot.
- [Substrate-currency ceremony 2026-05-18 YELLOW backlog](feedback_substrate_currency_ceremony_2026_05_18_yellow_backlog.md) — 3 next-cycle pickups: manifest-route worked example (overlaps with doorway/CLAUDE.md parent rewrite), query-composition aggregation pattern in rust-architect, ground-truth check on resilience-epic Part VII ReconcileController claim vs "no-op stubs" code comment.
