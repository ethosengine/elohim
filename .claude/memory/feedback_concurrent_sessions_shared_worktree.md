...
- **Recurred 2026-08-23 (twice in one session, both self-inflicted):** (1) `git add <4 backlog docs> && git commit` swallowed a rust-architect agent's 5 staged doorway files (cure + story) under a `backlog:` message — recovered with `reset --soft` + two `git commit -- <paths>` commits while nothing was pushed. (2) A storage agent ran `--amend` after a blind-reader pass and folded its two hunks into another agent's `fix(a2o)` commit; left as-is (second rewrite on a live tree is worse than the blur). Net rule, sharpened: **the orchestrator itself must use `git commit -m … -- <paths>` for EVERY commit, and every dispatched agent prompt must carry the form verbatim plus "never --amend; make a follow-up commit".**
