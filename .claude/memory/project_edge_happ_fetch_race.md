...
id: project-edge-happ-fetch-race
...
> UPDATE (2026-09-22): since 8ebce05a3 (09-14) `dependsOn` IS enforced as level ordering, and a selected long-running DNA is awaited (detached barrier) before edge starts — the same-wave race below needs DNA+edge in one push to recur only if that barrier is removed. See [[project_pipeline_dispatch_ordering]].
