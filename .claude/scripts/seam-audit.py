...
program is behind an explicit verb (`--file`, `--write`, `--record`, `--record-miss-of-canon`).
  ...
  --calibrate [--record]      spine-flip intake for the calibration ledger (append-only)
...
def main() -> int:
    ...
    if args.calibrate:
        ...
        if args.json:
            ...
            print("CALIBRATION  (plan P4.4 — append-only ledger, spine-flip intake)")
            print("=" * 72)
        ...
        if args.record:
            ...
            print(f"\nrecorded: {res['written']} calibration row(s)"
                  + (" + a fresh spine snapshot" if res["snapshot"] else ""))
