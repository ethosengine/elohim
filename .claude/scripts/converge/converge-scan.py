...
Reads:
  ...
  - The latest dated lens reports (cleanup-backlog-refresh.md,
    dedupe-clusters.md, sprint-digest.md, path-update-proposals.md) for
    item assignment

Outputs: .eprfs/status/lenses/<YYYY-MM-DD>/convergence-themes.md

Phase 2 (synthesis subagent) consumes this — see the converge skill.
"""
...
MEMORY_KIT_ROOT = _paths.reports_root(REPO_ROOT)
...
# ---------- Item assignment (consume the dated lens reports) ----------
...
def render_themes(themes: list[Theme], all_plans: list[Path]) -> str:
    ...
    out.append("")
    out.append(
        "Generated by `.claude/scripts/converge/converge-scan.py` (v2 — TF-IDF + bigrams + auto-stopwords + TextRank centrality). "
    ...
    if not themes:
        out.append("_No themes surfaced. Try lowering --min-composite or running the hygiene lenses first._")
...
def main() -> int:
    ...
    parser.add_argument("--reports-date", help="Date dir under .eprfs/status/lenses/ (default: latest)")
    ...
    if reports_date is None:
        print("error: no dated lens report directories found under .eprfs/status/lenses/; run the hygiene lenses first",
    ...
    # ----- Item assignment from the dated lens reports -----
