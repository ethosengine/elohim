...
Sibling of the hygiene lenses at `.epr-meta/elohim/lenses/`.
...
Output: .eprfs/status/lenses/<YYYY-MM-DD>/sprint-digest.md
...
EXCLUDE_FRAGMENTS = (
    ...
    ".claude/worktrees/",
    ".claude/archive/",
    "status/lenses/",  # don't recursively digest our own output
)
...
def render_digest(entries: list[SprintEntry], out_path: Path, since: date | None) -> str:
    ...
    out.append("")
    out.append("Generated by `.claude/scripts/dev-dashboard/sprint-distill.py`. Read-only.")
    out.append("")
    out.append(
...
def main() -> int:
    ...
    out_dir: Path = args.out_dir or _paths.reports_dir_for_today(REPO_ROOT, TODAY)
