...
Output: .eprfs/status/lenses/<YYYY-MM-DD>/plan-status.md

Sibling lenses live at `.epr-meta/elohim/lenses/`. This is the
...
def render_report(records: list[PlanRecord], include_complete: bool) -> str:
    ...
    out.append("")
    out.append(
        "Generated by `.claude/scripts/dev-dashboard/plan-status.py`. Read-only "
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Override output directory (default: .eprfs/status/lenses/<YYYY-MM-DD>/).",
    )
    args = parser.parse_args()

    out_dir = args.output_dir or (
        _paths.reports_dir_for_today(REPO_ROOT, TODAY)
    )
