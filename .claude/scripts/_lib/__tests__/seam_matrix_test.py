...
Two halves, deliberately:
  (a) SYNTHETIC — cell-state derivation, routing, ranking and digest stability over fixture repos
      under tempfile.TemporaryDirectory(). The real registries/catalog/habits are never written.
  (b) LIVE, READ-ONLY — the structural contract of `.claude/epr-meta/seam-catalog.yaml` against
      ...
      `seam-registry.yaml`, every `habits` entry must name a real habit, and every
...
with tempfile.TemporaryDirectory() as td:
    ...
    c4sx = _cell(d, "C4", "SX")
    c4sy = _cell(d, "C4", "SY")
    check("rung-proximity: a red+active habit outranks a green one",
    ...
    check("a seam with no habits linkage falls to the proximity floor, never to zero",
          sm.rung_proximity({"habits": []}, {})[0] == 0.5)
    ...
    # A substrate move (habits flip) must move the digest — otherwise the freshness check is inert.
    _w(root / sm.HABITS_REL, _HABITS.replace("id: fix-green\n    status: green",
                                            "id: fix-green\n    status: red"))
    d2 = sm.matrix_data(root)
    check("a habits flip changes the digest (the freshness check is not inert)",
...
habits_live, _ = sm.load_habits(REPO)
declared_articles = {n for s in live_cat["seams"] for n in (s.get("habits") or [])}
check("LIVE: every habit the catalog links is real in habits.yaml",
      declared_articles <= set(habits_live))
