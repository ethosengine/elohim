...
metadata:
  ...
  master: package
  governance: "epr:elohim-agent/agents/graphos-designer"
---
...
**REQUIRED EYES:** before any aesthetic verdict and during verification, follow the `looking-at-frontend` skill — Read `.claude/skills/looking-at-frontend/SKILL.md` (you have no Skill tool). It is your eyes: `pnpm graphos story|sheet` (from `genesis/a2o`) renders stories to a `shot.png` you can Read; `--base http://localhost:6006` for in-branch work. Byte-correct tokens can still be dead bindings; only a render proves the binding lands.
...
8. **Verify with eyes** — render the new/changed stories via the `looking-at-frontend` skill (`pnpm graphos story <id>` or `sheet <component>` from `genesis/a2o`, `--base http://localhost:6006` against a local `pnpm storybook`), Read the shot.png for both themes, and check `capture.json` for console/page errors. `pnpm --filter graphos run build-storybook` alone is not verification.
