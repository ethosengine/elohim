...
When verified canonical memory surfaces change, re-mine natively: `mempalace --palace .mempalace/palace sync --root . --apply`, then `mempalace --palace .mempalace/palace mine <surface>` for each surface declared on `mempalace-surfaces-changed-ceiling@1` in `.claude/epr-meta/measures.yaml` (`genesis/docs/content/elohim-protocol`, `.claude/memory`, `genesis/data/stories`), then stamp `date +%s.%N > .mempalace/.last-mine` ONLY if no step was lock-blocked — the honesty rule the wrapper it replaces existed to keep. Check `epr flow report --headline` for the `mempalace:` freshness slot. A failed refresh stays visible; do not claim the provider current or erase successful local work. No automatic diary/curation outside this authorized maintenance scope. Reuse the existing chronology and flow records rather than creating another ledger.
