...
>
> Output ≤ 500 words per shape: health summary, top findings, archive candidates, dedupe candidates, CLAUDE.md drift surfaces, **graduation-delivery-gap list**, **story-coverage numbers (features-on-disk, features-orphan, top orphans by leverage_score)**, **per-story sourcing-completeness flags**, chronicle-worthy moments, signals worth carrying forward.
...
>
> Output ≤ 400 words: trajectory read, max 3 resonant precedents (archived-at, what happened, why now, suggested action), no-resonance themes.
...
8. **Stasis implementation plan (REQUIRED, Round 4+)** — for each audited dimension in LIFECYCLE.md "Dimensions with stasis targets", produce a row in the stasis plan. This is the substrate Wave 6 executes against — **not aspirational, binding**. Authoring discipline:
   ...
   - **The plan is binding.** Wave 5 retros will reckon dimension-by-dimension against it; Wave 6 will execute it; the chronicle will record per-dimension achievement. Padding the defer column to escape work shows up in the next-cycle 3-cycle-clock as a forced-attempt.
...
| Dimension | Current | Target | 20%-floor | Proposed action | Resolution agent | Effort tier | Status |
|---|---|---|---|---|---|---|---|
| CLAUDE.md DRIFTED-FACTUAL count | 18 | 0 | 14 | Reword the 4 most-stale lines in app/elohim-app/CLAUDE.md per audit's per-line evidence | librarian | small | proposed |
| CLAUDE.md OVER-BUDGET count | 2 | 0 | (n/a — count of 2; advance = touch at least 1) | Trim root CLAUDE.md by ~200 lines via section consolidation | librarian | medium | proposed |
| Cleanup-scan review flags | 67 | 0 | 53 | Run cleanup-judge subagent over the 14 most-stale entries; apply operator-confirmed ARCHIVE | librarian | small | proposed |
| Agent catalog real findings (post-FP filter) | 3 | 0 | 2 | Fix dead-path citations in 2 agent prompts | librarian | small | proposed |
| Skill catalog overlap pairs | 1 | 0 | 1 | Disambiguate /memory-kit vs /converge descriptions | librarian | small | proposed |
| Story orphan-ratio | 0.42 | 0 | 0.34 | (deferred-with-rationale: storyteller-coverage-sprint owns this; ceremony surface ≤3/cycle) | storyteller | medium | deferred |
| Story sourcing-completeness flags | 2 | 0 | 2 | Backfill sourced_from on james-son + david-and-the-stewarded-hub | storyteller | small | proposed |
| MEMORY.md byte size | 30,025 | 24,400 | 28,900 | Tighten 6 longest index entries; graduate 2 stale memorialized topics | librarian | medium | proposed |
| Surface:Archive ratio | (current) | <100:1 | (current × 0.8) | (operator-confirmed graduations from Wave 4) | storyteller + librarian | small | proposed |
| /deliver pickup queue | (current) | drain over time | (current × 0.8) | (deferred-with-rationale: cycle has no /deliver-ready features this round) | /deliver | small | deferred |
```
...
- **Effort tier** — small (≤1 dispatch); medium (≤3 dispatches); large (needs split across cycles → must be marked deferred-with-rationale)
- **Status** — `proposed` (default; will be executed in Wave 6) or `deferred-with-rationale` (cycle cannot advance this dimension; rationale required)
...
Present compact summary to operator (≤15 lines):
- Librarian: archive/dedupe/CLAUDE.md drift counts
- Historian: precedent count + names
...
4. Signal candidates (max 2) — concrete hook/accumulator/cron with trigger + threshold + action
5. Cadence proposal — how often + what should trigger
6. Was Wave 2 operating shape (single-agent four-lens vs TeamCreate) the right call?

Output ≤ 250 words each (raised from 200 to accommodate item 3).
...
4. Dispatch with **anti-bail framing baked into the prompt**. Use this template (adapt per-dispatch):

   > "Advance this dimension to at least its 20%-floor as recorded in the stasis plan. **20% is the worst acceptable outcome, not the target — 50% bite is the default ambition when your lens supports it.**
   >
   > **Do NOT return with 'I propose to do X.' Return with X executed.** If scope feels too large to fully execute in one dispatch: split it. Do enough this cycle to clear the 20%-floor; surface the remainder as a backlog entry. Returning with 'this is too big to attempt' is not an available outcome — return with the executable bite plus the split-for-next-cycle plan.
   >
...
Each agent executes to achieve at least the 20% threshold for their dimension; the dispatch prompt instructs them to aim higher when their lens supports it. **Particularly for the librarian** (resolver for most stasis-plan rows): the dispatch prompt should enumerate concrete diffs from Wave 3 so execution is "apply this paste-ready set," not "go decide what to do."
...
### Phase 6d — Historian writes chronicle
...
### Phase 6e — Next-ceremony substrate
