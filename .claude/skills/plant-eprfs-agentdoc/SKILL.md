...
description: "Plant ONE runtime-authored agent-doc (CLAUDE.md / AGENTS.md) in its elohim-native package on the eprfs layer. The package becomes the authoritative root for the gospel doc's frontmatter (cite envelopes preserved verbatim) + body; the doc file stays BYTE-IDENTICAL because the projection is verbatim passthrough, so the cite writer is untouched and the cite fingerprints survive intact. The agent-doc member of the plant-eprfs family; the most sensitive plant type (gospel / managed-memory surfaces). Use when \"plant this CLAUDE.md\", \"plant AGENTS.md for <dir>\", or rooting a gospel / managed-memory doc in its package without changing a single byte of it. Siblings for other artifact types: plant-eprfs-skill (SKILL.md), plant-eprfs-agent (subagents), plant-eprfs-command (slash commands), plant-eprfs-hook (executable hooks)."
metadata:
...
## The FLIP that composes with the cite writer

Agent-docs DO flip (the package becomes `master`, like every other plant type). The flip composes WITH `epr flow cites` instead of fighting it, and this is the whole design:

- **The doc's frontmatter is a LIVE index.** `epr flow cites seal` writes `id:` + content-addressed `cites:` envelopes (`slug | desc | sha256:… | path: …`) whose fingerprints are computed from the *cited* docs' content. A generate-from-metadata projector (the skill/agent flip) could never reproduce those, and would fight the cite writer.
- **So the projector is VERBATIM PASSTHROUGH, not generate-from-metadata.** `project(package)` returns `source.body` unchanged — the entire raw file. The flip moves *authority* into the package; it rewrites **zero bytes** of the doc. `epr flow cites` stays the sole author of the doc surface, and every cite fingerprint survives byte-identical. This is why the flip composes: the eprfs/epr-meta root is a content-addressed substrate (cites are sha256 envelopes, the package graph is BlobCid), so the package absorbing cite provenance is a natural extension, not a foreign burden.
- **The authority marker lives in the PACKAGE, never in the doc.** `metadata.master: "package"` goes in the package JSON only. The loader reads authority from the package (package-aware skip), never from the doc frontmatter — so `master: package` is **never** injected into the CLAUDE.md. `epr flow cites` + `managed_surfaces.py` + the `.epr-meta` compose-gate remain the EDIT authority over the doc file itself.
...
- **Authority rooted in the package.** The package gains `metadata.master: "package"`. Editing the doc directly is still done through `epr flow cites` (the doc is the cite surface), but the package is now the authoritative composition root and the freshness anchor.
...
- **The composition note.** The package carries a short authored `composition` note describing how this doc composes / is-managed relative to the others — the one piece of authored intent that has nowhere to live in the doc's own frontmatter without perturbing the cite envelopes.
...
2. **The cite writer is UNTOUCHED.** Because the doc bytes never change on projection, `epr flow cites verify <doc>` resolves exactly as before and every `sha256:…` fingerprint is preserved. Planting must never trip the cite gate.
...
2. **Plant atomically** with the replant wrapper: `node elohim/sdk/domains/elohim-agent/scripts/replant.mjs --compose <id>`. It runs the whole project → verify → compose sequence and is the supported way to plant: it snapshots every file the plant could touch, writes fixtures then runtime scoped `--only <id>` (never a bare tree-wide `--write-runtime`, which rewrites every other package's runtime file — including un-planted Claude-authored and ambiently-modified ones), re-runs `verify`, and restores the snapshot byte-exactly if the plant introduced a failure that was NOT in the pre-plant baseline — so a red plant can never leave a half-planted gospel tree, and a tree already red for unrelated reasons still lets a clean plant land. Run `--dry-run <id>` FIRST: the doc's own path must NOT appear in the reported change list. For a doc the write is **byte-identical** to what was there (verbatim passthrough) — `git diff <X>` MUST be empty. A non-empty diff means the bytes were transformed somewhere and is a STOP condition. **If the flip cannot preserve the cite envelopes byte-identically, STOP — never ship a mangled gospel doc.**
3. **Confirm package-first.** `verify` green means X takes the package-first path (the loader skips re-importing a `master: package` doc), and the byte-identity assertion holds for both the fixture and the runtime doc. Underlying mechanism `replant.mjs` runs for you: `package-projections.mjs project --write-fixtures --only <id>`, `... project --write-runtime --only <id>`, `... verify`, `eprfs-agent compose-graph .epr-meta/elohim/packages --projections-root .epr-meta/elohim/projections`.
4. **Confirm the cites still resolve.** `epr flow cites verify <X>` exits 0 — every cite is still content-addressed and resolvable. The flip touched nothing the cite writer cares about.
...
6. **Confirm the composition record.** X's compose-graph node shows `kind: "AgentDocPackage"`, `master: "package"`, `packageCid` <-> projection CID (equal, since the mirror is verbatim), and `composedBy` (attribution comes from the `metadata.composedBy` set in step 1 — `replant.mjs` passes no `--composed-by`). Agent-doc support in the Rust compose-graph adapter may still be pending, and `--compose` warns-and-continues when the `eprfs-agent` binary is absent — byte-identity is the floor that needs no CID.
...
- **Never inject an authority marker into the doc.** `master: package` lives in the package JSON only. The doc frontmatter stays exactly as the cite writer authored it, so the `gospel-claude-md` classifier and every cite envelope stay intact.
- **`epr flow cites` owns the doc surface.** Any change to the doc's frontmatter (`id:`, `cites:`) goes through cite tooling, never a hand-edit and never the projector. Planting is orthogonal to authoring.
