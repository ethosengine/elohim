...
**The census is the enforcement backstop, not a courtesy reminder.** `python3 .claude/scripts/seam-audit.py --matrix` reads every crate's `seam-registry.yaml` and reports a missing registration or an uncited contract test as part of the concern×seam matrix; `--spot-check` independently re-derives a sampled set of decision points from disk and exits 2 on any mismatch, including a mirrored test (the anti-mirror fixture — a passing test measuring the same helper the code under test uses). The co-located `.epr-meta` `inject` rules on decision-surface `.rs` diffs (e.g. `doorway/doorway-service/src/.epr-meta`, `steward/node/src/.epr-meta`, `crates/seam-contracts/.epr-meta`) are this step's edit-time half — they fire on the same shapes this step asks you to walk through at design time. Skip this step and the gap doesn't vanish; it surfaces later, at the census or in production, which is exactly the cost this gate exists to avoid.
