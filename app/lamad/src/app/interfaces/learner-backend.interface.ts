 ...
 * Consumers inject the LEARNER_BACKEND token.
 *
 * Tests provide a mock ILearnerBackend via the same token — no
 * concrete service import needed.
 *
 * P-disposition: Moved from @app/elohim/interfaces (Slice 2.1c).
 * The factory is NOT included here — lamad's app.config.ts registers
 * LearnerBackendApiService as the concrete provider.
 *
 * @example
 * ```typescript
 ...
 *
 * The default provider is registered in lamad's app.config.ts:
