...
export interface ResilienceSnapshotView {
  ...
  /**
   * How many additional distinct-collective slots short of the resilience floor this content is. PRESENT as a non-negative integer whenever distributionState is "measured" — including the value 0, which is itself a measurement ("the floor is met"), never a missing lens. ABSENT when distributionState is "unmeasured": a shortfall computed over a non-measurement would fabricate a measurement. Absent is NOT 0, and the key is never a present null. The relational household_resilience::snapshot path computes it by comparing stewardingCollectives against the tier floor (elohim_facings::folds::resiliency::floor_for_tier, the same floor feltStatus.floor.wantsHouseholds states); the graph-backed branch derives it via CoverageRollup.deficit.measure().
   */
