...
describe('@elohim/identity/core boundary (elohim-sdk.md §4.1)', () => {
  ...
  it('walks a non-trivial closure (walker sanity)', () => {
    ...
    expect(visitedNames.some(f => f.endsWith('/lib/doorway-session-client.ts'))).toBe(true);
    expect(visitedNames.some(f => f.endsWith('/lib/attestations.model.ts'))).toBe(true);
    expect(visitedNames.some(f => f.endsWith('/lib/session-human.model.ts'))).toBe(true);
    expect(visitedNames.some(f => f.endsWith('/lib/content-access.model.ts'))).toBe(true);
  ...
  it('detector control: the Angular root entry (public-api.ts) DOES trip the detector', () => {
    ...
    // entry re-exports session-human.service, which imports @angular/core.
