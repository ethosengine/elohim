...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-imagodei-introspection-panel');
  if (!found) {
    throw new Error(
      'elohim-imagodei-introspection-panel declaration not found in custom-elements.json. ' +
...
describe('elohim-imagodei-introspection-panel custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-imagodei-introspection-panel');
    expect(decl.name).to.equal('ElohimImagodeiIntrospectionPanel');
  });

  it('declares the subjectId property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'subjectId');
    expect(prop).to.exist;
  });

  it('declares the subjectSettings property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'subjectSettings');
    expect(prop).to.exist;
  });

  it('declares the sampleAffordances property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'sampleAffordances');
    expect(prop).to.exist;
  });
});

describe('<elohim-imagodei-introspection-panel> — capabilityContract manifest', () => {
