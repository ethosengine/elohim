...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-imagodei-settings-palette');
  if (!found) {
    throw new Error(
      'elohim-imagodei-settings-palette declaration not found in custom-elements.json. ' +
...
describe('elohim-imagodei-settings-palette custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-imagodei-settings-palette');
    expect(decl.name).to.equal('ElohimImagodeiSettingsPalette');
  });

  it('declares the viewerId property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'viewerId');
    expect(prop).to.exist;
  });

  it('declares the targetId property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'targetId');
    expect(prop).to.exist;
  });
});

describe('<elohim-imagodei-settings-palette> — capabilityContract manifest', () => {
