...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-imagodei-setting-control');
  if (!found) {
    throw new Error(
      'elohim-imagodei-setting-control declaration not found in custom-elements.json. ' +
...
describe('elohim-imagodei-setting-control custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-imagodei-setting-control');
    expect(decl.name).to.equal('ElohimImagodeiSettingControl');
  });

  it('declares the setting property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'setting');
    expect(prop).to.exist;
  });

  it('declares the viewerCanEdit property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'viewerCanEdit');
    expect(prop).to.exist;
  });
});

describe('<elohim-imagodei-setting-control> — capabilityContract manifest', () => {
