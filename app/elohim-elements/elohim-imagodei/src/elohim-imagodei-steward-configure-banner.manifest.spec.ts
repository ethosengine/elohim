...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-imagodei-steward-configure-banner');
  if (!found) {
    throw new Error(
      'elohim-imagodei-steward-configure-banner declaration not found in custom-elements.json. ' +
...
describe('elohim-imagodei-steward-configure-banner custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-imagodei-steward-configure-banner');
    expect(decl.name).to.equal('ElohimImagodeiStewardConfigureBanner');
  });

  it('declares the stewardName property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'stewardName');
    expect(prop).to.exist;
  });

  it('declares the stewardeeName property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'stewardeeName');
    expect(prop).to.exist;
  });

  it('declares the witnessQahalName property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'witnessQahalName');
    expect(prop).to.exist;
  });
});

describe('<elohim-imagodei-steward-configure-banner> — capabilityContract manifest', () => {
