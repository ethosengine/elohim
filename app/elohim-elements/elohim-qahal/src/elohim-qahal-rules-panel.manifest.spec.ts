...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-rules-panel');
  if (!found) {
    throw new Error(
      'elohim-qahal-rules-panel declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-rules-panel custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-rules-panel');
    expect(decl.name).to.equal('ElohimQahalRulesPanel');
  });

  it('declares the rubric property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'rubric');
    expect(prop).to.exist;
  });
});

describe('<elohim-qahal-rules-panel> — capabilityContract manifest', () => {
