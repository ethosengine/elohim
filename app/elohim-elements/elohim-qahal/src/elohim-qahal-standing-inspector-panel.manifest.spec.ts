...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-standing-inspector-panel');
  if (!found) {
    throw new Error(
      'elohim-qahal-standing-inspector-panel declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-standing-inspector-panel custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-standing-inspector-panel');
    expect(decl.name).to.equal('ElohimQahalStandingInspectorPanel');
  });

  it('declares the standing property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'standing');
    expect(prop).to.exist;
  });
});

describe('<elohim-qahal-standing-inspector-panel> — capabilityContract manifest', () => {
