...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-standing-ring');
  if (!found) {
    throw new Error(
      'elohim-qahal-standing-ring declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-standing-ring custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-standing-ring');
    expect(decl.name).to.equal('ElohimQahalStandingRing');
  });

  it('declares the bloomTier property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'bloomTier');
    expect(prop).to.exist;
  });
});

describe('<elohim-qahal-standing-ring> — capabilityContract manifest', () => {
