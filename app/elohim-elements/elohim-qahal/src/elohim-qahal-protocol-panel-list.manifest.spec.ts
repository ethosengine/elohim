...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-protocol-panel-list');
  if (!found) {
    throw new Error(
      'elohim-qahal-protocol-panel-list declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-protocol-panel-list custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-protocol-panel-list');
    expect(decl.name).to.equal('ElohimQahalProtocolPanelList');
  });
...
describe('<elohim-qahal-protocol-panel-list> — capabilityContract manifest', () => {
