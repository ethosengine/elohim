...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-sidebar');
  if (!found) {
    throw new Error(
      'elohim-qahal-sidebar declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-sidebar custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-sidebar');
    expect(decl.name).to.equal('ElohimQahalSidebar');
  });

  it('declares the qahalName property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'qahalName');
    expect(prop).to.exist;
  });
});

describe('<elohim-qahal-sidebar> — capabilityContract manifest', () => {
