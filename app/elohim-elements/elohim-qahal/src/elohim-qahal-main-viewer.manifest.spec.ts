...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-main-viewer');
  if (!found) {
    throw new Error(
      'elohim-qahal-main-viewer declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-main-viewer custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-main-viewer');
    expect(decl.name).to.equal('ElohimQahalMainViewer');
  });

  it('declares the activePanelName property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'activePanelName');
    expect(prop).to.exist;
  });
});

describe('<elohim-qahal-main-viewer> — capabilityContract manifest', () => {
