...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-co-steward-panel');
  if (!found) {
    throw new Error(
      'elohim-qahal-co-steward-panel declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-co-steward-panel custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-co-steward-panel');
    expect(decl.name).to.equal('ElohimQahalCoStewardPanel');
  });

  it('declares the primaryObservation property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'primaryObservation');
    expect(prop).to.exist;
  });

  it('declares the pendingAcknowledgments property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'pendingAcknowledgments');
    expect(prop).to.exist;
  });
});

describe('<elohim-qahal-co-steward-panel> — capabilityContract manifest', () => {
