...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-attestations-panel');
  if (!found) {
    throw new Error(
      'elohim-qahal-attestations-panel declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-attestations-panel custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-attestations-panel');
    expect(decl.name).to.equal('ElohimQahalAttestationsPanel');
  });

  it('declares the attestations property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'attestations');
    expect(prop).to.exist;
  });
});

describe('<elohim-qahal-attestations-panel> — capabilityContract manifest', () => {
