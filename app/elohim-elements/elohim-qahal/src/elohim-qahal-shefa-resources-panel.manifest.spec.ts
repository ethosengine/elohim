...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-shefa-resources-panel');
  if (!found) {
    throw new Error(
      'elohim-qahal-shefa-resources-panel declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-shefa-resources-panel custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-shefa-resources-panel');
    expect(decl.name).to.equal('ElohimQahalShefaResourcesPanel');
  });

  it('declares the shefa property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'shefa');
    expect(prop).to.exist;
  });
});

describe('<elohim-qahal-shefa-resources-panel> — capabilityContract manifest', () => {
