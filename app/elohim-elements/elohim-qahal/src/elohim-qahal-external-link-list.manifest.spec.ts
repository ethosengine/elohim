...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-external-link-list');
  if (!found) {
    throw new Error(
      'elohim-qahal-external-link-list declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-external-link-list custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-external-link-list');
    expect(decl.name).to.equal('ElohimQahalExternalLinkList');
  });

  it('declares the links property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'links');
    expect(prop).to.exist;
  });

  it('declares the visibilityMap property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'visibilityMap');
    expect(prop).to.exist;
  });

  it('declares the viewerTier property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'viewerTier');
    expect(prop).to.exist;
  });

  it('declares the offline property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'offline');
    expect(prop).to.exist;
  });
});

describe('<elohim-qahal-external-link-list> — capabilityContract manifest', () => {
