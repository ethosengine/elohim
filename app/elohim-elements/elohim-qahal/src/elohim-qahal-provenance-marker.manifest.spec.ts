...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-provenance-marker');
  if (!found) {
    throw new Error(
      'elohim-qahal-provenance-marker declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-provenance-marker custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-provenance-marker');
    expect(decl.name).to.equal('ElohimQahalProvenanceMarker');
  });

  it('declares the category property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'category');
...
describe('<elohim-qahal-provenance-marker> — capabilityContract manifest', () => {
  ...
  it('claims contentCertainty=not-observed', () => {
