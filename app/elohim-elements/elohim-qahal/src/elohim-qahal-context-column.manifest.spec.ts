...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-context-column');
  if (!found) {
    throw new Error(
      'elohim-qahal-context-column declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-context-column custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-context-column');
    expect(decl.name).to.equal('ElohimQahalContextColumn');
  });
});

describe('<elohim-qahal-context-column> — capabilityContract manifest', () => {
