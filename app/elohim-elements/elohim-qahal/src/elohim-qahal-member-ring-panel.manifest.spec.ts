...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-member-ring-panel');
  if (!found) {
    throw new Error(
      'elohim-qahal-member-ring-panel declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-member-ring-panel custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-member-ring-panel');
    expect(decl.name).to.equal('ElohimQahalMemberRingPanel');
  });

  it('declares the reach property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'reach');
    expect(prop).to.exist;
  });

  it('declares the tiers property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'tiers');
    expect(prop).to.exist;
  });
});

describe('<elohim-qahal-member-ring-panel> — capabilityContract manifest', () => {
