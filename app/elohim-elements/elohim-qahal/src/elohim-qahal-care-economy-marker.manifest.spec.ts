...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-care-economy-marker');
  if (!found) {
    throw new Error(
      'elohim-qahal-care-economy-marker declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-care-economy-marker custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-care-economy-marker');
    expect(decl.name).to.equal('ElohimQahalCareEconomyMarker');
  });

  it('declares the kind property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'kind');
    expect(prop).to.exist;
  });

  it('declares the tokens property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'tokens');
    expect(prop).to.exist;
  });
});

describe('<elohim-qahal-care-economy-marker> — capabilityContract manifest', () => {
