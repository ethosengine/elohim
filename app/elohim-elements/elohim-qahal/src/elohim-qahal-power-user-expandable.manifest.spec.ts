...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-power-user-expandable');
  if (!found) {
    throw new Error(
      'elohim-qahal-power-user-expandable declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-power-user-expandable custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-power-user-expandable');
    expect(decl.name).to.equal('ElohimQahalPowerUserExpandable');
  });

  it('declares the panels property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'panels');
    expect(prop).to.exist;
  });

  it('declares the powerUserEnabled property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'powerUserEnabled');
    expect(prop).to.exist;
  });
});

describe('<elohim-qahal-power-user-expandable> — capabilityContract manifest', () => {
