...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-curated-epr-list');
  if (!found) {
    throw new Error(
      'elohim-qahal-curated-epr-list declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-curated-epr-list custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-curated-epr-list');
    expect(decl.name).to.equal('ElohimQahalCuratedEprList');
  });

  it('declares the eprs property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'eprs');
    expect(prop).to.exist;
  });
});

describe('<elohim-qahal-curated-epr-list> — capabilityContract manifest', () => {
