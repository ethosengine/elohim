...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-graph-discovery-panel');
  if (!found) {
    throw new Error(
      'elohim-qahal-graph-discovery-panel declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-graph-discovery-panel custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-graph-discovery-panel');
    expect(decl.name).to.equal('ElohimQahalGraphDiscoveryPanel');
  });

  it('declares the suggestions property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'suggestions');
    expect(prop).to.exist;
  });
});

describe('<elohim-qahal-graph-discovery-panel> — capabilityContract manifest', () => {
