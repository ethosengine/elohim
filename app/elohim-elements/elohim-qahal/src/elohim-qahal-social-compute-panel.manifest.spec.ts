...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-social-compute-panel');
  if (!found) {
    throw new Error(
      'elohim-qahal-social-compute-panel declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-social-compute-panel custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-social-compute-panel');
    expect(decl.name).to.equal('ElohimQahalSocialComputePanel');
  });

  it('declares the topology property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'topology');
    expect(prop).to.exist;
  });
});

describe('<elohim-qahal-social-compute-panel> — capabilityContract manifest', () => {
