...
before(async () => {
  ...
  const found = manifest.modules
    ...
    .find(d => d.tagName === 'elohim-qahal-stream-panel');
  if (!found) {
    throw new Error(
      'elohim-qahal-stream-panel declaration not found in custom-elements.json. ' +
...
describe('elohim-qahal-stream-panel custom-elements-manifest', () => {
  it('declares the tag', () => {
    expect(decl.tagName).to.equal('elohim-qahal-stream-panel');
    expect(decl.name).to.equal('ElohimQahalStreamPanel');
  });

  it('declares the events property', () => {
    const prop = decl.members?.find(m => m.kind === 'field' && m.name === 'events');
    expect(prop).to.exist;
  });
});

describe('<elohim-qahal-stream-panel> — capabilityContract manifest', () => {
