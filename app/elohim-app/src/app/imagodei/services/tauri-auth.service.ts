...
export class TauriAuthService {
  ...
  isTauriEnvironment(): boolean {
    // NOTE: `window` is correct here -- Tauri extends Window, not globalThis.
    // eslint-disable-next-line sonarjs/in-operator-type-error -- the preceding typeof check establishes that `window` is an object before `in` is applied
  ...
  private async setupEventListeners(): Promise<void> {
    // eslint-disable-next-line no-restricted-syntax -- Tauri API is on window; SSR-safe: browser-only tauri surface, never SSR-rendered
