...
description: "Plant ONE runtime-authored skill in its elohim-native package on the eprfs layer (the package becomes the authoritative source-of-truth; .claude/.codex become generated, content-addressed, provenance-recorded projections that grow from and trace back to it). The skill member of the plant-eprfs family; fidelity-gate-guaranteed lossless. Use when \"plant skill X\", \"root this skill in its package\", or flipping a .claude/skills/<X>/SKILL.md to package authority — the artifact is a SKILL.md whose name/description frontmatter is generated from package metadata. Siblings for other artifact types: plant-eprfs-agent (subagents), plant-eprfs-agentdoc (CLAUDE.md / AGENTS.md), plant-eprfs-command (slash commands), plant-eprfs-hook (executable hooks)."
metadata:
  runtime: codex
...
4. **Confirm what landed.** The projector GENERATED `X`'s `.claude`/`.codex` frontmatter (carrying `master: package` + `sourceRuntime` + the governance eprRef) instead of passing the old human frontmatter through; body is the package body. Green means `X` takes the package-first path (not re-imported), `project(package) === .claude/.codex` freshness holds, and `X`'s compose-graph node shows `master: "package"`, `packageCid` <-> projection CIDs, and `composedBy` (attribution comes from the `metadata.composedBy` set in step 2 — `replant.mjs` passes no `--composed-by`). Underlying mechanism it runs for you: `... project --write-fixtures --only X`, `... project --write-runtime --only X`, `... verify`, `eprfs-agent compose-graph .epr-meta/elohim/packages --projections-root .epr-meta/elohim/projections`.
