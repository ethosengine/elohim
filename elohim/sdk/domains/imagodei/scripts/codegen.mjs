#!/usr/bin/env node
/**
 * Imagodei codegen — generates TypeScript types from the imagodei domain manifest.
 *
 * Reads:
 *   - Imagodei manifest (elohim/sdk/domains/imagodei/manifest.json)
 *   - Imagodei companion schemas (elohim/sdk/domains/imagodei/schemas/*.schema.json)
 *
 * Produces (to app/elohim-app/src/app/imagodei/generated/):
...
const OUTPUT_DIRS = [
  resolve(REPO_ROOT, 'app/elohim-app/src/app/imagodei/generated'),
];

const DOMAIN_HEADER = `// AUTO-GENERATED from imagodei manifest + companion schemas.
...
`;
  ...
  if (VERIFY) {
    ...
    for (const dir of OUTPUT_DIRS) {
      for (const [filename, fileContent] of allFiles) {
        ...
        if (existing !== fileContent) {
          console.error(
            `FAIL: ${relative(REPO_ROOT, outPath)} is stale. Run: pnpm run imagodei:codegen`,
          );
          hasFailure = true;
        }
      }
    ...
    console.log('Imagodei codegen is up to date.');
    return;
  }
  ...
  console.log(
    `\nImagodei codegen complete: ${allFiles.size} files × ${OUTPUT_DIRS.length} locations`,
  );
}
