 ...
 */
function generateObservationKindTypes(manifest, prefix, relManifestPath) {
  const observationKinds = manifest.observation_kinds || [];
  const signalKinds = manifest.signalKinds || {};

  const blocks = [];

  // Shared interface — derived from observation-kind.schema.json structure
...
export interface ObservationKindDeclaration {
  ...
  // SignalKindDeclaration interface — derived from app-manifest.schema.json signalKinds shape
  ...
  // Domain observation_kinds const array
...
function generateManifestTypes(manifest) {
  ...
  blocks.push(generateObservationKindTypes(manifest, prefix, relManifestPath));
