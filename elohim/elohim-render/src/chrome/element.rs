...
//!
//! The element bytes/hash/path helpers (and the `omni-element.js` source),
//! plus the [`ChromeContext`] producer struct, now live in the **light**
//! `elohim-chrome-asset` crate (deps: `sha2`, `serde`, `serde_json` — no V8)
//! — so serving the static element does NOT require compiling V8.
...
mod tests {
    ...
    fn reexports_resolve_to_the_light_crate() {
        ...
        assert_eq!(
            element_script_path(),
            elohim_chrome_asset::element_script_path()
        );
        ...
        // ChromeContext is reachable through elohim-render too (the doorway
        // producer path); the minimal-shape contract holds through the shim.
