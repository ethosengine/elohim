...
//! Angular to materialize the hydratable `<app-root>` markup, not a full
...
//! bundle `<script>` tags, but its `<app-root>` is an empty placeholder (no
...
//! the rendered `<app-root>` (+ its `ngh` attributes, inner content, and
...
/// Compose a V8-rendered SSR document with the bundle-carrying browser shell.
///
/// The rendered doc carries the hydratable `<app-root>` (with `ngh`
...
/// client bundle `<script>` tags, but an EMPTY `<app-root></app-root>`
/// placeholder. This splices the rendered app-root chunk into the shell in
/// place of that placeholder, so the served document has BOTH the
/// server-rendered content AND the client bundles → it hydrates. Returns
/// `None` if either input lacks an `<app-root>` element (caller then serves
/// the un-composed render — never hard-fails).
#[must_use]
pub fn compose_ssr_with_shell(rendered_html: &str, shell_html: &str) -> Option<String> {
    let chunk = extract_rendered_chunk(rendered_html)?;
    let (shell_app_start, shell_app_end) = find_app_root_element(shell_html)?;

    let mut out =
        String::with_capacity(shell_html.len() - (shell_app_end - shell_app_start) + chunk.len());
    out.push_str(&shell_html[..shell_app_start]);
    out.push_str(chunk);
    out.push_str(&shell_html[shell_app_end..]);
    Some(out)
}

/// Locate + extract the transplant chunk from a V8-rendered SSR document: the
/// `<app-root>…</app-root>` element (optionally preceded by an immediately
/// adjacent `<!--nghm-->` hydration marker), plus — when immediately
/// present — the trailing TransferState `<script id="ng-state">…</script>`.
///
/// Returns `None` if `rendered_html` has no `<app-root` element at all, or if
/// the `<app-root` it does have is never closed with a `</app-root>`.
fn extract_rendered_chunk(rendered_html: &str) -> Option<&str> {
    let (app_start, app_close_end) = find_app_root_element(rendered_html)?;

    // (b) Optionally extend chunk_start LEFT over an immediately-preceding
    // `<!--nghm-->` marker (only whitespace allowed between it and `<app-root`).
    let chunk_start = extend_left_over_nghm_marker(rendered_html, app_start);

    // (d) After the closing tag, skip whitespace + HTML comments; if a
    // `<script id="ng-state"…>` immediately follows, extend the chunk through
    // its `</script>`.
    let chunk_end = extend_right_over_ng_state_script(rendered_html, app_close_end);

    Some(&rendered_html[chunk_start..chunk_end])
}

/// Byte range `(start, end)` of the FIRST `<app-root` element in `html`:
/// `start` is the index of the literal `<app-root` open, `end` is the index
/// just past its matching `</app-root>` close tag. Both needles are matched
/// case-sensitively lowercase — Angular always emits lowercase element names.
///
/// Returns `None` if no `<app-root` is present, or if one is present but never
/// closed.
fn find_app_root_element(html: &str) -> Option<(usize, usize)> {
    const OPEN: &str = "<app-root";
    const CLOSE: &str = "</app-root>";
...
<body><app-root></app-root></body></html>";
    ...
    #[test]
    fn returns_none_when_rendered_lacks_app_root() {
        let rendered = "<!DOCTYPE html><html><body><h1>no app root here</h1></body></html>";
        assert_eq!(compose_ssr_with_shell(rendered, SHELL), None);
    }

    #[test]
    fn returns_none_when_shell_lacks_app_root() {
