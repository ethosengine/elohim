...
---
DELTA 2026-09-03 (holochain 0.7 F2 re-check, household mesh + sweettest, status UNCHANGED — no fleet run id): the sweettest `rea_commitment_replication::project_epr_commitment_replicates_to_peer_b` PASSES on holochain 0.7.0 with the shared-rendezvous harness (Lane B, 186.9 s, branch upgrade/holochain-0.7 — the 0.7 line homes each conductor to its own relay, so isolated pairs must share ONE SweetLocalRendezvous or they never connect). The 2026-08-09 attribution ("cold-cell wasm warm-up eats the 60 s budget") did not carry the line change; treat that DELTA as superseded evidence, not as the standing explanation. Fleet-side this habit is re-measured only after the F5 cutover (connections>0 across BOTH relays is the new precondition — a 0.7 conductor against a wrong or missing relay is a booted conductor with 0 connections, silently). Guide: genesis/docs/superpowers/plans/2026-09-02-holochain-0-7-upgrade-guide.md §F2/§E10.
