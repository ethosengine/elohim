...
Multi-DNA Holochain application for decentralized learning, identity, and infrastructure.

## Architecture

The Elohim hApp consists of three DNAs:

1. **Lamad DNA** (`lamad`) - Content & Learning
   - ContentNode entries for learning materials
   - LearningPath orchestration
   - Progress tracking and mastery

2. **Imago Dei DNA** (`imagodei`) - Identity & Relationships
   - Human/Agent profiles
   - Social graph (HumanRelationship)
   - Attestations and credentials
   - Content mastery (Bloom's taxonomy)

3. **Infrastructure DNA** (`infrastructure`) - Doorway Federation
   - DoorwayRegistration for node operators
