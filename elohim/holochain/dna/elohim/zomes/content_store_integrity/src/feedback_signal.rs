...
mod tests {
    ...
    #[test]
    fn accepts_squelch_advisory_with_evidence_cid() {
        // evidence_cid is optional for squelch; squelch + advisory + evidence = valid
        assert!(
            make_signal("squelch", "advisory", Some("bafyreiabcdef_optional"))
                .validate()
                .is_ok()
        );
    }
