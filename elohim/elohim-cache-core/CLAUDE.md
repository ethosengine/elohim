...
`app/elohim-library/projects/elohim-service/src/cache/` contains pure TypeScript implementations that mirror the WASM API. Browser clients try WASM first, fall back to TS transparently. The TS types are in `cache/types.ts`. WASM is loaded from `/wasm/elohim-cache-core/elohim_cache_core.js`.
