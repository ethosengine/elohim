...
//!
//! The head is the gossip-friendly metadata envelope that travels ahead of the EPR
//! atom this crate encodes; it folds into `elohim-epr` from `elohim-storage`'s
//! `epr_codec` so the protocol's dag-cbor CID construction has a single home.
//!
...
///
/// Defined once for the crate in [`crate::cid`], which builds every CIDv1 the
/// substrate addresses with; re-exported here so `head::DAG_CBOR_CODEC` resolves.
pub use crate::cid::DAG_CBOR_CODEC;
...
mod tests {
    ...
    /// Golden: the head codec's address is pinned to an exact CID and an exact
    /// dag-cbor byte length, over the `sample_head()` fixture that predates this
    /// module's home. An EPR Head is content-addressed, so any change to field
    /// order, serde attributes, or the dag-cbor encoder re-addresses every head
    /// ever published — this test is what makes that change loud rather than silent.
    ///
    /// The head's CID must also be exactly what [`crate::cid::compute_cid`]
    /// derives from the same bytes: one dag-cbor CID construction for the crate.
    ///
    /// Both expected values were confirmed against an independent dag-cbor
    /// encoder written from the struct definition and its serde attributes
    /// (RFC 8949 §4.2.1 map-key ordering, float64, `skip_serializing_if`
    /// honoured) — not captured from this module's own output.
    #[test]
    fn golden_epr_head_address_is_pinned() {
        let (bytes, cid) = encode_epr_head(&sample_head()).unwrap();

        assert_eq!(
            bytes.len(),
            442,
            "dag-cbor byte length moved for the sample head"
        );
        assert_eq!(
            cid.to_string(),
            "bafyreifnahxlgzodfqcjfrkc3ipiaqka3w67i2duf3nzlmc5gzjn3ylfxa",
            "EPR Head CID moved for the sample head"
        );

        // The crate has one dag-cbor CID construction, not two.
        assert_eq!(cid, crate::cid::compute_cid(&bytes));
    }
}
