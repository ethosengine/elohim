...
DELTA 2026-09-06 (accountable correction, slice 1 — LOCAL ONLY; NO status change and no mesh
  ...
  WHAT IS PROVEN. `just gate sweettest-check` EXIT=0, `just gate schema-dna` EXIT=0,
  `just gate manifest-hygiene` EXIT=0; 96 native zome unit tests pass including 11 new ones;
  the storage lib+test suite passes locally.
  WHAT IS NOT PROVEN — the honest half. NOTHING here has run on the household mesh. The eight
  stations of @concern:accountable-correction have never executed; the new sweettests are
  written but not run (they need a packed DNA from the pipeline); no peer has ever discovered,
