...
checks:
  ...
  - "a2o @concern:accountable-correction (genesis/a2o/features/dataplane/accountable-correction.feature — RED-FIRST 2026-09-06: a correction filed by one household peer against another's record converges to the SAME verdict on every peer, and only the record's root author can accept it or publish its successor. Contract: genesis/docs/superpowers/specs/2026-09-06-accountable-correction-contract.md; the coordinator + storage surface it binds to is documented in features/dataplane/README-accountable-correction.md. Born red: the eight stations have never run on the mesh)"
guard: >
...
- 2026-09-05 06:08Z — RE-MEASURED GREEN after both blind readers' findings were folded into the story: 15/15 steps,
  exit 0, 1m18s, converged in 30s (receipt
  `genesis/a2o/reports/dataplane/2026-09-05/carried-election-organic-receipt-2026-09-05T06-06-22-481Z.json`;
  ledger 5/5, declarationCallsDuringCure 0). The extra step is a reader finding turned into an ASSERTION rather than a
  comment: the scenario stages on a page this run authored and then reads the REAL landing page, and that scope change
  is now guarded by `EPR "elohim-host-landing" was never staged by this run`, which fails if any staging-ledger entry
...
failures on rows that are not DHT-anchored (bulk-seed anchor gap) — same family, fleet lane.
  ...
  VERIFIED LIVE 2026-09-06 ~06:5xZ (edge #1434 deployed 0e9ac6c25): `GET /db/p2p/conductor-diagnostics` on BOTH doorways
  now returns `transportStats` with `transport_stats` + `blocked_message_counts` (string-keyed DnaHash projection,
  ee3b77975) — the probe is no longer dim. adam's wedge unchanged across the second restart (9 dead / stuck);
  validation 6/84/2 again with the same two rows. Edge #1434's storage test layer failed to COMPILE
...
DELTA 2026-09-06 (accountable correction, slice 1 — LOCAL ONLY; NO status change and no mesh
  claim). Landed on sprint/accountable-correction and verified on this desk, nowhere else.
  WHAT LANDED. Coordinator (elohim DNA `content_store`, COORDINATOR-ONLY — lamad.dna packs to
  uhC0k0UOVlaaZ-2pKSHmgaM30CS26IC3jdiShEMaXJy5iT5xYAoNK before AND after, while the bundle
  sha256 and the coordinator wasm size both move, so this heals by `update_coordinators`
  hot-swap and never re-keys an agent): `get_content_lineage` (verified EXACT root Create,
  per-candidate fetch outcomes, deduplicated links, observed-fork marker), `amend_content`
  (root-author-gated Update against an EXPLICIT predecessor, refusing when a cross-root
  canonical head means it could never be served), `get_feedback_signal_record` plus
  reference+outcome discovery queries, and correction ADMISSION binding the act to an immutable
  request embedded in a public Correction EPR. The read resolver and the declare path now share
  ONE version-DAG total order with an action-hash tiebreak — the previous bare
  `max_by_key(timestamp)` was enumeration-order dependent, so two peers holding the same two
  versions could serve different heads. Storage: four migrations (application keyed by the
  OPERATION GROUP, the local-intent outbox, the durable subscription set with a fairness cursor,
  and per-evaluator generations), `services/feedback_projector.rs` as the single application
  path, the §7 cutover that STOPS the pre-contract immediate debit at api/epr.rs, an additive
  `actRef` on the p2p feedback message with old<->new decode pinned, and
  `POST /api/v1/feedback/operations`.
  WHAT IS PROVEN (re-verified 2026-09-06 22:5xZ–23:4xZ after the box reboot, logs under
  genesis/a2o/reports/accountable-correction/). `just gate elohim-storage`: fmt + clippy -D warnings
  clean, lib tests 3618 passed / 1 failed — the one failure is
  `release_manifest_mirror_agrees_with_the_rakia_schema` reading `../rakia/...` with the
  `elohim/rakia` submodule absent in the worktree; with the submodule initialised it passes
  (3619/3619). `just gate sweettest-check` EXIT=0 (compile-only). The feedback sweettests were
  RUN, not just written: `cargo test --test feedback_signal -- --include-ignored` 14 passed /
  0 failed in 1805 s, including `content_lineage_names_exact_root_and_author`,
  `amend_content_by_non_root_author_rejected`, `feedback_signal_refs_report_references_and_resolve_on_request`,
  `correction_with{out,_mismatched}_embedded_request_rejected`. 19 projector unit tests + 27
  outbox/wire tests pass (fair rotation, late arrival, crash-window rollback, replay no-op, group
  dedup, non-root acceptance refused, unfetchable pending, rebuild == published generation).
  DNA HASH: integrity sources, dna.yaml and Cargo.lock are byte-identical to dev (git), which is
  the real proof of a coordinator-only change; the local `hc dna hash` numbers above and in the
  main tree differ from each other because Cargo's per-crate metadata includes the package PATH
  (worktree vs /projects/elohim) — see memory project_dna_hash_depends_on_build_path and the
  observation note on dna/Jenkinsfile. Never compare hashes across trees.
  WHAT IS NOT PROVEN — the honest half. NOTHING here has run on the household mesh. The eight
  stations of @concern:accountable-correction have never executed; no peer has ever discovered,
  verified and applied a correction authored by another peer. The projector's tick loop is
  written and unit-tested against a fake DHT reader but is NOT yet spawned from main — nothing
  drives it in a running node, so on a live peer today a correction is enqueued and never
...
DELTA 2026-09-07 (accountable correction live-loop follow-up; NO status change): mesh stations 1–8 NOT RUN, none passed or failed assertions. Receipt: genesis/a2o/reports/accountable-correction/mesh-20260907000336Z/start.log. After a clean stop, start EXIT=1 before any peer launched: missing binary /projects/.cargo-target-pool/family/dev/doorway__doorway-service/dev/debug/doorway. Stopped at this environment boundary as instructed; no prologue, no live Cucumber JSON/HTML, no @wip removed. Storage loop/reader, rebuild trigger and one-shot crash hook added; 45 of 53 step definitions bound, eight explicitly pending for delayed older-action discovery, notification timing/injection, and dependent-view rendering. Registration-only receipt: genesis/a2o/reports/accountable-correction/bindings-dry-run.json (8 scenarios / 91 steps skipped, zero undefined); this is not mesh proof. Final mesh stop EXIT=0.

DELTA 2026-09-07 (accountable correction ON THE HOUSEHOLD MESH; NO status change — the habit stays red). The stations RAN. Receipt: genesis/a2o/reports/accountable-correction/mesh-20260907T025002Z/{cucumber.json,cucumber.html,run.log}, cucumber EXIT=1. 8 scenarios / 91 steps: 3 PASSED, 3 PENDING, 2 FAILED, 69 steps passed. PASSED — station 4 (§7, crash window: storage killed mid-transaction, restarted, reapplied exactly once, replay of a settled correction changed nothing), station 6 (§6, contested fork: two updates on one predecessor mark the record contested, the deterministic pick serves a head without clearing the marker, a sequential amendment is not contested), station 8 (§8, lost response: a discarded response plus a retry under the same client-minted operation id resolves to one correction and at most one contribution). @wip removed from those three only. PENDING (each after its earlier steps passed) — station 1 at 13 steps: Matthew's UNNOTIFIED peer does discover and apply the correction on its own durable scan, which is §3's whole claim; it stops at the late-arrival fixture, because a newly authored Holochain action cannot be backdated. Station 2 at 12 steps: no notification-driven wake or observable next-scan deadline exists to measure acceleration against. Station 3 at 16 steps: acceptance, successor, three-peer head adoption and James's refusal all pass; it stops at "a dependent view re-renders", which names no browser view or route. FAILED — station 5 (§7) is an honest refusal by its own guard, not a defect: it asserts Jessica reads Unknown and by the time it runs she carries standing from earlier stations, so it needs per-scenario evaluator isolation. Station 7 (§7) rebuilt and published a fresh generation, but this scenario's own acceptance had not reached its tally inside the poll (expected 8, observed 6) — rebuild-convergence budget versus a real lag between republication and retained-member replay is NOT yet separated, and that is the next thing to measure. WHAT THE PROJECTION ITSELF SHOWED, read directly on jessica's peer mid-run: 17 application rows all 'applied' — three with accepted=1/contribution=2 and fourteen unaccepted allegations at contribution=0 — with standing_generation_aggregate.debit_weight_sum exactly 6. That is §7's rule (an allegation debits nobody; only an accepted correction contributes, once) observed rather than argued, and it supersedes the prior DELTA's "nothing has run on the household mesh". Environment: storage built from this branch with --features "p2p p2p-iroh" (the mesh defaults to dual transport and refuses a binary without the iroh marker), conductor+CLI pinned to the holochain 0.7.0 fork pair (stock on PATH is 0.6.0 and cannot load an hdk-0.7 DNA), lamad.dna repacked from this branch, the four role-filler DNAs copied unchanged, iroh-relay 1.0.3, ELOHIM_FEEDBACK_SWEEP_SECONDS=5, ELOHIM_TEST_FEEDBACK_CRASH_ONCE=1. Five harness defects were fixed to get here, each of which had been producing a green no-op or a false red rather than a measurement: A2O_RUN_WIP never reached cucumber's scenario filter (the profile's 'not @wip' ANDs with any CLI --tags, so a scoped @wip run reported "0 scenarios" and EXIT=0); peerEnv parsed the mesh pidfile as a bare pid when hc-mesh.sh writes "<pid> <start-ticks>"; hc-mesh.sh could not restart a peer that had already DIED, which is exactly what station 4's crash test produces; the standing baseline was sampled on its first read mid-projection; and the contribution assertion sampled an eventually-consistent tally instead of polling it. Still NOT proven: stations 5 and 7; the late-arrival, acceleration, foreign-DNA-injection and dependent-view fixtures named pending above; anything on the fleet.

DELTA 2026-09-07 (accountable correction, slice 1 close-out; NO status change — the habit stays red). Five mesh rounds, three completed, receipts under genesis/a2o/reports/accountable-correction/{mesh-20260907T045232Z,mesh-20260907T051633Z,mesh-20260907T055414Z}/run.log (+ sprint-report.json) and one aborted partial at mesh-20260907T082426Z-aborted/. Best completed round is 045232Z: 8 scenarios / 91 steps — 4 PASSED (stations 5, 6, 7, 8), 3 PENDING (1, 2, 3), 1 FAILED (station 4), 71 steps passed, 20m41s. Round 051633Z: passed 6, 7, 8; failed 4 and 5. Round 055414Z: passed 4, 5, 6; failed 7 and 8 (88m). Every one of stations 4-8 has now passed on the household mesh, but never all five in one round, so NO @wip was removed this pass — a station's tag flips on a round that carries it, not on a union across rounds. STATION 7 IS A HARNESS BUDGET, NOT A SUBSTRATE DEFECT. Measured with the poll ceiling raised to 600 s so the run reported lag instead of a deadline: the REBUILD itself requested-to-published in 59,974 ms — one sweep — and the fresh generation's canonical rows then matched the live generation's exactly, with zero pending members in the rebuilt generation. What the old 210 s budget was actually waiting on was the scenario's OWN acceptance traversing discovery: 212,471 ms to the application row and 272,528 ms to the republished tally. The budget expired two seconds before the row appeared; "8 expected, 6 observed" was never a wrong tally. THE LAG IS A ROTATION AND THE ROTATION GROWS. The projector visits MAX_MEMBERS_PER_SWEEP=8 subscription members per SWEEP_INTERVAL_SECS=60 tick and publish_generation only republishes on a tick ending with nothing unvisited and nothing pending, so an act is applied within ceil(N/8) sweeps plus a clean sweep. Measured acceptance-to-application-row across the day as N grew: 0.04 s (already-settled replay), 106.6 s, 212.5 s, 252.6 s at N≈14-30, then 332.8 s at N=44 — exactly ceil(44/8)=6 sweeps. N is every content record and every discovered correction the peer has ever seen: 14 members at the first round of the day, 44 by the third, 127 by the fifth. At the 60 s product default a single act's convergence passed 25 minutes by the fourth round and the derived budget exceeded the step timeout wrapping it. This lane therefore pins ELOHIM_FEEDBACK_SWEEP_SECONDS=5 and derives its budgets as DHT-propagation floor (300 s, measured 2-3 min, which no sweep setting accelerates) plus 2x(ceil(N/8)+2) sweeps read from the peer's own live environment. That growth is a real substrate property worth naming: feedback discovery on a long-lived peer is O(N) in everything it has ever subscribed to, with no cursor. STATION 5 IS AN HONEST ASSERTION CHANGE, NOT AN ISOLATION FIX. Its "Jessica reads Unknown, no row at all" cannot be made true mid-run by a fresh generation: a GENERATION is (evaluator, pinned policy), not (evaluator, scenario) — FeedbackProjector::tick replays every retained subscription member into whichever generation resolve_generation returns, so a fresh or rebuilt one lands on the same subject aggregate it started from; and a fresh evaluator IDENTITY reads Unknown whether or not the correction is ever accepted (no projector runs for a key nobody evaluates with), which would have made the acceptance half unpassable. Standing is kept per author, so Jessica accumulates across every station that corrects a record of hers. The station now asserts the generation's own arithmetic instead of a captured baseline: the unaccepted correction's group carries accepted=0/contribution=0, the served tally equals the sum of the accepted groups the generation holds, and the aggregate row count is zero exactly when that sum has no groups behind it — stronger than a delta and true in every run order. The same move removed the baseline from station 4/7/8's contribution assertion; the Background no longer waits for anything to settle. STATION 1's late arrival is now staged by PUBLICATION ORDER rather than by timestamp (a Holochain action cannot be backdated) and stays PENDING with the reason named exactly: create_feedback_signal commits the entry and BOTH index links in one extern call (content_store/src/feedback_signal.rs:168-192 — create_entry, then create_link TargetToFeedbackSignal, then create_link SignerToFeedbackSignal); there is no link-only extern, so an act's link cannot be published later than the act, and delaying it needs a coordinator extern authored for the product rather than for a fixture. Stations 2 and 3 keep their existing pending reasons (no notification-driven wake or observable next-scan deadline; no named dependent view or route). NO RUST CHANGED this pass — every fix is in genesis/a2o/steps/dataplane/accountable-correction.{steps,helpers}.ts and the feature file; the DNA hash did not move and the projector unit tests were not touched. `just mesh prologue` was NOT run: it requires built Angular dists and this worktree has no elohim-library build (elohim-core/register unresolved), and these stations author their own content and read peer HTTP, conductor calls and SQLite directly. A blind reader passed the feature READY on the second pass; its remaining named findings (station 4's double-negative atomicity step, the implementation-status header block preceding the Feature keyword) are deferred, not fixed.

DELTA 2026-09-08 (mesh ergonomics; NO status change — the habit stays red). preflight refuses in ~1 s before any launch; start returns in ~1 s detached (setsid); wait reached ready in 156 s on the 3-peer household mesh; a process-group kill of the caller leaves all peers up; T3 peer offsets to 8095/8898 beside the mesh, conductor ready in 16 s; relay auto-detected under MESH_TOOLS_DIR. Commits 87fc5c8a9 c9ed88a9d 71fd9937d 65839d9b4.

DELTA 2026-09-08 (seeder vocabulary drift closed; NO status change — the habit stays red). `just mesh prologue` on the 3-peer household mesh: all 30 MUST-SUCCEED legs EXIT=0, `WasmError Deserialize` 0 lines (seeder now carries `reach` from the corpus declaration, commit 42fc776a3), `relationship_type STEP is not valid` 0 lines (STEP declared at the lamad manifest and hand-synced into VALID_RELATIONSHIP_TYPES, commit fa4aa12c4), seed-base-corpus post-flight PASSED. Receipt: prologue-t3-run3.log (job tmp). Precondition for accountable-correction stations 1–3 and the fixtures-clone seeder leg.

DELTA 2026-09-07 (accountable correction stations 1 and 2 PASS on the household mesh; NO status change — the habit stays red until every station in the lane carries). Receipt: genesis/a2o/reports/accountable-correction/mesh-20260907T202913Z/{cucumber.json,cucumber.html,run.log,sprint-report.json}, cucumber EXIT=0, 5 scenarios / 57 steps, ALL PASSED in 4m23s: station 1 (§3 durable discovery + late arrival, 18 steps), station 2 (§4 notification accelerates + foreign-DNA refusal, 17 steps), station 4 (§7 crash window, 18), station 6 (§6 contested fork, 18), station 8 (§8 lost response, 16). Stations 3, 5 and 7 keep @wip and were not in this run, so nothing regressed and nothing new is claimed for them. TWO PRODUCT GAPS CLOSED, not two fixtures. (1) The coordinator could not separate an act from its INDEX: create_feedback_signal committed the entry and both index links in one call, so a link could never be published later than the act and §3's 'a late link is picked up on a later tick' was unexercisable. publish_feedback_signal_target_link now publishes the TargetToFeedbackSignal edge for an act that already exists — link only, target read from the act's own target_cid so it can restore an index but never forge one, idempotent — and create_feedback_signal gained an additive defaulted defer_target_link. Coordinator-only: packed from the same path before and after, DNA hash uhC0kEZTgq_RVas2rLzUsEDu17TIvJePpqfnTZWh2ZE5_GrPqdpHy unchanged and integrity wasm sha256 a9373f6b4eb79478d482be7f76226baa5d127a4e0446270ccca00d258a4ddef6 unchanged; only the coordinator wasm moved (fd51a686… → 9f98a956…). Commit a2055c21c. (2) THE NOTIFICATION PLANE CARRIED NOTHING AT ALL, and the measurement is what found it: EprAtomService::with_origin_dna_hash existed and was called from nowhere, so every peer refused every feedback-signal notification with 'no local content DNA hash bound' — station 2's 'notifications only enqueue subscriptions' pending reason was describing a plane that had never admitted a single message. Both transports now bind the hash from the hc registry they already hold (p2p/mod.rs epr_atom_service(), main.rs iroh construction), and the receive path calls T6's admit_notified_signal (epr_atom_service.rs:624) instead of its own inline add_member, so a notified act joins the durable set AND goes Hot in the live SweepScheduler. Measured on the mesh: the notified member is swept within two sweeps of admission with source='notified'. WHAT THIS COST TO LAND, which is the reusable finding: a coordinator-only change built in a WORKTREE CANNOT be hot-swapped onto a mesh installed from the main checkout. The DNA hash is path-dependent, so happ_manager::lineage_mismatch_error refuses update_coordinators across the two trees — measured verbatim against the running peer: role lamad drifted=true, error 'dnaHashMismatch … installed=uhC0keuLMYBe0sj4ZqLvIyuVqCXcxPL3PbHyiqx40UGgBh-bTbBOl, bundle=uhC0k8pow3EM3IVN9lZhCBlgnwyKrReAAXLKTEKnEFcPJQxjPZLNm', while the other four roles showed zero drift because their DNAs were copied unchanged. The guard is right (it cannot tell path drift from an integrity change) and the consequence is concrete: rung-1 coordswap is a MAIN-TREE vehicle, and a worktree needs a full mesh stop/start with MESH_HAPP_PATH pointing at its own bundle. That is what this run did (ready in 194s, fresh DHT and fresh agent keys), which is also why N was small and station 2's rotation comparison reports rather than asserts at that size. Second trap, one lost mesh cycle: the sprint pool slot was rebuilt at 20:09 WITHOUT --features "p2p p2p-iroh" and hc-mesh.sh refused to start every peer (it greps the binary for the p2p_iroh marker) — on a shared slot, the feature set is not sticky. New test surface, gated and named: POST/GET /admin/test/feedback-notify (ELOHIM_TEST_FEEDBACK_INGRESS=1, 404 otherwise) is a second ingress to the SAME EprAtomService::handle(IntegrityNotify) both transports call, so a foreign-DNA envelope can be delivered to the real receiver and its refusal observed rather than argued; a household mesh has no peer in another content space. Rust verification: cargo fmt clean, clippy -D warnings clean, 3803 lib tests pass (0 failed) including nine new ones covering the wake in both directions, the foreign-DNA refusal at both the projector ingress and the receiver, and the no-act-reference no-op. Pre-existing and NOT touched: eight DNA rustfmt diffs (six in correction.rs) that are red at HEAD, and one unused-import warning in the constitution crate. Fixed in passing because it blocked the test build: p2p_iroh/dual_publish/tests.rs had two FeedbackSignal initializers missing act_ref since that field landed — the p2p-iroh feature's test profile had not been compiled since.

DELTA 2026-09-09 (native network sprint, identity onboarding predecessor; status remains red): maintained seed-conductor-identities now registers the caller's existing Human as its own Agent EPR, refuses profile/key conflicts and verifies exact action readback. Focused 32 tests and just gate genesis (611 passed, 9 skipped) EXIT0; real owned-mesh replay at 19:57:47Z returned 3 existing matching Agent profiles, zero conflicts/failures (genesis/a2o/reports/network-sprint-20260909/custody/seed-conductor-identities-replay-final.json). This proves repeatable own onboarding, not binding projection or cold RS recovery; signed binding publication exposed a separate signal-subscription startup race.

DELTA 2026-09-09 (network custody prerequisites; status remains red): delivery eligibility now requires an active cross-signed binding and explicit unfinished content:reach provision; household lookup separates transport, Agent key and Human identity. Replication fetches prefer the connected inventory advertiser within existing budgets. Own binding mint waits for actual signal subscription and retries bounded missing-Agent onboarding. Focused readiness/retry13 passed; final just gate elohim-storage EXIT0 (3684 unit passed, 3 ignored, integrations/docs green), report genesis/a2o/reports/network-sprint-20260909/custody/owning-gate-subscription-readiness.log. Earlier immutable6d48 runtime proved direct witness delivery; subscription-order repair has local proof only. Missed-original binding recovery, timestamp lookup availability, notarized offer lifecycle and cold RS acceptance remain outstanding.

DELTA 2026-09-09 (network coordinator recovery predecessors; status remains red): observed REA lifecycle reads now follow the bounded signed Update graph and refuse foreign updates; a backward-compatible own-binding query carries original signed records for projection recovery. Baseline runtime tests failed on created-vs-active and unsupported recovery input; repaired Lamad and Imagodei Sweettests each passed, focused units 5+1 passed, and schema-dna/manifest-hygiene/sweettest-check gates exited0. Both integrity bytes and DNA identities remain unchanged; combined hApp SHA35bd683b01798ba63b029abf71951e651492d2d151ee912d10d07d22841b09e7, receipt genesis/a2o/reports/network-sprint-20260909/coordinator-combined-artifacts.json. This proves coordinator behavior, not household projection, cold RS recovery or global trust.
DELTA 2026-09-11 (fleet probe + household re-measure; RED preserved): live 22:33Z, both doorways healthy 4 h after edge #1453 — alpha (matthew) projectionReconcile converged=true, healedTotal 25, pull/replication caughtUp; apex (adam) converged=false, pending 32, failed 2, healedTotal 0, divergentAnchor 9 — and the two doorways serve two shell heads (alpha version.json 228fe686 built 16:19Z, apex face02de built 2026-09-09): the apex never heals what it detects, the stuck provide-loop finding 2b4761b2eaf6 (backlog dataplane-reanchor-dead-remaining-rekeyed-peer) owns it, and the fix sits in the storage WIP lane. Household lane 20260911T223432Z-466536cb: federation-version-convergence RED on `GET /db/content/elohim-host-landing` 404 at alpha-A — the prologue fixture is absent on this mesh, so this is an apparatus red, not a convergence verdict (the 2026-09-05 15/15 green stands as the last honest household measure); accountable-correction 3/5 (the two reds: `/admin/test/feedback-notify` 404 `test ingress is not armed on this node`, and the projector crash-arm deadline). The federation-deploy Act II clause is measured by the `[edge:validate-only]` run dispatched from 82610e30f; its build number is appended when terminal.
DELTA 2026-09-11 (fleet validate-only measure; RED preserved): edge #1454 (617d85866, validate-only, no roll) — federation-deploy passed=2 failed=0 pending=1 on the live fleet (alpha-A and elohim.host both fully resolve the landing EPR; the third clause pending), reconcile-inventory 3/0, inventory-convergence 1/0, fleet-quiesce sustained 362 s with A-converged 0→1 during the sustain window. The deploy-uniformity clause is green on this measure; the habit stays RED on the apex's projection reconcile (converged=false, healedTotal 0 at 22:33Z) and the two served shell heads — convergence of the served head, not the landing EPR, is the open invariant.
DELTA 2026-09-12 (household re-measure; RED preserved): on the re-cast mesh (run 20260912T010759Z-a9624007, storage = origin/dev 586e78085 + coverageShortfall fix, iroh-capable) federation-version-convergence "two doorways that disagree about a page converge on the elected version without anyone re-uploading it" PASSED — the 23:34 red was the missing fixture, not convergence; reach-enforced-http 3/3 again; accountable-correction 3/5 with the same two apparatus reds (test ingress not armed on the mesh storage; projector crash-arm deadline). The Act I clause is green on this binary; the fleet clause waits on the edge roll carrying 696ff3f77, which the orchestrator has not yet picked up (its #1855 died cloning GitHub at ~12 KiB/s, `Maximum checkout retry attempts reached`).
DELTA 2026-09-12b (household measure; RED preserved): doorway-footprint-convergence "two doorways testify the same footprint for one commons EPR" RED on run 20260912T122630Z-eaf84fb2 — for elohim-host-landing doorway A (pool matthew) answers stewardingCollectives 1 / at-risk / coverageShortfall 2 / intraHubPeers 3 while doorway B (pool jessica) answers 2 / partial / coverageShortfall 1 / intraHubPeers 1, and they do not agree inside the 60 s window. The household resilience projection is computed per pool peer from that peer's own commitment view and does not converge across pools — the resilience-view sibling of the served-head clause this habit already names.
DELTA 2026-09-12c (footprint convergence GREEN on the household; RED preserved on the served head and custody standing): doorway-footprint-convergence "two doorways testify the same footprint for one commons EPR" PASSED (runs 20260912T145035Z and 20260912T153349Z) after 776a3390a (the household resilience fold groups on the DHT collective_cid, presents the anchored slug, and resolves online peers through the same cid↔slug identity — onlinePeers 3/3 where every peer read 0/0), e3bafbbb4 (a cid-keyed placeholder no longer freezes the alias merge) and the seed's one-slug household (B2). Both doorways now answer stewardingCollectives 1 / household-dowell for elohim-host-landing. Root cause recorded: the collectives reconcile arm converged the cid while the fold grouped by the per-host routing alias (classify_collective_gap now names AliasDivergent instead of silent InSync), and household formation had never completed because the seeder bound the founder to a hosted human's cell. Named, not fixed: custody `state` does not travel on the wire (rows converge, standing does not — the rea_commitments arm compares anchors only); humans.agent_pub_key has no reconcile arm (intraHubPeers 3 vs 2 across peers); a mesh cold start keeps storage projection DBs as ghosts of the previous DHT.
DELTA 2026-09-12d (fleet validate-only after the roll; RED preserved): edge #1456 (29cff957a, `[edge:validate-only]`, no roll) 25 min after #1455 rolled the fleet: fleet-quiesce PASS sustained 362 s (A/B caughtUp at the gate, A-converged 1), doorway-failover 3/0, federation-deploy 2/0, reconcile-inventory 3/0, blob-durability 1/0 held green on a second measure; the run is FAILURE only because inventory-convergence "the seed-facing doorway peer catches its projection up under sustained gossip" red on `elohim.host /health p2p.caughtUp is false` — doorway B's new pod on shem was NotReady at #1455's 300 s rollout window and was still catching up when measured (1/0 on #1455 an hour earlier, before B's own roll). Restart churn of the doorway B leg after a roll, the ≈20 min class the trust-contract runbook names; not a convergence verdict on the served head.
DELTA 2026-09-12e (correction to 2026-09-12d): edge #1457 (2b87c1a2a, validate-only) reds the same inventory-convergence clause again — `elohim.host /health p2p.caughtUp=false` — while the fleet-quiesce gate reads B-caughtUp=true. Live at 18:20Z: elohim.host pull.caughtUp=true, failed=0 (the gate's field) but projectionReconcile caughtUp=false, converged=false, divergentAnchor=3, healedTotal=0 over 17 sweeps. So this is NOT post-roll catch-up as 12d read it; it is the apex doorway's chronic projector flap canonicalized this morning (runtime fingerprint 79f357281ca5, backlog self-heal-adam-projection-catchup-exhaustion-full-arc — the same mechanism at small amplitude: the windowed divergence scan comes back small without healing anything). The gate and the scenario read different fields; the scenario is the honest one. Blocker is operator-owned (per-space gossip budget × agents per conductor); the inventory-convergence clause stays RED on it.
DELTA 2026-09-12f (rung 1 deployed; RED preserved): doorway-footprint-convergence PASSED again on storage f6ce271db (run 20260912T220217Z) — the fold holds with custody state now travelling on the wire (ReaRowGap::StateDivergent counted and re-healed via the miss ledger's anchor|state key) and a humans arm converging agent-key bindings through membership_identity_reconcile (write path unchanged, refusals published as divergent_refused). Named next: custody activation for a non-primary provider through the primary's doorway answers 503 catching-up (see blob-durability 2026-09-12e).
DELTA 2026-09-12g (clean cold reseed on the full sprint stack; RED preserved on the served head): doorway-footprint-convergence PASSED (run 20260912T224651Z) on a mesh whose household formed cleanly (founder bound by steward app, custody on provider peers); peer-loss-failover 4/4; the custody-standing, humans-arm and liveness slices all deployed together. The one fleet clause stays the apex projector flap (2026-09-12e).
DELTA 2026-09-13a (a live dataplane fault, named; RED preserved): after the 2026-09-13 00:39Z cold start the primary peer (matthew) saturates at ~200% CPU with its HTTP listener starved (health 8 s+), from 00:55Z on the pre-sprint 23:25Z build and persisting on every later build. Diagnosed causes, two coupled drums on ONE 231 MB rs-4-7 object (sha256-7e0559559…, seven 58 MB shards): (1) a custody kick was a re-decision every pass with no in-flight or cooldown gate — five commitment rows name the object (one cancelled; no state filter), the only inventory row (`unknown-peer`) went stale at 600 s so every kick became a connected-peer fallback race, and connection churn triggered passes every 5–8 s instead of the 120 s cadence — fixed by KickGate (09a3fe428: one race per hash in flight, then a cooldown of the declared cadence, RAII release); (2) the non-holders' heal-on-read re-requests all seven shards ~2/min and never completes (`missing_shards 7` for an hour) — `fetch_blob_timeout_seconds` defaults to 5 s (config.rs:1223) for 58 MB shards; raised to 60 s on the mesh peers' config.toml as a knob, which did not clear it. With the pullers SIGSTOPped matthew still burns ~200% with a quiet log, so the remaining load is internal to the provider (not serving) — next slice: profile the provider (shard serving from the iroh store with the sha256→blake3 alias per request; rs-4-7 re-encode on serve; the projection/custody sweeps' full-table scans), size the fetch deadline to the object (as c8930c2f2 did for release pulls), and filter cancelled rows out of reconcile_pass. Until then the household lanes that need matthew's HTTP (served-under-standing, name-routing) cannot be measured on this mesh.
DELTA 2026-09-13b (thread evidence for 2026-09-13a): `top -H` on the saturated provider shows the two `http-server` worker threads at 99.9% each with every other thread (iroh-blob, elohim, import) idle — the spin is inside storage's HTTP serving path itself (shard streaming or the alias lookup on GET /shard/{hash} from the iroh store), not in the p2p or reconcile tasks. That is the profile target for the next slice.
DELTA 2026-09-13c (2026-09-13a/b CURED on the household; RED preserved on the served head): the provider's spin was serialization, not I/O or a stuck loop — `ShardResponse::Data(Vec<u8>)` carried no `serde_bytes`, so rmp_serde encoded each 58 MB shard as fifty-eight million MessagePack integers (87.7 MB on the wire for a 57.9 MB shard, 350.8 MB for the whole 231.8 MB object) against a 64 MB frame ceiling: every serve read the bytes, logged `Serving shard`, burned seconds encoding and then refused the frame, so `missing_shards 7` never moved and the requesters' retries were permanent. Evidence: `top -H` two http-server threads at 99.9% on a `worker_threads(2)` runtime, voluntary context switches frozen while nonvoluntary climbed, per-thread io flat (pure userspace compute). Cure 0bf0e17f2: `serde_bytes` on Data/Push/Found (a standalone rmp-serde 1.3 probe proved decode is lenient both ways — no protocol bump), `SHARD_PAYLOAD_MAX_BYTES` + `payload_fits_frame` asked from declared size BEFORE the read, oversize whole-object asks answered with the manifest pivot, SHA-256 on `spawn_blocking`. Companion adfa340fc: the swarm race deadline sized by `manifest.shard_size` (floored by config, capped 600 s) and withdrawn commitments (cancelled/terminated/superseded) filtered out of reconcile_pass, salvage_pass and the prioritizer. Measured after `storage-restart` of all three peers at 02:41Z: matthew /health 0.3 s, the hot thread idles between serves, and jessica logs `Q4: composite reassembled from swarm-fetched shards` shards=7 elapsed_ms=30221 then `Serving reassembled blob` size=231795670 — the genesis object crosses the mesh in 30 s where it never crossed before. Two named reds close with it (fetch deadline sized to the object; cancelled rows in custody reconcile_pass).
DELTA 2026-09-13d (fleet measure after the roll; RED preserved): edge #1459 (70f195e53) rolled the shard cure, the lifecycle-ordered classifier (fed6eb101: StateDivergent only when the peer is strictly ahead; withdrawn-local vs proposed-peer is InSync) and the heal guard (493adaabb: a conductor answer that would regress a settled state is refused as `conductor_behind`, never written) to all seven pods. Prometheus at 07:42Z on the fresh pods: `state_divergent{rea}` matthew 40 → 12, adam 0 (the ahead-side halves are gone); `divergent_actionable` on storage A held 14–25 for the whole quiesce window and the stage timed out (ABORTED). Matthew's heal outcomes over 7 sweeps — healed 5, refreshed 7, missing 2, timeout_exhausted 9, ~25 gaps admitted per sweep — read as three attempts per sweep before the leg's HealCircuit opens on a timeout streak and sheds the rest: the plateau is conductor read latency on the alpha pool's conductor (load the gate is right to see), not a classifier artefact. Ceiling named for the operator: pool-conductor capacity on matthew-alpha, or the gate's window. Household on the same binaries: chaos 3/3 three times on warm meshes; after a COLD restart the household resilience fold reads `onlinePeers.live 1 / known 3` while every storage peer reports 2 connected peers (ids unchanged), so the protection rung reads at-risk on 3 custody copies — the liveness join after a cold start is a named red (2/3), not the batch. Also named: the signal-driven state write (`rea_projection::handle_rea_signal`) emits no SSE; the commitment observation contract lags its own updates (DNA design-gate item in the sprint plan).
DELTA 2026-09-13e (fleet-quiesce PASSED once on the batch; RED preserved on the served head): edge #1460 (the timer's second roll of 70f195e53) read storage A's `divergent_actionable` 41 → 3 → 0 within ten minutes of the fresh pods coming up and sustained PASS for the 330 s window; blob-durability 1/0 and reconcile-inventory 3/0 followed; the build still ABORTED on the stage's overall timeout (the quiesce wait consumed the window). The classifier and the guard are proven by the gate they were written for. The app leg (#1711, retry ladder honouring retryAfter, 8 attempts / 407 s) still cannot stage the apex: `forwarded_to_storage:false` on elohim.host while adam's own reconcile is idle (actionable 0, 8 sweeps) and adam's CONDUCTOR admission gate sheds every interactive zome class (`elohim_conductor_admission_shed_total{class="interactive"}` content_store 8, infrastructure 7, imagodei 6, node_registry_coordinator 2) — the byte forward's registration calls among them. The fleet's remaining red is pool-conductor admission capacity on the apex pairing, operator-owned; storage-side there is nothing left to hide it behind.


DELTA 2026-09-13f (Codex recovery, local proof only; RED preserved): the sequenced-roll helper now requires a newer reconciliation sweep even when its first reading says converged, caps requests/sleeps to the remaining phase budget, and holds later conductors on unexpected helper errors. The hermetic sequenced-roll suite passed three consecutive runs after its stalled fixture received a 5-second observation budget; shell syntax and the validate-only pipeline regression passed. Independent review verified that these helpers execute in the Jenkins builder image with Python available. The peer identity-handshake liveness augmentation passed the full elohim-storage gate (fmt, clippy, cargo tests and doc-tests; exit 0). A default-feature binary was refused by the dual-transport mesh preflight, leaving existing peers running; the corrected dual artifact and live cold-start proof remain pending. None of these local results declares a fleet rollout or a served-head green.

DELTA 2026-09-13g (dual artifact adopted, remaining live red): all three owned peers adopted the capability-checked dual build 16a9205b95f5fa055b8b7a5be25b478bc5da473c72d73ce3669f3d39bd8a0dc2 and recovered their zome paths at 15:18Z. Matthew still reported onlinePeers live=1/known=3 with connectedPeers=2, despite fresh handshake-sourced bindings for the current agent keys on all peers. Source review found a second loss path: a repeated ConnectionEstablished replaced augmented labels, and ConnectionClosed removed every peer cache even when num_established remained nonzero. Label union and final-connection-only cleanup are now independently reviewed in source; the next gate, dual build and live measure must verify those additional edits. The running binary does not yet contain them.

DELTA 2026-09-13h (coordinator rollout precondition discovered; RED preserved): chain coordinator preview → rolling apply / missing node: every role's preview must be admissible before any role on that peer is changed. A local read-only preview returned HTTP 200 with lamad's dnaHashMismatch in roles[].error, while fleet-coordswap.sh reported drift-detected and exit 0; independent source review found apply mode would proceed and could update another compatible role first. No coordinator apply was attempted. The driver repair now stops on per-role errors in precheck, apply and verification; scripts/ci/fleet-coordswap.test.sh passed mixed-role refusal, zero-drift error, no-later-peer and clean-success checks, including Python-only parsing. The repaired driver repeated the local read-only preview with verdict=error, the exact lamad mismatch and exit 2; no apply was attempted. CORRECTION at 16:04Z: the 217.73-second cross-peer Sweettest loaded a packed DNA from 15:18:40, before the final commitment_observation.rs edit at 15:31:57. Its pass is NOT proof of the final committed coordinator; a later raw WASM artifact was stale too. Both candidate wrappers are withdrawn from application. Seven focused source tests remain separate evidence. Final-source build, repack and cross-peer rerun must pin source commit and the exact loaded DNA/coordinator hashes before a live hot-swap; preserve the installed integrity bytes. Storage live proof will use fresh process connections and cascade/rejoin, rather than a database wipe whose unseeded household view has no useful liveness oracle.

DELTA 2026-09-13i (final-source recovery proof; RED preserved): peer lifecycle fix 5f4ff1354 passed the full storage gate and explicit dual-transport build; binary sha256 109bbb17a55444bbf8a1ea7c36acf3e524d6d91bd57e36e2515c9fcd85a4ac8f was adopted and verified on all three household peers with their original PeerIds. Each reports two connected peers, but primary household resilience remains live 1 / known 3: hosted human keys differ from current steward handshake keys, and the remaining projection join must preserve device identity without equating people or counting one shared agent heartbeat as multiple live devices. Recovered coordinator source 074a1113 (dev f5fdd6f77) was repacked from its final clean source and tested against an immutable DNA snapshot: cross-peer test 1 passed / 0 failed / 230.79s. Exact tested coordinator sha256 257fd7e30c05adc54988289682cab93161a3f2a4eec9fccf127175d199995276 was wrapped with installed integrity bytes and applied through the guarded rolling driver; all three peers report that coordinator, all five DNA hashes are unchanged, and the other four roles are unchanged. Receipts and pack/test logs: genesis/a2o/reports/recovery/coordinator-head-observation-20260913/{verification,before,apply,after}.json and logs/. The older 217.73s pass remains withdrawn as final-source proof; no chaos recovery or fleet success is claimed.

DELTA 2026-09-13j (post-swap authoritative observation; RED preserved): five new routing commitments cancelled after the coordinator update were subsequently healed from each other peer's own conductor (10 Advanced CAS events); all three storage projections agree on each final cancelled state and action hash. Evidence: genesis/a2o/reports/recovery/coordinator-head-observation-20260913/live-followup.json (sha256 e587a7a92f453a44403f7c9a12baa21ba242f4331068ae95798f41dbd8d657d5). This proves new cross-peer observations reached the lifecycle fold, not which internal head-lookup branch or budget they used. Four historical custody commitments still diverge on Matthew and James: each has four refused/known-divergent REA items despite divergent_actionable=0 and converged=1. The existing retry law admits three attempts per round, then 12 dormant sweeps before readmission; no historical backfill or full convergence is claimed.

DELTA 2026-09-13k (household liveness join and churn proven; RED preserved): the household projection now resolves existing admissible agent-peer bindings into distinct transport PeerIds and counts those against an atomic snapshot of live connection keys; unverified aliases and a shared agent heartbeat cannot light another bound device. No person equivalence or physical-host independence is inferred. Twenty focused regressions passed; the full storage gate passed 4,640 / failed 0 / ignored 59. Final dual binary sha256 42283b406f79b9e36d2e7fabf7b164a95635852b88aea79180b2018fe48469dd was verified running on all three peers with unchanged PeerIds, DNA hashes and coordinators: manifesto and chaos-ladder both changed from onlinePeers 1/3 to 3/3. The existing chaos-peer-churn feature passed 3 scenarios / 34 steps in 174.778s, proving repeated restart, the 3→2→1 availability ladder and simultaneous-loss degradation; report genesis/a2o/reports/sprint-report-household-20260913T180403Z-d7992151.json, SUT ce009fdfedbd46b9. Teardown restored the mesh and both doorway login probes return 200. Source/diff, test, binary, adoption and restoration receipts: genesis/a2o/reports/recovery/household-peer-bindings-20260913/verification.json. Three earlier compilations were shed before tests; an in-place conductor restart reclaimed heap without changing limits or identities, and those attempts remain failed evidence. Historical missing anchors, fleet convergence, household diversity and browser mid-transfer rescue remain unproven by this scoped run.

DELTA 2026-09-22a (story 4.3 measurement slice; RED preserved): origin-to-successful-serving-projection staleness is measured on libp2p and iroh eager/pull/periodic receive paths using existing advisory Automerge timestamps; missing/future clocks are excluded, concurrent duplicate suppression is best effort, and no global sync lock or announce guard change is introduced. Focused dual-feature regression covers new, duplicate and dependent raw/save_after changes; invalid-clock and timestamp-origin tests pass. Household binary a163463094a1bbc2cac18f9afd9b7dd63c1a50daf53126afc6ab03dbd3a32f67 passed isolated libp2p (1 scenario/12 steps, receiver samples 0.901s and 0.903s) and mixed dual-author/iroh-Jessica (1 scenario/11 steps, sample 38.597s), with exact-head serving convergence in both: sprint-report-household-20260922T140839Z-19c78d4a and sprint-report-household-20260922T140956Z-19c78d4a. Reports name the base commit; the measured candidate was uncommitted. Aggregate sample deltas are not unique-document transit measurements. The mixed lane exposed and fixed a harness classifier that read pure iroh with irohNodeId as dual; its regression passes. All three peers restored to dual, original identities/data retained. Fleet proof and the eager dual-mode iroh doorbell remain owed; 1.4b stays held.

DELTA 2026-09-22b (shipment split at a real serving red): cell-membership cure 19c78d4af landed on origin/dev at 14:21:17Z after its exact-commit green full gate and 5/5 serving receipt; orchestrator #1891 started on that revision, fleet outcome pending. Measurement candidate fef7f7702 remains local: final serving report sprint-report-household-20260922T141415Z-fef7f770 passed 4/5, but the next-browser upgrade at elohim.host crossed 75s. Jessica already held/served the expected new immutable blob at 14:16:41Z; prompt incoming changes met the existing green-row guard retaining the old pointer, and reconciliation healed it at 14:17:57Z, too late for the remaining file probes. No authority change, relaxed deadline or passing shipment claim is made for that candidate.

DELTA 2026-09-22c (story 4.3 delivery slice; RED preserved): a dual node now rings iroh-only peers itself — the libp2p arm's forwarder hands each projected change to an iroh announce bridge scoped to book peers without a libp2p_peer_id, so one projection feeds two announcers and dual peers get no duplicate push. The mixed-transport scenario tightens to a 10s window. Control on the old binary, same topology (Matthew+James dual, Jessica iroh): 0/1, jessica=[] at 10s — sprint-report-household-20260922T191738Z-6039f41f (report names the worktree commit; peers ran the old mesh-bin). Candidate 6039f41f2, binary sha256 022b238542067a1c3ffcb4f88bad556000e2b931c3e329428d91a56414f0187c, adopted on Matthew only via the release-adoption slot: 1/1, 11/11 steps, Matthew logged `iroh announced local change … peers=1 accepted=1`, Jessica's iroh origin-to-projected-apply sample 0.32s (was 38.6s) — sprint-report-household-20260922T191849Z-6039f41f. Full storage gate EXIT=0. Household restored to all-dual; Matthew keeps the candidate. One sample, not a distribution; fleet proof owed; 1.4b stays held.

DELTA 2026-09-22d (story 1.4d on the household; RED preserved): a bounded pointer-audit sweep (25 declared rows carrying a blob pointer per 300s tick, keyset cursor, Background admission, ELOHIM_POINTER_AUDIT default on) asks the own conductor for each row's head record and lets the unchanged pointer_heal_patch + StampMode::HealCanonical refresh a torn pointer — never authors, declares, contests or moves a head. Branch commit 4fb90a920 (storage gate EXIT=0, lib 4060 passed; elohim-settings 42 passed), shipped as a cherry-pick onto 571d9ea54; binary sha256 674f872fb9a78a8e… on all three peers via release-adoption slots. epr-app-deliverability: full run 3/5 — the restart scenario's storage-restart command errored after matthew had booted in 27s, and a cleanup assertion followed from it; @deliverability-restart alone then passed 19/19 (sprint-report-household-20260922T232142Z-9ea23a63 and the rerun). Control on the pre-batch binary a1634630…: 5/5, 102/102. The sweep logged no heals and no errors on the household (no torn rows there). The fleet's torn row (edge #1474 seam-smoke: one head, two blobs) also waits on cell health — both doorways answered UNREADABLE-HEAD-RECORD reason=cell-disabled. Story 3.2 was HELD from this ship: with it, the same feature failed 75s propagation steps twice (4/5, then a different step), and its selection sits on the blob-fetch path.

DELTA 2026-09-24 (native-delivery Lane N — app bundle as elected content, authored; RED preserved): an app's browser+server zips now ride ONE release on `runtime:app-bundle:<network>:<channel>` and each peer's AppBundleVehicle points every bound slug row at them in one transaction; a slug bound in its own metadata (`releaseChannel`) is Held by head adoption and skipped at the projection stamp, and its storage head read names the channel's staged candidate (ee5f9e54a, packager/ceremony 31c0bfac6, CI publish/verify 2cb1b6b2a, feature a6ab548cb; rakia 720c132 on pin/lane-N, not on dev). Unit evidence only: storage lib 4123 passed / 0 failed, clippy clean. The household proof (app-bundle-elected-delivery, 6 scenarios) is UNRUN — mesh down, preflight refuses the pool storage binary (no p2p-iroh marker); fleet follows the channel at observe only; the per-host crutch (leg 4b, stageSpaBlobs, app→edge dependsOn) stays until that proof is green.

DELTA 2026-09-25 (RED preserved; N6 has NO household reading — the lane never ran, and why it could not is the finding): the app-bundle-elected-delivery feature (Lane N6) was not executed. Three prologue attempts on pin 25dd2d0be failed to cast the household, every failing leg wearing a convergence face rather than a code face — `seed-projections`, `seed-presences`, `seed-collectives` and `stamp-server-projection-peers` all exhausted their 12×15s retries on `Projector catching-up (503)`, and the doorway relayed storage's own 503 with `cause: upstream` after `POST /api/v1/commitments` timed out into an open breaker. Underneath: matthew's `projectionReconcile` sat at `caughtUp:false, converged:false` with `divergentAnchor` climbing 0 → 97 → 102 → 191 and `failed: 7` against `healedTotal: 156` over 42 sweeps, and the reconciler's loudest repeated verdict (120 of the last 400 WARNs) was `projection-reconcile[humans]: KEY-DIVERGENT binding — this peer and the advertiser bind the same human to different agent keys` for `human-jessica-spouse` and `human-james-son`, 83 occurrences each on all three peers. That is a convergence red this habit owns, observed at rest rather than under a lane: two household humans are bound to different agent keys by different peers and the membership-truth pass has not settled it. It is ALSO the proximate reason the N6 reading is missing, so the absence is evidence, not a gap in effort. Window cost that framed the whole attempt: `genesis/orchestrator/.epr-meta/push-delivers-within-budget.habit.md` DELTA 2026-09-25.

DELTA 2026-09-25b (RED preserved; N6 still has no reading, now for a tooling reason): on the recast household (fork e0bfc6c7a) the prologue cast cleanly, which removes the convergence face that blocked N6 before. app-bundle-elected-delivery ran only after the 30-minute verify bound (12:58Z), so it is NOT RUN as evidence. It failed 6/6 at packaging (sprint-report-household-20260925T125821Z-381825f7): the release-manifest schema at the dev rakia pin d3329b2 refuses `artifactClass: app-bundle`, `appliesTo.apps` and `artifacts[].app/kind`. That class exists only at 720c132 (feat/app-bundle-class). A rerun with ELOHIM_RAKIA_ROOT pointed at that tree was stopped at the bound with Station 1 already failing, unread. Deliverability's cross-peer pointer station passed twice (see doorway-failover).

DELTA 2026-09-25c (RED preserved): the storage bridge wedge after conductors-restart is cured in e95535e72 — the re-mint now adopts fresh sockets into the handle every boot-captured consumer holds, and 3 transport-closed zome calls re-mint without a ping; on the household all roles re-minted by +15 s and recovered by +27 s with 0 `No connection` afterwards (was 292/376 and a storage restart). refuses-fast 3/4 (sprint-report-household-20260925T134059Z-e95535e7: the probe answered not-ready once right after caughtUp; doorway A's ZomeCaller reconnected only at +99 s), deliverability 5/5 (sprint-report-household-20260925T134528Z-e95535e7).

DELTA 2026-09-25d (RED preserved): a quiet role no longer latches storage's zome path dead after conductors-restart — f21326f4b folds an answered app_info ping as transport-responsive (lifts Dead, never ends a not-running episode) and drops transport failures from an already-replaced mint; household refuses-fast restart window now closes on its own in ~7 s (storage-refused 14:41:02–05Z, ready from 14:41:09Z; sprint-report-household-20260925T144050Z-53b54c2c, 3/4 — only the face-vocabulary step reds), deliverability 5/5 (sprint-report-household-20260925T144400Z-2590ca59).

DELTA 2026-09-25e (RED preserved): the conductor-restart window closes on its own on a tree carrying e95535e72 + f21326f4b + 9a01b6902 + 195aacbdc. Household refuses-fast 4/4 (sprint-report-household-20260925T152943Z-b6ff7d53): both doorways answered storage-refused from 15:29:57.96Z, A cleared by 15:30:17.64Z, B by 15:30:20.94Z (about 23 s); the ready poll after caughtUp answered ready on its first ask (timeToReadyMs 485). Deliverability 5/5 (sprint-report-household-20260925T153355Z-adaa6b7c). One sample on the household; the federation, transport-parity, trust-priced-sync, accountable-correction and fleet Dataplane Validation checks have no new reading.
