...
DELTA 2026-09-25 (household re-proof GREEN twice — status red → GREEN; checks (1), (2) and (3) all
green). Two runs of `just test mesh-browser '@concern:attention-witnessed-privately'` on household-dowell
(transport dual, 3 peers, processControl true, commit 46883085c). Receipts
`genesis/a2o/reports/sprint-report-household-20260925T135856Z-46883085.{json,md}` (sut
sha256:d862c63a39b22f20) and `…T135922Z-46883085.{json,md}` (sut sha256:d05d629d5af26a9a), identical:
1 scenario passed, 14 of 14 steps passed, and the clean-console After hook found no browser error.
Jessica's dwell and scroll depth on the manifesto reached her own storage peer as her agent-private
observation, her `/lamad/me/stream` listed it with the recipe CID the peer returns, James's stream (his
own sign-in) had no such entry, and her peer counted the note as withheld from other peers, never
announced. Fix commits under measure: bab140f95 (the emitter witnesses with the session bearer on both
paths; 401 for an anonymous write), f45fb527e (the stream read carries the bearer), cc90d86dc (the a2o
harness's injected sign-in persists the provider and expiry a real sign-in persists, R-A14),
cfbfbe404 (the content viewer registers its renderer before the view is checked — no NG0100, R-A15),
5c7fbbcae (storage peers get an absolute pillar-manifest directory, R-A16), 46883085c (signals read
`GET /api/v1/status/write-through` once per session and never post to a declared-off path, R-A16).
NO BYPASS: `MESH_ALLOW_NO_PROLOGUE` was not set; the lamad readiness rail passed on matthew, jessica and
james, and the portal preflight answered 200. Setup: the development dists rebuilt at 46883085c
(`ContentViewerComponent` carries the signal and no plain field write; the shell's emitter chunk carries
the write-through status URL; the lamad bundle holds neither the viewer nor a signal emitter, so neither
marker applies there), stamped, staged via doorway A and propagated to B with the carried record (the
server leg's first declare hit `Stamp declared head failed: database is locked` on B and propagated on a
single retry, attempt 1); doorway A served the new chunks byte-identically. The running storage peers
predate 5c7fbbcae and still report `effective: []` from the status route, so the green rests on
46883085c's client side (no post, no 503); a peer restarted on 5c7fbbcae exercises the other side.
Checks (2) and (3) unchanged and green, as re-measured in the 2026-09-24 A1-A9 delta: (2)
`cargo test --lib -- gossip_gate` 2 passed EXIT=0; (3) re-measured now, after cfbfbe404 touched the viewer: the grep on content-viewer.component.ts = 0, EXIT=0.
Environment: conductor fork e0bfc6c7a (the pin target, 81e77bcae), the recast 9adf9f01 made at 12:22Z;
the receipts' `sutParts.conductor` names the submodule tree 25dd2d0be, not the running binary.
