...
---
DELTA 2026-09-09 (SDK retention prerequisite): `just gate elohim-sdk` EXIT=0 across eight feature selections and clippy; seven identical regressions failed against the frozen pre-repair source and pass against the repair. Native sprint gap 1 fulfilled and independently approved; `genesis/a2o/reports/network-sprint-20260909/task-sdk-report.md` bounds proof to in-process HTTP-client retention. Ambiguous create reconciliation, actual remote custody and cold RS source-loss proof remain open; no habit status flip.
...
DELTA 2026-09-12 (cure landed on origin/dev, RED until the fleet carries it): 696ff3f77 — `household_resilience::snapshot` now states `coverageShortfall` as a number on every MEASURED snapshot (0 = the floor is met, itself a measurement) and omits it when unmeasured, from the same `floor_for_tier("standard")` the felt projection already compares against; the retired graph branch (`graph_views/shefa/resilience_snapshot.rs`) got the identical present-vs-absent gate (it had emitted `Some(3)` over content the node never saw). Golden baseline amended (present-0 on the lit card, 1 on intra, no key on unmeasured), views_shefa pins both branches, schema prose + ts-rs/codegen TS regenerated; 38+228+11 tests, clippy -D warnings, fmt, lib tests green in an isolated worktree. Live on the household mesh after the storage restart: both doorways and the storage peer answer `coverageShortfall: 2` for the 1-collective content where the old bytes omitted the key. Flip-to-green = the fleet scenario ("coverageShortfall with value 0", 3 collectives on elohim-protocol) green on the edge run that DEPLOYS 696ff3f77 — a validate-only run against the current fleet would still red because the field is produced in storage.
DELTA 2026-09-12b (household chaos drills run LIVE on the mesh; RED preserved): run 20260912T122630Z-eaf84fb2 (`just test mesh '@resilience and (@chaos or @federation)'`, three storage peers + two doorways on the iroh-capable pool build; report genesis/a2o/reports/sprint-report-household-20260912T122630Z-eaf84fb2.md). What holds: peer-loss-failover 4/4 (Jessica's peer SIGSTOPped — reads still serve from a surviving peer, the returning peer re-syncs on its own, adjacency is bidirectional, a single device serves alone), peer-recovery "a wiped device recovers its stewarded content from the mesh" PASSED, doorway-multi-address-failover 3/3 (the SDK's sticky read-fallback / honest-write logic, measured against synthetic URLs — a client-level proof, not a live doorway going dark). chaos-peer-churn 0/3, two named causes and neither is a durability verdict: (1) "manifesto" has no custody footprint on this mesh (`distributionState: unmeasured`, stewardingCollectives 0), so the flapping and simultaneous-loss drills refuse before killing anything — the drill must seed custody for its content the way the cascade scenario seeds "chaos-ladder"; (2) the cascade drill seeds "chaos-ladder" on all 3 peers and then asserts protectionStatus "protected" while the snapshot answers "at-risk": protectionStatus is the COLLECTIVE-diversity floor (one household, wantsHouseholds 3, coverageShortfall 2) and the feature's ladder is the intra-household COPY count (details.stewardingCollectives[].intraHubPeers 3/2/1). On a one-household mesh "protected" is unreachable by construction; the ladder step must read intraHubPeers (or the feature must declare a diversity ladder) before any kill rung can pass or fail for the right reason. doorway-pool-degrade 3 HELD: no storage verb shades a peer's projections, so the degraded-primary shape cannot be constructed on the mesh. Mesh handed back intact (3 peers, 2 connected each, caughtUp).
DELTA 2026-09-12c (chaos drills now fail for mesh reasons; RED preserved): clean cold-started mesh, storage 772999bd3 (+ shade verb, alias fixes), founder binding repaired in the seeder (6d78c76e3: the formation/custody-spool binders bound to a hosted human's app on the hosting conductor; selectStewardApp now, formation 3/3 affirmed, spool 3/3). Runs 20260912T145758Z and 20260912T152941Z: peer-loss-failover 4/4 both times; chaos-peer-churn 0/3 with three NAMED dataplane reds, none glue: (1) custody standing does not converge — the same custody-blob commitment rows reach every peer through the rea_commitments reconcile arm but CommitmentWireFields carries no `state`, so non-authoring peers hold them as "proposed" forever while the author holds "active"; the arm reports converged because it compares dht_anchor_hash only (the record comparison is now state-agnostic and attaches a per-peer state histogram); (2) liveness for the felt badge is a 900 s peer_statuses heartbeat — a SIGKILLed peer writes no offline row, so after two peers die "manifesto" still reads 3 live copies (the rung is now min(custody copies, onlinePeers.live), and onlinePeers is real after 776a3390a: live 3/known 3 where it read 0/0) — the badge needs the p2p connected-peer view (ping timeout ≈35 s), not the heartbeat cutoff; (3) seed-drill-custody EXIT=1 on the last prologue (custody authored only through matthew's conductor; jessica's row absent), so the cascade Given refuses naming the missing provider rather than asserting rungs on a broken baseline. Flapping's remaining step "holds the same inventory as the rest of the mesh" compares row counts (3544 vs 19) — the seeding peer legitimately carries the corpus; that step's claim needs the parity definition peer-loss-failover already uses. Fleet clause unchanged: the orchestrator has not picked up fe60b07e9 (no run since 09:00Z), so coverageShortfall is still absent on alpha.
DELTA 2026-09-12d (RED → GREEN on fleet evidence, by this atom's own 2026-09-11 rule): edge #1455 (Jenkinsfile from ca6df0789, a full deploy — 7/7 edge rollouts, doorway A rolled; result UNSTABLE only because the doorway-alpha-b rollout testcase failed while B answered 200 throughout the quiesce window) carries 696ff3f77 to the alpha fleet. Dataplane Validation on the deployed build: fleet-quiesce PASS sustained 364 s (A/B caughtUp, A quiesced actionable=0, A-converged 1) after an 8-minute A-not-caught-up dip during catch-up; byConcern `[OK] blob-durability: passed=1 failed=0 pending=13` — the household-diversity-dataplane scenario ("a standard-tier content item with 3+ distinct stewarding collectives … coverageShortfall with value 0") is GREEN on the fleet, the exact assertion that flipped this habit red on #1452/#1454. doorway-failover 3/0, federation-deploy 2/0, reconcile-inventory 3/0, inventory-convergence 1/0; 99 scenarios HELD @act:i by LAYERS design. Household clause unchanged from 2026-09-12c: chaos-peer-churn stays 0/3 with three named dataplane causes (custody standing on the wire, heartbeat liveness, drill custody seed) — they were red under the previous green too and remain the next slices, recorded here rather than masked.
DELTA 2026-09-12e (rung 1 of the served-under-standing sprint deployed to the household mesh; RED preserved, one drill green): storage f6ce271db (custody state on the wire + StateDivergent outcome; a humans reconcile arm; liveness from the connected-peer view) on all three peers, doorway 983c73a22. Run 20260912T215749Z chaos-peer-churn 1/3: "Simultaneous loss of two peers leaves the survivor degrading honestly" PASSED — after two SIGKILLs the survivor's badge reads at-risk inside the ping budget where it read protected for 15 min before; "A flapping peer never corrupts what the mesh believes" now clears the settled-record precondition (custody standing converges) and stops at "holds the same inventory as the rest of the mesh" (matthew 3546 vs jessica 25, james 20 — the step's parity definition counts rows, which peer-loss-failover already says is never the claim: the seeding peer carries the corpus; glue); "Cascading peer loss" refuses at seeding custody on jessica: the prologue's seed-drill-custody leg creates jessica's and james's custody-blob commitments through doorway A but activating them answers `503 catching-up` on PATCH for the non-primary providers (12 retries), so only matthew's copy is on record — a cross-pool activation path to diagnose. peer-loss-failover 4/4 (run 20260912T220220Z).
DELTA 2026-09-12f (the cascade's custody red diagnosed to a DHT identity fork; RED preserved): the prologue's drill-custody seeder authored every custody-blob commitment through doorway A, so matthew's cell wrote a second root under the deterministic commitment id that jessica's own runtime self-custody minter had already rooted — commitment_observation then fails closed forever ("multiple root Creates for ID") and update_rea_commitment_state is root-author-only, so activation from another peer is structurally impossible; doorway A relabelled that permanent storage 503 as "catching-up" (12 × 15 s retries for a call that could never succeed). Rulings: a custody commitment is authored and activated by the PROVIDER's own peer (seeder + drill fixed in flight); the coordinator zome's create_rea_commitment returns the existing root idempotently instead of minting a second (coordinator-only, hash-neutral, in flight); the doorway relays a storage 503 that declares no backpressure verbatim (1df5ae3a2). custody-blob-abca87ccc3b77518 and -5cae43215ad73563 are forked on this mesh until a cold reseed. The flapping drill's "2 custody copies for jessica" is the same red seen from the other side: its seeding gate counted only active rows while its assertion counts non-withdrawn, so it minted a second pledge beside the stuck proposed one (gate aligned; a canonical marker for deterministic_custody_id — bafkrei vs sha256-hex of the same bytes make two ids — is a fleet-wide id migration, named not done).
DELTA 2026-09-12g (clean cold reseed on the full sprint stack; RED preserved, two of three drills green): storage f6ce271db pinned, doorway b47b440b5, coordinator guard hot-swapped then carried by the cold start's bundle (DNA hash unchanged), custody authored on the provider's own peer (48a992b3d) — the prologue's seed-drill-custody leg is EXIT=0 for the first time (jessica and james activated). Run 20260912T224420Z chaos-peer-churn 2/3: "A flapping peer never corrupts what the mesh believes" PASSED end to end (settled record, three kills, neighbours back, inventory parity, exactly one custody copy per peer, no phantom holder); "Simultaneous loss" PASSED; "Cascading peer loss" red at its FIRST rung before any kill — 3 custody copies, 2 live peers: the connected-peer registry counts the peers matthew is connected to and never matthew himself, so a three-peer household can never read three live from one fold (fix in flight: the answering peer is live by definition). peer-loss-failover 4/4 (run 20260912T224653Z). Still named: a canonical marker for deterministic_custody_id (fleet-wide id migration); one author per undertaking for the base-corpus/stewardship-fixture legs, which still seed the same EPR through both doorways inside one gossip window ("multiple root Creates" on PUT /api/v1/epr/… in the prologue — the guard closes the wide window, not this one).
DELTA 2026-09-12h (household chaos drills GREEN): storage 9a43811bc (the answering peer counts itself live; local and remote labels resolved through the same identity binding) on all three peers — run 20260912T233146Z chaos-peer-churn 3/3: flapping (three kills, custody record idempotent, inventory parity, one copy per peer), cascade (protected → partial → at-risk with reads outliving the badge; the last rung reads inside the ping budget, never "protected"), simultaneous loss (survivor honest, at-risk). With the fleet clause green since edge #1455, this habit's checks are green on both planes; the @browser-only mid-transfer drill (serving credit for the rescue) is the remaining unmeasured row.
