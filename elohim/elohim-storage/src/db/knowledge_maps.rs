...
pub fn get_knowledge_map(
    conn: &Connection,
    id: &str,
) -> Result<Option<KnowledgeMapRow>, StorageError> {
...
) -> Result<Vec<KnowledgeMapRow>, StorageError> {
...
pub fn create_knowledge_map(
...
) -> Result<KnowledgeMapRow, StorageError> {
    ...
    get_knowledge_map(conn, &id)?.ok_or_else(|| {
        StorageError::Internal("Failed to retrieve created knowledge map".to_string())
    })
}
...
pub fn update_knowledge_map(
    ...
    get_knowledge_map(conn, id)?.ok_or_else(|| {
        StorageError::Internal("Failed to retrieve updated knowledge map".to_string())
    })
}
...
pub fn delete_knowledge_map(conn: &mut Connection, id: &str) -> Result<bool, StorageError> {
    let rows = conn
        .execute("DELETE FROM knowledge_maps WHERE id = ?", params![id])
