...
//!
//! ## Legacy content-attestation routes (pre-consolidation) — REMOVED
//! The content-attestations table was dropped in the Phase-2a consolidation
//! (migration 2026-05-12-100300). Prerequisite requirements are now PREREQUISITE
//! content-graph edges (gated by `epr_service::check_prerequisite_mastery`); the
//! Category-A DHT attestation projection is served by the `/unified` routes
//! above. The legacy `GET/POST /api/v1/attestations[/{id}[/revoke]]` arms were
//! removed alongside the dropped table.
...
use crate::views::AttestationView;
...
pub async fn handle(
    ...
    // Legacy content-attestation routes were removed alongside the dropped
    // content-attestations table (Phase-2a consolidation). Only the `/unified`
    // projection routes remain.
    let _ = (method, query_str);
