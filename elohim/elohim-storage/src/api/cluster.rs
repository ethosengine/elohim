...
//!
//! NOTE: graph-backed path returns graph-derived fields (OPERATES_DEVICE edges)
//! with placeholders for system_metrics / blob_inventory / peer-binding fields.
//! Full composition lands in the follow-on sprint per Phase 5 architectural decision.
...
pub async fn handle(
    ...
    handle_cluster_self(req, pool, p2p_handle, graph_engine).await
}
...
///
/// When `graph_engine` is `Some` and the `graph-native` feature is ON, the graph-backed
/// path is used in preference to the federated relational path.
///
...
async fn handle_cluster_self(
    ...
    // graph-native branch: if the engine is available, derive cluster from graph edges.
    // Falls through to the legacy federated path when engine is absent.
    #[cfg(feature = "graph-native")]
    if let Some(engine) = graph_engine {
        return match crate::graph_views::shefa::cluster::build(engine, &agent_cid) {
            Ok(view) => Ok(response::ok(&view)),
            Err(e) => Ok(response::internal_error(&format!(
                "graph cluster build failed: {e}"
            ))),
        };
    }

    // Legacy federated path (thin builds or when graph engine not yet open).
