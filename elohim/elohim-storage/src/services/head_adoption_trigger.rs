...
async fn process_trigger(
    ...
    // ── STORY 1.4b: THE HEAD ARRIVED WITH THE CONTENT ────────────────────────
    //
    // Before spending a `resolve_content_head` probe on a head this conductor
    // probably cannot walk yet, look for the evidence in the doc that just
    // landed. `headRecord` is the author's SIGNED `Record` for the action
    // `headActionHash` names; handed to THIS node's own conductor it makes the
    // declare succeed without waiting for DHT gossip to deliver the action —
    // which is the entire race the 1/2/4/8/15/30 s ladder below exists to lose
    // slowly.
    //
    // Placement is deliberate: this runs on the SERIAL worker, behind the
    // per-id claim/cooldown, behind the row-presence gate, and behind
    // `decide` — never on the sync apply path, where a conductor call cannot be
    // cancelled and the rate is a peer's to choose. The work is bounded before
    // the call is made: one single-id declare, with a size-capped payload, at
    // most once per id per cooldown window.
    //
    // ANY non-adopt outcome falls through to the unchanged probe path below, so
    // a doc with no record — or a record the conductor refuses — behaves
    // exactly as it did before 1.4b existed.
    let doc_record = sync
        .get_doc_field(
            crate::sync::projector::PROJECTION_NAMESPACE,
            &doc_id,
            crate::sync::projector::HEAD_RECORD_DOC_KEY,
        )
        .await
        .ok();
    match head_adoption::decode_carried_head_record(doc_hint.as_deref(), doc_record.as_deref()) {
        Ok((claimed_head, record)) => {
            let carried = crate::chain_write_gate::as_writer(
                crate::chain_write_gate::WriterKind::TriggerAdopt,
                head_adoption::adopt_carried_head_record(
                    &hc,
                    pool,
                    ctx,
                    id,
            ...
            .await;
            if carried == AdoptOutcome::Adopted {
                crate::metrics::inc_head_adoption_trigger("carried_adopted");
                tracing::info!(
                    target: "elohim_storage::head_adoption_trigger",
                    content_id = %id,
                    source_peer = %trigger.peer,
                    head = %claimed_head,
                    trigger_to_adopt_ms = trigger.raised_at.elapsed().as_millis(),
                    attempt = trigger.attempt,
                    trust_path = "carried-doc",
                    "head-adoption trigger: ADOPTED a head whose signed record ARRIVED WITH \
                     THE CONTENT — verified once by this node's own conductor, no DHT walk, \
                     no re-probe ladder"
                );
                return;
            }
            // Refused, shed, held or contested. The conductor said no (or said
            // nothing); the probe path below is the unchanged fallback.
            crate::metrics::inc_head_adoption_trigger("carried_refused");
            tracing::debug!(
                target: "elohim_storage::head_adoption_trigger",
                content_id = %id, source_peer = %trigger.peer, outcome = ?carried,
                "head-adoption trigger: a doc-carried head record did not adopt — \
                 falling through to the conductor probe exactly as before"
            );
        }
        Err(refusal) => {
            // Honest absence is the overwhelmingly common arm (every pre-1.4b
            // peer), so it is counted and not logged.
            crate::metrics::inc_head_adoption_trigger(refusal.label());
        }
    }
