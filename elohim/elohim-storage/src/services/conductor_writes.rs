...
pub async fn call_update_rea_commitment_state(
    ...
    // Serialization + the bounded head-moved retry now live at the ONE choke
    // point every zome write passes through (`crate::chain_write_gate`), so
    // this call site keeps only its encoding. The retry that used to live here
    // covered exactly one write out of the crate's dozens; the gate covers all
    // of them, uses the same exact matcher, and no longer competes with the
    // in-process writers that were the actual cause of most refusals.
