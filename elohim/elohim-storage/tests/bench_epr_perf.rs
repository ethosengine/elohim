...
mod iroh_bench {
    ...
    async fn send_one(conn: &iroh::endpoint::Connection, req: &EprRequest) -> EprResponse {
    ...
    pub async fn run_reuse_conn(
        ...
        let head = build_payload(payload_size, 0x00C0_FFEE);
...
async fn compare_epr_perf() {
    ...
    for &size in payload_sizes {
        ...
        all_results.push(iroh_bench::run_reuse_conn(ids.clone(), size, warmup, measured).await);
        all_results.push(libp2p_bench::run_reuse_conn(ids.clone(), size, warmup, measured).await);

        // Fresh scenario — handshake-per-request.
        all_results.push(iroh_bench::run_fresh_conn(ids.clone(), size, warmup, measured).await);
        all_results.push(libp2p_bench::run_fresh_conn(ids, size, warmup, measured).await);
    }
