...
//!   handshake cost" comparison; production iroh
//!   ([`IrohSyncClient::request`]) operates in this regime.
...
// ---------------------------------------------------------------------------
// Iroh-side bench — IrohNode + sync ALPN handler. Two scenarios:
//   • run_reuse_conn — open one QUIC Connection up front, fresh bidi stream
//     per request (matches Phase 11 connection-pool ceiling).
//   • run_fresh_conn — open a fresh QUIC Connection per request (matches
//     Phase 5 IrohSyncClient::request as shipped today).
// ---------------------------------------------------------------------------
...
mod iroh_bench {
    ...
    pub async fn run_reuse_conn(
        ...
        assert!(
            ...
            "iroh reuse: need at least warmup+measured doc_ids ({total_iters}), got {}",
            doc_ids.len()
        );
        ...
        BenchResult::from_durations("reuse", "iroh", heads_per_response, durations)
    }

    /// Fresh-conn scenario: open a new QUIC connection for every request,
    /// drop it after the response. Mirrors what
    /// [`IrohSyncClient::request`] does in production today (Phase 5,
    /// pre-pool).
    pub async fn run_fresh_conn(
        ...
        assert!(
            doc_ids.len() >= total_iters,
            "iroh fresh: need at least warmup+measured doc_ids ({total_iters}), got {}",
            doc_ids.len()
        );
        ...
        for doc_id in &doc_ids[..warmup] {
            ...
            let conn = fetcher
                .endpoint()
                .connect(provider_addr.clone(), SYNC_ALPN)
                .await
        ...
        BenchResult::from_durations("fresh", "iroh", heads_per_response, durations)
    }
}
...
mod libp2p_bench {
    ...
    pub async fn run_reuse_conn(
        ...
        assert!(
            doc_ids.len() >= total_iters,
            "libp2p reuse: need at least warmup+measured doc_ids ({total_iters}), got {}",
            doc_ids.len()
        );
        ...
        BenchResult::from_durations("reuse", "libp2p", heads_per_response, durations)
    }
    ...
    pub async fn run_fresh_conn(
        ...
        assert!(
            doc_ids.len() >= total_iters,
            "libp2p fresh: need at least warmup+measured doc_ids ({total_iters}), got {}",
            doc_ids.len()
        );
        ...
        BenchResult::from_durations("fresh", "libp2p", heads_per_response, durations)
    }
}
...
async fn compare_sync_perf() {
    ...
    for &heads_per_response in head_counts {
        ...
        all_results.push(
            iroh_bench::run_reuse_conn(doc_ids.clone(), heads_per_response, warmup, measured).await,
        );
        all_results.push(
            libp2p_bench::run_reuse_conn(doc_ids.clone(), heads_per_response, warmup, measured)
                .await,
        );

        // Fresh scenario — handshake-per-request. iroh "as we ship" via
        // IrohSyncClient::request, libp2p with a brand-new fetcher swarm
        // each iteration.
        all_results.push(
            iroh_bench::run_fresh_conn(doc_ids.clone(), heads_per_response, warmup, measured).await,
        );
        all_results.push(
            libp2p_bench::run_fresh_conn(doc_ids, heads_per_response, warmup, measured).await,
        );
    }
    ...
    print_scenario_table(
        &all_results,
        "reuse",
        "Sync plane perf — REUSE scenario (engine ceiling)",
    ...
    print_scenario_table(
        &all_results,
        "fresh",
        "Sync plane perf — FRESH scenario (handshake per request)",
        "Each iteration pays full handshake cost. iroh: fresh `endpoint.connect()` per request \
         (matches IrohSyncClient::request as shipped today). libp2p: fresh fetcher swarm spawned \
         and dialed per request (TCP+Noise+Yamux handshake on each iteration).",
        warmup,
        measured,
    );
    ...
    for (heads, (iroh, libp2p)) in &by_class_fresh {
        let (Some(i), Some(l)) = (iroh, libp2p) else {
            continue;
        };
        let ratio_p50 = l.p50.as_secs_f64() / i.p50.as_secs_f64().max(f64::EPSILON);
    ...
    assert!(
        ...
        Expected: iroh p50 < libp2p p50 on at least one of {head_counts:?} for the reuse \
...
fn print_scenario_table(
    ...
    println!("## {title}");
    println!();
    println!("| heads/response | transport | p50 (ms) | p95 (ms) | p99 (ms) | mean (ms) | rps |");
    println!("|---|---|---|---|---|---|---|");
    ...
    println!();
    println!(
        "Iters: {warmup} warmup discarded + {measured} measured per (transport, head_count). {footnote}"
    );
    println!();
}
