...
//!
//! This bench measures **per-request latency** for the request-response shard
...
struct BenchResult {
    ...
    /// Workload class — bytes per `Data` response payload.
    payload_size: usize,
    p50: Duration,
    p95: Duration,
    p99: Duration,
...
// ---------------------------------------------------------------------------
// Iroh-side bench — IrohNode + shard ALPN handler. Two scenarios:
...
mod iroh_bench {
    ...
    /// Drive a single Get request over an existing QUIC connection.
    /// Used by both scenarios.
    async fn send_one(conn: &iroh::endpoint::Connection, req: &ShardRequest) -> ShardResponse {
    ...
    pub async fn run_reuse_conn(
        ...
        assert!(
            hashes.len() >= total_iters,
            "iroh reuse: need at least warmup+measured hashes ({total_iters}), got {}",
            hashes.len()
        );
    ...
    pub async fn run_fresh_conn(
        ...
        assert!(
            hashes.len() >= total_iters,
            "iroh fresh: need at least warmup+measured hashes ({total_iters}), got {}",
            hashes.len()
        );
        ...
        for hash in &hashes[..warmup] {
            ...
            let conn = fetcher
                .endpoint()
                .connect(provider_addr.clone(), SHARD_ALPN)
                .await
...
// ---------------------------------------------------------------------------
// Libp2p-side bench — minimal two-swarm setup for the shard ALPN.
...
mod libp2p_bench {
    ...
    pub async fn run_reuse_conn(
        ...
        assert!(
            hashes.len() >= total_iters,
            "libp2p reuse: need at least warmup+measured hashes ({total_iters}), got {}",
            hashes.len()
        );

        let data = build_payload(payload_size, 0xC0DE_C0DE);

        let provider = spawn_node(data.clone()).await;
    ...
    pub async fn run_fresh_conn(
        ...
        assert!(
            hashes.len() >= total_iters,
            "libp2p fresh: need at least warmup+measured hashes ({total_iters}), got {}",
            hashes.len()
        );
...
async fn compare_shard_perf() {
    ...
    for &size in payload_sizes {
        ...
        all_results.push(iroh_bench::run_reuse_conn(hashes.clone(), size, warmup, measured).await);
        all_results
            .push(libp2p_bench::run_reuse_conn(hashes.clone(), size, warmup, measured).await);

        // Fresh scenario — handshake-per-request.
        all_results.push(iroh_bench::run_fresh_conn(hashes.clone(), size, warmup, measured).await);
        all_results.push(libp2p_bench::run_fresh_conn(hashes, size, warmup, measured).await);
    }
    ...
    print_scenario_table(
        &all_results,
        "reuse",
        "Shard plane perf — REUSE scenario (engine ceiling)",
        "One QUIC Connection / TCP+yamux Connection reused across iterations on each side; \
         ...
         Reed-Solomon shard `Get` round-trip.",
        warmup,
        measured,
    );
    print_scenario_table(
        &all_results,
        "fresh",
        "Shard plane perf — FRESH scenario (handshake per request)",
