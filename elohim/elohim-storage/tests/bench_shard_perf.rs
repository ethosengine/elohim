...
async fn compare_shard_perf() {
    ...
    for &size in payload_sizes {
        ...
        all_results.push(
            iroh_bench::run_reuse_conn(hashes.clone(), size, warmup, measured).await,
        );
        all_results.push(
            libp2p_bench::run_reuse_conn(hashes.clone(), size, warmup, measured).await,
        );

        // Fresh scenario — handshake-per-request.
        all_results.push(
            iroh_bench::run_fresh_conn(hashes.clone(), size, warmup, measured).await,
        );
        all_results.push(
            libp2p_bench::run_fresh_conn(hashes, size, warmup, measured).await,
        );
    }
