...
//! both transports under two scenarios:
//!
...
//!
//! The EPR-atom plane uses CBOR (`read_frame_cbor_default` /
//! `write_frame_cbor`) rather than MessagePack — this bench measures that
//! codec specifically.
//!
...
//!
//! This bench measures **per-request latency** for the request-response EPR-atom
...
struct BenchResult {
    ...
    /// Workload class — bytes per `Atom` envelope response payload.
    payload_size: usize,
    p50: Duration,
    p95: Duration,
    p99: Duration,
...
// ---------------------------------------------------------------------------
// Iroh-side bench — IrohNode + EPR-atom ALPN handler. Two scenarios:
//   • run_reuse_conn — open one QUIC Connection up front, fresh bidi stream
//     per request (engine ceiling). Uses CBOR codec.
//   • run_fresh_conn — open a fresh QUIC Connection per request (handshake
//     cost included in measurement). Uses CBOR codec.
// ---------------------------------------------------------------------------
...
mod iroh_bench {
    ...
    async fn send_one(conn: &iroh::endpoint::Connection, req: &EprAtomRequest) -> EprAtomResponse {
        ...
        tokio::time::timeout(
            READ_TIMEOUT,
            elohim_storage::p2p_iroh::codec::read_frame_cbor_default::<EprAtomResponse, _>(
                &mut recv,
            ),
        )
        .await
        .expect("iroh read_frame_cbor timed out — handler may be wedged")
    ...
    pub async fn run_reuse_conn(
        ...
        assert!(
            cids.len() >= total_iters,
            "iroh reuse: need at least warmup+measured cids ({total_iters}), got {}",
            cids.len()
        );
    ...
    pub async fn run_fresh_conn(
        ...
        assert!(
            cids.len() >= total_iters,
            "iroh fresh: need at least warmup+measured cids ({total_iters}), got {}",
            cids.len()
        );
        ...
        for cid in &cids[..warmup] {
            ...
            let conn = fetcher
                .endpoint()
                .connect(provider_addr.clone(), EPR_ATOM_ALPN)
                .await
...
// ---------------------------------------------------------------------------
// Libp2p-side bench — minimal two-swarm setup for the EPR-atom ALPN.
...
mod libp2p_bench {
    ...
    pub async fn run_reuse_conn(
        ...
        assert!(
            cids.len() >= total_iters,
            "libp2p reuse: need at least warmup+measured cids ({total_iters}), got {}",
            cids.len()
        );

        let envelope = build_payload(payload_size, 0xDEAD_BEEF);

        let provider = spawn_node(envelope.clone()).await;
    ...
    pub async fn run_fresh_conn(
        ...
        assert!(
            cids.len() >= total_iters,
            "libp2p fresh: need at least warmup+measured cids ({total_iters}), got {}",
            cids.len()
        );
...
async fn compare_epr_atom_perf() {
    ...
    print_scenario_table(
        &all_results,
        "reuse",
        "EPR-atom plane perf — REUSE scenario (engine ceiling, CBOR codec)",
        "One QUIC Connection / TCP+yamux Connection reused across iterations on each side; \
         ...
         CBOR-encoded `Fetch`/`Atom` round-trip.",
        warmup,
        measured,
    );
    print_scenario_table(
        &all_results,
        "fresh",
        "EPR-atom plane perf — FRESH scenario (handshake per request, CBOR codec)",
        "Each iteration pays full handshake cost. iroh: fresh `endpoint.connect()` per request. \
         ...
         (TCP+Noise+Yamux handshake on each iteration). CBOR codec throughout.",
        warmup,
        measured,
    );
