...
/// parallel iroh-based stack — see elohim-storage's `p2p_iroh` and the staged-cutover
...
/// Every `pub fn set_*` boot publisher this module exposes, named by the part
/// that follows `set_`, in file order.
///
/// **Why a declared list and not a source scan.** A publisher nobody calls is
/// not a compile error: the key is registered, it is read live, and it silently
/// rides its default while the operator's env var is ignored. That shipped on
/// 2026-09-19 for `REANCHOR_HELD_BACKOFF_SECONDS` and
/// `CONTEST_REMINT_WINDOW_SECONDS` — setters with zero call sites, through
/// review. The guard that catches it must compare this module's publishers
/// against `elohim-storage`'s `main.rs`, which now sits in a different crate,
/// and a relative `include_str!("../../elohim-storage/src/main.rs")` cannot
/// make that trip: the storage Dockerfile flattens sibling crates into `/app`
/// and rewrites only Cargo paths, so a cross-crate source include resolves in
/// the dev tree and breaks in the image.
///
/// So each half reads only its OWN crate's source. Here,
/// `tests::the_publisher_registry_matches_this_module` keeps this list honest
/// against `config.rs`. There, `config::tests::every_boot_publisher_is_called_from_main`
/// checks each name against `main.rs`. Adding a publisher without adding it
/// here fails the first; adding it here without a call site fails the second.
///
/// This is the static half of the boot assertion. The runtime half —
/// "the call actually landed on the key" — is
/// [`crate::runtime_config::assert_boot_published`].
pub const BOOT_PUBLISHERS: &[&str] = &[
    "contest_two_way_declared",
    "contest_undeclared_divergence",
    "adopt_before_author",
    "obey_carried_election",
    "ghost_declaration_decay",
    "head_corpus_digest_enabled",
    "trust_memo_reads_enabled",
...
/// How many ALTERNATE advertisers one peer-head hint retains.
///
/// Bounded, not unbounded: the fallback ladder in
/// `services::head_adoption::resolve_peer_evidence` walks up to
/// [`evidence_fallback_max_alternates`] of these, healthiest first, per evidence
/// resolution — this cap is what makes that ladder a FIXED, pre-sized walk
/// rather than a retry loop. Raised 3 → 6 (2026-08-09, B2b, drain lever 4): the
/// household mesh the harvest observes routinely advertises the same
/// genesis-seeded row from more than 3 couriers, so the old cap silently
/// discarded plurality the fallback ladder could have used. The memory cost is
/// `ids × 6` short strings — still bounded by construction, not by convention.
///
/// This cap and [`evidence_fallback_max_alternates`] are BOTH held under the
/// SAME write-guard invariant as `adopt_contest_fanout` — see
/// [`assert_courier_ladder_budget`]'s doc for the adam 2026-07-20 melt this
/// exists to prevent a repeat of.
///
/// Defined HERE and re-exported by `services::head_adoption`, where the harvest
/// that enforces it reads it. The clamp below is why: a definition in that
/// service would make the settings layer depend upward on it.
...
///
/// Hard-clamped to [`ALTERNATE_ADVERTISER_CAP`] because that is how many the
/// harvest ever retains — a larger value cannot mean more probes, only a
/// misleading config. `0` restores the single-ask path exactly (same effect as
/// `evidence_fallback_enabled = false`).
pub fn evidence_fallback_max_alternates() -> usize {
    (*EVIDENCE_FALLBACK_MAX_ALTERNATES
        .get()
        .unwrap_or(&DEFAULT_EVIDENCE_FALLBACK_MAX_ALTERNATES))
    .min(ALTERNATE_ADVERTISER_CAP)
}
...
/// rationale as [`CONTEST_TWO_WAY_DECLARED`]: the reconcile sweep carries no
/// `Config`, and an env read on the hot path is the parallel-test flake this
/// codebase has already paid for once. (It once named `CONTEST_BACKOFF_SECONDS`,
/// which stopped being a `OnceLock` when that window moved into
/// [`crate::runtime_config`] — a dangling link, fixed with the extraction.)
...
/// `p2p::projection_reconcile::HealPacing::head_batch_fanout`, never this
/// constant). Kept ONLY as the fixed historical baseline
...
///
/// Plainly `pub`, not `#[cfg(test)] pub(crate)`: its one reader is a test in
/// ANOTHER crate now, and `cfg(test)` is per-crate — gated here it would simply
/// not exist for that reader. A `pub` constant in a library crate raises no
/// dead-code warning, so nothing is being silenced to make this compile.
...
/// now retains up to [`ALTERNATE_ADVERTISER_CAP`]
...
pub struct Config {
    ...
    /// [`ALTERNATE_ADVERTISER_CAP`] couriers per
...
mod transport_backend_tests {
    ...
    /// [`super::BOOT_PUBLISHERS`] names exactly the `pub fn set_*` this module
    /// declares, in file order.
    ///
    /// The half of the publisher guard that reads THIS crate's source. The
    /// other half — "and every one of them is called from `main`" — needs
    /// `main.rs` and so lives in `elohim-storage`, reading this list rather
    /// than this file. See [`super::BOOT_PUBLISHERS`] for why the registry is
    /// declared instead of scanned across the crate boundary.
    #[test]
    fn the_publisher_registry_matches_this_module() {
        let config_src = include_str!("config.rs");
        let declared: Vec<&str> = config_src
            .lines()
        ...
        assert_eq!(
            declared,
            super::BOOT_PUBLISHERS,
            "BOOT_PUBLISHERS has drifted from this module's `pub fn set_*` surface — a publisher \
             missing from the list is one elohim-storage will never check for a call site, which \
             is exactly how a lever ships inert"
        );
    }
