...
pub fn select_diverse(candidates: &[Candidate], min_capability: u8, n: usize) -> Vec<ScoredPeer> {
    ...
    let household = |cid: &str| {
        candidates
            .iter()
            .find(|c| c.agent_cid == cid)
            .map(|c| c.household_id.as_str())
    };
...
mod tests {
    ...
    fn capability_floor_filters_out_incapable_peers() {
        let cs = vec![
            cand("low", 1, 0.1, Some(10), "h1", true, 1.0),
            cand("ok", 5, 0.1, Some(10), "h2", true, 1.0),
        ];
        let r = rank(&cs, 3);
        assert_eq!(r.len(), 1);
    ...
    fn more_headroom_ranks_higher() {
        let cs = vec![
            cand("busy", 5, 0.9, Some(10), "h1", true, 1.0),
            cand("idle", 5, 0.1, Some(10), "h2", true, 1.0),
        ];
        let r = rank(&cs, 0);
        assert_eq!(
            r[0].agent_cid, "idle",
            "the less-loaded peer should rank first"
        );
    }
    ...
    fn saturated_peers_are_excluded_so_caller_can_shed() {
        ...
        assert!(
            rank(&cs, 0).is_empty(),
            "load>=1.0 means no headroom → not a candidate → caller sheds"
        );
    }
    ...
    fn unknown_rtt_degrades_gracefully_not_crashes() {
        ...
        assert_eq!(
            r.len(),
            1,
            "a peer with no attested RTT is still rankable (neutral rtt factor)"
        );
    }
    ...
    fn lower_rtt_ranks_higher_when_all_else_equal() {
        let cs = vec![
            cand("far", 5, 0.1, Some(300), "h1", true, 1.0),
            cand("near", 5, 0.1, Some(10), "h2", true, 1.0),
        ];
        let r = rank(&cs, 0);
    ...
    fn diverse_selection_spreads_across_households() {
        ...
        assert_eq!(
            cids[1], "h2a",
            "second pick crosses to a new fault domain, not the same-household runner-up"
        );
    }
    ...
    fn diverse_selection_falls_back_when_no_new_household() {
        ...
        let cs = vec![
            cand("a", 5, 0.05, Some(10), "h1", true, 1.0),
            cand("b", 5, 0.10, Some(10), "h1", true, 1.0),
        ];
        let picked = select_diverse(&cs, 0, 2);
        assert_eq!(
            picked.len(),
            2,
            "no new household → fall back to best remaining rather than under-fill"
        );
    }
}
