...
use eprfs_agent::memory::{Contribution, Locality};
use serde_json::{json, Value};

use super::validation::Governance;

pub fn input_guide(governance: &Governance) -> Value {
    let reference = &governance.reference;
    let example = Contribution {
        version: 1,
        collective: reference.clone(),
        author: "REQUIRED: current registered session identity".into(),
        steward: governance.default_steward(),
        scope: "workspace".into(),
        reach: Locality::Workspace,
    ...
    json!({
        ...
        "source":{"resource":"FileRef", "reach":"private|workspace|repository locality, checked against the most-specific source rule's maxLocality; the source must lie in this collective, not a nearer child"},
        ...
        "graduate":{"readOnly":true,"fields":{"contribution":"FileRef","review":"exact independent native approved verdict event CID on contribution","audience":"repository"},"approver":"the verdict's provider must hold a Steward affiliation in this collective and must not be (or stand for) the contribution's author; an approval resting on a fixture co-steward is reported as bootstrap, never peer validation","meaning":"Local rehearsal only, never publication or experiential acceptance"},
