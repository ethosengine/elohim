//! Stewarded resources routes — transparent proxy to elohim-storage `/api/v1/resources/*`
...
async fn forward_to_storage(
    ...
    match builder.send().await {
        ...
        Err(e) => {
            warn!(error = %e, path = %path, "Failed to forward stewarded-resources request to storage");
            Response::builder()
                .status(StatusCode::BAD_GATEWAY)
                .header("Content-Type", "application/json")
                .body(Full::new(Bytes::from(format!(
...
mod tests {
    ...
    fn storage_url_built_with_query() {
        ...
        let query = "steward=abc";
        let url = format!("{}{}?{}", base.trim_end_matches('/'), path, query);
        assert_eq!(url, "http://localhost:8090/api/v1/resources?steward=abc");
    }

    #[test]
    fn storage_url_trailing_slash_trimmed() {
        let base = "http://localhost:8090/";
        let path = "/api/v1/resources/abc-123";
        let url = format!("{}{}", base.trim_end_matches('/'), path);
        assert_eq!(url, "http://localhost:8090/api/v1/resources/abc-123");
    }
}
