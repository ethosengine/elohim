//! Compute routes — transparent proxy to elohim-storage `/api/v1/compute/*`
...
pub async fn handle_compute_request(
    ...
    let storage_url = match &state.args.storage_url {
        Some(url) => url.clone(),
        None => {
            warn!("Compute proxy called but STORAGE_URL not configured");
...
async fn forward_to_storage(
    ...
    debug!(method = %method, url = %full_url, "Forwarding compute request to elohim-storage");
    ...
    match builder.send().await {
        ...
        Err(e) => {
            warn!(error = %e, path = %path, "Failed to forward compute request to storage");
            Response::builder()
                .status(StatusCode::BAD_GATEWAY)
                .header("Content-Type", "application/json")
                .body(Full::new(Bytes::from(format!(
...
mod tests {
    ...
    fn storage_url_built_with_query() {
        ...
        assert_eq!(url, "http://localhost:8090/api/v1/compute?agent=abc");
    }

    #[test]
    fn storage_url_trailing_slash_trimmed() {
        let base = "http://localhost:8090/";
        let path = "/api/v1/compute/abc-123";
        let url = format!("{}{}", base.trim_end_matches('/'), path);
        assert_eq!(url, "http://localhost:8090/api/v1/compute/abc-123");
    }
}
