//! Custodians routes — transparent proxy to elohim-storage `/api/v1/custodians/*`
...
pub async fn handle_custodians_api_request(
    ...
    let storage_url = match &state.args.storage_url {
        Some(url) => url.clone(),
        None => {
            warn!("Custodians proxy called but STORAGE_URL not configured");
...
async fn forward_to_storage(
    ...
    debug!(method = %method, url = %full_url, "Forwarding custodians request to elohim-storage");
    ...
    match builder.send().await {
        ...
        Err(e) => {
            warn!(error = %e, path = %path, "Failed to forward custodians request to storage");
            Response::builder()
                .status(StatusCode::BAD_GATEWAY)
                .header("Content-Type", "application/json")
                .body(Full::new(Bytes::from(format!(
...
mod tests {
    ...
    fn storage_url_built_with_query() {
        ...
        let path = "/api/v1/custodians";
        let query = "agent=abc";
        let url = format!("{}{}?{}", base.trim_end_matches('/'), path, query);
        assert_eq!(url, "http://localhost:8090/api/v1/custodians?agent=abc");
    }

    #[test]
    fn storage_url_trailing_slash_trimmed() {
        let base = "http://localhost:8090/";
        let path = "/api/v1/custodians/abc-123";
        let url = format!("{}{}", base.trim_end_matches('/'), path);
        assert_eq!(url, "http://localhost:8090/api/v1/custodians/abc-123");
    }
}
