//! Stewardship policy routes — transparent proxy to elohim-storage `/api/v1/stewardship/*`
...
async fn forward_to_storage(
    ...
    match builder.send().await {
        ...
        Err(e) => {
            warn!(error = %e, path = %path, "Failed to forward stewardship request to storage");
            Response::builder()
                .status(StatusCode::BAD_GATEWAY)
                .header("Content-Type", "application/json")
                .body(Full::new(Bytes::from(format!(
...
mod tests {
    ...
    fn storage_url_trailing_slash_trimmed() {
        let base = "http://localhost:8090/";
        let path = "/api/v1/stewardship/stewards";
        let url = format!("{}{}", base.trim_end_matches('/'), path);
        assert_eq!(url, "http://localhost:8090/api/v1/stewardship/stewards");
    }
}
