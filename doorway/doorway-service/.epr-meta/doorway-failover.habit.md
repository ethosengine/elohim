...
checks:
  ...
  - "a2o @concern:doorway-failover (genesis/a2o/features/dataplane/served-shell-boots.feature, scenario outline `A visitor's browser actually starts the app it was handed` — @act:ii @browser-only, the DYNAMIC clause the static one cannot reach: a headless browser loads `/` through each doorway and the app must actually start (no pageerror, every same-origin request answered, `<app-root>` non-empty) and the served `/version.json` stamp must equal the one inside the declared browser head's own bundle at `/apps/{slug}/version.json`. Run: `cd genesis/a2o && E2E_DEVICE_MODE=playwright E2E_DOORWAY_ALPHA=https://doorway-alpha.elohim.host node_modules/.bin/cucumber-js --tags '@concern:doorway-failover and @browser-only'`. Born red on the fleet 2026-09-06: both apex names render an intact landing whose /version.json 404s.)"
  - "a2o @concern:doorway-failover (genesis/a2o/features/dataplane/epr-app-deliverability.feature — @act:i @requires:multi-node, the household-lane deploy story for spec genesis/docs/superpowers/specs/2026-09-08-epr-app-deliverability-through-doorway.md D1-D4: a bundle byte-seeded to both doorways and declared through doorway A only must reach a visitor able to RUN at either door within 75 s (2x BUNDLE_HEADS_TICK_SECS + slack); one serverBlobHash PATCH must reach every household peer; a doorway restarted while its storage peer is down must converge unaided; and an incoherent bundle must be refused by name and never reach a visitor as a blank page. TWO lanes, neither covering the other: `just test mesh features/dataplane/epr-app-deliverability.feature` and `just test mesh-browser features/dataplane/epr-app-deliverability.feature` (the mesh profile excludes @browser-only, which is the station that proves an app STARTS). Born red 2026-09-08: D1/D2/D3 unlanded, and the restart station is pending on a missing `hc-mesh.sh doorway-restart` arm.)"
refs:
