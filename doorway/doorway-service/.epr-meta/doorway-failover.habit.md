...
checks:
  ...
  - "Actual public-name transition authority: just test mesh features/dataplane/doorway-apex-transition.feature (no longer @wip: every step has a real definition). The household membership apparatus is relay-addr-beacon's `file` membership sink (relay-addr-beacon/src/sinks/file.rs), staged by app/elohim-app/scripts/hc-mesh.sh (start_membership_beacons, MESH_MEMBERSHIP=1) as one beacon leg per owned doorway writing only its OWN entry into $MESH_DIR/membership/elohim.local.json, decided by the SAME reconcile_membership call, serving probe and join2/leave3 hysteresis the fleet's Cloudflare multi-A lane runs; hc-mesh-prologue.sh declares it in the household fixture (membershipAuthority) and the scenarios resolve the public name THROUGH that document with the shipped client's try-in-order-and-stick selection. This born-red feature must have every scenario and step passed before graduation; WIP filtering, undefined steps, pending, skips, absence, a household that staged no authority, or steady-state doorway-failover.feature green cannot discharge it. It separately requires induced shed, owner-only membership withdrawal/rejoin, sibling delivery through the unchanged public name, browser bootstrap and recovery. WAN ingress continuity on the fleet is a DISTINCT prerequisite, not implied by household membership or by multi-A membership: the household document proves the routing decision, never that the resolved address is reachable across the WAN."
  - "Beacon shared-membership bridge: relay-addr-beacon/justfile gate with explicit native cargo-pool slot and empty RUSTFLAGS; production-cycle mock tests prove join2/leave3 hysteresis, WAN-independent withdrawal, exact-owner writes and retries, on BOTH projections of the one eligible-origin set — the Cloudflare multi-A lane and the household-ownable `file` membership sink (document materialisation, exact-owner writes with a byte-identical sibling entry across a withdrawal, no duplicate owner on rejoin, corrupt-document rebuild, concurrent two-leg writes with no lock/temp residue, abandoned-lock break, restart re-adoption). This foreign bridge measure does not certify public DNS propagation, WAN balancing or actual apex host selection; the household document proves the routing decision, never that the resolved address is reachable."
...
DELTA 2026-09-12 (household re-measure on a re-cast mesh; RED preserved): storage peers restarted on a fresh iroh-capable pool build (origin/dev 586e78085 + the coverageShortfall fix), `just mesh prologue` re-run (landing + manifesto staged, cast recast), then the same scoped lane as 20260911T223432Z — run 20260912T010759Z-a9624007 (report genesis/a2o/reports/sprint-report-household-20260912T010759Z-a9624007.md): doorway-failover 15 passed / 5 failed / 3 skipped (was 7/13/3). Every prerequisite red of the evening cleared — fixture-readiness 2/2, pair floor, apex landing 200, declared-head agreement, freshness receipt, warmup servable head, sibling-reader — so the 23:34 reds were the absent fixture, as recorded. The five that remain, read from the report: (a) doorway-runtime-endpoint ×2 `content routing must not escape to the compiled production host` — a real red on the packaged-app clause; (b) epr-app-deliverability `root is already claimed; cannot borrow it for this fixture` — the check's own ordering rule (the unclaimed-root proof must run before the canonical root is staged; the prologue stages it first), apparatus; (c) the two process-control chapters REFUSED again (`recovery executable … differs from the running bytes`) because another session rebuilt the pool binary at 00:59Z while the peers ran the 00:33Z build — apparatus. federation-version-convergence green on the same run. Apex-transition remains the authority; RED.

DELTA 2026-09-12 (household membership apparatus LANDED; habit RED preserved — the apex-transition run is the authority and has not been measured on this commit): the household now owns a public-name membership authority, which is the missing apparatus every earlier apex-transition delta named. relay-addr-beacon grew a `file` membership sink (`--membership-file`, `--member-origin`; relay-addr-beacon/src/sinks/file.rs) that projects the SAME eligible-origin set the Cloudflare shared lane projects as multi-A, through the SAME reconcile_membership call, serving probe and join2/leave3 hysteresis, into a JSON document the household owns — exact-owner writes under an O_EXCL lock with atomic rename, exactly one entry per owner, restart-starts-withdrawn. `just gate` in that crate: EXIT=0, 61 tests (13 new: 9 unit + 4 production-cycle tests driving serving_cycle itself). hc-mesh.sh stages two legs at `just mesh start` (owner alpha -> :8888, owner apex -> :8889, public name elohim.local, 3s probe cadence, MESH_MEMBERSHIP=0 opts out), reaps them in stop, reports them in status, and REFUSES preflight without the binary; hc-mesh-prologue.sh copies the staging declaration into household-fixture.json as membershipAuthority. apex-transition.steps.ts replaced every "no household-owned membership authority" honest-fail with real assertions read from that document, and resolves the public name through it with the shipped client's try-in-order-and-stick selection (dual-WAN design §3a) instead of a doorway's fixture URL — the test-only proxy the feature's preamble refuses. Both scenarios shed @wip (@requires:owned-substrate kept). Scratch proof of the apparatus itself (two legs, two dummy endpoints, SIGSTOP one): withdrawal of only the frozen owner in ~12s with the sibling's entry byte-identical, stamp included; rejoin in ~4s after SIGCONT; exactly one entry per owner throughout. WAN ingress continuity on the fleet is unchanged and still distinct. The feature has NOT been run on the household mesh for this commit — the measurement is the next move, and until it passes every step the habit stays RED.
DELTA 2026-09-12c (household authority discharged AT HOME; RED preserved by the WAN clause): on a clean cold-started mesh (storage 772999bd3+ fixes, doorway 05:31 build, two relay-addr-beacon legs writing $MESH_DIR/membership/elohim.local.json) the born-red authority doorway-apex-transition.feature is 2/2 (run 20260912T153241Z; also 20260912T144926Z and 20260912T143132Z): shared membership withdrew only the shedding owner after three failed probes and readmitted it after two serving probes without duplicating the sibling, and a new visitor resolving elohim.local through the membership document received the landing page (200, app-root, same declared build stamp) from the sibling while doorway A was SIGSTOPped, then the shed doorway rejoined inside the declared bound. doorway-multi-address-failover 3/3 in owned-substrate mode (run 20260912T153057Z: the real ElohimClient against doorway A and B, A frozen for real; reads move to B and stick, the write fails honestly, no-fallback degrades as before). doorway-pool-degrade 3/3 (run 20260912T153133Z) through the new storage verb POST /admin/projections/shade: the router populates from a pool peer and WARNs "EPR router DEGRADED" naming the shaded primary and the serving peer; all-shaded logs the consulted list at INFO; the apex serves through the degraded primary. What this does NOT certify, so the habit stays RED: the fleet's public name — elohim.host is pinned to doorway B's ingress, the shared record fronts A only, apex multi-A was reverted (single-slot beacon lane, ingress-conflict policy) — the household file sink is the same reconcile_membership loop projected into a document the household owns, and the Cloudflare sink is that loop projected into DNS; the fleet leg is Task 21 of the 2026-09-10 plan and needs the operator's ingress-topology decision (per-leg ingress class or controller split) before the beacon's shared lane can carry the apex. Operator menu, evidence-backed: (1) let both doorway ingresses accept elohim.host and widen the beacon's shared lane to two records, or (2) borrow a health-checked LB for the apex as the filed interim (spec §3b) — either flips this habit on the next fleet measure; nothing else here is agent work. Mesh trap recorded: an agent sourcing hc-mesh.sh and calling stop_all with a fake MESH_DIR reaped the live mesh by owned port at 12:58Z.
RULING 2026-09-12 (operator): option 1. Doorways are the federated load balancer at the WAN addresses of the apex; shared membership is the one authority and every sink projects it — Cloudflare now, the household file at home, an authoritative DNS server the federation runs next, routing a visitor to the cluster of doorways that can provide ingress, whose own ingress picks the pod. No borrowed LB. Cluster prerequisite (operator): per-premise ingress classes pinned to their nodes. Repo side (parked branch until the classes exist, so no edge deploy applies an ingress nothing serves): check-ingress-conflicts keyed on host+class; doorway A's ingress accepts elohim.host with the TLS name; relay-addr-beacon shared lane repeatable (two records per leg); both beacon manifests contribute the apex. Flip = apex-transition green on the fleet (Task 21 of the 2026-09-10 plan).
DELTA 2026-09-12d (household re-measure on the full sprint stack; RED preserved by the WAN clause): doorway b47b440b5 (name routing with DHT-seeded registry and federation-aware specificity, permanent storage errors relayed not masked, reach re-asked at serve time) on both household doorways — doorway-apex-transition 2/2 (run 20260912T225049Z), doorway-multi-address-failover 3/3 real mode (run 20260912T225156Z), doorway-pool-degrade 3/3 (run 20260912T224930Z), name-routing 3/3 of its de-WIPed scenarios (run 20260912T224749Z). The fleet leg (Task 21) still waits on the operator's premise-pinned ingress classes and the parked branch sprint/apex-multi-a-ingress.
DELTA 2026-09-13a (household measure; RED preserved on the fleet apex): name-routing 3/4 on every run tonight (runs 20260913T045317Z-fed6eb10, 063426Z-1e9d80e9, 091419Z-52cf701f): the registry fold, the one-hop relay with `x-federation-hop`, the authoritative 403 and the no-live-contract refusal hold; scenario 4 "a doorway never forwards a forwarded request" stays red after four distinct mechanisms were found and closed — (1) the glue's unscoped mount root let two stagings share one route key (nonce-scoped, 955357b9d), (2) a stale peer echo re-installed a revoked contract through the fallback (revocation shield b5c180221, single entry point `EprRouter::install_from_fallback` a8082c113), (3) the heal refreshed a cancelled row from a conductor still answering the pre-cancel root (storage guard 493adaabb), (4) the periodic self-heal fell back to the sibling before the revoke SSE armed the shield (primary omission arms it, 1e9d80e97); the remaining miss sits inside 300 ms of the lapse and needs the doorway's own log around the ask read once more with all four in place. Rung 4 slice 1 landed (78347f1c7..779f086e0: contracts name hostnames + channel, router keyed by RouteKey{host,path}, bundle heads by (slug, channel), second public name alpha.elohim.local on the beacons) — unmeasured by a scenario yet; the candidate-channel scenario is the next write. Fleet: edge #1459 rolled everything; apex `/status` is the HTML operator page by design (seeder now reads /status.json).


DELTA 2026-09-13b (final-source serving receipt exposes the root-fixture premise again; RED preserved): the mandatory epr-app-deliverability lane on 4d3786b1d ran all five stations: four passed, one refused at setup, 83 steps passed / 1 failed / 18 skipped in 228.605s. Report: genesis/a2o/reports/sprint-report-household-20260913T181441Z-4d3786b1.json. Browser-root setup found the legitimate elohim-host-landing projection at / on both alpha-elohim-host and apex-elohim-host and refused before creating root commitments; that site was preserved. The other server-pointer, rendered-version/cache, restart-recovery and incoherent-bundle refusal stations passed. Chain: populated household mesh → real-root browser visit / missing node: a separately owned doorway pair can project run-owned sites over the same authoritative peers while preserving the household entrances. The 2026-09-12 delta already named the conflicting pre-prologue premise; a command alone was an incomplete handoff. The isolated pair fixture is implemented and independently reviewed; all 384 A2O unit tests and the canonical A2O gate pass, but no fresh five-station pass or serving receipt is claimed yet. The corrected name-routing lane separately passed 4 scenarios / 67 steps on the final tested coordinator (report genesis/a2o/reports/sprint-report-household-20260913T162959Z-8f61c824.json); WAN ingress-class premises remain operator-owned.

DELTA 2026-09-13c (isolated household entrances and complete serving lane measured; RED preserved for WAN): on c326b589b, epr-app-deliverability passed all 5 scenarios / 102 steps, including required cleanup, in 350.852s with zero findings; report genesis/a2o/reports/sprint-report-household-20260913T190958Z-c326b589.json. Each scenario owns two ephemeral doorway entrances over the same three authoritative storage peers, with copied running executable bytes and private process/database lifecycles; raw pair identity/topology/cleanup attachments are retained in reports/recovery/isolated-doorway-pair-20260913/second-cucumber.json. The first isolated run passed every serving step but failed root cleanup at 45s; its final B poll at 19:01:12.068 preceded the next 30s refresh by about 0.23s. Cleanup now uses the existing 75s convergence contract (two ticks plus slack), preserving author cancellation/readback and both route-disappearance assertions. Chain: author cancellation → peer lifecycle convergence → doorway refresh → root absent; cancellation and peer convergence are separate observable stations, not one acknowledgement. First-run after-probes confirm both temporary roots cancelled with matching action hashes on all three peers; stable peer IDs, all five DNA/coordinator roles, canonical doorway identities and household root heads remain unchanged, and the canonical login route returns 200. The complete lane now measures the previously missing isolated-entrance node without claiming DNS/anycast or fleet apex failover; those still require the operator-owned ingress-class premise.
