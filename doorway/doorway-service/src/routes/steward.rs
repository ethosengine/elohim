//! Steward routes — transparent proxy to elohim-storage `/api/v1/steward/*`
...
pub async fn handle_steward_api_request(
    ...
    let storage_url = match &state.args.storage_url {
        Some(url) => url.clone(),
        None => {
            warn!("Steward proxy called but STORAGE_URL not configured");
...
async fn forward_to_storage(
    ...
    debug!(method = %method, url = %full_url, "Forwarding steward request to elohim-storage");
    ...
    match builder.send().await {
        ...
        Err(e) => {
            warn!(error = %e, path = %path, "Failed to forward steward request to storage");
            Response::builder()
                .status(StatusCode::BAD_GATEWAY)
                .header("Content-Type", "application/json")
                .body(Full::new(Bytes::from(format!(
