//! Attestations routes — transparent proxy to elohim-storage `/api/v1/attestations/*`
...
pub async fn handle_attestations_request(
    ...
    let storage_url = match &state.args.storage_url {
        Some(url) => url.clone(),
        None => {
            warn!("Attestations proxy called but STORAGE_URL not configured");
...
async fn forward_to_storage(
    ...
    debug!(method = %method, url = %full_url, "Forwarding attestations request to elohim-storage");
    ...
    match builder.send().await {
        ...
        Err(e) => {
            warn!(error = %e, path = %path, "Failed to forward attestations request to storage");
            Response::builder()
                .status(StatusCode::BAD_GATEWAY)
                .header("Content-Type", "application/json")
                .body(Full::new(Bytes::from(format!(
