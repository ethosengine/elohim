//! Presence lifecycle routes — transparent proxy to elohim-storage `/api/v1/presence/*`
...
async fn forward_to_storage(
    ...
    match builder.send().await {
        ...
        Err(e) => {
            warn!(error = %e, path = %path, "Failed to forward presence request to storage");
            Response::builder()
                .status(StatusCode::BAD_GATEWAY)
                .header("Content-Type", "application/json")
                .body(Full::new(Bytes::from(format!(
...
mod tests {
    ...
    fn storage_url_built_with_query() {
        ...
        let path = "/api/v1/presence";
        let query = "state=unclaimed";
        let url = format!("{}{}?{}", base.trim_end_matches('/'), path, query);
        assert_eq!(url, "http://localhost:8090/api/v1/presence?state=unclaimed");
    }

    #[test]
    fn storage_url_built_without_query() {
        let base = "http://localhost:8090/";
        let path = "/api/v1/presence/abc-123";
        let url = format!("{}{}", base.trim_end_matches('/'), path);
        assert_eq!(url, "http://localhost:8090/api/v1/presence/abc-123");
    }
}
