...
And the operator never carries a runaway bill, as fediverse admins do today. What you can reasonably offer the commons, in kind, sets the size of your flywheel, and you are compensated for the compute, infrastructure, and effort it takes to supply it. No more passing the hat to cover the cloud bill: the contract exists from the start, with its limits and bounds set when each relationship begins.
