    ...
    match (&method, path_only) {
        ...
        (&Method::GET, "/states") => {
            ...
            if entity_type.is_empty() {
                return Ok(response::bad_request(
                    "entityType query param is required",
                ));
            }
            let mut conn = get_conn(pool)?;
        ...
        (&Method::GET, p) if p.starts_with("/challenges/") => {
            ...
            let id = extract_id(sub).ok_or_else(|| {
                StorageError::InvalidInput("Challenge ID required".to_string())
            })?;
            let mut conn = get_conn(pool)?;
        ...
        (&Method::GET, "/challenges") => {
            ...
            let views: Vec<ChallengeView> =
                results.into_iter().map(ChallengeView::from).collect();
            Ok(response::ok(&views))
        }
        ...
        (&Method::GET, p) if p.starts_with("/proposals/") => {
            ...
            let id = extract_id(sub).ok_or_else(|| {
                StorageError::InvalidInput("Proposal ID required".to_string())
            })?;
            let mut conn = get_conn(pool)?;
        ...
        (&Method::GET, "/proposals") => {
            ...
            let views: Vec<ProposalView> =
                results.into_iter().map(ProposalView::from).collect();
            Ok(response::ok(&views))
        }
        ...
        (&Method::GET, p) if p.starts_with("/precedents/") => {
            ...
            let id = extract_id(sub).ok_or_else(|| {
                StorageError::InvalidInput("Precedent ID required".to_string())
            })?;
            let mut conn = get_conn(pool)?;
        ...
        (&Method::GET, "/precedents") => {
            ...
            let views: Vec<PrecedentView> =
                results.into_iter().map(PrecedentView::from).collect();
            Ok(response::ok(&views))
        }
