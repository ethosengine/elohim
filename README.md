...
- Projection. The doorway ([`doorway/`](./doorway/)) is the layer that faces the legacy web. It brings browser traffic into the mesh and serves canonical content back out over HTTP as a read-optimized view. The doorway service does not itself join the libp2p or iroh swarms; it proxies to a conductor, reads and writes bytes through a storage peer's HTTP API, caches projections, and is never the source of truth.

The same content addressing reaches the repository itself; [The repository is the first node](#the-repository-is-the-first-node) below says how.

### How a node takes part

The [seam map](./genesis/docs/content/elohim-protocol/architecture/2026-06-21-elohim-seam-map-concern-routing.md) names four participation tracks. They are not tiers: a device can be on several at once, and what hardware it has bounds which.
...
### The repository is the first node

Why govern a repository with the protocol it builds, and call it a node when it is not yet a peer? Because the hardest change to move is a change to a DNA's rules, and the network cannot be upgraded by force. In Holochain the hash of a DNA's integrity rules is the identity of its network: change the rules and you have a new network whose peers cannot see the old one. No operator can push a new rule set onto running peers. Peers cross (move from the old network to the new) only by running both versions and choosing, so the protocol evolves at the rate its participants adopt it, and no faster. Peers who must choose whether to adopt a change need to see why it was made, and the cheapest way to make those reasons legible is to keep them as the same kind of records the network already reads. Two decisions are involved, and the evolution design keeps them apart. Which lineage is canonical is decided by a council of elohim (an earned roster; its members are elohim, not people, so the decision sits one degree removed from any person), recorded as a bounded, revocable, notarized commitment on the mishpat DNA, the DNA that holds collectives, proposals and judgments. When a node follows is still that node's own choice, and a node that has not yet followed is visibly behind rather than silently forked. So the network's evolution is neither an operator's push nor a per-node veto, and no one person and no operator owns it. The council's commitment is made under a constitutional root, a content address of the constitution text the rules serve, which every DNA is meant to carry in its declared settings and which today only the node-registry DNA declares. [Upgrade, Revert, and Constitutional Consensus](./genesis/docs/content/elohim-protocol/architecture/2026-07-14-upgrade-revert-and-constitutional-consensus.md) is the design; its core crossing steps pass their tests, and the household mesh has run them end to end with a test key standing in for the council. What the council and each choosing peer read when they decide are the change's scenario, its value-chain entry and its review. A network that can only evolve by honest adoption needs a development process that produces those records, and that is what the rest of this section describes.

The repository is not yet a peer on the network, but it is already the first thing the protocol governs: each file a content-addressed record (a document's body is hashed the way the codec addresses any record, and that hash is its identity), each directory's rules its governance, the work itself its value chain. A citation from one document to another carries a fingerprint of the cited body, so when the cited document changes the citation is detectably stale instead of silently wrong, and a document's identity is what it says rather than where it sits. Each directory declares, in a `.epr-meta` manifest, the rules a record born there must satisfy: what frontmatter it carries, which policies bind it, and where a kind of file is routed if it lands in the wrong directory. The rules cascade down the tree, root manifest first, each directory adding to what it inherits. [`elohim/epr-rea/`](./elohim/epr-rea/) models the work itself as a value chain in REA terms (resources, events, agents, in the vocabulary that [ValueFlows](https://valueflo.ws/) standardizes): a specification is a process, an unclaimed piece of work is an intent, a claimed task is a commitment, and a passing scenario fulfills that commitment once its test report is recorded in the chain, so what is left to finish is a walk of the chain rather than an opinion. The `epr` CLI in [`elohim/eprfs/`](./elohim/eprfs/), the layer that projects protocol records onto a filesystem, reads all of this. Like the directory rules, the habit register (the list of what the system reliably does, each entry bound to a runnable check; [How we work](#how-we-work) explains it) and the recall budget a coding agent works under (how much evidence it may pull in to answer one question, so that its evidence gathering is itself bounded and inspectable) are declared records: bounds and algorithms the tree carries, not constants in code. Coding agent here means an AI working in this repository, such as the ones [How we work](#how-we-work) introduces; it is not an elohim.

A change graduates locally before it asks for reach. It is authored in private, and only what is said or done is judged, for humans and coding agents alike; a draft or a chain of thought is nobody's business. So every workspace is a place to play: a clone, a branch or a hosted workspace can diverge, experiment and fail in private, with the full stack runnable inside it, and nothing is judged until it asks to travel. That is what makes the repository a playground for computational democracy, in the plain sense that the rules which will govern a network are readable and changeable by the people and coding agents they bind, and are tried on them first. It is also the path to a social coding commons: a tree no one owns, where what earns reach is what showed its reasons rather than who pushed it. Today a maintainer still merges to `dev`; the ceiling below is where that stops being a person's call. The coding agents' tooling checks a change against its directory's rules as it is written, and a human contributor meets the same rules at the pre-push hook. The scoped gate (`just gate`, the build-and-test check for the projects a change touches) proves that it builds and its tests pass; for feature work, the household mesh proves its scenario (the Gherkin story that specifies it); and a README or scenario is read cold by a fresh-context reader, a reviewer given the finished document and nothing else. Only then does the change ask to travel, and today that ask is a git push: the pre-push hook re-runs the directory rules over the whole push range, checks that the generated summary of the habit register (its projection) is fresh when a habit changed, and runs the gates. A push that passed those checks is therefore the minimum, mechanical form of the protocol's earned-reach attestation: the change has shown, by checks anyone can re-run, that it may travel. Call that the floor. The designed ceiling is that the push becomes a governance ceremony in the protocol's own terms. In that design the change, its scenario, its value chain and its review would travel together as records; the commit would carry EPR provenance ([brit](./elohim/brit/), a fork of gitoxide, a Rust implementation of git, can verify such provenance today, and writing it into commits is being built); builds would be peer-attested ([rakia](./elohim/rakia/), a distributed build system); and the result would graduate into the same peer-to-peer store as any other content, where reach and updates propagate under the reach rules that govern everything else. eprfs is built so that what it projects onto a filesystem can later be handed to the dataplane for exactly that. None of the ceiling runs end to end yet; [What runs today](#what-runs-today-september-2026) lists it.

The floor, kept honestly, is the bar for production. The ceiling changes where the records live, on the network instead of in git, not what the bar requires. Before this protocol is called production ready, the self-governing process that develops it, guided by viable-system design (Stafford Beer's recursive feedback and Ross Ashby's requisite variety, which [shefa](./genesis/docs/content/elohim-protocol/shefa.md) introduces), has to demonstrate on its own records that the architecture respects the world it enters as a complex ecology. In observable terms: the habit register green on its own checks, every feature change carrying its scenario, its value-chain entry and its review, and the reason for every rule readable by the people and coding agents the rule binds. That bar, a process that can show its own reasons on its own records, is the one this project sets for itself, and it is not one it has seen other peer-to-peer systems, the fediverse, Holochain on its own or REA on its own set. Interpretability, governance, value, meaning, attribution, review over the process and honesty in relationship are each meant to be a record the system can read, carried by whatever is built on it, so that the explanation of the system is part of the system. Today the rules, the citations, the habits and the value chain are such records; the rest is the design, and What runs today keeps the line between them.

### Surfaces

- Doorway ([`doorway/`](./doorway/)) is the projection layer's running service. Today it serves browsers and publishes a W3C DID document for its identity. It also signs an endpoint record in the pkarr format, where records are addressed by public key, but publishing that record is not yet wired. AT Protocol and ActivityPub bridges are planned. Doorways are plural and replaceable: several can serve the same content, and none of them owns it.
...
- The repository's own governance ([The repository is the first node](#the-repository-is-the-first-node)): directory-local `.epr-meta` rules checked at edit time and over every push range, content-addressed citations that report their own staleness, the habit register, the recall budget, and the developer value chain that `epr flow` projects from the tree.

Designed, not yet running:

- The push as a governance ceremony that graduates a change into the peer-to-peer store, with brit's provenance and rakia's attestation in the path. The DNA-lineage crossing under the elohim's commitment has its core crossing steps passing their tests and run end to end on the household mesh; no crossing has run on the alpha fleet.
...
- The feed protocol (the specification's `/elohim/feed/1.0.0`, by which peers exchange what has changed).
...
Production deploys are paused, and the bare domain elohim.host is served by an alpha doorway. The live source of truth is the habit register ([How we work](#how-we-work) explains it): `just status habits` lists what the system reliably does, each entry bound to a runnable check (its generated projection, [`genesis/manifests/habits.yaml`](./genesis/manifests/habits.yaml), reads without a clone), and it is where the pillar-level answer (what a person can do in each app today) lives; the list above is scoped to the substrate.
...
│   ├── eprfs/                     # Filesystem projection layer: eight crates; epr-cli is the `epr` governance tool, eprfs-cli the projection tool
...
│   ├── constitution/              # Loads the constitution and enforces it in an agent process (none deployed yet)
...
│   ├── brit/                      # Fork of gitoxide; verifies EPR provenance in commits, writing it is in progress (git submodule)
...
│   │   └── src/app/<pillar>/      # four pillars (imagodei · avodah · qahal · shefa) plus elohim (core) and doorway (gateway)
...
│   ├── manifests/                 # cluster-state.yaml and the generated habits.yaml projection of the habit register
...
Agents work here under directory-local governance. A directory's `.epr-meta` declares the rules an edit there must satisfy, a hook checks them before a write lands, and READMEs and scenarios pass a fresh-context blind reader (a reviewer given the finished document and nothing else) before they land. [The repository is the first node](#the-repository-is-the-first-node) says why the repository is governed this way. The root developer interface is eight verbs, which `just --list` shows and CLAUDE.md explains: `gate`, `test`, `dev`, `mesh`, `seed`, `look`, `status` and `codegen`.
...
- The frontend against live alpha data, with no local backend and no submodules (a production build fetches Sophia's renderers as a prebuilt bundle; the dev server runs without it, so assessments will not render on this path). From a clone of [the repository](https://github.com/ethosengine/elohim) with Node 24 and pnpm 10 (pinned in the root `package.json`) and a Rust toolchain with the `wasm32-unknown-unknown` target and `wasm-pack`, run `pnpm install`, then `pnpm --filter elohim-app run prestart` once (it compiles a browser-side WASM module), then `pnpm --filter elohim-app start:alpha`, and open http://localhost:4200/. You should see the landing page, with content served from doorway-alpha; browsing needs no account.
- The full local stack: a conductor, a storage peer and a doorway on your machine. Beyond the frontend's needs it takes `just`, Holochain 0.7 (`holochain` and `hc` on your `PATH`) and `git submodule update --init --recursive`; `just dev start` launches it and, from the repository root, `pnpm app:dev` serves the app against it; open http://localhost:4200/ and you should see the landing page served from your own doorway, empty until you seed content (`just seed apply local content`). `just mesh start` runs the multi-peer household mesh instead, which needs the pinned conductor fork that CLAUDE.md covers.
