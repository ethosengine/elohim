# Scenarios TODO - Idd Community (Value Scanner)

## Overview

This file tracks which scenario files need to be created for the `idd_community` user type in the `value_scanner` epic.
...
[**TODO**: Check which of these layers are relevant for idd_community, then create Gherkin .feature files accordingly]
...
[**TODO**: Check which of these functional domains are relevant for idd_community]
...
```gherkin
@epic:value_scanner
@user_type:idd_community
@governance_layer:[layer_name]
@related_users:[user1,user2]
@related_layers:[layer1,layer2]
@elohim_agents:[agent1,agent2]

Feature: [Layer Name] [Epic Context] for IDD Community
  As a idd_community in the value_scanner system
  ...
  Background:
    ...
    And the idd_community user is registered in the system
...
[**TODO**: Add any special considerations for idd_community scenarios in value_scanner epic]
