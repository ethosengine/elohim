# Scenarios TODO - Single Parent (Value Scanner)

## Overview

This file tracks which scenario files need to be created for the `single_parent` user type in the `value_scanner` epic.
...
[**TODO**: Check which of these layers are relevant for single_parent, then create Gherkin .feature files accordingly]
...
[**TODO**: Check which of these functional domains are relevant for single_parent]
...
Feature: [Layer Name] [Epic Context] for Single Parent
  As a single_parent in the value_scanner system
  ...
  Background:
    ...
    And the single_parent user is registered in the system
...
[**TODO**: Add any special considerations for single_parent scenarios in value_scanner epic]
