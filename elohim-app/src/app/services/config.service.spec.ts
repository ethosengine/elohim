...
describe('ConfigService', () => {
  ...
  describe('getConfig in development environment', () => {
    ...
    it('should return config from environment file', () =>
  ...
  describe('getConfig in production environment', () => {
    ...
    it('should load config from HTTP request', () =>
      new Promise<void>(done => {
    ...
    it('should use default config when HTTP request returns null', () =>
      new Promise<void>(done => {
        service.getConfig().subscribe(config => {
          expect(config).toEqual({
            logLevel: 'error',
    ...
    it('should use default config when HTTP request returns undefined', () =>
      new Promise<void>(done => {
        service.getConfig().subscribe(config => {
          expect(config).toEqual({
            logLevel: 'error',
    ...
    it('should handle HTTP errors gracefully with default config', () =>
      new Promise<void>(done => {
        service.getConfig().subscribe(config => {
          expect(config).toEqual({
            logLevel: 'error',
    ...
    it('should cache config after successful load using shareReplay', () =>
      new Promise<void>(done => {
        const mockConfig: AppConfig = {
          logLevel: 'info',
          environment: 'production',
  ...
  describe('config interface and validation', () => {
    it('should handle missing environment values with defaults', () =>
      new Promise<void>(done => {
        (environment as any).production = false;
