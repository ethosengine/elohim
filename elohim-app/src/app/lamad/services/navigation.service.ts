...
export class NavigationService {
  ...
  parsePathSegments(urlPath: string, queryParams: any = {}): NavigationContext | null {
    ...
    if (!urlPath || urlPath.length === 0) {
      return {
        ...
        currentNode: null,
        collectionType: null,
        children: this.getNodesByType(graph, 'epic'),
        parent: null,
        queryParams
      };
    }
    ...
    let parent: ContentNode | null = null;
    ...
    for (let i = 0; i < parts.length - (isCollectionView ? 1 : 0); i += 2) {
      ...
      if (node.contentType !== type) {
        console.warn(`Type mismatch: expected ${type}, got ${node.contentType} for ID ${id}`);
        return null;
      }
    ...
    if (isCollectionView) {
      ...
      // For now, we default to showing the "natural" next level children
      // epic -> feature, feature -> scenario
   ...
   */
  private getChildren(node: ContentNode): ContentNode[] {
    const graph = this.graphService.getGraph();
    if (!graph) return [];

    // Logic for "natural" children based on type
    let childType = '';
    ...
    // Filter related nodes by this type
    return node.relatedNodeIds
        .map(id => graph.nodes.get(id))
        .filter((n): n is ContentNode => n !== undefined && n.contentType === childType);
  }
   ...
   */
  private getChildrenOfType(node: ContentNode | null, childType: string): ContentNode[] {
    const graph = this.graphService.getGraph();
    if (!graph) return [];

    // Root-level collection (e.g., /lamad/epic)
    if (!node) {
      return this.getNodesByType(graph, childType);
    }

    // Node-specific collection
    // Filter related nodes by childType
    return node.relatedNodeIds
        .map(id => graph.nodes.get(id))
        .filter((n): n is ContentNode => n !== undefined && n.contentType === childType);
  }
  
  private getNodesByType(graph: ContentGraph, type: string): ContentNode[] {
   ...
   * Find a node by ID
   */
  private findNodeById(id: string): ContentNode | undefined {
   ...
   */
  nodeToUrlSegment(node: ContentNode): string {
    return `${node.contentType}:${node.id}`;
  }
   ...
   */
  getSuggestedNext(context: NavigationContext): ContentNode[] {
