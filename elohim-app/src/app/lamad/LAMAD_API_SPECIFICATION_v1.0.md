...
### 1.9 Governance Dimension

> **Cross-Reference:** The governance dimension is defined in `qahal/QAHAL_API_SPECIFICATION_v1.0.md`.

Every Lamad entity (content, path, human, Elohim, assessment) has a governance dimension accessible via:
```
/lamad/{entityType}:{entityId}/governance/{view}
```

The Qahal specification defines:
- Loomio-inspired graduated feedback and deliberation proposals
- Polis-inspired opinion clustering and sensemaking
- Wikipedia-style audit trails and version history
- Constitutional challenge/appeal system with SLA guarantees
- GovernanceService interface

---

### 1.10 Feedback Profile Dimension

> **Cross-Reference:** Feedback profiles are defined in `qahal/QAHAL_API_SPECIFICATION_v1.0.md`.

The Qahal specification defines:
- Core insight: "Virality is a privilege, not an entitlement"
- Key principles: No likes, emotional reaction constraints, intellectual humility
- Feedback mechanism hierarchy (approval-vote → view-only)
- Emotional reaction types (supportive vs critical)
- Route patterns for profile management and reactions

**Route Pattern (local context):**
```
/lamad/{entityType}:{entityId}/feedback-profile
/lamad/{entityType}:{entityId}/reactions
```
...
> **Cross-Reference:** The GovernanceService is defined in `qahal/QAHAL_API_SPECIFICATION_v1.0.md`.

The governance service manages the constitutional feedback layer. It provides:
- Context menu and governance state access
- Graduated feedback (Loomio-inspired)
- Deliberation proposals and voting
- Sensemaking (Polis-inspired)
- History and audit trails (Wikipedia-style)
- Discussion threads (talk pages)
- Challenges and appeals (constitutional accountability)
