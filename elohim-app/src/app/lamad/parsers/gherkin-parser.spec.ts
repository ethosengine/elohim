...
Examples:
  ...
  | bob      | wrong    | failure |`;
      ...
      expect(examples![0].rows[0]).toEqual(['alice', 'pass123', 'success']);
    });
...
Scenario: Test
  Given step`;
    ...
    it('should set correct content format for feature and scenarios', () => {
      const content = `Feature: Test
Scenario: Test
  Given step`;
    ...
    it('should set correct content type for scenarios', () => {
      const content = `Feature: Test
Scenario: Test
  Given step`;
      ...
      expect(result.scenarios[0].contentType).toBe('scenario');
    });
  });
});
